...
| `editor/html-text-utils.js` | `escHtml`, `escapeHtml` (riesportati da `core/dom-utils.js`), `escHtmlStrict`, `escTexJs`, `nl2br`, `containsInlineHtml`, ... | Escape HTML/TeX |
...
|--------|---------------|------|
| `editor/tex-workspace-service.js` | `texWorkspace`: `.load()`, `.refresh()`, `.invalidate()`, `.getCached()`, `.setCache(data)`, `.onChange(fn)` | Fetch chain: `/tikz/workspace` → `/tikz/effective-templates` → `/modelli_tikz_elements.json` |
