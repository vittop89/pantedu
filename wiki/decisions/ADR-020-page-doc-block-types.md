...
esempi vivi su ProgettoPrecedente:
...
[[decisions/ADR-002-lit3-web-components]] su accessibilità).
...
- **Server-side (authoritative):** riusa `App\Services\Security\HtmlSanitizer` introdotto da [[decisions/ADR-015-xss-sanitization]]. Stessa whitelist (h2-h4/p/ul/ol/li/a/strong/em/blockquote/code/pre/br/hr).
...
Estrazione one-shot da repo `progetto-precedente` (branch master, path `storage/templates/strcomp/ALTRO/*.php`) via script `tools/scripts/extract-seed-from-progetto-precedente.php`. Output: 4 file `schemas/risdoc/_pt/seeds/*.pt.json`. Review manuale obbligatoria prima merge.
...
- **Riusa sanitizer server**: `HtmlSanitizer` di [[decisions/ADR-015-xss-sanitization]] già deployato e testato.
...
- Source pages: https://www.progetto-precedente.example/strcomp_bes_altro/ALTRO/
