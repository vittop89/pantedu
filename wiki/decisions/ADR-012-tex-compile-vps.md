...
**Aggiornamento 2026-09-19.** Dal 24/5 il servizio sta sulla stessa macchina
dell'applicazione, e dall'8/9 l'applicazione gira in un container Docker. Il
servizio resta un'unità systemd sull'host e ascolta sul **gateway del bridge
Docker**, l'unico indirizzo valido sia dall'host sia dal container, con una
regola del firewall ristretta a `docker0`. Scartati il socket Unix montato e il
TeX in un container: il perché, i passi e i controlli (`/health/tex`, la
diagnostica `tex`) stanno in `docs/ops/tex-dal-container.md`.
