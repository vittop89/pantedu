...
I percorsi sono relativi alla **cartella dei dati d'istanza** (`app.paths.data_base`,
cioè `PANTEDU_DATA_PATH`), non alla radice del repository. Fino al 19/9/2026
`TikzRenderService` partiva dalla radice del repository: nel container è
l'immagine, e dall'8/9 le figure compilate si perdevano a ogni rilascio mentre
quelle già in cache non si leggevano più. Prova: `tests/Unit/Services/Tikz/CacheTikzNeiDatiTest.php`.

#### Encryption envelope (teacher scope)

Riusa `app/Services/Crypto/TeacherCryptoService.php` di
