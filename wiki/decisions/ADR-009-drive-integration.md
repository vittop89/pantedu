...
> **Aggiornato da [[ADR-038-drive-stati]]** (14/9/2026): Drive ha uno stato dell'installazione
> (`DRIVE_ENABLED`: spento, guasto, acceso) e uno per ogni collegamento (attivo o da ricollegare).
...
- Privacy: il browser carica XML dal nostro server (no upload a JGraph
  ...
  Nella pagina di studio l'XML arriva al visualizzatore dal browser: nel
  frammento `#R` per le mappe piccole, via `postMessage` per le grandi. Fino
  al 15/9/2026 le grandi andavano con `#U` e un URL firmato, che il
  visualizzatore scarica dal **proxy di diagrams.net**: il contenuto decifrato
  passava da JGraph, contro questa riga, e il WAF fermava il proxy («File non
  trovato»).
