...
**Contesto:** ADR-024/025/026 (documento PT unificato, dati curricolari dinamici), [[decisions/ADR-029-decomposizione-god-controller]] è un altro lavoro (decomposizione God-controller) — qui è solo un numero successivo.
...
## File principali
- `js/modules/risdoc/pt/terna-binding.js` + `app/Services/Risdoc/Pt/TernaBinding.php` — motore (extract/apply/split/attach, ensureBindingIds, accessor) e il suo mirror PHP per il render server e la migrazione.
...
## Verifica
Test live su pantedu.eu (doc terna_scoped): valori dei campi 🔗 **isolati e persistenti** fra SCI/2/MAT e SCI/3/MAT; toggle "Valori per classe" attiva + migra (verificato anche nel DB: `terna_scoped=true`, `ternaStore` con lo slot della terna).

Fino al 23/9/2026 «test unitari del motore verdi» non era una prova
verificabile: nessun test esisteva nel repository (stesso guasto misurato per
ADR-031, rilievo A-20 della revisione architetturale del 23/9/2026, voce 125
del registro del debito). Ora esistono, sullo stesso modello delle formule:
`tests/fixtures/terna-binding/*.json`, `tests/Unit/Risdoc/Pt/TernaBindingParitaTest.php`
(PHP), `tests/js-unit/terna-binding-parita.test.js` (JS) — vedi ADR-031 §
«Prove di parità» per come si aggiunge un caso.
