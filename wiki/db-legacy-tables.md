...
tipo: database
status: superseded
aliases: ["db-legacy-tables", "tabelle legacy"]
cssclasses: []
---

# DB Legacy Tables (G22.S15.bis Fase 5+)

> [!warning] Superata (2026-09-04)
> Censimento del 2026-05-09. Da allora: `teacher_exercises` e
> `teacher_verifiche` sono state droppate (migration 038),
> `VerificaBuilderController` e le rotte `/api/verifiche/*` non esistono
> più, `teacher_recovery_audit` è stata ricreata dalla migration 098 come
> registro append-only. Restano legacy `exercises` (vista su `exercises_data`,
> sola lettura) e `verifica_documents`, che però è oggi la tabella viva delle
> verifiche. Lo stato corrente è in [[database-schema]].
