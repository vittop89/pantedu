...
tipo: security
status: finale
aliases: ["_security", "toolchain sicurezza", "audit assistito"]
cssclasses: []
---
...
| FND-002 unsafe-inline/eval CSP | Medium | ✅ FIXED (Track 7, 2026-06-03) | `'unsafe-eval'` rimosso (nessun eval/`Function` nel sorgente). Tutti gli `on*=` inline bonificati (view `.php` + template editor) → delegation/data-*. CSP `strict` (nonce + strict-dynamic) pronta, attivabile da `/admin/waf/config` o `CSP_MODE`; resta `'unsafe-inline'` per `<script>` finché non si flippa a strict. CI guard anti-regressione. Dal 2026-09-23 (A-16) il nonce lo scrivono solo gli script dell'app (`App\Support\Csp`), non più il middleware su ogni `<script>`: regola in [[security-notes]]. |
