...
$router->get('/health/backup',          [\App\Controllers\HealthController::class, 'backupFreshness']);
// 19/9/2026 — il container raggiunge il servizio TeX? Solo sì/no e la classe
// dell'errore, mai indirizzi (HealthController::tex).
$router->get('/health/tex',             [\App\Controllers\HealthController::class, 'tex']);
$router->get ('/login',                 [AuthController::class, 'showLogin']);
// ADR-032 — scenari 1 e 2: gli studenti entrano con la credenziale del
// docente, senza account. La pagina posta su /api/access/student-login.
$router->get ('/accesso-classe',        [AuthController::class, 'showClassAccess']);
$router->post('/accesso-classe/esci',   [AuthController::class, 'classAccessLogout'])->middleware('csrf');
...
// 2026-09-01 — secondo passaggio del login. Non richiede sessione autenticata:
// a questo punto la password e' corretta ma l'utente NON e' ancora entrato, e
// lo stato vive in `pending_2fa`. Rate limit stretto: sei cifre si indovinano.
$router->get ('/login/2fa',             [AuthController::class, 'show2fa']);
$router->post('/login/2fa',             [AuthController::class, 'verify2fa'])->middleware('csrf', 'rate:login,10');
$router->any ('/logout',                [AuthController::class, 'logout']);

// 2026-09-01 — recupero password. Pubbliche per necessita': chi le usa non
// riesce ad autenticarsi. Rate limit stretto sui POST: il modulo di richiesta
// manda email a indirizzi altrui, quello di reset prova token.
$router->get ('/password/forgot',       [\App\Controllers\PasswordResetController::class, 'showForgot']);
$router->post('/password/forgot',       [\App\Controllers\PasswordResetController::class, 'submitForgot'])->middleware('csrf', 'rate:login,5');
$router->get ('/password/reset',        [\App\Controllers\PasswordResetController::class, 'showReset']);
$router->post('/password/reset',        [\App\Controllers\PasswordResetController::class, 'submitReset'])->middleware('csrf', 'rate:login,10');
...
// 23/9/2026 — le rotte /me/* che lavorano sulla sessione portano `auth`
// (revisione 2026-09, rilievo A-79). Senza, AuthMiddleware non le vedeva: chi
// aveva `must_change_password` o `must_enrol_2fa` raggiungeva esportazione,
// consensi, cancellazione e profilo, contro il confinamento, e una sessione di
// un account disattivato non si chiudeva passando da qui. Restano senza `auth`,
// di proposito:
//   - le conferme che arrivano per email (/me/account/email/conferma,
//     /me/confirm-deletion; la GET mostra la pagina, conferma il pulsante con
//     la POST): le apre il gettone del collegamento, non la sessione;
//   - /me/change-password e le rotte dell'iscrizione al secondo fattore
//     (/me/2fa, setup, enable, setup-email, enable-email): sono quelle a cui
//     il confinamento manda, e i controller chiedono già la sessione.
// La regola la verifica tests/Unit/Core/ConfinamentoSulleRotteMeTest.php.
//
// «Il mio account» (15/9/2026): email con conferma, password, due fattori. La
// conferma dell'email è un POST dietro un bottone: vedi AccountController.
$router->get ('/me/account',                  [\App\Controllers\AccountController::class, 'page'])->middleware('auth');
$router->post('/me/account/email',            [\App\Controllers\AccountController::class, 'email'])->middleware('auth', 'csrf', 'rate:login,10');
$router->get ('/me/account/email/conferma',   [\App\Controllers\AccountController::class, 'confermaPagina']);
$router->post('/me/account/email/conferma',   [\App\Controllers\AccountController::class, 'conferma'])->middleware('csrf', 'rate:login,10');
...
$router->post('/me/2fa/setup-email',    [\App\Controllers\TotpController::class, 'setupEmail'])->middleware('csrf');
$router->post('/me/2fa/enable-email',   [\App\Controllers\TotpController::class, 'enableEmail'])->middleware('csrf');
// Spegnere il secondo fattore non serve a completare l'iscrizione: `auth`.
$router->post('/me/2fa/disable',        [\App\Controllers\TotpController::class, 'disable'])->middleware('auth', 'csrf');

// Phase 25.C — self-service GDPR (Art. 7, 16, 17, 20): auth e CSRF, salvo le conferme via email (sopra).
$router->get ('/me/consents',           [\App\Controllers\SelfServiceController::class, 'consentsList'])->middleware('auth');
// 2026-09-04 — accessi amministrativi ai propri contenuti cifrati (ToS §3(c)).
$router->get ('/me/custody-events',     [\App\Controllers\SelfServiceController::class, 'custodyEvents'])->middleware('auth');
$router->post('/me/consents/grant',     [\App\Controllers\SelfServiceController::class, 'consentGrant'])->middleware('auth', 'csrf');
$router->post('/me/consents/revoke',    [\App\Controllers\SelfServiceController::class, 'consentRevoke'])->middleware('auth', 'csrf');

$router->post('/me/request-deletion',   [\App\Controllers\SelfServiceController::class, 'requestDeletion'])->middleware('auth', 'csrf', 'rate:deletion,5');
$router->get ('/me/confirm-deletion',   [\App\Controllers\SelfServiceController::class, 'confirmDeletion']);
$router->post('/me/confirm-deletion',   [\App\Controllers\SelfServiceController::class, 'confirmDeletionSubmit'])->middleware('csrf', 'rate:deletion,5');
$router->post('/me/cancel-deletion',    [\App\Controllers\SelfServiceController::class, 'cancelDeletion'])->middleware('auth', 'csrf');
$router->get ('/me/deletion-status',    [\App\Controllers\SelfServiceController::class, 'deletionStatus'])->middleware('auth');

$router->get ('/me/export-data',        [\App\Controllers\SelfServiceController::class, 'exportData'])->middleware('auth', 'rate:export,3');
$router->post('/me/profile',            [\App\Controllers\SelfServiceController::class, 'profilePatch'])->middleware('auth', 'csrf');
...
$router->post('/dpo-contact',           [\App\Controllers\DpoContactController::class, 'submit'])
       ->middleware('csrf', 'rate:dpo,3,3600');
...
$router->post('/segnalazione-contenuti', [\App\Controllers\Public\PublicTakedownController::class, 'submit'])
       ->middleware('rate:takedown,3,3600');

// Phase 25.P — Admin Takedown queue (super_admin only).
// `sadmin_audit` registra le LETTURE del super-admin in privileged_access_log:
// qui dentro si aprono richieste GDPR, registro dei data breach, log altrui.
// Fino al 2026-09-02 il middleware esisteva ma non era applicato da nessuna
// parte, e quelle letture non lasciavano traccia. Le scritture passano dal
// registro globale in Kernel e da `audit_reason`, che scrive il perché:
// fino al 23/9/2026 qui lo portavano sette POST su trentotto (A-69), e il
// commento diceva «tutte». Ora ogni mutazione amministrativa lo porta, o sta
// fra le esenzioni scritte di tests/Unit/Core/CoperturaMotivazioneTest.php.
$router->group(['middleware' => ['auth', 'role:admin', 'super_admin_required', 'log', 'sadmin_audit:admin_read,governance']], function (\App\Core\Router $r) {
    ...
    $r->post('/admin/takedown/{id}/action',     [\App\Controllers\Admin\AdminTakedownController::class, 'action'])
       ->middleware('csrf', 'audit_reason:takedown_decision,takedown_request');
    ...
    $r->post('/admin/sidebar-config/save',      [\App\Controllers\Admin\AdminSidebarConfigController::class, 'save'])
       ->middleware('csrf', 'audit_reason:sidebar_section_save,sidebar_config');
    $r->post('/admin/sidebar-config/delete',    [\App\Controllers\Admin\AdminSidebarConfigController::class, 'delete'])
       ->middleware('csrf', 'audit_reason:sidebar_section_delete,sidebar_config');
    $r->post('/admin/sidebar-config/reorder',   [\App\Controllers\Admin\AdminSidebarConfigController::class, 'reorder'])
       ->middleware('csrf', 'audit_reason:sidebar_sections_reorder,sidebar_config');
    // Chi pubblica in rete (migrazione 129): rende visibili senza login i
    // contenuti pubblicati di un docente, quindi con il motivo.
    $r->post('/admin/sidebar-config/pubblica-in-rete', [\App\Controllers\Admin\AdminSidebarConfigController::class, 'pubblicaInRete'])
       ->middleware('csrf', 'audit_reason');

    // Migration 099/100 — governo delle sezioni: chi insegna dove e in quale
    // sezione sta ogni studente. Entrambe decisioni della scuola, non del
    // docente: per questo stanno qui e non in /area-docente/profilo.
    // Lo spostamento di uno studente e' una rettifica Art. 16 GDPR e va a
    // registro col prima e il dopo — da qui `audit_reason`.
    $r->get ('/admin/sections',          [\App\Controllers\Admin\AdminSectionsController::class, 'index']);
    $r->post('/admin/sections/assign',   [\App\Controllers\Admin\AdminSectionsController::class, 'assign'])
       ->middleware('csrf', 'audit_reason:teacher_sections_assign,teacher_sections');
    $r->post('/admin/sections/revoke',   [\App\Controllers\Admin\AdminSectionsController::class, 'revoke'])
       ->middleware('csrf', 'audit_reason:teacher_sections_revoke,teacher_sections');
    // Prima di togliere incarichi: che cosa ha il docente su quelle sezioni (15/9/2026).
    $r->get ('/admin/sections/anteprima-revoca', [\App\Controllers\Admin\AdminSectionsController::class, 'anteprimaRevoca']);
    $r->post('/admin/sections/student',  [\App\Controllers\Admin\AdminSectionsController::class, 'student'])
       ->middleware('csrf', 'audit_reason');
    // ADR-041 — i materiali di un docente rimasti su una sezione di cui non ha
    // più l'incarico: l'amministratore li porta sull'anno, a mano e con il
    // motivo; la data è solo l'avviso che i docenti vedono.
    $r->post('/admin/sections/materiali', [\App\Controllers\Admin\AdminSectionsController::class, 'materiali'])
       ->middleware('csrf', 'audit_reason');
    $r->post('/admin/sections/scadenza',  [\App\Controllers\Admin\AdminSectionsController::class, 'scadenza'])
       ->middleware('csrf', 'audit_reason:leftover_materials_deadline,teacher_sections');
    // Materie del docente: stesso pannello, stessa logica di governo. E' un
    // "set" — arrivano le caselle spuntate, il resto viene disattivato.
    $r->post('/admin/sections/subjects', [\App\Controllers\Admin\AdminSectionsController::class, 'subjects'])
       ->middleware('csrf', 'audit_reason:teacher_subjects_set,teacher_subjects');
    ...
    $r->post('/admin/institutes/new',           [\App\Controllers\Admin\AdminInstitutesController::class, 'create'])
       ->middleware('csrf', 'audit_reason:institute_create,institute');
    // Sospende/riattiva: toglie l'istituto dalle liste in cui lo si SCEGLIE,
    // senza cancellarlo e senza scollegare chi c'e' gia' dentro.
    $r->post('/admin/institutes/{id}/active',   [\App\Controllers\Admin\AdminInstitutesController::class, 'toggleActive'])
       ->middleware('csrf', 'audit_reason:institute_active_toggle,institute');
    // 2026-09-04 — le compilazioni dei modelli istituzionali dei docenti di un
    // Istituto si salvano sul server o restano nel browser: scelta del Titolare,
    // impostata per Istituto (CompilationStoragePolicy).
    $r->post('/admin/institutes/{id}/compilation-storage', [\App\Controllers\Admin\AdminInstitutesController::class, 'toggleCompilationStorage'])
       ->middleware('csrf', 'audit_reason:institute_compilation_storage,institute');
    // ADR-041 — chi dei docenti dell'istituto può legarsi a una sezione.
    $r->post('/admin/institutes/{id}/sezioni-docenti', [\App\Controllers\Admin\AdminInstitutesController::class, 'impostaSezioniDocenti'])
       ->middleware('csrf', 'audit_reason:institute_teacher_sections_policy,institute');
    ...
    $r->post('/admin/institutes/miur/update',   [\App\Controllers\Admin\AdminInstitutesController::class, 'miurUpdate'])
       ->middleware('csrf', 'audit_reason:miur_schools_update,miur_registry');
    // L'ALTRO dataset MIUR: le adozioni, da cui si ricavano indirizzi e sezioni
    // realmente attivi. Tre passi separati perche' i codici degli indirizzi
    // sono proposte, e la conferma di chi legge sta in mezzo.
    $r->post('/admin/institutes/miur/adozioni', [\App\Controllers\Admin\AdminInstitutesController::class, 'adozioniUpload'])
       ->middleware('csrf', 'audit_reason:miur_adoptions_upload,adoptions');
    $r->get ('/admin/institutes/adozioni',      [\App\Controllers\Admin\AdminInstitutesController::class, 'adozioniPreview']);
    $r->post('/admin/institutes/adozioni/apply',[\App\Controllers\Admin\AdminInstitutesController::class, 'adozioniApply'])
       ->middleware('csrf', 'audit_reason:miur_adoptions_apply,adoptions');
    // ADR-035 — il catalogo dell'istituto: il vocabolario con la provenienza
    // di ogni voce, quanti docenti l'hanno spuntata e quanti contenuti ci
    // puntano. E' l'unico posto dove un'etichetta si corregge una volta per
    // tutti, e dove una voce promossa da una copia di un docente si accetta
    // o si toglie. Ogni modifica e' una mutazione privilegiata: motivazione.
    $r->get ('/admin/institutes/{id}/catalogo',         [\App\Controllers\Admin\AdminCatalogoController::class, 'index']);
    $r->post('/admin/institutes/{id}/catalogo',         [\App\Controllers\Admin\AdminCatalogoController::class, 'aggiungi'])
       ->middleware('csrf', 'audit_reason');
    $r->post('/admin/institutes/{id}/catalogo/{entry}', [\App\Controllers\Admin\AdminCatalogoController::class, 'modifica'])
       ->middleware('csrf', 'audit_reason');
    ...
    $r->post('/admin/gdpr/authority-export',  [\App\Controllers\Admin\AdminGdprController::class, 'authorityExportSubmit'])
       ->middleware('csrf', 'audit_reason:authority_export,gdpr_export');
    ...
    $r->post('/admin/data-requests/{id}/action', [\App\Controllers\Admin\AdminGdprController::class, 'dataRequestsAction'])
       ->middleware('csrf', 'audit_reason:data_request_action,gdpr_request');

    $r->get ('/admin/data-breach',                   [\App\Controllers\Admin\AdminGdprController::class, 'dataBreachIndex']);
    // Prima di `{id}`, altrimenti «piano» verrebbe letto come un numero.
    $r->get ('/admin/data-breach/piano',             [\App\Controllers\Admin\AdminGdprController::class, 'dataBreachPiano']);
    ...
    $r->post('/admin/data-breach/new',               [\App\Controllers\Admin\AdminGdprController::class, 'dataBreachCreate'])
       ->middleware('csrf', 'audit_reason:data_breach_create,data_breach');
    ...
    $r->post('/admin/data-breach/{id}/action',   [\App\Controllers\Admin\AdminGdprController::class, 'dataBreachAction'])
       ->middleware('csrf', 'audit_reason:data_breach_action,data_breach');
    ...
    $r->post('/admin/subprocessors/save',            [\App\Controllers\Admin\AdminGdprController::class, 'subprocessorsSave'])
       ->middleware('csrf', 'audit_reason:subprocessor_save,subprocessor');
    $r->post('/admin/subprocessors/{id}/delete', [\App\Controllers\Admin\AdminGdprController::class, 'subprocessorsDelete'])
       ->middleware('csrf', 'audit_reason:subprocessor_delete,subprocessor');
    ...
    $r->post('/admin/crypto-status/event',     [\App\Controllers\Admin\AdminCryptoStatusController::class, 'recordEvent'])
       ->middleware('csrf', 'audit_reason:crypto_event_record,crypto_custody');
    ...
    $r->post('/admin/backup/cold-completed',   [\App\Controllers\Admin\AdminBackupController::class, 'coldCompleted'])
       ->middleware('csrf', 'audit_reason:backup_cold_completed,backup');
    $r->post('/admin/backup/b2-verified',      [\App\Controllers\Admin\AdminBackupController::class, 'b2Verified'])
       ->middleware('csrf', 'audit_reason:backup_b2_verified,backup');
    ...
    $r->post('/admin/system/deployment/switch',   [\App\Controllers\Admin\AdminSystemController::class, 'deploymentSwitch'])
       ->middleware('csrf', 'audit_reason:deployment_switch,deployment');
    ...
    $r->post('/admin/system/registration-classes/add',    [\App\Controllers\Admin\AdminSystemController::class, 'registrationClassAdd'])
       ->middleware('csrf', 'audit_reason:registration_class_add,registration');
    $r->post('/admin/system/registration-classes/remove', [\App\Controllers\Admin\AdminSystemController::class, 'registrationClassRemove'])
       ->middleware('csrf', 'audit_reason:registration_class_remove,registration');
    ...
    $r->post('/admin/system/registration-mode', [\App\Controllers\Admin\AdminSystemController::class, 'registrationModeSet'])
       ->middleware('csrf', 'audit_reason:registration_mode_set,registration');
    // Phase 25.P — accende/spegne il gate ToS/AUP senza toccare .env né php-fpm.
    // `audit_reason` è deliberato: impedire l'accesso al servizio è una decisione,
    // e va a registro con una motivazione e con chi l'ha presa.
    $r->post('/admin/system/2fa-enforce', [\App\Controllers\Admin\AdminSystemController::class, 'twoFactorEnforceSet'])
       ->middleware('csrf', 'audit_reason');
    $r->post('/admin/system/tos-enforce', [\App\Controllers\Admin\AdminSystemController::class, 'tosEnforceSet'])
       ->middleware('csrf', 'audit_reason');
...
// WAF). 429 JSON al superamento; inerte se RATE_LIMIT_DISABLED=1 (solo prove e sviluppo).
...
// Riusa la stessa shell+sidebar+stile di /studio (StudyPageRenderer). Dal 2026-09-04
// (revisione P5) le rotte pubbliche stanno in PublicStudyController, separato dal
// controller autenticato: la superficie esposta ai guest si legge in un file solo.
$router->get('/public/studio/{id}',    [\App\Controllers\PublicStudyController::class, 'publicView'])
...
$router->get('/api/public/study/topics.json',  [\App\Controllers\PublicStudyController::class, 'publicTopicsJson'])
       ->middleware('rate:pub_study,120');
$router->get('/api/public/study/content.json', [\App\Controllers\PublicStudyController::class, 'publicContentJson'])
       ->middleware('rate:pub_study,120');
...
// AI Act (Reg. UE 2024/1689) — trasparenza art. 4 + art. 50.
$router->get('/legal/ai-act',               [\App\Controllers\TrustPagesController::class, 'aiAct']);
$router->get('/legal/ai-literacy',          [\App\Controllers\TrustPagesController::class, 'aiLiteracy']);
...
// 2026-09-05 — report delle violazioni CSP (report-uri): senza sessione, solo
// append su file, rate limit. Serve a tenere la produzione in report-only.
$router->post('/api/csp-report',         [\App\Controllers\CspReportController::class, 'collect'])->middleware('rate:csp,60');
...
// Piano classi, C — ingresso con il QR della credenziale (permanente o a
// tempo; piu' codici separati da virgola = pacchetto di classe) e QR del
// portachiavi corrente. Stesso limite di tentativi della password.
$router->get ('/accesso-classe/qr/{token}',     [\App\Controllers\TeacherCredentialController::class, 'qrLogin'])
       ->middleware('rate:qr,20');
$router->get ('/accesso-classe/pacchetto.svg',  [\App\Controllers\TeacherCredentialController::class, 'bundleSvg']);
...
$router->group(['prefix' => ''], function (Router $r) {
    ...
    // /tikzjax.js RIMOSSA 2026-09-23 (revisione architetturale A-54, D-17):
    // rimandava con un 301 a /js/vendor/tikzjax.js, tolto il 2026-06-01, e
    // nessuna pagina la chiedeva più. Con lei sono usciti il motore
    // WebAssembly del vecchio TikZJax in wasm/ e la sua `location` di nginx.
    // Il TikZ si rende sul server (ADR-013), e /tikzjax.js risponde 404.
...
/* Phase 25.E17 — Apache native serving per static asset.
   ...
   /js/, /css/, /img/, /tikzjax-develop/, /node_modules/,
   /scriptGoogle_sync/, /build/ vengono serviti direttamente da Apache,
   skippando il front controller PHP. Niente più catch-all qui. */

// ───────────── STUDIO: studenti e ospiti con credenziale di classe ─────────────
// 14/9/2026 — le pagine e le API con cui si studia, di sola lettura. Stavano nel
// gruppo degli studenti (auth + role:student), che un ospite entrato con la
// credenziale di classe non soddisfa: riceveva 401 da tutte e la pagina lo
// mandava al login, mentre i controller (ADR-032) l'ospite lo sanno servire.
// StudioMiddleware lascia passare chi ha un account da studente in su, alle stesse
// condizioni di prima, e l'ospite con almeno una credenziale valida, riverificata
// sul database a ogni richiesta. Nessuna rotta che scrive **i dati di qualcuno**:
// l'unica POST qui è la compilazione di una figura, che riempie una cache
// indirizzata dal contenuto e non appartiene a nessuno (21/9/2026).
//
// IMPORTANTE: queste rotte stanno PRIMA di quelle M11 legacy del gruppo studenti,
// perché i pattern a 5 segmenti coincidono nel conteggio dei parametri:
// /studio/{type}/{ind}/{cls}/{subj} contro /studio/{ind}/{cls}/{materia}/{topic}.
// Il router prende la prima registrata. Se il primo segmento non è un tipo
// valido, ContentStudyController passa a ExerciseStudyController — ma solo a chi
// ha un account: la tabella legacy non conosce la credenziale.
$router->group(['middleware' => ['studio', 'log']], function (Router $r) {
    ...
    // 19/9/2026 — le materie del selettore di chi studia, per la classe
    // chiesta (anni frequentati): solo quelle con materiali visibili.
    $r->get('/api/study/materie.json',
            [\App\Controllers\ContentStudyController::class, 'materieJson']);
    // Le verifiche che vede chi studia (ADR-037, fase 3): pubblicate per la classe.
    $r->get('/api/study/verifica/list',
            [\App\Controllers\VerificaController::class, 'listForStudent']);
    // Phase 25.Q.16 — l'intestazione del docente di riferimento (per l'ospite,
    // quella del docente della credenziale).
    ...
    // 21/9/2026 — anche la compilazione, non solo la lettura della cache.
    //
    // La GET qui sopra trova la figura **se c'è già**; quando manca, il client
    // la fa compilare con questa POST. Stava nel gruppo degli account
    // (`auth` + `role:student`), che l'ospite con la credenziale di classe non
    // soddisfa: riceveva 401 «unauthenticated» e al posto della figura
    // compariva «[TikZ render error] unauthenticated». Lo stesso commento di
    // allora diceva «studenti devono poter vedere TikZ in pagine esercizi e
    // verifiche»: era vero solo per chi ha un account.
    //
    // Chi compila non scrive niente di suo: la cache è indirizzata dal
    // contenuto (sha256 del sorgente normalizzato), quindi due sorgenti uguali
    // sono la stessa voce e non c'è niente da enumerare. Restano il gettone
    // CSRF, il limite dedicato (200/min), il tetto sulla dimensione del
    // sorgente e il semaforo del microservizio.
    //
    // G27 PERF — bucket 'tikz' con limite alto: il rendering di una pagina
    // ricca di grafici è a raffica ma legittimo, e va in cache. Senza, una
    // verifica con molte figure sforava il limite e le figure restavano vuote.
...
$router->group(['middleware' => ['auth', 'role:student', 'log']], function (Router $r) use ($legacyGoneHandler) {
    ...
    // Il riferimento del super-admin (/api/admin/latex-shortcuts) sta nel
    // gruppo dell'amministratore dal 23/9/2026 (A-78).
    ...
    // Il PUT (editor /area-docente/fonti) sta nel gruppo TEACHER + (19/9/2026).
    ...
    // La gestione dei preset (/api/admin/badge-style-presets) sta nel gruppo
    // dell'amministratore dal 23/9/2026 (A-78).
...
// ───────────── TEACHER + ─────────────
$router->group(['middleware' => ['auth', 'role:teacher', 'teacher_subjects', 'log']], function (Router $r) {
    // 19/9/2026 — le scritture delle preferenze del docente (fonti, stile del
    // badge, filtri delle origini, intestazione). Stavano nel gruppo STUDENT +
    // accanto alle loro letture: uno studente con account passava la porta e
    // lo fermava solo il controller, per caso (nessuna scuola in
    // teacher_institutes: 404). Le letture restano dov'erano. La regola:
    // tests/Unit/Core/CoperturaRuoliTest.php.
    $r->put('/api/teacher/sources.registry.json',
            [\App\Controllers\StudySourcesController::class, 'sourcesRegistrySave'])->middleware('csrf');
    $r->put('/api/teacher/badge-style',
            [\App\Controllers\BadgeStyleController::class, 'teacherPut'])->middleware('csrf');
    $r->put('/api/teacher/sources.json',
            [\App\Controllers\StudySourcesController::class, 'sourcesSave'])->middleware('csrf');
    $r->put('/api/teacher/checked-origins.json',
            [\App\Controllers\StudySourcesController::class, 'checkedOriginsSave'])->middleware('csrf');
    $r->put('/api/teacher/header-page.json',
            [\App\Controllers\StudyHeaderController::class, 'headerPageSave'])->middleware('csrf');

    // Materie del docente: dichiarate al primo accesso, correggibili dopo.
    // Il middleware del gruppo ci manda chi non le ha ancora scelte.
    $r->get ('/area-docente/materie', [\App\Controllers\TeacherSubjectsController::class, 'form']);
    $r->post('/area-docente/materie', [\App\Controllers\TeacherSubjectsController::class, 'save'])
       ->middleware('csrf');
    ...
    // tool Python pdf-scraping-tools (progetto-precedente), hardenata (LLM-PY-001).
    ...
    // /provider-models rimossa il 2026-08-26 insieme al provider OpenRouter
    // (era il suo unico caso d'uso). Vedi PdfImportController.
    ...
    $r->group(['middleware' => ['csrf', 'rate']], function (Router $rr) {
        ...
        $rr->post('/api/verifica/{id}/pubblicazioni',             [\App\Controllers\TeacherPublicationsController::class, 'aggiungiVerifica']);
        $rr->post('/api/verifica/{id}/pubblicazioni/{pub}/stato', [\App\Controllers\TeacherPublicationsController::class, 'statoVerifica']);
        $rr->post('/api/verifica/{id}/pubblicazioni/{pub}/togli', [\App\Controllers\TeacherPublicationsController::class, 'togliVerifica']);
        $rr->post('/api/verifica/{id}/duplica',                   [\App\Controllers\TeacherPublicationsController::class, 'duplicaVerifica'])
        ...
        $rr->post('/api/teacher/pdf-import/session',
           ...
           ->middleware('rate:pdf_import_llm,config:pdf_import.rate.pdf_import_llm');
        $rr->post('/api/teacher/pdf-import/session/{id}/cell',
                  [\App\Controllers\Teacher\PdfImportController::class, 'editCell'])
           ->middleware('rate:pdf_import,config:pdf_import.rate.pdf_import');
        ...
        $rr->post('/api/teacher/pdf-import/session/{id}/translate',
           ...
           // Un numero suo, di proposito: il client chiama /translate a
           // ripetizione, un pezzo alla volta, e 12 lo fermerebbe a metà.
    ...
    // ADR-037, fase 3 — i posti di una verifica («Dove vale»).
    $r->get('/api/verifica/{id}/pubblicazioni', [\App\Controllers\TeacherPublicationsController::class, 'elencoVerifica']);
    ...
    $r->group(['middleware' => ['csrf', 'rate']], function (Router $rr) {
        ...
        // 14/9/2026 — recupero di una verifica condivisa (tutte le varianti).
        $rr->post('/api/teacher/pool/recover-verifica/{id}',
            [\App\Controllers\PoolController::class, 'recoverVerifica'])
           ->middleware('rate:pool_recover,30');
    ...
    // ADR-036 — i libri in adozione dell'istituto, per le classi e le materie
    // che il docente ha spuntato: da qui prende le proprie fonti.
    $r->get('/api/teacher/adozioni',
            [\App\Controllers\TeacherAdozioniController::class, 'index']);
    // Piano classi, C — QR della credenziale (?mode=temp: codice a tempo da proiettare).
    $r->get('/api/teacher/credentials/{id}/qr.svg',
            [\App\Controllers\TeacherCredentialController::class, 'qrSvg']);
    // ADR-044 — l'etichetta che il server comporrebbe, per l'anteprima del modulo.
    $r->get('/api/teacher/credentials/anteprima-etichetta',
            [\App\Controllers\TeacherCredentialController::class, 'labelPreview']);
    $r->group(['middleware' => ['csrf', 'rate']], function (Router $rr) {
        ...
        // Piano classi, C — rotazione della sola password, scadenza, QR nuovo.
        $rr->post('/api/teacher/credentials/{id}/password',
                  [\App\Controllers\TeacherCredentialController::class, 'rotate']);
        $rr->post('/api/teacher/credentials/{id}/expiry',
                  [\App\Controllers\TeacherCredentialController::class, 'expiry']);
        $rr->post('/api/teacher/credentials/{id}/qr/regenerate',
                  [\App\Controllers\TeacherCredentialController::class, 'qrRegenerate']);
        // ADR-044 — ricompone l'etichetta (materie e aggiunta scelte ora).
        $rr->post('/api/teacher/credentials/{id}/etichetta',
                  [\App\Controllers\TeacherCredentialController::class, 'relabel']);
    ...
    // NB /api/teacher/subjects (GET/POST/delete) rimosso il 2026-09-02: leggeva
    // e scriveva la pivot curriculum_users, droppata dalla migration 044. Tutte
    // e tre le operazioni fallivano in SQL. Le materie del docente stanno in
    // curriculum_entries (owner_user_id) e si governano da /area-docente/materie
    // e /admin/sections; per leggerle: /api/teacher/curriculum?scope=all.
    ...
    // ADR-037, fase 2 — «Dove vale»: le pubblicazioni di un contenuto e i posti
    // (scuole e voci spuntate) in cui il docente può pubblicare.
    $r->get('/api/teacher/content/{id}/pubblicazioni',
            [\App\Controllers\TeacherPublicationsController::class, 'elenco']);
    $r->get('/api/teacher/pubblicazioni/luoghi',
            [\App\Controllers\TeacherPublicationsController::class, 'luoghi']);
    ...
    $r->group(['middleware' => ['csrf', 'rate']], function (Router $rr) {
        ...
        // ADR-037, fase 2 — «Dove vale» (aggiungi, stato, togli) e «Duplica in…».
        $rr->post('/api/teacher/content/{id}/pubblicazioni',
                  [\App\Controllers\TeacherPublicationsController::class, 'aggiungi']);
        $rr->post('/api/teacher/pubblicazioni/{id}/stato',
                  [\App\Controllers\TeacherPublicationsController::class, 'stato']);
        $rr->post('/api/teacher/pubblicazioni/{id}/togli',
                  [\App\Controllers\TeacherPublicationsController::class, 'togli']);
        $rr->post('/api/teacher/content/{id}/duplica',
                  [\App\Controllers\TeacherPublicationsController::class, 'duplica'])
                  ->middleware('rate:content,60');
        ...
        // 2026-09-22 — il docente ha scaricato il documento: da qui parte la
        // grazia prima della cancellazione (ADR-046). Lo scaricamento avviene
        // tutto nel browser — il server non lo vedrebbe mai — quindi e' il
        // client a dirlo. Secchio dedicato: un docente che scarica piu' volte
        // di seguito non deve consumare il bilancio dei suoi salvataggi.
        $rr->post('/api/risdoc/compilations/{id}/scaricata',
                  [\App\Controllers\Risdoc\CompilationController::class, 'scaricata'])
                  ->middleware('rate:scaricata,60');
    });
    ...
    // Spostati dal group role:admin (bug pre-esistente: docente.due
    ...
    // I contenuti senza indirizzo non compaiono nella navigazione, che filtra
    // per indirizzo: senza questa pagina non c'e' modo di arrivarci.
    $r->get ('/area-docente/da-categorizzare', [\App\Controllers\TeacherUncategorizedController::class, 'index']);
    $r->post('/area-docente/da-categorizzare', [\App\Controllers\TeacherUncategorizedController::class, 'save'])
       ->middleware('csrf');
    // Spostare i materiali da una classe a un'altra, a blocchi, dal sito:
    // il caso vero e' anno → sezione (ADR-035). Solo classi spuntate.
    $r->get ('/area-docente/sposta-di-classe', [\App\Controllers\TeacherMoveController::class, 'index']);
    $r->post('/area-docente/sposta-di-classe', [\App\Controllers\TeacherMoveController::class, 'save'])
       ->middleware('csrf');
...
// ───────────── AMMINISTRATORE DI ISTITUTO (ADR-040) ─────────────
// La zona `istituto`: l'amministratore di istituto non sta in nessun'altra, e
// fuori dallo scenario 3 nemmeno in questa (Auth::hasAccess). Ogni funzione
// nuova sull'istituto si dichiara qui, con l'ambito verificato da
// Auth::isAdminOfInstitute e una prova d'integrazione nei due versi.
$router->group(['middleware' => ['auth', 'role:istituto', 'log']], function (Router $r) {
    $r->get('/istituto', [\App\Controllers\IstitutoController::class, 'page']);
});
...
$router->group(['middleware' => ['auth', 'role:admin', 'log']], function (Router $r) use ($legacyGoneHandler) {
    ...
    // 2026-09-15 — erano nella zona del collaboratore, che conteneva solo loro
    // e l'amministratore: tolto il collaboratore restano all'amministratore.
    ...
    $r->group(['middleware' => ['csrf', 'rate']], function (Router $rr) {
        ...
        $rr->post('/admin/migrate/run', [\App\Controllers\AdminMigrateController::class, 'run'])
           ->middleware('audit_reason:db_migrate,database');
    });

    // Ported FileService endpoints (new, type-safe, path-validated)
    //
    // 2026-09-20 — `/files/save-tex` tolta: nessun chiamante (nemmeno
    // `Endpoints.files.saveTex`, che era una voce senza lettori) e nessuna
    // richiesta nel registro degli accessi di produzione. I sorgenti TeX si
    // salvano da `/api/verifica/save-tex` e dagli endpoint `tex-files/save`.
    $r->group(['middleware' => ['csrf', 'rate']], function (Router $rr) {
        $rr->post('/files/save-latex',    [FileController::class, 'saveLatex']);
        $rr->post('/files/delete',        [FileController::class, 'deleteFile'])
           ->middleware('audit_reason:webroot_file_delete,webroot_files');
        $rr->post('/files/delete-folder', [FileController::class, 'deleteFolder'])
           ->middleware('audit_reason:webroot_folder_delete,webroot_files');
        // /files/list-php promosso a student+ (usato dalla sidebar Mappe/Eser/Lab)
        //
        // 23/9/2026 (A-77) — solo POST. Era `any()`: il gettone qui
        // sopra si controlla solo sui verbi di scrittura, e il gestore svuota
        // i temporanei con qualunque verbo, quindi una GET da un altro sito
        // aperto da un amministratore li cancellava. Nessun client la chiama
        // oggi; `Endpoints.files.deleteTemp` viene da `/delete_temp.php`, che
        // la vecchia pagina chiamava in POST.
        $rr->post('/files/clear-temp',    [FileController::class, 'clearTemp']);
    });
    ...
    $r->group(['middleware' => ['csrf', 'rate']], function (Router $rr) {
        $rr->post('/tikz/save-svg',   [\App\Controllers\TikzController::class, 'saveSvg'])
           ->middleware('audit_reason:tikz_svg_save,tikz_svg');
        $rr->post('/tikz/delete-svg', [\App\Controllers\TikzController::class, 'deleteSvg'])
           ->middleware('audit_reason:tikz_svg_delete,tikz_svg');
    });

    // AdminController — new JSON endpoints replacing ad-hoc log/admin PHP pages
    // 2026-09-04 — /admin E' la pagina degli strumenti (utenti, registrazioni,
    // log, notifiche): e' quello che si apre per lavorare. /admin/tools resta
    // come rimando per i vecchi collegamenti; il dashboard con i riquadri sta
    // su /admin/dashboard, raggiungibile dalla barra.
    $r->get ('/admin',              [\App\Controllers\AdminToolsController::class, 'page']);
    ...
    $r->get ('/admin/tools',        [\App\Controllers\AdminController::class, 'index']);
    ...
    $r->group(['middleware' => ['super_admin_required', 'sadmin_audit:admin_read,cross_teacher_analytics']], function (Router $rr) {
    ...
    // ADR-036 — il catalogo delle adozioni a mano: un libro adottato dopo la
    // chiusura del dataset MIUR, o un testo consigliato.
    $r->get ('/api/admin/adozioni',                       [\App\Controllers\Admin\AdminAdozioniController::class, 'index']);

    // Phase 13 — admin gestisce istituti + users + security
    $r->group(['middleware' => ['csrf', 'rate']], function (Router $rr) {
        $rr->post('/api/admin/adozioni',                              [\App\Controllers\Admin\AdminAdozioniController::class, 'create'])
           ->middleware('audit_reason:adoption_create,adoptions');
        $rr->post('/api/admin/adozioni/{id}/delete',                  [\App\Controllers\Admin\AdminAdozioniController::class, 'delete'])
           ->middleware('audit_reason:adoption_delete,adoptions');
        $rr->post('/api/institutes',                                  [\App\Controllers\InstituteController::class, 'create'])
           ->middleware('audit_reason:institute_create,institute');
        $rr->post('/api/admin/users/{id}/active',                     [\App\Controllers\UsersAdminController::class, 'setActive'])
           ->middleware('audit_reason:user_active_set,user');
        $rr->post('/api/admin/users/{id}/role',                       [\App\Controllers\UsersAdminController::class, 'setRole'])
           ->middleware('audit_reason:user_role_set,user');
        $rr->post('/api/admin/users/{id}/delete',                     [\App\Controllers\UsersAdminController::class, 'delete'])
           ->middleware('audit_reason:user_delete,user');
        $rr->post('/api/admin/security/credentials/block',            [\App\Controllers\SecurityAdminController::class, 'blockCredential'])
           ->middleware('audit_reason:credential_block,security_blocks');
        $rr->post('/api/admin/security/credentials/unblock',          [\App\Controllers\SecurityAdminController::class, 'unblockCredential'])
           ->middleware('audit_reason:credential_unblock,security_blocks');
        $rr->post('/api/admin/security/ips/block',                    [\App\Controllers\SecurityAdminController::class, 'blockIp'])
           ->middleware('audit_reason:ip_block,security_blocks');
        $rr->post('/api/admin/security/ips/unblock',                  [\App\Controllers\SecurityAdminController::class, 'unblockIp'])
           ->middleware('audit_reason:ip_unblock,security_blocks');
        $rr->post('/api/admin/security/config',                       [\App\Controllers\SecurityAdminController::class, 'setConfig'])
           ->middleware('audit_reason:security_alerts_config,security_config');
    ...
    $r->group(['middleware' => ['super_admin_required', 'sadmin_audit:admin_read,user_records']], function (Router $rr) {
        ...
        $rr->group(['middleware' => ['csrf', 'rate']], function (Router $rrr) {
            $rrr->post('/admin/registrations/{id}/approve', [\App\Controllers\RegistrationController::class, 'approve'])
               ->middleware('audit_reason:registration_approve,registration_request');
            $rrr->post('/admin/registrations/{id}/reject',  [\App\Controllers\RegistrationController::class, 'reject'])
               ->middleware('audit_reason:registration_reject,registration_request');
        });
    });
    ...
    $r->group(['middleware' => ['csrf', 'rate']], function (Router $rr) {
        ...
        // Files
        //
        // 2026-09-20 — `/files/save-image` tolta con il giro morto del vecchio
        // editor TikZ: la chiamava solo `saveSvgToServer` in
        // `js/modules/editor/content-processor.js`, ramo raggiungibile solo da
        // `svg[data-tikz-script-id]`, attributo che scriveva soltanto lui. I
        // file che produceva non li serviva nessuna rotta.
        $rr->post('/files/save-pdf',      [FileController::class, 'savePdf'])
           ->middleware('audit_reason:verifica_pdf_save,webroot_files');
        // /files/save-latex, delete, delete-folder, list-php, clear-temp già registrati sopra
        ...
        $rr->post('/tikz/save-new-element', [\App\Controllers\TikzController::class, 'saveNewElement'])
           ->middleware('audit_reason:tikz_template_create,tikz_templates');
        $rr->post('/tikz/edit-element',     [\App\Controllers\TikzController::class, 'editElement'])
           ->middleware('audit_reason:tikz_template_edit,tikz_templates');
        $rr->post('/tikz/delete-element',   [\App\Controllers\TikzController::class, 'deleteElement'])
           ->middleware('audit_reason:tikz_template_delete,tikz_templates');
        ...
        // dal 2026-09-05 la logica sta nel controller, niente file eseguito.
        $rr->any ('/delete_temp.php', [\App\Controllers\CronController::class, 'deleteTemp']);
    });

    // I pannelli legacy /log/admin, /log/logging e /log/security sono stati
    // rimossi il 2026-09-04 (revisione P10): nessun chiamante nel frontend.

    // G22.S20 v2.C2 — Rotte /area-docente/* spostate al group role:teacher
    // (vedi sopra). Erano per errore nel group role:admin → docente.due
    ...
    $r->group(['middleware' => ['super_admin_required', 'sadmin_audit:admin_read,teacher_content']], function (Router $rr) {
    ...
    // 23/9/2026 (A-78) — erano nel gruppo `role:student`, e le fermava solo
    // il controllo dentro ogni metodo: lo stesso schema delle PUT del docente
    // spostate il 19/9. Qui con gli stessi middleware locali di prima, così
    // chi le usa resta lo stesso: i preset vogliono Auth::hasAccess('admin'),
    // cioè questa porta; il riferimento delle scorciatoie vuole
    // Auth::isSuperAdmin(), che il controller continua a verificare (il
    // super-admin ha sempre la zona admin, e il database non ammette un
    // amministratore di istituto super-admin). Sorveglia CoperturaRuoliTest.
    //
    // Phase 25 — riferimento super-admin delle scorciatoie LaTeX, che il
    // docente forka per scorciatoia da /api/latex-shortcuts/*.
    ...
    $r->group(['middleware' => ['csrf', 'rate']], function (Router $rr) {
        $rr->post('/api/admin/latex-shortcuts', [\App\Controllers\LatexShortcutsController::class, 'adminSave'])
           ->middleware('audit_reason:latex_shortcuts_save,latex_shortcuts');
    });
    ...
    // e sono cascade _default → istituto.
    ...
    $r->put('/api/admin/badge-style-presets/{name}',
            [\App\Controllers\BadgeStyleController::class, 'adminPut'])->middleware('csrf', 'audit_reason:badge_style_preset_save,badge_style_presets');
    $r->delete('/api/admin/badge-style-presets/{name}',
            [\App\Controllers\BadgeStyleController::class, 'adminDelete'])->middleware('csrf', 'audit_reason:badge_style_preset_delete,badge_style_presets');
});
...
$router->group(['middleware' => ['auth', 'role:admin', 'log']], function (Router $r) {
    ...
    $r->group(['middleware' => ['super_admin_required', 'csrf', 'rate', 'audit_reason']], function (Router $rr) {
        ...
        $rr->get   ('/admin/waf/api/cti',               [\App\Controllers\Admin\WafAdminController::class, 'apiCti']);
...
// ───────────── API authenticated ─────────────
// 2026-09-15 — qui c'era un gruppo /api per la zona del collaboratore, vuoto
// dal 26/8: tolto con il ruolo.
    /* Copilot RIMOSSO (2026-08-26). Erano /api/copilot/chat + gli alias legacy
       /copilot.php e /copilot_proxy.php verso CopilotController.

       Motivi: (1) il pannello non si caricava piu' — il loader cercava
       api/copilot-ai.js sotto la document root, che e' public/, quindi la
       condizione era sempre falsa; (2) l'etichetta diceva "GitHub Copilot"
       mentre il traffico andava a OpenAI/Anthropic, informazione errata sotto
       l'art. 50(1) del Reg. UE 2024/1689; (3) il proxy accettava la chiave API
       del provider nel body della POST dal client e ne logava il prefisso.

       Se serve di nuovo un assistente di scrittura, il pattern corretto e'
       quello di PDF-Import: chiavi lato server in ProviderKeyStore, cifrate
       con la KEK del docente, mai in transito dal browser.
       Vedi docs/legal/ai-act-assessment.md § 3.2. */
