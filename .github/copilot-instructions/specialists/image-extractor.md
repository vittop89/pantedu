...
### 4. Grafici/Figure/Immagini (⚠️ OBBLIGATORIO)

**REGOLA CRITICA:** Prima di procedere con qualsiasi esercizio, VERIFICARE SEMPRE se l'immagine contiene:
- Grafici di funzioni (parabole, rette, iperboli, ecc.)
- Figure geometriche (triangoli, cerchi, poligoni, ecc.)
- Diagrammi, schemi, disegni fisici (circuiti, forze, ecc.)

**Se presente un grafico o una figura → è OBBLIGATORIO:**
1. Impostare `hasGraph: true` nel contract
2. Popolare `tikz_figures[]` con analisi dettagliata
3. Segnalare al workflow che serve la generazione TikZ
...
"tikz_figures": [
  {
    ...
    "description": "Parabola con a<0, passante per l'origine e (3,0), vertice in (1.5, 2.25)",
    "key_elements": {
      "function": "y = -x^2 + 3x",
      "points": [{"x": 0, "y": 0}, {"x": 3, "y": 0}, {"x": 1.5, "y": 2.25}],
      "domain": [-1, 4],
      "range": [-3, 3]
    }
  }
...
**⚠️ ATTENZIONE:** NON saltare mai questo passo. Se un esercizio ha un grafico o una figura e il campo `tikz_figures` è vuoto, il TikZ non verrà generato e l'esercizio sarà incompleto.
...
## [TIKZ_DETECTION] Rilevamento Figure Geometriche/Grafici

Quando l'immagine contiene figure geometriche, grafici o diagrammi fisici, aggiungi `tikz_figures` al contract.

### Detection Algorithm

```javascript
function detectTikZFigures(image) {
  const figures = [];
  
  // 1. Rileva elementi visuali
  const hasGeometricShapes = detectShapes(image); // triangoli, cerchi, poligoni
  const hasCartesianPlane = detectAxes(image);    // assi x,y con griglia
  const hasPhysicsDiagram = detectPhysicsElements(image); // vettori, molle, corpi
  const hasTables = detectTables(image);          // tabelle segni, matrici
  
  if (hasGeometricShapes) {
    figures.push(analyzeGeometricFigure(image));
  }
  
  if (hasCartesianPlane) {
    figures.push(analyzeGraph(image));
  }
  
  if (hasPhysicsDiagram) {
    figures.push(analyzePhysicsDiagram(image));
  }
  
  if (hasTables) {
    figures.push(analyzeTable(image));
  }
...
```javascript
{
  ...
  "detected_shape": "triangle|polygon|circle",
  "image_analysis": {
    "points": [
      {"name": "A", "x": 0, "y": 0, "label": "$A$", "position": "below left"}
    ],
    "segments": [
      {"from": "A", "to": "B", "label": "5 cm", "style": "thick"}
    ],
    "angles": [
      {"vertex": "B", "rays": ["BA", "BC"], "measure": "90°"}
    ],
    "colors": {"fill": "yellow!30", "lines": "black"}
  },
  "template_suggestion": {
...
{
  "figure_type": "graph",
  "detected_shape": "function_graph",
  "image_analysis": {
    "axes": {"xmin": -4, "xmax": 6, "ymin": -4, "ymax": 6},
    "functions": [
      {"expression": "y = (2/3)*x + 2", "color": "blue", "label": "s"}
    ],
    "points": [
      {"x": 4.5, "y": 5, "label": "(9/2, 5)", "color": "blue"}
    ],
    "grid": true
  },
  "template_suggestion": {
    "group": "gruppo-grafici e funzioni",
    "label": "grafico di funzione (axis)"
  }
}
```
...
{
  "figure_type": "physics",
  "detected_shape": "cinematics_1d",
  "image_analysis": {
    "objects": ["Alex(A)", "Antonio(B)"],
    "states": [
      {"time": "t0=0s", "posA": -3, "posB": 2, "velA": 1.5, "velB": -1}
    ]
  },
  "template_suggestion": {
    "group": "gruppo-FISICA",
