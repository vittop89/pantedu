...
final class ClsNormalizer
{
    ...
    /**
     * Shrink legacy → short, dropping the suffix `s`.
     * "2s" → "2", "2" → "2" (idempotent). "2B" resta "2B": e una SEZIONE.
     * Used per la display nel dropdown e per i nuovi URL puliti.
     */
    public static function shrink(string $cls): string
    {
        $c = trim($cls);
        // Il suffisso "b" (Breve/Bilinguismo) non e' piu' collassato: quel
        // percorso non esiste piu' in nessun dato (zero righe in
        // teacher_content, users e print_info, cartella eser/ vuota) e con le
        // sezioni "1b" significa ora sezione B, non Bilinguismo. Continuare a
        // ridurlo a "1" cancellerebbe la sezione in silenzio.
        if (preg_match('/^([1-5])s$/', $c, $m)) {
            return $m[1];
        }
        return $c;
    }
