 ...
 *   - Dark theme: applica `.fm-dark` sul wrapper con la decisione della
 *     politica del tema (js/tema-iniziale.js, window.fmTemaScuro), che nel
 *     documento a sé è in linea nel <head> e nelle altre due forme arriva
 *     dall'impaginazione dell'app (23/9/2026, A-50).
...
final class StandalonePageRenderer
{
    ...
    public static function render(string $title, string $body, array $options = []): string
    {
        ...
        // Il nonce della CSP sullo script del tema (23/9/2026, A-16): prima lo
        // metteva la timbratura del middleware su ogni <script> del corpo.
        $csp = Csp::attributo();
        $tema = TemaIniziale::script();
    ...
    private static function darkModeScript(): string
    {
        return <<<'JS'
            (function () {
                ...
                /* La decisione è della politica del tema (js/tema-iniziale.js,
                   in linea nel <head>): qui non se ne prende un'altra. Se il
                   file non si è letto, chiaro come il CSS di base. */
                var decidi = window.fmTemaScuro || function () { return false; };
                apply(decidi());
                // Toggle on click
                ...
                window.addEventListener('storage', function (e) {
                    if (e.key === STORAGE_KEY) apply(decidi());
                });
            })();
        JS;
    }
