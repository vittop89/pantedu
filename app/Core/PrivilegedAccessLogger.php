...
final class PrivilegedAccessLogger
{
    public static function log(
        ...
        // actorRole() e non $user['role']: distingue chi ha agito da super-admin.
        $role  = $user === [] ? 'guest' : Auth::actorRole();
        // Hash, non in chiaro (2026-09-03): il registro art. 30 lo
        // dichiarava da tempo, il codice scriveva l'IP e lo User-Agent
        // cosi' com'erano. Vedi RequestFingerprint e migration 100.
        $ip    = \App\Services\Audit\RequestFingerprint::ipHash();
        $ua    = \App\Services\Audit\RequestFingerprint::uaHash();
                ...
                $stmt = $pdo->prepare(
                    'INSERT INTO privileged_access_log
                       (user_id, actor_name, actor_role, action, resource_type, resource_id,
                        reason, ip_hash, ua_hash, outcome)
                     VALUES (?,?,?,?,?,?,?,?,?,?)'
                );
                $stmt->execute([
        ...
        self::appendFile([
            ...
            'reason'        => $reason,
            'ip_hash'       => $ip === null ? null : bin2hex($ip),
            'ua_hash'       => $ua === null ? null : bin2hex($ua),
            'outcome'       => $outcome,
        ]);
    }
