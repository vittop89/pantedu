...
final class Database
{
    ...
    /**
     * La stringa di connessione, in un posto solo.
     *
     * C'erano tre copie identiche — `connection()`, `maintenanceConnection()`
     * e `migrationConnection()` — e aggiungere il socket avrebbe voluto dire
     * aggiungerlo tre volte, con due occasioni di dimenticarsene. Quale delle
     * tre si dimentica lo si scopre solo quando serve.
     *
     * Con `DB_SOCKET` valorizzata si passa dal socket Unix e host e porta non
     * si scrivono affatto: PDO li ignorerebbe, ma un DSN che dichiara due
     * strade quando ne usa una sola e' un DSN che mente a chi lo legge.
     */
    private static function dsn(): string
    {
        $driver  = (string)Config::get('database.driver');
        $name    = (string)Config::get('database.name');
        $charset = (string)Config::get('database.charset');
        $socket  = (string)(Config::get('database.socket') ?? '');

        if ($socket !== '') {
            return sprintf('%s:unix_socket=%s;dbname=%s;charset=%s', $driver, $socket, $name, $charset);
        }
