...
class JobRepository
{
    ...
    {
        // Interpolato non lo era per distrazione: `$id` è dichiarato `int`, e
        // PHP lo garantisce. Ma è l'unico punto del codice che costruisce una
        // query concatenando, e basta allargare la firma una volta perché
        // diventi un'iniezione. Costa una riga tenerlo preparato come tutti
        // gli altri.
        $st = $this->pdo->prepare('SELECT attempts, max_attempts FROM jobs WHERE id = ?');
        $st->execute([$id]);
