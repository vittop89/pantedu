    ...
    public function assegnaCategorie(
        ...
        $chk = $db->prepare(
            'SELECT 1 FROM curriculum_entries
              WHERE id = ? AND kind = ? AND owner_user_id = ? LIMIT 1'
        );

        $cambiate = 0;
        $place = implode(',', array_fill(0, count($ids), '?'));
        foreach ($campi as $colonna => [$kind, $valore]) {
            $chk->execute([$valore, $kind, $teacherId]);
            if (!$chk->fetchColumn()) {
                throw new \InvalidArgumentException('voce_non_del_docente:' . $kind);
            }
            // Una UPDATE per campo, ognuna col proprio "solo se vuoto": in una
            // sola, un contenuto che ha gia' la materia ma non la classe
            // sarebbe escluso da entrambe.
            $upd = $db->prepare(
                "UPDATE teacher_content_data SET $colonna = ?
                  WHERE teacher_id = ? AND $colonna IS NULL AND id IN ($place)"
            );
            $upd->execute(array_merge([$valore, $teacherId], $ids));
            $cambiate += $upd->rowCount();
        }
        return $cambiate;
    }
}
