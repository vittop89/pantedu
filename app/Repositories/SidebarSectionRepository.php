...
/* tabella override assente → ignora */
    ...
    private function shape(array $r, ?array $override): array
    {
        ...
        $s = [
            ...
            'template_origin'       => isset($r['template_origin']) ? (string)$r['template_origin'] : null,
    ...
    public function teacherMatchesScope(int $teacherId, array $scope): bool
    {
        ...
        try {
            ...
            if ($ind !== '' || $cls !== '') {
                if ($inst > 0) {
                    // ADR-037, fase 1 (2026-09-13) — con la scuola nello scope, i
                    // contenuti devono essere pubblicati in quella scuola. Prima
                    // si guardavano le sigle della riga, e un docente di due
                    // scuole con le stesse sigle risultava nello scope di
                    // entrambe per i contenuti di una sola.
                    $w = ['d.teacher_id = ?', 'p.institute_id = ?'];
                    $a = [$teacherId, $inst];
                    if ($ind !== '') {
                        $w[] = 'pi.code = ?';
                        $a[] = $ind;
                    ...
                    $s = $pdo->prepare(
                        'SELECT 1 FROM content_publications p
                           ...
                           LEFT JOIN curriculum_entries pi ON pi.id = p.indirizzo_id
                           LEFT JOIN curriculum_entries pc ON pc.id = p.classe_id
                          WHERE ' . implode(' AND ', $w) . ' LIMIT 1'
                    );
                } else {
                    $w = ['teacher_id = ?'];
                    $a = [$teacherId];
    ...
    /**
     * Id delle sezioni publish_public attive (qualunque istituto) che hanno quel
     * tipo come tipo predefinito: le sole in cui la lista pubblica «per tipo»
     * cerca (PublicContentPolicy::scopedFilters, 16/9/2026).
     * @return list<int>
     */
    public function publicSectionIdsForType(string $type): array
    {
        if (!Database::isAvailable() || $type === '') {
            return [];
        }
        try {
            $stmt = Database::connection()->prepare(
                'SELECT id FROM sidebar_sections WHERE active = 1 AND publish_public = 1 AND default_content_type = ?'
            );
            $stmt->execute([$type]);
            return array_map('intval', $stmt->fetchAll(PDO::FETCH_COLUMN) ?: []);
        } catch (\Throwable) {
