...
final class CompilationRepository
{
    ...
    public function save(int $teacherId, int $templateId, string $compilationKey, string $label, ?string $classe, ?string $sezione, ?string $indirizzo, ?string $disciplina, string $dataJson): int
    {
        ...
        $clsId = $classe !== null && $classe !== ''
            ? $L::idFromCodeForTeacher('classi', (string)$classe, $teacherId, $indirizzo !== null && $indirizzo !== '' ? (string)$indirizzo : null) : null;
    ...
    public function listByTeacher(int $teacherId, int $templateId, ?string $classe = null, ?string $sezione = null, ?string $indirizzo = null, ?string $disciplina = null): array
    {
        $sql = 'SELECT id, compilation_key, label, classe, sezione, indirizzo,
                       disciplina, created_at, updated_at, exported_at
    ...
    /**
     * Segna che il docente ha scaricato il documento di questa compilazione.
     *
     * Da qui parte la grazia prima della cancellazione (ADR-046,
     * {@see \App\Services\Risdoc\ScadenzaDelleBozze}).
     *
     * TRE COSE CHE SEMBRANO DETTAGLI E NON LO SONO
     *
     * 1. `updated_at = updated_at`. La colonna ha `ON UPDATE
     *    CURRENT_TIMESTAMP` (migrazione 007): un UPDATE qualunque la sposta ad
     *    adesso. Misurato il 22/9/2026 nei due versi — senza questa
     *    assegnazione `updated_at` salta, con essa resta ferma. Se saltasse,
     *    segnare uno scaricamento verrebbe contato come una modifica del
     *    docente: la lista delle compilazioni si riordinerebbe sotto le sue
     *    mani, e la grazia ripartirebbe da capo a ogni scaricamento senza che
     *    nessuno l'abbia deciso.
     *
     * 2. `NOW()` e non l'ora di PHP. L'applicazione gira su Europe/Rome e il
     *    database in UTC: una data scritta da PHP nasce due ore nel futuro
     *    rispetto all'orologio con cui verra' confrontata. E' lo stesso errore
     *    corretto su `detected_at` lo stesso giorno.
     *
     * 3. La scrittura va sulla TABELLA. `risdoc_compilations` e' una vista.
     *
     * Torna false se la riga non esiste o non e' di questo docente: i due casi
     * non si distinguono, come in {@see delete()}.
     */
    public function segnaScaricata(int $teacherId, int $id): bool
    {
        if ($teacherId <= 0 || $id <= 0) {
            return false;
        }
        ...
        $stmt = Database::connection()->prepare(
            'UPDATE risdoc_compilations_data
                SET exported_at = NOW(), updated_at = updated_at
              WHERE teacher_id = ? AND id = ?'
        );
        $stmt->execute([$teacherId, $id]);

        // rowCount() e' 0 anche quando la riga c'e' ma il valore non cambia.
        // Qui non puo' succedere — NOW() e' sempre diverso dal secondo prima,
        // e al primo scaricamento la colonna e' NULL — ma se un giorno si
        // scaricasse due volte nello stesso secondo, un false vorrebbe dire
        // «non l'ho trovata» a torto. Per questo si ricontrolla.
        if ($stmt->rowCount() > 0) {
            return true;
        }

        $q = Database::connection()->prepare(
            'SELECT 1 FROM risdoc_compilations_data WHERE teacher_id = ? AND id = ? LIMIT 1'
        );
        $q->execute([$teacherId, $id]);
     ...
     *
     * 23/9/2026 (A-36) — «non configurato» voleva dire anche «chiave presente
     * ma malformata», e allora la compilazione si scriveva in chiaro, in
     * silenzio. Adesso il chiaro vale solo senza chiave: con una chiave
     * malformata il salvataggio fallisce, e l'errore si registra (lo stato,
     * mai il valore).
     *
    ...
    private function seal(int $teacherId, string $plain): array
    {
        ...
        if ($teacherId > 0 && $this->crypto()->hasMalformedKey()) {
            \error_log('[CompilationRepository] KMS_MASTER_KEY malformata (servono 64 caratteri esadecimali): '
                . 'compilazione del docente ' . $teacherId . ' non salvata, per non scriverla in chiaro');
            throw new \RuntimeException('kms_malformata: compilazione non salvata');
        }
