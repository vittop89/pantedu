 ...
 *
 * Tutti gli utenti (administrator, teacher, student, institute_admin) vivono ora
...
class UserRepository implements UserRepositoryInterface
{
    ...
    private function hydrate(array $row): User
    {
        return User::fromArray($row['username'], [
            ...
            // 23/9/2026 (A-75) — il corso dello studente, dalle colonne della
            // migrazione 091 e nel formato della registrazione ("{indirizzo}.{classe}").
            // Mancava: User::canAccessSection() era sempre falso per uno
            // studente del database, e il login da un vecchio indirizzo di
            // sezione falliva anche nella sua.
            'course'        => StudentProfileService::course(
                isset($row['indirizzo']) ? (string)$row['indirizzo'] : null,
                isset($row['classe']) ? (string)$row['classe'] : null,
            ),
        ], 'db');
    }
}
