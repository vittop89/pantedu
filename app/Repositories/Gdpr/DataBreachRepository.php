...
final class DataBreachRepository
{
    ...
    public function create(array $data, int $reporterId): int
    {
        ...
        // 2026-09-22 — di chi è il dato. Il rischio R20 dice che un docente
        // può scrivere in un campo libero un dato che riguarda uno studente:
        // di quello è titolare il suo Istituto, non l'operatore. Senza queste
        // due colonne l'incidente non si poteva nemmeno attribuire, e senza
        // attribuzione non si sa chi avvisare (migrazione 136).
        $istituto = trim((string)($data['institute_id'] ?? ''));
        $stmt = Database::connection()->prepare(
            'INSERT INTO data_breach_incidents
                (occurred_at, detected_at, severity, affected_users_count, data_categories,
                 institute_id, titolare_esterno,
        ...
        $stmt->execute([
            ...
            ($istituto !== '' && ctype_digit($istituto)) ? (int)$istituto : null,
            trim((string)($data['titolare_esterno'] ?? '')) ?: null,
            $description,
            ($data['root_cause'] ?? '')       ?: null,
            ($data['remedial_actions'] ?? '') ?: null,
            // 2026-09-22 — zero non è un utente: la chiave esterna lo
            // rifiuta e l'incidente non nasce. Succede quando l'azione
            // parte senza una sessione — da riga di comando, o da una
            // prova. Un incidente senza autore è meglio di nessun
            // incidente: la colonna è nullable apposta.
    ...
    /**
     * Segna che il titolare esterno dei dati — di regola l'Istituto del
     * docente — è stato avvisato.
     *
     * Non è una cortesia: quando il dato trapelato è di un altro titolare,
     * l'operatore è l'unico che può saperlo, e solo quel titolare può
     * adempiere verso i propri interessati. Il trigger della migrazione 136
     * impedisce di disdire un avviso già dato.
     */
    public function setControllerNotified(int $id, string $method): bool
    {
        $stmt = Database::connection()->prepare(
            'UPDATE data_breach_incidents
                SET notified_controller_at = NOW(), controller_notification_method = ?
              WHERE id = ? AND notified_controller_at IS NULL'
        );
