...
final class CurriculumService
{
    ...
    private const CODE_PATTERNS = [
        'indirizzi' => '#^[A-Z]{3,6}$#',
        'classi'    => '#^[1-9][A-Z0-9]{0,5}$#', // anno + sezione (es. 1, 2B, 1ALSS: nel dataset MIUR il suffisso codifica anche l'indirizzo)
        'materie'   => '#^[A-Z]{3,6}$#',
    ];
    ...
    private function loadFromDb(?int $instituteId, ?int $teacherId = null): array
    {
        ...
        $base = "SELECT ce.id, ce.kind, ce.institute_id, ce.owner_user_id,
                        ce.code, ce.label, ce.grp, ce.indirizzo, ce.active, ce.shared_with_pool,
    ...
    public function add(
        ...
        // per le classi: 1A e' dello scientifico, 1BLSS dello sportivo, e
        // senza questo il form di registrazione le offrirebbe indistintamente.
        // NULL = trasversale, come gli anni senza sezione.
        ...
        // Indirizzi, classi e materie sono una proprieta' della SCUOLA, non del
        // docente: il Esempio ha scientifico e artistico, ha la 1A e insegna
        // Matematica a prescindere da chi ci lavora.
        //
        // Senza questo guard due docenti dello stesso istituto possono creare
        // 'SCI' e 'SCIE' entrambi etichettati "Scientifico":
        // listActiveForInstitute() deduplica per CODICE, quindi allo studente
        // in registrazione arrivano due voci identiche a vedersi. Chi sceglie
        // quella sbagliata si ancora a un codice che i contenuti dell'altro
        // docente non usano e non vede nulla, senza alcun errore.
        //
        // Vale per tutti e tre i kind, non solo per gli indirizzi. Una materia
        // inventata da un docente non compare nel catalogo di nessun altro, e
        // una classe inventata non corrisponde a nessuna sezione reale: in
        // entrambi i casi il contenuto finisce agganciato a una voce che gli
        // studenti non hanno, e sparisce senza un errore.
        //
        // Il vocabolario lo fissa l'ISTITUTO (righe owner_user_id IS NULL), e
        // quelle righe le scrive l'import dei dati MIUR da /admin/institutes.
        // Il docente puo' solo attivare o disattivare cio' che esiste gia'.
        // Aggiungere una voce davvero nuova e' un'azione esplicita dell'admin,
        // non l'effetto collaterale di aver digitato una sigla.
        if ($owner !== null && $owner > 0) {
            $stmt = Database::connection()->prepare(
            ...
            $stmt->execute([$kind, $instituteId, $code]);
            if (!$stmt->fetchColumn()) {
    ...
    private function rowToShape(array $row): array
    {
        return [
            ...
            'label'          => $row['label'],
            'group'          => $row['grp'],
            // Solo per kind=classi: corso di appartenenza (migration 100).
            // NULL = trasversale a tutti gli indirizzi dell'istituto.
            'indirizzo'      => isset($row['indirizzo']) && $row['indirizzo'] !== null ? (string)$row['indirizzo'] : null,
