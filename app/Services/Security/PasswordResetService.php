...
final class PasswordResetService
{
    ...
    public function request(string $email, string $ip): void
    {
        ...
        try {
            ...
            // 23/9/2026 — la radice prima del gettone. Senza `app.url` non
            // c'è un collegamento da mandare, e un gettone che nessuno può
            // ricevere è solo una riga in più: IndirizzoPubblico registra
            // l'anomalia e lancia, e il catch qui sotto tace come per ogni
            // altro errore (proprietà 1). Fino a quel giorno il ripiego era il
            // dominio di produzione: un'altra istanza mandava il gettone al
            // sito di qualcun altro. Senza mailer il gettone si emette ancora
            // (lo dice .env.example) e la radice non serve.
            $sito = $this->mailer instanceof Mailer
            ...
            $st->execute([
                ...
                // Impronta con chiave, non l'IP in chiaro né uno SHA-256 nudo
                // (dal 24/9/2026): vedi App\Support\ImprontaIp.
                ImprontaIp::esadecimale($ip),
                self::now(),
            ]);

            $this->sendLink($email, (string)$user['username'], $token, $sito);
        } catch (Throwable) {
    ...
    private function sendLink(string $email, string $username, string $token, string $sito): void
    {
        if (!$this->mailer instanceof Mailer) {
            return;
        }
        $link = $sito . '/password/reset?token=' . $token;
