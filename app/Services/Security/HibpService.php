...
final class HibpService
{
    ...
    /**
     * 23/9/2026 — proprietà dichiarata a parte e non promossa: semgrep legge
     * solo in parte i file con una proprietà promossa `readonly`.
     */
    public function __construct(int $timeoutSeconds = self::TIMEOUT_S)
    {
    ...
    private function fetchRange(string $prefix): ?array
    {
        ...
        try {
            ...
            $opts = [
                ...
                // Chi chiama e dove trovarlo: l'istanza (`app.url`), non il dominio
                // di produzione che c'era scritto fino al 23/9/2026.
                CURLOPT_USERAGENT      => \App\Support\IndirizzoPubblico::agente('pantedu-waf/25.J'),
                CURLOPT_HTTPHEADER     => [
            ...
            $ca = \App\Support\BundleCa::percorso();
            if ($ca !== null) {
