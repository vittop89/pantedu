...
final class LatexShortcutsService
{
    ...
    /** Dove si scrive: i dati d'istanza (PANTEDU_DATA_PATH). */
    private string $basePath;
    /** Dove sta il codice: serve solo per leggere il seed versionato. */
    private string $radiceDelCodice;

    public function __construct(?string $basePath = null)
    {
        // Nel container la radice del repository è l'immagine: gli override
        // del docente scritti lì sparivano al rilascio, e il riferimento admin
        // finiva in uno strato sopra la copia versionata dell'immagine.
        $this->radiceDelCodice = rtrim(str_replace('\\', '/', $basePath ?? dirname(__DIR__, 3)), '/');
        $this->basePath = \App\Support\PercorsiDati::base(dirname(__DIR__, 3), $basePath);
    }

    /** Il riferimento admin si SCRIVE nei dati d'istanza. */
    private function adminFile(): string
    {
        return $this->basePath . '/' . self::ADMIN_FILE_REL;
    }

    /** Il riferimento admin si LEGGE prima dai dati, poi dal codice: il seed
     *  `storage/data/latex_shortcuts_default.json` è versionato e viaggia
     *  dentro l'immagine. */
    private function adminFileDaLeggere(): ?string
    {
        return \App\Support\PercorsiDati::inCascata(
            self::ADMIN_FILE_REL,
            $this->radiceDelCodice,
            $this->basePath,
        );
    }

    /** Il file degli override del docente. Non crea niente: la cartella la fa
     *  chi scrive davvero ({@see self::writeOverrides()}). Prima il mkdir stava
     *  qui, e bastava una lettura a creare `storage/objects/teachers/<id>`
     *  vuota — è così che è comparsa in produzione dentro il container. */
    private function teacherFile(int $teacherId): string
    {
        if ($teacherId <= 0) {
            throw new RuntimeException('invalid_teacher_id');
        }
        return $this->basePath . '/' . self::TEACHER_DIR_REL . '/' . $teacherId . '/' . self::TEACHER_FILE;
    }
