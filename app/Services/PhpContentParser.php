...
final class PhpContentParser
{
    ...
    private function parseGroup(DOMXPath $xp, DOMElement $problem): array
    {
        ...
        foreach ($xp->query('.//div[contains(@class,"fm-collection__item")]', $problem) as $cxItem) {
            if (!$cxItem instanceof DOMElement) {
                continue;
            }
    ...
    private function parseItem(DOMXPath $xp, DOMElement $it, string $groupKind = 'Collect'): array
    {
        ...
        $base = [
            ...
            'question'       => $questionNode instanceof DOMElement ? $this->parseBlocks($questionNode) : [],
        ];
        ...
        if ($groupKind === 'VF') {
            ...
            $base['solution'] = $solutionNode instanceof DOMElement ? $this->parseBlocks($solutionNode) : [];
        }
    ...
    private function extractBadge(array $questionBlocks): ?array
    {
        ...
        if (preg_match_all('/\\\\tiny\{\\\\text\{([^}]+)\}\}/', $tex, $m)) {
            // Niente `isset` sul primo: se `preg_match_all` ha trovato
            // qualcosa, la cattura zero c'è per definizione. Sul secondo serve.
    ...
    private function parseVfExtras(DOMXPath $xp, DOMElement $it): array
    {
        ...
        } elseif ($solDiv instanceof DOMElement) {
        ...
        return [
            'answer'        => $answer,
            'justification' => $justifyNode instanceof DOMElement ? $this->parseBlocks($justifyNode) : [],
        ];
    }
    ...
    {
        $justifyNode = $xp->query('.//div[contains(@class,"fm-giustsol")]', $it)->item(0);
        $justBlocks  = $justifyNode instanceof DOMElement ? $this->parseBlocks($justifyNode) : [];
        ...
        foreach ($xp->query('.//table//td', $it) as $td) {
            ...
            if ($input instanceof DOMElement) {
                ...
                $options[] = [
                    'correct' => $correct,
                    'content' => $labelNode instanceof DOMElement ? $this->parseBlocks($labelNode) : [],
                ];
                continue;
            }
    ...
    private function parseBlocks(DOMElement $root): array
    {
        ...
        // Il tipo dichiarato serve al controllore statico: senza, restringe
        // `$textBuf` alla stringa vuota nel punto in cui la closure viene
        // definita e conclude che il confronto qui sotto è sempre vero. Ma la
        // cattura è per riferimento e il valore cambia dopo.
        /** @var string $textBuf */
        $textBuf = '';
