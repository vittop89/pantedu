 ...
 *   64 hex chars (32 bytes), letta e giudicata da ChiaveMadre. Generabile con:
...
final class TeacherCryptoService
{
    ...
    /** Lo stato della chiave letta: ChiaveMadre::ASSENTE, VALIDA o MALFORMATA. */
    private string $statoChiave;

    /**
     * @param ?string $kmsMasterHex la chiave data a mano (le prove, gli
     *        strumenti); null = quella dell'ambiente.
     */
    public function __construct(?string $kmsMasterHex = null)
    {
        // 23/9/2026 (A-36) — la chiave si legge e si giudica in ChiaveMadre,
        // con le stesse regole della firma dell'export e del recupero. Qui
        // prima c'era una regola propria, e un ripiego su
        // Config::get('crypto.kms_master_key') che non esisteva: la chiave non
        // passa dalla configurazione (app/Config/crypto.php).
        // Lazy: con una chiave non valida $kmsMaster resta null, errore
        // esplicito al primo uso.
        $chiave = $kmsMasterHex !== null
            ? ChiaveMadre::daValore($kmsMasterHex)
            : ChiaveMadre::dallAmbiente();
        $this->statoChiave = $chiave->stato();
        if ($chiave->valida()) {
    ...
    /**
     * La chiave c'è ma non ha la forma giusta (23/9/2026, A-36). Non è il caso
     * «sviluppo senza chiave»: chi scriverebbe in chiaro senza chiave, con una
     * chiave malformata deve fermarsi.
     */
    public function hasMalformedKey(): bool
    {
        return $this->statoChiave === ChiaveMadre::MALFORMATA;
    }
    ...
    public function ensureTeacherKey(int $teacherId): int
    {
        ...
        //
        // Override esplicito via ALLOW_CRYPTO_REGENERATE=1 (crypto.allow_regenerate)
        // per casi legittimi (es. nuovo teacher dopo shred volontario in audit log).
        $allowRegenerate = (bool)\App\Core\Config::get('crypto.allow_regenerate', false);
        if (!$allowRegenerate) {
    ...
    /** Verifica che KMS_MASTER_KEY sia presente e valida, lancia eccezione se no. */
    private function requireKms(): void
    {
        if ($this->kmsMaster === null) {
            // Lo stato (assente o malformata), mai il valore.
            throw new RuntimeException(
                'kms_not_configured: KMS_MASTER_KEY ' . $this->statoChiave
                . ' (servono 64 caratteri esadecimali). '
