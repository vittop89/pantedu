...
final class ClasseKeyService
{
    ...
    {
        $this->requireKms();
        // CANONICALIZZA i codici per lookup e risoluzione FK (es. '__TEST__',
        // '1A'). La forma usata nel salt HKDF e' invece garantita da deriveCkek,
        // che canonicalizza da se': la VIEW classe_keys puo' restituire il codice
        // com'e' scritto in tabella (anche minuscolo), non com'e' stato passato qui.
    ...
    /**
     * CKEK per (indirizzo, classe, anno, key_version), derivata SEMPRE dalla
     * forma canonica dei codici, in qualunque forma arrivino.
     *
     * Il salt HKDF e' case-sensitive; il catalogo no. `curriculum_entries.code`
     * e' utf8mb4_unicode_ci, per cui resolveCurriculumId (`code = '2B'`) puo'
     * agganciare una riga salvata come "2b" — nel catalogo convivono "3B" e
     * "1b". In wrap il chiamante passa il codice canonico ("2B"); in unwrap la
     * VIEW classe_keys restituisce quello memorizzato ("2b"): due salt diversi,
     * class_key_unwrap_failed, e quella classe non ha piu' una chiave usabile.
     * Canonicalizzare qui, nell'unico punto in cui il salt nasce, rende wrap e
     * unwrap indipendenti da come il codice e' scritto in tabella. Sui codici
     * gia' canonici e' l'identita', quindi le chiavi esistenti restano valide.
     */
    private function deriveCkek(string $ind, string $cls, string $anno, int $kv): string
    {
        $ind = \App\Support\CurriculumLookup::canonicalize('indirizzi', $ind);
        $cls = \App\Support\CurriculumLookup::canonicalize('classi', $cls);
