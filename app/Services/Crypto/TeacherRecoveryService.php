 ...
 *   1) firmare il manifest del pacchetto esportato con HMAC(R, payload), che
 *      l'import verifica con il codice R digitato da chi importa, su questa
 *      istanza o su un'altra (vedi «Verifica import» qui sotto);
 ...
 *
 * Verifica import (verifyManifestHmac):
 ...
 *     Non legge il database né la chiave master. NO check ownership: la
 *     Recovery Key è una chiave fisica condivisibile (modello "passa il PDF
 *     cassaforte").
 *   - Che cosa prova (scritto il 23/9/2026): che il manifest, e con le sue
 *     impronte i file, non l'ha cambiato chi non conosce la R di chi l'ha
 *     esportato. Chi importa digita il codice dal PDF dell'esportatore; se
 *     il manifest è cambiato dopo la firma, o il codice non è quello con cui
 *     è stato firmato, la verifica non passa.
 *   - Che cosa NON prova: chi l'ha esportato, rispetto al server. Il server
 *     non controlla che R sia la chiave del docente `exporter_user_id`, né
 *     che sia una chiave registrata o attiva su questa istanza, né che il
 *     manifest sia uscito dall'export: chiunque abbia una R, anche la
 *     propria, firma un manifest suo con l'esportatore, il nome e la data
 *     che vuole. `exporter_username` nell'anteprima è una dichiarazione del
 *     pacchetto. Revocare o rigenerare la chiave non invalida i pacchetti
 *     già firmati: la verifica non guarda teacher_recovery_keys.
 *   - Perché così: la verifica non dipende dallo stato del server, e servono
 *     tutti e due gli usi previsti dalle guide: portare i propri contenuti
 *     su un'altra istanza (docs/architecture/sync-strategy.md, scenario B) e
 *     reimportarli dopo la perdita o la rotazione della chiave master, che
 *     la Recovery Key esiste per sopravvivere. L'import scrive solo
 *     nell'account di chi importa. Legare la verifica alla chiave salvata
 *     sul server toglieva quei due usi e non fermava un docente con una
 *     chiave sua: scartato il 23/9/2026.
 *
 * Audit:
...
final class TeacherRecoveryService
{
    ...
    {
        // 23/9/2026 (A-36) — stessa lettura e stesse regole di
        // TeacherCryptoService, in ChiaveMadre.
        $chiave = $kmsMasterHex !== null
            ? ChiaveMadre::daValore($kmsMasterHex)
            : ChiaveMadre::dallAmbiente();
        if ($chiave->valida()) {
            $this->kmsMaster = $chiave->byte();
        }
    }
    ...
    public function generate(int $teacherId, ?string $ip = null, ?string $userAgent = null): array
    {
        ...
        // d'integrazione girano dentro la loro e la annullano alla fine, come
        // per MapsController::create (23/9/2026).
     ...
     *
     * Prova che il manifest non l'ha cambiato chi non conosce R; non prova
     * chi l'ha esportato, né che R sia una chiave di questa istanza (il
     * docblock della classe, «Verifica import», dice perché). Senza database
     * e senza chiave master di proposito: funziona su un'altra istanza e
     * dopo la perdita della master. Prove: tests/Unit/Services/Crypto/
     * FirmaDelManifestoTest.php e tests/Integration/FirmaDelPacchettoTest.php.
     *
     * @param string $rHexOrB32 codice R inserito dall'utente (64 hex chars o base32)
     * @param array  $payload   manifest senza campo `hmac`
     * @param string $hmacB64   HMAC da verificare (base64 standard, come computeHmac)
    ...
    private function audit(int $teacherId, string $action, bool $success, ?string $note, ?string $ip, ?string $userAgent): void
    {
        try {
            $stmt = Database::connection()->prepare(
                'INSERT INTO teacher_recovery_audit (user_id, action, ip_hash, ua_hash, success, note)
                 VALUES (?, ?, ?, ?, ?, ?)'
            );
            // I chiamanti passano IP e User-Agent in chiaro; nel registro
            // entrano solo come hash (2026-09-03, vedi RequestFingerprint).
            $stmt->execute([
                $teacherId,
                $action,
                \App\Services\Audit\RequestFingerprint::ipHash($ip),
                \App\Services\Audit\RequestFingerprint::uaHash($userAgent),
                $success ? 1 : 0,
                $note,
            ]);
        } catch (\Throwable $e) {
            // Best-effort, ma non muto: la migration 037 aveva droppato
            // `teacher_recovery_audit` come "scaffold mai utilizzata", e da
            // allora generate/revoke di una chiave di recupero finivano qui,
            // in silenzio. La 098 ricrea la tabella; se sparisce di nuovo,
            // adesso lo si legge.
            \error_log('[TeacherRecoveryService] audit ' . $action . ' fallita: ' . $e->getMessage());
        }
    }
