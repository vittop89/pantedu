...
final class InstituteMergeService
{
    ...
    public function merge(int $canonicalId, int $duplicateId, ?string $adoptCode = null): array
    {
        ...
        try {
            ...
            // curriculum_entries — UNIQUE(kind, code, institute_id) (ADR-035, migrazione 116).
    ...
    {
        $pdo = $this->db();
        // Mappa delle righe canoniche per chiave logica (kind|code|corso di un
        // anno): dal 15/9/2026 (ADR-042) «3» dello scientifico e «3»
        // dell'artistico sono due voci, e fonderle ne perderebbe una.
        $chiave = static fn(array $r): string => $r['kind'] . '|' . $r['code'] . '|'
            . ($r['kind'] === 'classi' && \App\Domain\ClassCode::isAnno((string)$r['code']) ? (string)($r['indirizzo'] ?? '') : '');
        $canon = [];
        $stmt = $pdo->prepare(
            'SELECT id, kind, code, indirizzo FROM curriculum_entries WHERE institute_id = ?'
        );
        $stmt->execute([$canonical]);
        foreach ($stmt->fetchAll(PDO::FETCH_ASSOC) as $r) {
            $canon[$chiave($r)] = (int)$r['id'];
        }
        ...
        foreach ($dupRows as $r) {
            ...
            if (isset($canon[$key])) {
                ...
                // ADR-035 — le spunte dei docenti (curriculum_teacher) hanno
                // un unico su (voce, docente): chi ha spuntato la voce su
                // entrambi gli istituti collide sul ri-puntamento. La spunta
                // sul canonico c'è già, quella sul duplicato si toglie.
                if ($this->hasColumn('curriculum_teacher', 'curriculum_id')) {
                    $this->run(
                        'DELETE d FROM curriculum_teacher d
                           JOIN curriculum_teacher c ON c.user_id = d.user_id AND c.curriculum_id = ?
                          WHERE d.curriculum_id = ?',
                        [$canonRowId, $dupRowId]
                    );
                }
     ...
     */
    // Pubblico perche' non e' logica di merge ma lettura dello schema, e
    // serve a chiunque debba ri-puntare riferimenti prima di cancellare una
    // riga: gli strumenti di bonifica del curriculum hanno lo stesso
    // problema e non devono ricostruirsi l'elenco a mano, che e' il modo in
    // cui si dimentica una tabella e si scopre l'errore da una FK violata.
    public function referrersTo(string $table): array
    {
