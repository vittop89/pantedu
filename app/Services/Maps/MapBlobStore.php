...
{
    /**
     * Il messaggio dell'eccezione quando il file del blob non c'è. Chi riconosce
     * una mappa orfana lo confronta con questa costante, non con una stringa
     * scritta a mano: fino al 15/9/2026 MapSyncService cercava
     * `blob_store_not_found`, che è il messaggio di EncryptedBlobStore, e le
     * orfane risultavano errori del giro.
     */
    public const NON_TROVATO = 'map_blob_not_found';
    ...
    public function get(int $ownerTeacherId, string $relPath): string
    {
        ...
        if (!is_file($absPath)) {
            throw new RuntimeException(self::NON_TROVATO);
        }
    ...
    /**
     * Copia il file cifrato così com'è, senza decifrarlo, in `$destinazione`
     * (un percorso assoluto fuori da maps_enc). Serve a chi sta per
     * sovrascrivere un disegno e vuole poter tornare indietro: la copia si
     * rimette al suo posto con un cp. Fallisce se la copia non è identica.
     */
    public function copiaCifrata(string $relPath, string $destinazione): void
    {
        $this->guardPath($relPath);
        $sorgente = $this->rootDir . '/' . $relPath;
        if (!is_file($sorgente)) {
            throw new RuntimeException(self::NON_TROVATO);
        }
        $cartella = dirname($destinazione);
        if (!is_dir($cartella) && !@mkdir($cartella, 0o770, true) && !is_dir($cartella)) {
            throw new RuntimeException('map_blob_copy_mkdir_failed');
        }
        if (!@copy($sorgente, $destinazione) || hash_file('sha256', $sorgente) !== hash_file('sha256', $destinazione)) {
            throw new RuntimeException('map_blob_copy_failed');
        }
    }
