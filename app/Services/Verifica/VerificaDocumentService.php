...
final class VerificaDocumentService
{
    ...
    public function saveTex(array $input): array
    {
        ...
        try {
            $id = $this->tx->run(fn(): int => $this->docs->create([
                ...
                'tex_size'      => strlen($tex),
                'tex_sha256'    => $texSha,
                'source_type'   => self::classifyProblems((array)($input['problems'] ?? [])),
            ]));
        } catch (Throwable $e) {
    ...
    /**
     * Allega un PDF caricato (compilazione esterna: TeXworks, VSCode).
    ...
    /**
     * @param ?int $scuola la scuola attiva della barra (ADR-037, fase 3): con,
     *                     le sigle si confrontano sulle pubblicazioni in quella
     *                     scuola; senza, sulla riga in tutte.
     */
    public function listForTeacher(
        ...
        return $this->docs->listForTeacher($teacherId, $materia, null, $indirizzo, $classe, $scuola);
    }
     ...
     *
     * Le chiavi, con quel che significano (2026-09-08: le descrizioni stavano
     * DENTRO le graffe della forma, e da lì non si possono leggere — la forma
     * risultava malformata e il controllore non la usava affatto):
     *
     *   - `selection`         payload di Selection
     *   - `template_context`  segnaposto del modello
     *   - `compensa`          casella «Compensa»
     *   - `includeGriglia`    casella «griglie»
     *   - `includeMisure`     casella «misure»
     *   - `variants`          forzatura esplicita, es. ['A_SOL','A_NOR']
     *
     * @param array{
     *   teacher_id: int,
     *   materia: string,
    ...
    public function saveBatch(array $input): array
    {
        ...
        // Phase 25.P.3 — fonte (copyright) della verifica dai quesiti selezionati:
        // stessa regola del contratto (ContractAggregate::classifyItems). Fino al
        // 2026-09-05 restava NULL e nessuna verifica salvata dallo studio si
        // poteva condividere («non classificata»).
        $sourceType = self::classifyProblems((array)$sel->problems);
    ...
    /**
     * Fonte della verifica (personal | book_textbook | mixed | null) dai quesiti
     * dei problemi selezionati: ogni item porta origin/source/badge come nel
     * contratto, quindi vale la stessa regola.
     *
     * @param array<int, mixed> $problems
     */
    private static function classifyProblems(array $problems): ?string
    {
        $items = [];
        ...
        return \App\Services\Contract\ContractAggregate::classifyItems($items)['source_type'];
    }
