...
final class ExerciseInserter
{
    ...
    public function run(
        ...
        // Il contratto del documento nuovo sta nell'istituto attivo, come per i
        // contenuti creati da POST /api/teacher/content: è la scuola in cui
        // create() risolve indirizzo, classe e materia. Fino al 23/9/2026 qui
        // c'era la casa dei file privati (A-7): per un docente di due scuole,
        // con la seconda attiva, la riga nasceva nella seconda e il contratto
        // sotto `institutes/{prima}/`. I file di lavoro della sessione restano
        // nella casa (PdfImportController::createSession).
        $iid = TeacherContextResolver::activeInstituteId($teacherId);
        if ($iid <= 0) {
    ...
    {
        // AI Act art. 50(2) — marcatura di provenienza. Presente SOLO se la riga
        // contiene campi prodotti da operazioni generative (soluzioni, argomenti,
        // traduzioni); assente per contenuto solo trascritto dalla pagina.
        // Vedi AiProvenance per la distinzione assistivo/generativo.
        $ai = AiProvenance::itemBlock($row);

        return ($ai !== null ? ['ai' => $ai] : []) + [
