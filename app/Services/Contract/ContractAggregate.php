...
final class ContractAggregate
{
    public function __construct(
        ...
        // pagina. Il blocco sta nella scrittura: dal 23/9/2026
        // ContractRepository::save() rifiuta gli errori di schema che il
        // contratto di prima non aveva (ADR-005, A-76).
        if ($data !== []) {
    ...
    public function classifyShareability(): array
    {
        ...
        foreach ($this->groups() as $g) {
            foreach ((array)($g['items'] ?? []) as $it) {
                $items[] = is_array($it) ? $it : [];
            }
        }
        return self::classifyItems($items);
    }

    /** Valore di origin che dichiara un quesito proprio del docente. */
    public const ORIGIN_PERSONAL = 'personal';
    /** Valore di origin per «fonte non dichiarata» (select ORIGINE non compilata). */
    public const ORIGIN_UNKNOWN = 'unknown';

    /**
     * Chiave di fonte di un quesito: il primo valore NON vuoto fra item.origin,
     * item.source e item.badge.source_key (la select ORIGINE scrive origin;
     * l'import dal registro scrive badge.source_key; source è il campo legacy).
     */
    public static function itemSourceKey(array $item): string
    {
        $badge = is_array($item['badge'] ?? null) ? $item['badge'] : [];
        foreach ([$item['origin'] ?? null, $item['source'] ?? null, $badge['source_key'] ?? null] as $v) {
    ...
    /**
     * Classificazione di fonte di una lista di quesiti (contratto o selezione
     * di una verifica): la regola è UNA, qui.
     *
     *   - chiave vuota o 'personal'  → quesito proprio
     *   - 'unknown'                  → fonte non dichiarata
     *   - qualsiasi altra chiave     → dal libro di testo (registro fonti)
     *
     *   source_type:
     *   - 'personal'      → solo quesiti propri
     *   - 'book_textbook' → dal libro, nessuno proprio
     *   - 'mixed'         → dal libro + propri
     *   - null            → nessun quesito, oppure propri + non dichiarati
     *                       (resta «non classificato»: il docente deve dichiarare
     *                       la fonte prima di condividere)
     *
     * Fino al 2026-09-05 qualsiasi origin non vuoto (anche 'personal' e
     * 'unknown') contava come libro, mentre il badge del registro veniva
     * ignorato se source era la stringa vuota: gli esercizi del wizard
     * finivano bloccati dal copyright e quelli importati passavano.
     *
     * @param iterable<array<string, mixed>> $items
     * @return array{source_type: ?string, items_personal: int, items_from_book: int, items_unknown: int, items_total: int}
     */
    public static function classifyItems(iterable $items): array
    {
        $totalItems = 0;
        $itemsFromBook = 0;
        $itemsPersonal = 0;
        ...
        foreach ($items as $it) {
            $totalItems++;
            $key = strtolower(self::itemSourceKey(is_array($it) ? $it : []));
            if ($key === '' || $key === self::ORIGIN_PERSONAL) {
                $itemsPersonal++;
            } elseif ($key === self::ORIGIN_UNKNOWN) {
                $itemsUnknown++;
            } else {
                $itemsFromBook++;
            }
        ...
        $type = null;
        if ($itemsFromBook > 0) {
            $type = $itemsPersonal > 0 ? 'mixed' : 'book_textbook';
        } elseif ($itemsUnknown === 0 && $totalItems > 0) {
            $type = 'personal';
        }
