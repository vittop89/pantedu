...
final class FileService
{
    ...
    private array $roots;
    /** @var list<string> le radici versionate: si leggono e basta */
    private array $solaLettura;
    ...
    public function __construct(?array $config = null)
    {
        ...
        $this->solaLettura        = $cfg['roots_sola_lettura'] ?? [];
        $this->allowedExtensions  = $cfg['allowed_extensions'] ?? [];
        $this->maxSizes           = $cfg['max_sizes']          ?? [];
    }

    /**
     * Le radici versionate si leggono, non si scrivono.
     *
     * `img/` è dentro l'immagine del container: una scrittura lì riesce, e
     * sparisce al rilascio; una `delete` toglie davvero un file che il
     * rilascio successivo rimette. Meglio un errore adesso.
     */
    private function vietaLaScrittura(string $rootLabel): void
    {
        if (in_array($rootLabel, $this->solaLettura, true)) {
            throw new RuntimeException('read_only_root');
        }
    }
    ...
    {
        $this->vietaLaScrittura($rootLabel);
     ...
     * img/verifiche_temp/storage_backups.
     */
