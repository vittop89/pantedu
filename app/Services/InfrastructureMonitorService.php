...
/* lascia null */
    ...
    private function counts(): array
    {
        $out = ['users_total' => 0, 'users_teacher' => 0, 'users_student' => 0,
                'institutes' => 0, 'materials' => 0, 'pending_regs' => $this->domandeInAttesa()];
        if (!Database::isAvailable()) {
            return $out;
        }
        try {
...
/* best-effort */
    ...
    /**
     * Le domande di iscrizione in attesa, dal file dove stanno davvero.
     *
     * 2026-09-24 — si contavano con `SELECT COUNT(*) FROM registrations`, una
     * tabella che nessun codice scrive: la dashboard diceva 0 anche con dieci
     * domande in coda. Le domande stanno in `auth.paths.registrations`.
     */
    private function domandeInAttesa(): int
    {
        $percorso = (string)Config::get('auth.paths.registrations', '');
        if ($percorso === '' || !is_file($percorso)) {
            return 0;
        }
        $dati = json_decode((string)@file_get_contents($percorso), true);
        // Solo quelle nel termine: una domanda scaduta resta nel file fino al
        // giro notturno, ma non è più da decidere (24/9/2026).
        return \is_array($dati)
            ? \count(\App\Services\Gdpr\ConservazioneDelleIscrizioni::domandeInTermine($dati))
            : 0;
    }
