...
final class ContractRenderer
{
    ...
    public function renderContract(array $contract): string
    {
        ...
        // AI Act art. 50(2) — marcatura a livello di pagina, derivata dagli item
        // (non da un flag in DB: così vale per tutte e tre le vie di
        // inserimento di PDF-Import, non solo per i documenti che crea da zero).
        $h .= $this->renderAiPageMarking($contract);
    ...
    /**
     * Un esercizio preso da un libro (il badge porta la chiave del libro), a
     * chi non può modificarlo: mai la traccia (24/9/2026, Termini 1.6).
     *
     * Agli studenti arrivano il riferimento (libro, pagina, numero,
     * difficoltà, nel badge) e la soluzione scritta dal docente. Fino a quel
     * giorno la garanzia dipendeva da come il docente aveva copiato
     * l'esercizio: la copia «solo fonte» toglieva la traccia, quella
     * «completa» no, e una traccia scritta a mano nel campo arrivava agli
     * studenti così com'era. Adesso il campo non conta: qui c'è il segnaposto.
     * Le opzioni di una domanda a scelta fanno parte della traccia: restano
     * le lettere e quale è giusta (la soluzione), non il testo.
     *
     * @param array<string,mixed> $it
     * @return array<string,mixed>
     */
    public static function senzaLaTracciaDelLibro(array $it): array
    {
        $badge = $it['badge'] ?? null;
        if (!\is_array($badge) || trim((string)($badge['source_key'] ?? '')) === '') {
            return $it;
        }
        $it['question'] = [['type' => 'text', 'content' => \App\Repositories\Contract\ContractRepository::SOURCE_PLACEHOLDER]];
        if (!empty($it['options']) && \is_array($it['options'])) {
            $n = 0;
            foreach ($it['options'] as $k => $opzione) {
                $n++;
                if (\is_array($opzione)) {
                    $it['options'][$k]['content'] = [['type' => 'text', 'content' => 'Es' . $n]];
                }
            }
        }
        return $it;
    ...
    private function renderItem(array $it, string $groupType, int $idx = 0, string $groupId = ''): string
    {
        $esc = fn(?string $s) => htmlspecialchars((string)$s, ENT_QUOTES, 'UTF-8');
        if (!$this->canEdit) {
            $it = self::senzaLaTracciaDelLibro($it);
        }
        ...
        // AI Act art. 50(2) — marcatura leggibile da macchina degli output
        // sintetici. `$it['ai']` è scritto da ExerciseInserter solo per i campi
        // prodotti da operazioni generative (cfr. AiProvenance).
        $aiFields = self::aiFields($it);
        $aiAttr = $aiFields === []
            ? ''
            : ' data-ai-generated="true" data-ai-fields="' . $esc(implode(',', $aiFields)) . '"';

        // Il gancio per CSS/JS è `data-ai-generated`, non una classe: una
        // classe in più sarebbe stata solo un secondo nome per lo stesso fatto
        // (e `fm-ai-generated` violava pure il selector-class-pattern).
        $h  = '<div class="fm-collection__item diff' . $diff . ($src ? ' ' . $src : '')
            . '" data-id="' . $id . '"' . $aiAttr . '>';
        ...
        // Il marcatore IA condivide la badge-row quando c'è un badge; se l'item
        // non ha badge la row viene emessa comunque, altrimenti la marcatura
        // visibile sparirebbe proprio sugli item senza numero d'esercizio.
        if (!empty($it['badge']) || $aiFields !== []) {
            $h .= '<span class="fm-badge-row">';
            if (!empty($it['badge'])) {
                $h .= $this->renderBadge($it['badge']);
            }
            if ($aiFields !== []) {
                $h .= $this->renderAiMarker($aiFields);
            }
            $h .= '</span>';
        }
     ...
     */
    // ─────────────────────────────────────────────────────────────────────
    // AI Act art. 50(2) — marcatura degli output sintetici.
    //
    // Non esiste (ago 2026) uno standard armonizzato per il watermarking del
    // testo: l'art. 50(7) affida all'AI Office la facilitazione dei codici di
    // condotta e nessuno è ad oggi vincolante. La norma chiede soluzioni
    // "efficaci, interoperabili, solide e affidabili nella misura in cui ciò
    // sia tecnicamente possibile", tenuto conto dei costi e dello stato
    // dell'arte. Qui: attributi data-* sull'item + <meta> di pagina + JSON-LD
    // schema.org. La convenzione è documentata in docs/legal/ai-act-assessment.md.
    // ─────────────────────────────────────────────────────────────────────

    /** Etichette italiane dei campi marcati, per la disclosure visibile. */
    private const AI_FIELD_LABELS = [
        'solution'       => 'soluzione',
        'category_label' => 'argomento',
        'question'       => 'testo tradotto',
    ];

    /**
     * Campi generati dell'item, normalizzati. Vuoto = niente da marcare.
     *
     * @return list<string>
     */
    private static function aiFields(array $it): array
    {
        $ai = $it['ai'] ?? null;
        if (!is_array($ai) || empty($ai['generated'])) {
            return [];
        }
        $out = [];
        foreach ((array)($ai['fields'] ?? []) as $f) {
            if (is_string($f) && $f !== '') {
                $out[] = $f;
            }
        }
    ...
    /** Marcatore visibile accanto al badge. */
    private function renderAiMarker(array $fields): string
    {
        $esc = fn(?string $s) => htmlspecialchars((string)$s, ENT_QUOTES, 'UTF-8');
        $labels = [];
        foreach ($fields as $f) {
            $labels[] = self::AI_FIELD_LABELS[$f] ?? $f;
        }
        $what = $labels === [] ? 'contenuto' : implode(', ', $labels);
        $text = 'Generato da IA (' . $what . '), rivisto dal docente';

        return '<span class="fm-aimark" role="note" title="' . $esc($text) . '"'
             . ' aria-label="' . $esc($text) . '">'
             . '<span class="fm-aimark__tag" aria-hidden="true">IA</span>'
             . '</span>';
    }

    /**
     * Marcatura di pagina: <meta> + JSON-LD schema.org con l'elenco degli item
     * generati. Stringa vuota se il contract non contiene nulla di generato.
     */
    private function renderAiPageMarking(array $contract): string
    {
        $esc = fn(?string $s) => htmlspecialchars((string)$s, ENT_QUOTES, 'UTF-8');
        ...
        foreach ($groups as $g) {
            ...
            // L'id DEVE coincidere con il data-id emesso da renderItem, altrimenti
            // il JSON-LD non è correlabile al DOM e la marcatura perde di senso.
            // Fallback replicato alla lettera da renderGroup/renderItem.
            $groupId = (string)($g['id'] ?? 'g');
        ...
        $jsonLd = [
            ...
            // Nessun termine schema.org stabile per la provenienza IA: si usa
            // una proprietà esplicita, documentata nell'assessment.
        ...
        return '<meta name="fm-ai-generated" content="partial">'
             . '<meta name="fm-ai-generated-fields" content="' . $esc(implode(',', $fields)) . '">'
