 ...
 *          nel database va il suo hash, il token solo nell'email
...
final class ParentConsentService
{
     ...
     * chiaro, e chi leggeva il database — un dump, una copia locale, un
     * backup — poteva dare al posto del genitore il consenso dell'art. 8, o
     * rifiutarlo e far cancellare l'account del minore.
     *
     * @return string token 64 hex char: l'unica copia in chiaro, per il
     *                collegamento. Non si ricostruisce dal database: chi deve
     *                rimandarlo chiede una richiesta nuova.
     */
    public function request(int $studentUserId, string $parentEmail, ?string $parentName = null): string
    {
        ...
        $stmt->execute([
            ...
            $parentName ?: null, self::hashDelGettone($token), $expires,
        ]);
    ...
    /**
     * La richiesta a cui porta un collegamento, con il nome dello studente,
     * per la pagina che il genitore apre (GET /parent-consent/{token}).
     *
     * Stava nel controller, con la sua SELECT per `confirm_token = ?`: dal
     * 23/9/2026 la colonna tiene l'hash, e l'hash si calcola in un posto solo,
     * qui. Nessun filtro sullo stato: la pagina dice anche «gia' confermato»,
     * «revocato», «scaduto».
     *
    ...
    /**
     * Il gettone come si conserva: SHA-256 in esadecimale, 64 caratteri, la
     * stessa funzione di `password_resets.token_hash` e
     * `email_change_requests.token_hash`. Sta nella colonna `confirm_token`
     * (VARCHAR(64)) che prima teneva il gettone: la migrazione 139 ha ritirato
     * i gettoni in chiaro rimasti.
     */
