...
class TosAcceptanceService
{
    ...
    public function effectiveVersions(): array
    {
        ...
        if ($this->pdo !== null) {
            try {
                ...
                // Più righe per documento quando due versioni entrano in
                // vigore lo stesso giorno: sceglie prevalenti(), non l'ordine
                // di lettura.
                foreach (self::prevalenti($stmt->fetchAll(PDO::FETCH_ASSOC) ?: []) as $type => $row) {
                    $out[$type] = $row['version'];
                    $this->effectiveFrom[$type] = $row['effective_from'];
                }
            } catch (Throwable $e) {
    ...
    public function targetVersions(): array
    {
        ...
        foreach (self::prevalenti($this->pendingVersions()) as $type => $v) {
    ...
    /**
     * Per ciascun documento (`tos`, `aup`), la riga che prevale fra quelle
     * date: la `effective_from` più recente e, a parità di data, il numero di
     * versione più alto, confrontato come versione (1.10 viene dopo 1.9) e non
     * come stringa.
     *
     * 23/9/2026. Prima vinceva l'ultima riga letta, e le query non hanno
     * ORDER BY. Con due versioni in vigore dalla stessa data (i Termini 1.4 e
     * 1.5 dal 22/9, la 1.1 e la 1.2 dal 1/9) la versione vincolante dipendeva
     * dall'ordine in cui il database restituiva le righe: misurato su MariaDB,
     * inserite al contrario davano la 1.4.
     *
     * Le date si confrontano come stringhe: vengono tutte dalla stessa colonna
     * DATETIME, nello stesso formato `AAAA-MM-GG hh:mm:ss`.
     *
     * @param iterable<array<string, mixed>> $righe con doc_type, version, effective_from
     * @return array<string, array{version: string, effective_from: string}>
     */
    private static function prevalenti(iterable $righe): array
    {
        $scelte = [];
        foreach ($righe as $riga) {
            $tipo = (string)($riga['doc_type'] ?? '');
            if ($tipo !== 'tos' && $tipo !== 'aup') {
                continue;
            }
            $candidata = [
                'version'        => (string)($riga['version'] ?? ''),
                'effective_from' => (string)($riga['effective_from'] ?? ''),
            ];
            $attuale = $scelte[$tipo] ?? null;
            if ($attuale === null) {
                $scelte[$tipo] = $candidata;
                continue;
            }
            $perData = strcmp($candidata['effective_from'], $attuale['effective_from']);
            if (
                $perData > 0
                || ($perData === 0 && version_compare($candidata['version'], $attuale['version'], '>'))
            ) {
                $scelte[$tipo] = $candidata;
            }
        }
