 ...
 *   4. uploaderContact() + markUploaderNotified() → uploader avvisato
 *
 * NOTA: il service resta CRUD su DB. L'invio vero è in
 * AdminTakedownController::notifyUploader(), che usa Mailer (Resend): qui
 * l'unica cosa che serve al chiamante è il recapito e il flag di avvenuta
 * notifica, così il service resta testabile senza transport email.
 */
class TakedownRequestService
{
    ...
    /**
     * Recapito dell'uploader del contenuto contestato, per la notifica di
     * Fase 4 (docs/legal/takedown_procedure.md §3).
     *
     * Ritorna null quando la notifica automatica non è possibile: segnalazione
     * inesistente, `uploader_user_id` non valorizzato (il segnalante indica un
     * URL, non sempre si risale all'autore), utente cancellato (la FK è
     * ON DELETE SET NULL) o email mancante. In quei casi resta la procedura
     * manuale via abuse@ e `notified_uploader` NON va marcato.
     *
    ...
    public function uploaderContact(int $requestId): ?array
    {
        $stmt = $this->pdo()->prepare(
            'SELECT u.email, u.username, u.first_name, u.last_name
               ...
               JOIN users u ON u.id = t.uploader_user_id
              WHERE t.id = :id
              LIMIT 1'
        );
        $stmt->execute([':id' => $requestId]);
