 ...
 *   - Delete-orphan: cancella i .tex presenti su Drive ma assenti in DB,
 *     solo nelle cartelle `verifiche/` del docente (dal 23/9/2026, A-83)
 *
...
final class VerificaSyncService
{
    ...
    public function syncOne(int $docId, int $teacherId): array
    {
        ...
        try {
            ...
            if ($hasMultiFile) {
                $tex = $this->servizioDocumenti()->readTex($teacherId, $docId);
            } else {
     ...
     * scope `drive.file` non più presenti in DB (per quel teacher), nelle sue
     * cartelle `verifiche/` e in nessun altro posto (deleteOrphansFromDrive).
     *
    ...
    public function syncAllForTeacher(int $teacherId, ?int $limit = null, bool $onlyChanged = false, bool $deleteOrphans = true): array
    {
        ...
        foreach ($ids as $id) {
            ...
            } elseif (in_array($r['error'] ?? '', MapSyncService::FERMANO_IL_LOTTO, true)) {
                // Vale per tutto il docente (ADR-038): le verifiche che restano fallirebbero allo stesso modo.
        ...
        if ($deleteOrphans && !isset($report['fermato']) && $this->oauthRepo->isConnected($teacherId)) {
            try {
     ...
     *
     * 23/9/2026 (A-83) — il filtro sulle cartelle, promesso qui sopra, non
     * c'era: si cancellava ogni `.tex` creato dall'app per quell'account
     * Google e sconosciuto a questo docente. Con `drive.file` di solito
     * coincide con le sue verifiche, ma un account Google collegato a due
     * docenti perdeva quelle dell'altro, e un `.tex` dell'app fuori da
     * `verifiche/` spariva comunque. Adesso un file si cancella solo se una
     * delle sue cartelle è una `verifiche/` del docente o una sua
     * sottocartella, come le conosce `teacher_drive_folder_cache` (la cache di
     * FolderTreeBuilder, per docente). Nel dubbio si lascia: una cartella che
     * la cache non conosce più (si svuota a ogni nuovo collegamento) tiene i
     * suoi file finché una sincronizzazione non la risolve di nuovo. Un file
     * rimasto su Drive si cancella a mano; uno cancellato per sbaglio no.
     *
    ...
    private function deleteOrphansFromDrive(int $teacherId): array
    {
        ...
        $cartelle = $this->cartelleVerificheDelDocente($teacherId);
        if ($cartelle === []) {
            return [];
        }

        // Lista dei .tex creati dall'app (scope `drive.file`); il filtro sulle
        // cartelle `verifiche/` del docente si fa qui sotto, sui `parents`.
        $drive = $this->driveClient->getDriveFor($teacherId);
        $deleted = [];
        try {
            ...
            do {
                ...
                foreach ($resp->getFiles() as $f) {
                    ...
                    $nelleVerifiche = false;
                    foreach ((array)$f->getParents() as $genitore) {
                        if (isset($cartelle[(string)$genitore])) {
                            $nelleVerifiche = true;
                            break;
                        }
                    }
    ...
    /**
     * Gli id Drive delle cartelle `verifiche/` del docente e di quelle sotto
     * (titolo, versione), dalla cache di FolderTreeBuilder. Il percorso è
     * quello di BundlePathBuilder::folderPath():
     * `{istituto}/{indirizzo}/{classe}/{materia}/verifiche/...`, quindi la
     * sezione è il quinto segmento; un «verifiche» altrove (un titolo, una
     * cartella delle mappe) non conta.
     *
     * @return array<string, true>
     */
    private function cartelleVerificheDelDocente(int $teacherId): array
    {
        $stmt = Database::connection()->prepare(
            'SELECT folder_path, drive_folder_id FROM teacher_drive_folder_cache WHERE teacher_id = ?'
        );
        $stmt->execute([$teacherId]);
        $cartelle = [];
        foreach ($stmt->fetchAll(PDO::FETCH_ASSOC) as $r) {
            $segmenti = explode('/', trim((string)$r['folder_path'], '/'));
            if (($segmenti[4] ?? '') === 'verifiche' && (string)$r['drive_folder_id'] !== '') {
                $cartelle[(string)$r['drive_folder_id']] = true;
            }
        }
        return $cartelle;
    }
    ...
    /**
     * Il servizio che ricompone il .tex di una verifica a più file, con lo stesso
     * deposito cifrato di questo servizio.
     *
     * Fino al 15/9/2026 il deposito si passava come terzo argomento, che è la
     * transazione: ogni verifica a più file finiva in un errore di tipo
     * («Argument #3 ($tx) must be of type ?TransactionRunner»), misurato nel
     * registro di produzione. PHPStan lo segnalava, e la segnalazione stava nella
     * baseline.
     */
    private function servizioDocumenti(): \App\Services\Verifica\VerificaDocumentService
    {
        return new \App\Services\Verifica\VerificaDocumentService(null, $this->blobStore);
    }
