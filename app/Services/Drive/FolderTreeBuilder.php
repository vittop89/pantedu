...
final class FolderTreeBuilder
{
    ...
    private function resolveRoot(): string
    {
        ...
        // Prima la radice marcata `fm_root`, dovunque il docente l'abbia
        // spostata; poi, per le radici di prima del marcatore, per nome in
        // «Il mio Drive». Fino al 15/9/2026 si cercava solo per nome in «Il mio
        // Drive»: dopo un nuovo collegamento, una cartella spostata non si
        // ritrovava e se ne creava un'altra.
        $existing = $this->findMarkedRoot() ?? $this->findFolderInParent('root', $rootName);
        if ($existing === null) {
    ...
    /**
     * La radice creata dall'app (`appProperties.fm_root = 1`), in qualunque
     * cartella stia. Con più radici, la più vecchia: è quella con le cartelle.
     */
    private function findMarkedRoot(): ?string
    {
        $resp = $this->drive->files->listFiles([
            'q'        => sprintf(
                "mimeType='%s' and trashed=false and appProperties has { key='fm_root' and value='1' }",
                self::FOLDER_MIME
            ),
        ...
        $files = $resp->getFiles();
        return \count($files) === 0 ? null : (string)$files[0]->getId();
    }
