...
final class DriveClient
{
    ...
    /** @var callable|null il gestore HTTP di Guzzle; le prove ne passano uno che risponde al posto di Google */
    private $gestoreHttp;

    public function __construct(?DriveOAuthRepository $repo = null, ?callable $gestoreHttp = null)
    {
        $this->repo = $repo ?? new DriveOAuthRepository();
        $this->gestoreHttp = $gestoreHttp;
    }
    ...
    public function buildAuthUrl(string $state, array $scopes): string
    {
        ...
        // Fino al 15/9/2026 qui c'era setApprovalPrompt('force'), cioè il
        // parametro vecchio `approval_prompt`, che Google ignora: misurato in
        // produzione, scollegare e ricollegare dava «drive_oauth_no_refresh_token»
        // e il docente restava senza collegamento.
     ...
     *
     * Dal 14/9/2026 (ADR-038) guarda prima lo stato:
     *   - Drive spento o guasto sull'installazione: drive_spento, drive_guasto;
     *   - collegamento già da ricollegare: DriveDaRicollegare, senza chiamare
     *     Google;
     *   - Google rifiuta il rinnovo (invalid_grant), o concede il token senza
     *     il permesso su Drive: il collegamento si segna da ricollegare, poi
     *     DriveDaRicollegare. Ogni altro errore resta quello che è.
     */
    public function getDriveFor(int $teacherId): GoogleDriveService
    {
        $istanza = StatoDiDrive::attuale();
        if ($istanza !== StatoDiDrive::ACCESO) {
            throw new RuntimeException('drive_' . $istanza);
        }
        $collegamento = $this->repo->getMetadata($teacherId);
        if ($collegamento === null) {
            throw new RuntimeException('drive_oauth_not_connected');
        }
        if ($collegamento['stato'] !== 'attivo') {
            throw new DriveDaRicollegare((string)$collegamento['stato_motivo']);
        }
        ...
        try {
            ...
            $motivo = StatoDiDrive::motivoDaRicollegare($e);
            if ($motivo === null) {
                throw $e;
            }
            $this->repo->segnaDaRicollegare($teacherId, $motivo);
            throw new DriveDaRicollegare($motivo, $e);
        }

        // Il rinnovo dice quali permessi valgono adesso. Senza il campo non si
        // deduce niente: decide la prima chiamata a Drive.
        $concessi = is_array($token) && is_string($token['scope'] ?? null) ? $token['scope'] : '';
        if ($concessi !== '' && !StatoDiDrive::permessoConcesso($concessi)) {
            $this->repo->segnaDaRicollegare($teacherId, StatoDiDrive::PERMESSI_INSUFFICIENTI);
            throw new DriveDaRicollegare(StatoDiDrive::PERMESSI_INSUFFICIENTI);
        }
    ...
    private function newGoogleClient(): GoogleClient
    {
        ...
        // Il bundle delle CA di App\Support\BundleCa (CA_BUNDLE, poi quello
        // di sistema di Linux); senza, Guzzle usa il suo (23/9/2026, A-42:
        // prima il default di `drive.ca_bundle` era un percorso XAMPP di
        // Windows).
        ...
        if ($this->gestoreHttp !== null) {
            $opzioni['handler'] = $this->gestoreHttp;
        }
        $client->setHttpClient(new GuzzleClient($opzioni));
    ...
    private function resolveRedirectUri(): string
    {
        ...
        // La radice passa da IndirizzoPubblico (23/9/2026): la regola è una,
        // e senza `app.url` si lancia come prima, in più con l'anomalia.
        return \App\Support\IndirizzoPubblico::radice('drive_oauth') . '/teacher/drive/callback';
    }
}
