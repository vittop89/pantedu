...
final class TikzRenderClient
{
     ...
     * `errors`: gli errori di pdflatex (ErroriTex), vuota se il disegno è
     * riuscito o se è fallito per altro (tempo, rete, dvisvgm). Dal 24/9/2026
     * il servizio li manda; con un servizio di prima si ricavano dal log.
     *
     * @return array{ok:bool, svg:?string, log:string, errors:list<array{line:?int, message:string, context:string}>, http_status:int, duration_ms:?int, tempo_scaduto?:bool}
     */
    public function render(
        ...
        if ($errno !== 0 || $rawResponse === false) {
            // Al docente arriva questo `log` (TikzRenderController): l'errore di
            // cURL, con l'indirizzo interno, resta nel registro.
            return [
                'ok'          => false,
                'svg'         => null,
                'log'         => TexIrraggiungibile::segnala($errno, $this->endpoint, 'render-tikz', $connesso),
                'errors'      => [],
                'http_status' => 0,
                'duration_ms' => null,
        ...
        if (is_array($decoded) && isset($decoded['log'])) {
            ...
            if (is_array($decoded['errors'] ?? null)) {
                $errori = ErroriTex::normalizza($decoded['errors']);
            }
        }
        return [
            'ok'          => false,
            'svg'         => null,
            'log'         => $log,
            'errors'      => $errori ?? ErroriTex::daLog($log),
            'http_status' => $status,
            'duration_ms' => $duration,
        ];
    }
