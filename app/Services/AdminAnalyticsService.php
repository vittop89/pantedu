 ...
 *   - users by role           (administrator/teacher/institute_admin/student)
...
final class AdminAnalyticsService
{
    ...
    {
        // Dati d'istanza, e il percorso lo dice chi scrive il registro:
        // `<dati>/log/data/access_log.json` non lo scrive più nessuno dalla
        // Phase 25.J, quindi la pagina delle statistiche leggeva un file che
        // non esiste e mostrava zero accessi senza dirlo.
        $base = \App\Support\PercorsiDati::base(dirname(__DIR__, 2));
        return new self(accessLogPath: \App\Core\AccessLogger::percorsoDelRegistro($base));
    }
