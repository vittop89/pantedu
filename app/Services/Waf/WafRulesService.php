...
final class WafRulesService
{
    ...
    public static function isRegexConditionSafe(string $pattern): bool
    {
        ...
        //
        // 2026-09-08 — qui c'era `self::safeRegexMatch($pattern, $bad) !== null`.
        // Quel metodo restituisce `bool`, e un booleano non è mai `null`: il
        // confronto era **sempre vero**, cioè questa funzione approvava
        // qualunque pattern, compresi quelli con backtracking catastrofico che
        // doveva rifiutare. Trovato dal controllore statico aggiornato.
        //
        // Non basta guardare il valore di ritorno: `safeRegexMatch` risponde
        // `false` sia quando il pattern non trova niente — che sull'input di
        // prova è l'esito normale — sia quando è esploso. Le due cose si
        // distinguono solo dal codice d'errore di PCRE, che è quello che qui
        // interessa davvero.
        $bad = str_repeat('a', 2048) . '!';
        return self::regexReggeSottoStress($pattern, $bad);
    }

    /**
     * Il pattern regge un input avversariale senza sfondare i limiti di PCRE?
     *
     * Vero se la ricerca si conclude — con o senza riscontro, non importa —
     * senza che PCRE segnali un errore. Falso se sfonda il limite di
     * backtracking o di ricorsione: quello è il blow-up da cui la validazione
     * deve proteggere.
     */
    private static function regexReggeSottoStress(string $pattern, string $subject): bool
    {
        $prevBt = ini_get('pcre.backtrack_limit');
        $prevRc = ini_get('pcre.recursion_limit');
        @ini_set('pcre.backtrack_limit', '50000');
    ...
    /**
     * True se una voce è stata tolta.
     *
     * 23/9/2026 (revisione architetturale, A-5) — prima restituiva l'esito
     * della DELETE, vero anche quando l'id non corrispondeva a niente: il
     * pannello rispondeva «ok» a un'eliminazione mai avvenuta. Qui non c'è
     * un find() come per le regole, e l'unico chiamante è
     * WafAdminController::apiDeleteBlacklist, che con false risponde 404.
     */
    public function deleteBlacklist(int $id): bool
    {
        try {
            $stmt = $this->db()->prepare('DELETE FROM waf_blocked_ips WHERE id = ?');
    ...
    /** True se una voce è stata tolta: vedi {@see self::deleteBlacklist()}. */
    public function deleteWhitelist(int $id): bool
    {
        try {
            $stmt = $this->db()->prepare('DELETE FROM waf_whitelisted_ips WHERE id = ?');
            return $stmt->execute([$id]) && $stmt->rowCount() > 0;
        } catch (\Throwable) {
            return false;
        }
