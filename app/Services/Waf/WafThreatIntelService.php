 ...
 *
 * Usage:
 *   php tools/waf/sync_threat_intel.php [--source=all|asn|spamhaus|x4b|tor]
...
final class WafThreatIntelService
{
    ...
    /*
     * 2026-09-08 — la fonte CrowdSec è stata tolta da qui.
     *
     * `importCrowdSec()` chiamava `api.crowdsec.net/v3/decisions/stream` (il
     * CAPI) con l'intestazione `X-Api-Key`. Ma il CAPI vuole credenziali
     * macchina — `/v3/watchers/login` risponde «Please register prior to
     * login» — mentre `x-api-key` appartiene alla CTI, che sta su un host
     * diverso. Due servizi diversi mescolati: non era configurazione mancante,
     * era una chiamata che non poteva riuscire.
     *
     * Verificato l'8 settembre dal server con una chiave CTI vera:
     * `/v2/smoke/{ip}` risponde 200, `/v2/fire` — l'unico endpoint a forma di
     * elenco — risponde 403 perché non è nel piano gratuito, e questa
     * sincronizzazione rispondeva 401. Falliva ogni due ore da mesi.
     *
     * Non è stata sostituita da un'altra sincronizzazione perché non ce n'è
     * una possibile a questo piano. La chiave però adesso serve: la usa
     * `WafCtiService`, che chiede la reputazione di UN indirizzo quando
     * l'amministratore preme «Chi è» nel pannello. È la domanda a cui la CTI
     * gratuita sa rispondere.
     *
     * Le altre quattro fonti (ASN, Spamhaus, X4B, Tor) sono intatte e
     * riempiono `waf_threat_ips` come prima. Voce 82 del debito.
     */
    ...
    private function fetch(string $url, array $headers = []): string
    {
        ...
        // Il bundle delle CA: una regola sola per tutto il codice
        // (App\Support\BundleCa, 23/9/2026, A-42).
