 ...
 *
 * REGOLA DI MATCHING (decisa una volta: vive in App\Domain\ClassCode, dal
 * 2026-09-05 condivisa con il filtro dei contenuti; qui si applica agli incarichi)
 *
...
{
    /**
     * Anno + sezione opzionale: "1", "1A", "3BS". La regola di corrispondenza
     * vive in ClassCode (2026-09-05): qui restano i punti d'ingresso storici,
     * cosi' pannello e test continuano a chiamare il servizio.
     */
    private const CLASSE_PATTERN = ClassCode::PATTERN;
    ...
    public function assign(
        ...
        CurriculumLookup::ensureEntryForTeacher('classi', $userId, $classe, $instituteId, $indirizzo);
        // ADR-041 — con «solo incaricati» l'incarico è ciò che ammette la
        // sezione: una spunta sospesa si riprende.
        (new SezioniDeiDocenti($this->pdo()))->riallinea($instituteId, $userId);
    }
    ...
    public function revokeSection(int $userId, int $instituteId, string $indirizzo, string $classe): bool
    {
        ...
        // ADR-041 — senza incarico, con «solo incaricati», la spunta si sospende.
        (new SezioniDeiDocenti($this->pdo()))->riallinea($instituteId, $userId);
        return $tolto;
    }

    /**
     * Studenti con account che nessun incarico puo' raggiungere: senza
     * indirizzo, senza classe, o iscritti al solo anno («1») senza sezione.
     * Stessa definizione del pannello sezioni; qui solo il numero, per il
     * riquadro della dashboard nello scenario 3. Con $instituteId null conta
     * su tutti gli istituti.
     */
    public function countStudentsWithoutSection(?int $instituteId = null): int
    {
    ...
    {
        $chi = $this->pdo()->prepare('SELECT user_id, institute_id FROM teacher_sections WHERE id = ?');
        $chi->execute([$assignmentId]);
        $incarico = $chi->fetch(PDO::FETCH_ASSOC);
        $stmt = $this->pdo()->prepare('DELETE FROM teacher_sections WHERE id = ?');
        $stmt->execute([$assignmentId]);
        $tolto = $stmt->rowCount() > 0;
        if (is_array($incarico)) {
            // ADR-041 — senza incarico, con «solo incaricati», la spunta si sospende.
            (new SezioniDeiDocenti($this->pdo()))->riallinea((int)$incarico['institute_id'], (int)$incarico['user_id']);
        }
        return $tolto;
    }
}
