...
final class GeoGebraCatalogService
{
    ...
    {
        // Dati d'istanza (PANTEDU_DATA_PATH): il catalogo non ha nessun
        // riscontro nel database, il file è l'unica copia. Nel container la
        // radice del repository è l'immagine e il catalogo sparirebbe al
        // rilascio senza lasciare traccia.
        $this->basePath = \App\Support\PercorsiDati::base(dirname(__DIR__, 3), $basePath);
    }

    /** Non crea niente: la cartella la fa chi scrive ({@see self::writeCatalog()}).
     *  Prima il mkdir stava qui e bastava una lettura a creare
     *  `storage/objects/teachers/<id>` vuota. */
    private function teacherDir(int $teacherId): string
    {
        if ($teacherId <= 0) {
            throw new RuntimeException('invalid_teacher_id');
        }
        return $this->basePath . '/' . self::TEACHER_DIR_REL . '/' . $teacherId;
    }
