 ...
 *
 * Source: il registro degli accessi dove lo scrive `AccessLogger`
 * (`<dati>/storage/logs/access_log.json`; vedi
 * `AccessLogger::percorsoDelRegistro()`, che ripiega sul vecchio
 * `log/data/` solo se quel file c'e' ancora).
 *
 * Config: legge `<dati>/storage/security/alerts/config.json` se presente,
 * altrimenti usa i defaults (mirror master). Output array di anomalie con
 * tutti i dettagli necessari per l'UI admin.
 */
final class AnomalyDetectionService
{
    ...
    {
        // Dati d'istanza: `app.paths.base` è sempre la radice del repository,
        // che nel container è l'immagine. Le soglie le scrive
        // SecurityAdminController nella cartella dei dati, e i due file dei
        // blocchi li scrive WafSecurityRepository: qui si legge di lì.
        $base = \App\Support\PercorsiDati::base(dirname(__DIR__, 2));

        return new self(
            // Phase 25.J — AccessLogger scrive in app.paths.logs (default
            // storage/logs/), NON nel vecchio log/data/. La scelta del
            // percorso, con il ripiego sul vecchio, sta ora in un posto solo.
            accessLogPath:    \App\Core\AccessLogger::percorsoDelRegistro($base),
            configPath:       $base . '/storage/security/alerts/config.json',
