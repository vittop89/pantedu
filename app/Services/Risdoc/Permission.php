...
final class Permission
{
    ...
    public static function canView(int $templateId, int $teacherId): bool
    {
        ...
        try {
            ...
            // 23/9/2026 (A-82) — qui c'era il ripiego «il modello esiste →
            // visibile», pensato per un database senza la migrazione 013: con
            // qualunque errore di questa query ogni modello si apriva a ogni
            // docente, contro il fail-safe dichiarato sopra e contro gli altri
            // rami di questo file, che negano. La 013 è applicata ovunque; se
            // la lettura fallisce, si nega e si dice perché.
            \error_log('[Permission] canView: scope del modello ' . $templateId
                . ' non leggibile, accesso negato: ' . $e->getMessage());
            return false;
        }
        if (!$row) {
            return false;
