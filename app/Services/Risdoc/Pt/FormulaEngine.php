 ...
 * Mantenere allineata la semantica con la versione JS. Gli errori sono
 * FormulaError (file a parte dal 2026-09-04).
 */
final class FormulaEngine
{
    ...
    {
        if (($node['t'] ?? '') === 'range') {
            $a = self::parseRef($node['v'][0]);
            $b = self::parseRef($node['v'][1]);
            if (!$a || !$b) {
                throw new FormulaError('#REF!', 'range non valido');
            }
            $r0 = \min($a['r'], $b['r']);
            $r1 = \max($a['r'], $b['r']);
            $c0 = \min($a['c'], $b['c']);
    ...
    private static function tokenize(string $s): array
    {
        ...
        while ($i < $L) {
            ...
            if ($ch === '"' || $ch === "'") {
                ...
                if (($s[$j] ?? '') !== $q) {
                    throw new FormulaError('#ERR!', 'stringa non chiusa');
                }
                $t[] = ['t' => 'str', 'v' => $buf];
                $i = $j + 1;
                continue;
    ...
    {
        $p = 0;
        $peek = static function () use (&$p, $tokens) {
            return $tokens[$p] ?? null;
        };
        $next = static function () use (&$p, $tokens) {
            return $tokens[$p++] ?? null;
        };
        ...
        // Discesa ricorsiva con closure che si richiamano a vicenda: ognuna e'
        // dichiarata prima come segnaposto (mai invocato: le assegnazioni vere
        // seguono subito) cosi' le catture per riferimento sono gia' callable.
        // Prima erano `null`, e PHPStan segnalava dieci «Trying to invoke null».
        $placeholder = static fn(): array => [];
        $parseExpr = $placeholder;
        $parseCompare = $placeholder;
        $parseConcat = $placeholder;
        $parseAddSub = $placeholder;
        $parseMulDiv = $placeholder;
        $parsePow = $placeholder;
        $parseUnary = $placeholder;
        $parsePrimary = $placeholder;
        ...
        $parseExpr = function () use (&$parseCompare) {
            return $parseCompare();
        };
        ...
        $parseUnary = function () use (&$parseUnary, &$parsePrimary, $peek, $next) {
            ...
            if ($tk && ($tk['t'] ?? '') === 'op' && ($tk['v'] === '-' || $tk['v'] === '+')) {
                $next();
                return ['t' => 'unary', 'op' => $tk['v'], 'arg' => $parseUnary()];
            }
            return $parsePrimary();
        };
        $parsePrimary = function () use (&$parseExpr, $peek, $next, $expect) {
            $tk = $next();
            if (!$tk) {
                throw new FormulaError('#ERR!', 'formula incompleta');
            }
            $tt = $tk['t'] ?? '';
            if ($tt === 'num') {
                return ['t' => 'num', 'v' => $tk['v']];
    ...
    private static function toNum(float|string $v): float
    {
        ...
        if (!$pn['isNum']) {
            throw new FormulaError('#VALUE!', 'atteso un numero');
        }
        return $pn['num'];
    }
    ...
    private static function evalNode(array $node, callable $cell): float|string
    {
        switch ($node['t']) {
            case 'num':
                return (float)$node['v'];
            case 'str':
                return (string)$node['v']; // testo: valido come valore
            case 'ref':
                $ref = self::parseRef($node['v']);
                if (!$ref) {
                    throw new FormulaError('#REF!', $node['v']);
                }
                $cv = $cell($ref['r'], $ref['c']);
            ...
            case 'unary':
                ...
                return $node['op'] === '-' ? -$a : $a;
            case 'bin':
                $a = self::toNum(self::evalNode($node['left'], $cell));
                $b = self::toNum(self::evalNode($node['right'], $cell));
                switch ($node['op']) {
                    case '+':
                        return $a + $b;
                    case '-':
    ...
    private static function applyFunc(string $name, array $args, callable $cell): float|string
    {
        $nums = static function () use ($args, $cell) {
            $out = [];
            foreach ($args as $a) {
                foreach (self::collectNumbers($a, $cell, false) as $x) {
                    $out[] = $x;
                }
            }
            return $out;
        ...
        switch ($name) {
            ...
            case 'AVERAGE':
                ...
                $cnt = 0;
                foreach ($args as $x) {
                    $cnt += \count(self::collectNumbers($x, $cell, true));
                }
                if ($cnt === 0) {
                    throw new FormulaError('#DIV/0!');
                }
            ...
            case 'COUNT':
                $cnt = 0;
                foreach ($args as $a) {
                    $cnt += \count(self::collectNumbers($a, $cell, true));
                }
                return (float)$cnt;
            case 'ABS':
                return \abs($en(0));
            ...
            case 'IF':
                if (\count($args) < 2) {
                    throw new FormulaError('#ERR!', 'SE richiede almeno 2 argomenti');
                }
            ...
            case 'IFERROR':
                try {
                    return self::evalNode($args[0], $cell); // preserva il tipo
                } catch (FormulaError $e) {
                    return self::evalNode($args[1], $cell);
                }
            case 'AND':
                foreach ($args as $a) {
                    if (self::toNum(self::evalNode($a, $cell)) == 0.0) {
                        return 0.0;
                    }
                }
                return 1.0;
            case 'OR':
                ...
                return 0.0;
            case 'NOT':
                return self::toNum(self::evalNode($args[0], $cell)) != 0.0 ? 0.0 : 1.0;
            case 'COUNTIF':
                $coords = self::rangeCoords($args[0]);
                $pred = self::makeCriterion($args[1]);
                $n = 0;
                foreach ($coords as $co) {
                    $cv = $cell($co['r'], $co['c']);
                    if ($cv['isNum'] && $pred($cv['num'])) {
                        $n++;
                    }
                }
                return (float)$n;
            case 'SUMIF':
                ...
                foreach ($coords as $k => $co) {
                    $cc = $cell($co['r'], $co['c']);
                    if ($pred($cc['num'])) {
                        $sc = isset($sumCoords[$k]) ? $cell($sumCoords[$k]['r'], $sumCoords[$k]['c']) : $cc;
                        $s += $sc['num'];
                    }
                }
                return $s;
        ...
        $computeCell = function (int $r, int $c) use (&$computeCell, &$memo, &$computing, $cellAt): array {
            ...
            if (isset($cell['formula']) && self::isFormula($cell['formula'])) {
                if (isset($computing[$key])) {
                    throw new FormulaError('#CIRC!', 'riferimento circolare');
                }
                $computing[$key] = true;
                try {
