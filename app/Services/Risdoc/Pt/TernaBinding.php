...
final class TernaBinding
{
    ...
    private static function walkApply(array $blocks, array $t): array
    {
        foreach ($blocks as &$n) {
            ...
            if (self::blockIsLinked($n)) {
                ...
                switch ($type) {
                    case 'checkboxGroup':
                        $n['items']   = self::cbExpand($v);
                        break;
                    case 'select':
                        $n['value']   = \is_string($v) ? $v : '';
                        break;
                    case 'textField':
                        $n['value']   = \is_string($v) ? $v : '';
                        break;
                    case 'rawTex':
                        $n['content'] = \is_string($v) ? $v : '';
                        break;
                    case 'formCheckbox':
                        $n['checked'] = (bool)$v;
                        break;
    ...
    private static function walkExtract(array $blocks, array &$delta): array
    {
        foreach ($blocks as &$n) {
            ...
            if (self::blockIsLinked($n)) {
                // La chiave dell'accumulatore è dichiarata `string`: qui il
                // valore arriva da un array non tipizzato, quindi lo si dice.
                $name = (string)($n['name'] ?? '');
                if ($name === '') {
                    continue;
                }
                switch ($type) {
                    ...
                    case 'textField':
                        ...
                        break;
                    case 'rawTex':
                        $delta[$name] = $n['content'] ?? '';
                        $n['content'] = '';
                        break;
                    case 'formCheckbox':
                        $delta[$name] = (bool)($n['checked'] ?? false);
                        $n['checked'] = false;
                        break;
                }
            }
