...
// Phase 24.29 + 24.31 — sectionbox auto-wrap level-aware:
     ...
     *
     *  In CASCATA: prima i dati d'istanza, poi l'immagine. Dal 20/9/2026 gli
     *  scostamenti dell'amministratore stanno in `<dati>/storage/templates/
     *  risdoc/…` (RisdocAdminController) e la base versionata nell'immagine:
     *  leggendo dalla sola immagine, il TeX generato avrebbe continuato a
     *  usare le etichette vecchie mentre il pannello mostrava le nuove. */
    private static function fetchOptionsSource(array $src): array
    {
        ...
        if (!empty($src['file'])) {
            ...
            $matL = strtolower($mat);
            $rel = (string)$src['folder'] . '/' . $ind . '/' . $matL . '/'
                 . $ind . '_' . $cls . '_' . $matL . '.json';
        }
        $path = $rel === null
            ? null
            : \App\Support\PercorsiDati::inCascata('storage/templates/risdoc/' . $rel, $root);
        ...
        if (\is_array($rows)) {
            ...
            for ($r = 0; $r < $rowCount; $r++) {
                ...
                foreach ($dataRows[$r] as $cell) {
                    ...
                    if (isset($norm['formula']) && FormulaEngine::isFormula($norm['formula'])) {
                        $fr = $fres[$r][$col] ?? null;
...
// Phase 24.29 — skip header vuoto (era duplicato col title del parent)
        ...
        // master progetto-precedente. (I loghi IIS arrivano da intestaLAteX_IIS.tex.)
    ...
    /**
     * Intestazione documento (master progetto-precedente): titolo centrato + "Anno
