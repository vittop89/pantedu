...
// Section completamente PT-unified: usa il saved PT se disponibile,
     ...
     *
     * In CASCATA: prima i dati d'istanza, poi l'immagine. Dal 20/9/2026 il
     * pannello dell'amministratore salva questi file in
     * `<dati>/storage/templates/risdoc/…` (RisdocAdminController), mentre la
     * base versionata resta nell'immagine. Leggendo dalla sola immagine, il
     * pannello avrebbe mostrato il testo nuovo e il documento generato le
     * etichette vecchie — per sempre, senza un errore da nessuna parte.
     */
    private function fetchOptionsSource(array $src, array $state): array
    {
        ...
        $path = $rel === null
            ? null
            : \App\Support\PercorsiDati::inCascata(
                'storage/templates/risdoc/' . $rel,
                $this->rootDir,
            );
