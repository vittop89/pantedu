...
final class TexBuilder
{
     ...
     * 2026-09-08 — la forma non elencava `teacher_id` e `institute_id`, che il
     * metodo legge (righe 89-90) per caricare gli stili dei badge: chi leggeva
     * la firma non sapeva di doverli passare, e il controllore statico
     * concludeva che valessero sempre zero, cioè che quel ramo fosse morto.
     * `mode` è diventato facoltativo perchè il valore predefinito del
     * parametro è l'array vuoto.
     *
     * @param array{
     *   mode?: string,                // 'zip' | 'vsc', default 'zip'
     *   teacher_id?: int,             // docente proprietario: serve agli stili dei badge
     *   institute_id?: int,           // istituto: idem
    ...
    public function build(Selection $sel, string $variant = VersionPicker::NORMAL, array $opts = []): BuildResult
    {
        ...
        if ($isSol) {
            ...
            if ($badges !== null && $teacherId > 0 && $instituteId > 0) {
                ...
                $resolvedStyle = \App\Repositories\TexBuilder\BadgeStyleRepository::loadResolved(
                    $instituteId,
                    $teacherId,
                    $instituteCode,
                );
