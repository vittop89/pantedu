...
final class BadgeStylePresetStore
{
    ...
    /**
     * Dove si SCRIVE: i dati d'istanza (PANTEDU_DATA_PATH). Stesso albero di
     * {@see \App\Services\Verifica\TemplateFileStore} e stessa ragione: nel
     * container la radice del repository è l'immagine, e un preset salvato lì
     * sparisce al rilascio.
     */
    public static function rootDir(): string
    {
        return \App\Support\PercorsiDati::base(\dirname(__DIR__, 3)) . '/storage/templates/verifiche';
    }

    /** Il file da leggere per (scope, nome): prima i dati, poi il codice —
     *  i tre preset di `_default/badge_styles` sono versionati. */
    private static function percorsoDiLettura(string $scope, string $name): ?string
    {
        return \App\Support\PercorsiDati::inCascata(
            'storage/templates/verifiche/' . $scope . '/badge_styles/' . $name . '.json',
            \dirname(__DIR__, 3),
        );
    }

    /** @return list<string> le cartelle dei preset da guardare in lettura. */
    private static function cartelleDiLettura(string $scope): array
    {
        return \App\Support\PercorsiDati::radiciInCascata(
            'storage/templates/verifiche/' . $scope . '/badge_styles',
            \dirname(__DIR__, 3),
        );
    }
    ...
    public static function listAvailable(string $scope): array
    {
        ...
        foreach (array_unique([$scope, self::SCOPE_DEFAULT]) as $sc) {
            foreach (self::cartelleDiLettura($sc) as $dir) {
                if (!is_dir($dir)) {
                    continue;
                }
    ...
    public static function loadPreset(string $scope, string $name): BadgeStyle
    {
        ...
        foreach (array_unique([$scope, self::SCOPE_DEFAULT]) as $sc) {
            $path = self::percorsoDiLettura($sc, $name);
