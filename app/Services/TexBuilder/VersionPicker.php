...
final class VersionPicker
{
    ...
    /**
     * Path file override (assoluto), nei dati d'istanza (PANTEDU_DATA_PATH).
     *
     * Il preambolo personalizzato non ha nessun riscontro nel database: il
     * file è l'unica copia. Nel container la radice del repository è
     * l'immagine, e ciò che ci si scrive sparisce al rilascio successivo —
     * l'amministratore vedrebbe il preambolo tornare a quello di serie senza
     * un avviso. Il file non è versionato, quindi non c'è una base
     * nell'immagine da leggere: niente cascata, solo i dati.
     */
    public static function overrideFilePath(): string
    {
        return \App\Support\PercorsiDati::base(\dirname(__DIR__, 3)) . '/' . self::OVERRIDE_FILE;
    }
