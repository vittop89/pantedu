 ...
 *
 * Setup (.env):
 *   APP_MAIL_FROM=noreply@example.org
...
final class Mailer
{
    ...
    /**
     * Mailer con il mittente di app/Config/mail.php. Null se APP_MAIL_FROM e'
     * vuoto, cioe' «questa istanza non manda posta»: chi chiama decide se e'
     * un errore (job, reset password) o un flusso da saltare (2026-09-04, P7).
     */
    public static function fromConfig(): ?self
    {
        $from = (string)\App\Core\Config::get('mail.from', '');
        if ($from === '') {
            return null;
        }
        return new self($from, (string)\App\Core\Config::get('mail.from_name', 'Pantedu'));
    }

    /**
     * Invia un'email plain text.
     *
     * $replyTo serve quando il From è una casella send-only (noreply@): senza,
     * il Reply-To ricadrebbe sul From e la risposta dell'utente finirebbe in
     * una casella che nessuno legge.
     */
    ...
    private static function resendTransport(): callable
    {
        return static function (
            ...
            $apiKey = (string)\App\Core\Config::get('mail.resend_api_key', '');
