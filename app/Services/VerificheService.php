...
final class VerificheService
{
        ...
        if ($useDb) {
            ...
            if ($uid > 0) {
                ...
                $stmt->execute([
                    ...
                    $classe    !== '' ? $L::idFromCodeForTeacher('classi', (string)$classe, (int)$uid, $indirizzo !== '' ? (string)$indirizzo : null) : null,
    ...
    /** Dati d'istanza (PANTEDU_DATA_PATH), non la radice del repository: nel
     *  container quella è l'immagine, e ciò che ci si scrive sparisce al
     *  rilascio. */
    private function radiceDati(): string
    {
        return \App\Support\PercorsiDati::base(dirname(__DIR__, 2));
    }

    private function jsonPath(): string
    {
        return $this->radiceDati() . '/storage/data/print_info.json';
    }
     ...
     *  Es: user=`docente.uno`, verFilePath=`/studio/esercizio/ar/2s/MAT/1`
     *      → `storage/data/scelte/docente.uno/studio_esercizio_ar_2s_MAT_1.json`
     */
    private function scelteJsonPath(string $verFilePath, ?string $username): string
    {
        ...
        return $this->radiceDati() . '/storage/data/scelte/' . $userSafe . '/' . $slug . '.json';
    }
    ...
    public function handleScelte(string $action, string $verFilePath, string $versionKey, array $post, ?string $username = null): array
    {
        ...
        // La cartella si crea solo quando si salva davvero. Prima la creava
        // anche 'load' e 'check': è così che in produzione, dentro il
        // container, sono comparse cartelle `storage/data/scelte/<utente>`
        // vuote — la firma del difetto, non un salvataggio.
        if ($action === 'save') {
