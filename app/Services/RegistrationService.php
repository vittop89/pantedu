...
final class RegistrationService
{
    ...
    public function approve(string $id, string $actor): array
    {
        ...
        if (Config::get('database.enabled') && Database::isAvailable()) {
            ...
            // vigenti alla data della spunta e i metadati raccolti allora.
            if (!empty($entry['tos_accepted_at'])) {
                try {
                    $uidStmt = Database::connection()->prepare(
                        'SELECT id FROM users WHERE username = ?'
                    );
                    $uidStmt->execute([$entry['username']]);
                    $newUserId = (int)($uidStmt->fetchColumn() ?: 0);
                    if ($newUserId > 0) {
                        (new \App\Services\Gdpr\TosAcceptanceService())->recordHistoricAcceptance(
                            $newUserId,
                            (string)$entry['tos_accepted_at'],
                            (string)($entry['ip'] ?? ''),
                            isset($entry['user_agent']) ? (string)$entry['user_agent'] : null,
                        );
                    }
                } catch (\Throwable $e) {
                ...
                if ($studentId > 0) {
                    try {
                        ...
                        if ($mailFrom !== '') {
                            ...
                            // e' disaccoppiato dal checkout (/var/lib/pantedu-data),
                            // e questo e' il log da cui si recupera il token se
                            // l'invio fallisce — scriverlo nel repo lo rende
                            // introvabile proprio quando serve.
                            ...
                            $mailer = new \App\Services\ParentConsentMailer(
                                ...
                                $siteUrl,
                                $logFile,
                                (string)($_ENV['DPO_EMAIL'] ?? '') ?: 'operatore@example.net'
                            );
