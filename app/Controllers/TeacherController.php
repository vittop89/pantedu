...
final class TeacherController
{
    ...
    public function dashboard(Request $req): Response
    {
        ...
        $drive  = $this->driveDelCruscotto((int)($user['id'] ?? 0));
        $avvisoSezioni = $this->avvisoSezioni((int)($user['id'] ?? 0));
        ob_start();
    ...
    /**
     * ADR-041 — i materiali del docente su sezioni di cui non ha più l'incarico.
     * Un guasto qui non toglie il cruscotto: l'avviso manca e basta.
     *
     * @return list<array<string,mixed>>
     */
    private function avvisoSezioni(int $teacherId): array
    {
        if ($teacherId <= 0 || !Config::get('database.enabled') || !Database::isAvailable()) {
            return [];
        }
        try {
            return (new \App\Services\Contenuti\MaterialiSuSezioniNonAmmesse())->avviso($teacherId);
        } catch (Throwable $e) {
            error_log('TeacherController.avvisoSezioni: ' . $e->getMessage());
            return [];
        }
    }

    /**
     * Drive nel cruscotto (ADR-038): la sezione c'è se Drive è acceso o guasto
     * sull'installazione, o se il docente ha un collegamento di prima, che
     * deve poter scollegare in ogni stato.
     *
     * @return array{istanza: string, collegato: bool}
     */
    private function driveDelCruscotto(int $teacherId): array
    {
        $collegato = false;
        if ($teacherId > 0 && Config::get('database.enabled') && Database::isAvailable()) {
            try {
                $collegato = (new DriveOAuthRepository())->isConnected($teacherId);
            } catch (Throwable $e) {
                error_log('TeacherController.driveDelCruscotto: ' . $e->getMessage());
            }
        }
        return ['istanza' => StatoDiDrive::attuale(), 'collegato' => $collegato];
    }
