...
final class VerificaSyncController
{
    ...
    public function syncAll(Request $req): Response
    {
        try {
            ...
            if ($istanza !== StatoDiDrive::ACCESO) {
                // Drive spento o guasto sull'installazione (ADR-038): non si prova nemmeno.
    ...
    public function manifestSigned(Request $req): Response
    {
        try {
            ...
            if (!$recovery->isConfigured()) {
                return Response::fail('kms_not_configured', 500);
            }
            ...
            if ($hmac === null) {
                return Response::fail('recovery_key_missing', 412);
            }
            $payload['hmac'] = $hmac;
    ...
    public function buildLocalBundleManifest(int $teacherId): array
    {
        ...
        // Dati d'istanza (PANTEDU_DATA_PATH): l'indice TikZ e i file dei docenti stanno li', non nel repo.
        $dataBase = rtrim((string) \App\Core\Config::get('app.paths.data_base', $appRoot), '/');
        // Doppio sbaglio, corretto il 20/9/2026: la radice era quella del
        // repository (nel container, l'immagine) e il nome della cartella del
        // docente non combaciava con quello che scrive TemplateFileStore, che
        // usa lo scope `t_<id>` e non `teachers/<id>`. Quel ramo non ha mai
        // trovato niente. Adesso le cartelle le dice TemplateFileStore: dati
        // d'istanza e, per `_default`, anche la copia versionata dell'immagine.
        $defaultRoots = TemplateFileStore::cartelleDiLettura(
            TemplateFileStore::SCOPE_DEFAULT,
            'texCommon',
        );
        $teacherRoots = TemplateFileStore::cartelleDiLettura(
            TemplateFileStore::teacherScope($teacherId),
            'texCommon',
        );
        $emitted = []; // dedup per path relativo
        $walkTpl = function (string $absRoot, string $relPrefix) use (&$emitted): void {
            if (!is_dir($absRoot)) {
                return;
            }
        ...
        // `$emitted` tiene l'ultimo che passa, quindi si cammina dal meno al
        // più specifico: codice, poi dati d'istanza, e il docente dopo il
        // default. `cartelleDiLettura()` le dà nell'ordine inverso (prima i
        // dati), per questo si girano.
        foreach (array_reverse($defaultRoots) as $root) {
            $walkTpl($root, 'texCommon');
        }
        foreach (array_reverse($teacherRoots) as $root) {
            $walkTpl($root, 'texCommon'); // sovrascrive default
        }
...
/* repo non disponibile, skip */
        ...
        $tikzAdminDefaults = $dataBase . '/storage/data/modelli_tikz_elements.json';
        ...
        $tikzTeacherOverrides = $dataBase . "/storage/objects/teachers/{$teacherId}/tikz-overrides.json";
        ...
        // Le librerie drawio stanno nei dati d'istanza (TeacherDrawioLibraryController):
        // di quell'albero in git non c'è niente, quindi dall'immagine non si
        // leggerebbe mai nulla e il pacchetto uscirebbe senza le librerie.
        $radiceDrawio = \App\Support\PercorsiDati::base($appRoot) . '/storage/templates/drawio';
        $walkDrawio($radiceDrawio . '/_default');
        $walkDrawio($radiceDrawio . "/teachers/{$teacherId}");
    ...
    public function materializeBundleEntry(int $teacherId, array $entry): array
    {
        switch ($entry['type']) {
            ...
            case 'esercizio':
                ...
                // Dati d'istanza (PANTEDU_DATA_PATH): i contratti li scrive
                // LocalStorageProvider nella cartella dei dati
                // (app/Config/storage.php), non nella radice del repository —
                // che nel container è l'immagine. Cercandoli lì il file non si
                // trovava e il pacchetto usciva con 'contract': null, in
                // silenzio. Due righe sopra, a 371, lo stesso file usava già
                // `app.paths.data_base` per l'indice TikZ.
                $contractAbs = $contractRel !== ''
                    ? \App\Support\PercorsiDati::base(dirname(__DIR__, 2)) . '/storage/objects/' . $contractRel
                    : '';
