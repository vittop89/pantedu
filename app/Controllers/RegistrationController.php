...
{
    /**
     * Le risposte del servizio che il modulo non deve distinguere da una
     * domanda ricevuta (24/9/2026): direbbero a chiunque quali indirizzi hanno
     * un account o una domanda in attesa.
     */
    private const EMAIL_GIA_USATA = ['email_taken', 'email_pending'];

    /**
     * Ogni quanto, al più, parte l'avviso a un indirizzo già usato: senza,
     * il modulo diventerebbe un modo di mandare posta a ripetizione a
     * chiunque. Non oltre la conservazione di `rate_limits`
     * ({@see RateLimitStore::CONSERVAZIONE_SECONDI}), che toglierebbe i colpi
     * prima della fine della finestra.
     */
    public const AVVISO_OGNI_SECONDI = RateLimitStore::CONSERVAZIONE_SECONDI;

    private RegistrationService $svc;
    private ?RegistrationMailer $mailer;
    private RateLimitStore $limitatore;

    public function __construct(
        ?RegistrationService $svc = null,
        ?RegistrationMailer $mailer = null,
        ?RateLimitStore $limitatore = null,
    ) {
        ...
        $this->limitatore = $limitatore ?? new RateLimitStore();
    }

    private function defaultMailer(): ?RegistrationMailer
    {
        // Mittente da app/Config/mail.php: vuoto significa "non inviare",
        // com'è documentato in .env.example.
        $mailer = Mailer::fromConfig();
        if ($mailer === null) {
            return null;
        }
        // La radice la chiede il mailer all'invio (null = `app.url`, 23/9/2026:
        // prima qui si ripiegava sul dominio di produzione); le risposte vanno
        // alla casella generale dell'istanza.
        $log = Config::get('app.paths.storage') . '/logs/mail.log';
        return new RegistrationMailer($mailer, null, $log, (string)Config::get('mail.contact_email', ''));
    }
    ...
    public function showForm(Request $req): Response
    {
        ...
        $body  = $view->render('auth/register', [
            ...
            // 2026-09-24 — il nome utente assegnato non passa più di qui
            // (`?u=`): nome.cognome2 diceva a chiunque che nome.cognome
            // esisteva già. La pagina non lo mostrava nemmeno; arriva
            // nell'email di approvazione.
            ...
            // ADR-032 — i ruoli ammessi dipendono dallo scenario: nessuno (1),
            // solo docenti (2), docenti e studenti (3).
            'allowedRoles'    => \App\Support\DeploymentScenario::allowedRegistrationRoles(),
    ...
    private function errorMessage(string $code): ?string
    {
        ...
        return match ($code) {
            ...
            // `email_taken` ed `email_pending` non arrivano più qui: il
            // modulo risponde come a una domanda ricevuta (24/9/2026).
            'institute_required'      => 'Seleziona la tua scuola dai risultati della ricerca: '
                . 'serve quella con il codice meccanografico MIUR, non un nome digitato a mano. '
                . 'Se la tua scuola non compare, scrivi all\'amministratore.',
            ...
            'teacher_signup_disabled_single' => 'L\'iscrizione dei docenti non è aperta su questa istanza. Contatta l\'amministratore.',
            'registration_closed'            => 'Le iscrizioni non sono aperte: la piattaforma è in uso personale. Gli studenti entrano con la credenziale della classe.',
    ...
    public function submit(Request $req): Response
    {
        ...
        // ADR-032 — i ruoli ammessi dipendono dallo scenario: nessuno (1),
        // solo docenti (2), docenti e studenti (3, se gli account studente
        // sono attivi). Il form li rispecchia; una POST diretta no, quindi si
        // rivalida qui.
        $role    = (string)($req->post['role'] ?? '');
        $allowed = \App\Support\DeploymentScenario::allowedRegistrationRoles();
        if ($allowed === []) {
            return Response::redirect('/register?error=registration_closed', 303);
        }
        if ($role === '' && count($allowed) === 1) {
            $role = $allowed[0];
        }
        if (!in_array($role, $allowed, true)) {
            return Response::redirect(
                '/register?error=' . ($role === 'student' ? 'student_registration_disabled' : 'teacher_signup_disabled_single'),
                303
            );
        }
        ...
        try {
            ...
            $out = $this->svc->submit([
                ...
                // 2026-09-24 — IP e User-Agent non si passano più: la domanda
                // non li conserva (RegistrationService, ConservazioneDelleIscrizioni).
...
/* log via Mailer::logSend già fatto */
            ...
            // Senza il nome utente (24/9/2026): vedi showForm().
            return Response::redirect('/register?ok=1');
        } catch (Throwable $e) {
            if (\in_array($e->getMessage(), self::EMAIL_GIA_USATA, true)) {
                return $this->comeUnaDomandaRicevuta(
                    $e->getMessage(),
                    strtolower(trim((string)($req->post['email'] ?? ''))),
                );
            }
    ...
    /**
     * Un'email già usata riceve la stessa risposta di una domanda nuova
     * (24/9/2026). Fino a quel giorno il modulo rispondeva «Email già
     * registrata» o «Email già in attesa di approvazione» per qualunque riga
     * di `users`: bastava provare un indirizzo per sapere se aveva un account.
     *
     * Chi legge la casella riceve la spiegazione: con un account, come
     * recuperare la password; con una domanda in attesa, che vale quella. Al
     * più un avviso all'ora per indirizzo.
     */
    private function comeUnaDomandaRicevuta(string $codice, string $email): Response
    {
        Csrf::rotate();
        if ($this->mailer !== null && $this->avvisoConcesso($email)) {
            try {
                if ($codice === 'email_taken') {
                    $this->mailer->giaRegistrato($email);
                } else {
                    $this->mailer->giaInAttesa($email);
                }
            } catch (Throwable $e) {
/* log via Mailer::logSend già fatto */
            }
    ...
    /**
     * Un avviso a questo indirizzo nell'ultima ora? Il secchio porta l'hash
     * dell'indirizzo, non l'indirizzo, e nessun IP.
     */
    private function avvisoConcesso(string $email): bool
    {
        $secchio = 'avviso_iscrizione:' . hash('sha256', $email);
        if ($this->limitatore->hits($secchio, self::AVVISO_OGNI_SECONDI) !== []) {
            return false;
        }
        $this->limitatore->append($secchio);
        return true;
    }
...
/* best-effort */
    ...
    private function resolveInstituteId(string $miurCode, string $denom, string $city): int
    {
        ...
        $code = $miurCode !== '' ? strtoupper((string)preg_replace('/[^A-Za-z0-9_-]/', '', $miurCode)) : '';
        // Qui prima si fabbricava un codice "MIUR-NOME-CITTA" quando la scelta
        // non ne portava uno. Sembrava un ripiego innocuo e non lo era: una
        // scuola con quel codice non viene trovata dalla anagrafica ne'
        // dall'import delle adozioni, quindi resta per sempre senza indirizzi,
        // sezioni e materie - e chi ci si iscrive non vede nulla, senza che
        // niente segnali il perche'. E' cosi' che e' nato l'istituto 108.
        //
        // Senza codice reale non si crea niente: chi si iscrive deve scegliere
        // la scuola dalla ricerca, che il codice ce l'ha. Chi resta senza
        // riceve 'institute_required', gestito a valle.
        if (!InstituteRepository::isRealMiurCode($code)) {
            return 0;
        }
