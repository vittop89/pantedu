...
final class TikzController
{
    ...
    {
        try {
            $data = $req->body(); // JSON dal JS o form: `post` con un body JSON è vuoto
    ...
    /** POST /tikz/edit-element — modifica un elemento, rinomina il suo gruppo o lo sposta */
    public function editElement(Request $req): Response
    {
        try {
            ...
            $elementIndex = isset($data['elementIndex']) && $data['elementIndex'] !== ''
...
/* migrazione orfani non blocca la response admin */
    ...
    /** POST /tikz/delete-element — elimina un elemento o un gruppo intero */
    public function deleteElement(Request $req): Response
    {
        try {
            ...
            $v = new Validator($data);
            $groupName   = $v->string('groupName', max: 120);
            $wholeGroup  = filter_var($data['deleteWholeGroup'] ?? false, FILTER_VALIDATE_BOOLEAN);
...
/* cleanup orfani non blocca la response admin */
    ...
    /** POST /tikz/save-new-element — aggiunge un elemento tikz|latex, creando il gruppo se manca */
