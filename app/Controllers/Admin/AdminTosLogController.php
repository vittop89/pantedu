...
final class AdminTosLogController
{
    ...
    {
        // Versione EFFICACE, non la costante: durante il preavviso la nuova
        // versione è pubblicata ma non ancora vincolante, e segnare come
        // "non in regola" chi non l'ha ancora accettata sarebbe falso.
        $svc = new \App\Services\Gdpr\TosAcceptanceService();
        $effective = $svc->effectiveVersions();
        $currentVersion = $effective['tos'];
        $rows = $this->fetchRows();
        $outdated = 0;
        foreach ($rows as $r) {
            if (
                ($r['tos_version'] ?? null) !== $effective['tos']
                || ($r['aup_version'] ?? null) !== $effective['aup']
            ) {
                $outdated++;
            }
        }
    ...
    private function fetchRows(): array
    {
        ...
        try {
            ...
            // ADR-040 — `admin` non esisteva; il super-amministratore il middleware lo esenta.
            $stmt = $pdo->prepare(
                // L'ultima accettazione è la più RECENTE, non quella con la
                // stringa di versione più alta: MAX(tos_version) ordina come
                // testo e mette '1.9' sopra '1.10'.
                "SELECT u.id, u.username, u.role, u.created_at,
                 ...
                 LEFT JOIN user_tos_acceptance a
                   ON a.user_id = u.id
                  AND a.accepted_at = (
                      SELECT MAX(accepted_at) FROM user_tos_acceptance WHERE user_id = u.id
                  )
