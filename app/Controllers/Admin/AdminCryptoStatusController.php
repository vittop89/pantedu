...
final class AdminCryptoStatusController
{
    ...
    public function index(Request $req): Response
    {
        ...
        // Drill di verifica custodia (kms_backup_verified).
        //
        // La DPIA promette una verifica semestrale delle copie di
        // KMS_MASTER_KEY, ma fino al 2026-08-26 non ne era mai stata
        // registrata una: la promessa non aveva nulla che la ricordasse.
        // 183 giorni = sei mesi; oltre, il riquadro diventa un avviso.
        $drillLast     = null;
        $drillDaysAgo  = null;
        $drillStale    = true;   // mai eseguito => da fare
        $drillDueDate  = null;
        try {
            $drillLast = $pdo->query(
                "SELECT occurred_at FROM crypto_custody_events
                  WHERE event_type = 'kms_backup_verified'
                  ORDER BY occurred_at DESC LIMIT 1"
            )->fetchColumn() ?: null;
            if ($drillLast) {
                $ts = \strtotime((string)$drillLast);
                if ($ts) {
                    $drillDaysAgo = (int)\floor((\time() - $ts) / 86400);
                    $drillStale   = $drillDaysAgo > 183;
                    $drillDueDate = \date('d/m/Y', $ts + 183 * 86400);
                }
            }
        } catch (Throwable $e) {
            // tabella non ancora migrata: lo stato resta "da fare"
        }
        $view = View::default();
        $body = $view->render('admin/crypto_status', [
            ...
            'recent'      => $recent,
            'drillLast'    => $drillLast,
            'drillDaysAgo' => $drillDaysAgo,
            'drillStale'   => $drillStale,
            'drillDueDate' => $drillDueDate,
    ...
    {
        // 23/9/2026 (A-36) — la chiave si legge come la legge la cifratura,
        // in ChiaveMadre. Prima qui valeva una regola propria: qualunque
        // stringa di almeno 16 byte, esadecimale o no, e anche da $_SERVER.
        // Una chiave che la cifratura rifiuta dava comunque una firma, e il
        // manifest la dichiarava forte.
        $chiave = ChiaveMadre::dallAmbiente();
        if (!$chiave->valida()) {
            ...
            if ($chiave->malformata()) {
                // Assente è il caso previsto (sviluppo); malformata no, e va detto.
                \error_log('[AdminCryptoStatus] KMS_MASTER_KEY malformata (servono 64 caratteri esadecimali): export senza firma');
            }
            return null;
        }
        return hash_hmac('sha256', 'pantedu-export-signing-v1', $chiave->byte(), true);
    }
    ...
    public function recordEvent(Request $req): Response
    {
        ...
        try {
            ...
            $legalBasis = trim((string)($req->post['legal_basis'] ?? '')) ?: null;
            $stmt->execute([
                $type,
                $tid > 0 ? $tid : null,
            ...
            // 2026-09-04 — l'interessato viene avvisato dell'accesso ai suoi
            // contenuti nel momento in cui lo si registra: il registro lo
            // controlla chi lo scrive, l'avviso lo legge chi lo subisce.
            $notified = \App\Services\Crypto\CustodyNotifier::notify($type, $tid, $occurredAt, $legalBasis);
            $_SESSION['flash'] = [
                'type'    => 'success',
                'message' => 'Evento registrato.' . ($notified ? ' Il docente è stato avvisato via email.' : ''),
            ];
        } catch (Throwable $e) {
