 ...
 *   2. solo nello scenario 3 (ADR-040): l'amministratore di istituto,
 *      `role='institute_admin'` con `admin_institute_id` = N e una password
 *      da cambiare al primo accesso, mostrata UNA SOLA VOLTA.
 *
 * Fino al 14/9/2026 il ruolo scritto era `admin`, che nessuna zona di accesso
 * conosceva, e l'amministratore si creava in ogni scenario. Che cosa può fare
 * l'amministratore di istituto lo decide ADR-040 (fase 2).
 */
final class AdminInstitutesController
{
    ...
    public function newForm(Request $req): Response
    {
        ...
        $body = $view->render('admin/institutes_new', [
            'con_amministratore' => \App\Services\Institutes\AmministratoreDiIstituto::previsto(),
            'csrf'  => Csrf::token(),
    ...
    public function create(Request $req): Response
    {
        ...
        // ADR-040 — l'amministratore di istituto solo nello scenario 3: negli
        // altri i suoi campi non si leggono nemmeno.
        $conAmministratore = \App\Services\Institutes\AmministratoreDiIstituto::previsto();
        ...
        if ($conAmministratore && ($adminUsername === '' || !preg_match('/^[a-z0-9._-]{3,32}$/', $adminUsername))) {
            $errs[] = 'Username admin non valido (3-32 char minuscole/numeri/._-).';
        }
        if ($conAmministratore && ($adminEmail === '' || !filter_var($adminEmail, FILTER_VALIDATE_EMAIL))) {
        ...
        try {
            ...
            // 2. L'amministratore di istituto, solo nello scenario 3 (ADR-040).
            $plainPassword = null;
            if ($conAmministratore) {
                $plainPassword = (new \App\Services\Institutes\AmministratoreDiIstituto($pdo))
                    ->crea($iid, $adminUsername, $adminFirst, $adminLast, $adminEmail);
            }
        ...
        // Le credenziali si mostrano UNA SOLA VOLTA. Il titolo lo escapa la
        // vista; il messaggio no (porta il <code> della password).
        $_SESSION['flash'] = $plainPassword !== null
            ? [
                'type'    => 'success',
                'title'   => "Istituto «{$name}» creato.",
                'message' => 'Amministratore di istituto: ' . htmlspecialchars($adminUsername, ENT_QUOTES)
                    . " — password da cambiare al primo accesso: <code>{$plainPassword}</code> (annotala: non si rivede).",
            ]
            : [
                'type'    => 'success',
                'title'   => "Istituto «{$name}» creato.",
                'message' => 'Nessun amministratore di istituto: esiste solo quando la piattaforma è adottata da un Istituto (scenario 3).',
            ];
     ...
     * documentale: piano annuale e relazione finale sono bozze di atti della
     * scuola e parlano di studenti. Il Titolare della piattaforma puo' decidere
     * che per i docenti di un Istituto la bozza non resti su questo server
     * (2026-09-22: e' una sua scelta, non un'istruzione dell'Istituto o del suo
     * DPO); l'amministratore lo imposta
    ...
    public function toggleCompilationStorage(Request $req, array $params): Response
    {
        ...
        \App\Core\PrivilegedAccessLogger::log(
            ...
            sprintf(
                ...
                (string)($inst['name'] ?? $id),
                // 2026-09-22 — fino a qui la riga terminava con
                // «(indicazione del DPO dell'Istituto)»: il registro degli
                // accessi privilegiati attribuiva a un soggetto esterno una
                // scelta del Titolare, a ogni cambio. Prova:
                // tests/Unit/IlCodiceNonAttribuisceAlDpoLeScelteTest.php.
    ...
    /**
     * POST /admin/institutes/{id}/sezioni-docenti — chi dei docenti di questo
     * istituto può legarsi a una sezione (ADR-041): tutti, solo gli incaricati,
     * nessuno. Le spunte che la modalità non ammette si sospendono e si
     * riprendono se torna a permetterle; il flash dice quante.
     */
    public function impostaSezioniDocenti(Request $req, array $params): Response
    {
        $id = (int)($params['id'] ?? 0);
        $modalita = (string)($req->post['modalita'] ?? '');
        if ($id <= 0 || !\in_array($modalita, \App\Services\SezioniDeiDocenti::MODALITA, true)) {
            $_SESSION['flash'] = ['type' => 'error', 'title' => 'Istituto o modalità non validi.'];
            return Response::redirect('/admin/institutes');
        }
        if (!Database::isAvailable()) {
            $_SESSION['flash'] = ['type' => 'error', 'title' => 'DB non disponibile.'];
        ...
        $inst = (new InstituteRepository())->findById($id);
        if (!$inst) {
            $_SESSION['flash'] = ['type' => 'error', 'title' => 'Istituto inesistente.'];
            return Response::redirect('/admin/institutes');
        }
        $sezioni = new \App\Services\SezioniDeiDocenti();
        $prima = $sezioni->modalita($id);
        try {
            $esito = $sezioni->imposta($id, $modalita);
        } catch (\Throwable $e) {
            error_log('[admin/institutes/sezioni-docenti] ' . $e->getMessage());
            $_SESSION['flash'] = ['type' => 'error', 'title' => 'Impostazione non disponibile: migrazione 126 non applicata.'];
            return Response::redirect('/admin/institutes');
        }
        $nomi = [
            \App\Services\SezioniDeiDocenti::TUTTI           => 'tutti i docenti',
            \App\Services\SezioniDeiDocenti::SOLO_INCARICATI => 'solo i docenti incaricati',
            \App\Services\SezioniDeiDocenti::NESSUNO         => 'nessun docente',
        ];
        ActivityLogger::event(
            'institute_sezioni_docenti',
            subjectType: 'institute',
            subjectId:   (string)$id,
            details:     ['code' => (string)($inst['code'] ?? ''), 'da' => $prima, 'a' => $modalita] + $esito,
            outcome:     'ok',
        );
        \App\Core\PrivilegedAccessLogger::log(
            'institute_sezioni_docenti',
            'institute',
            (string)$id,
            sprintf('sezioni delle classi per «%s»: da «%s» a «%s»', (string)($inst['name'] ?? $id), $prima, $modalita),
        );
        $_SESSION['flash'] = [
            'type'    => 'success',
            'title'   => sprintf('Istituto «%s»: le sezioni le usa %s.', (string)($inst['name'] ?? $id), $nomi[$modalita]),
            'message' => sprintf(
                'Spunte dei docenti su una sezione sospese: %d; riprese: %d. Gli anni valgono sempre per tutte le loro sezioni. I dati non si cancellano.',
                $esito['sospese'],
                $esito['riprese']
            ),
        ];
    ...
    public function miurUpdate(Request $req): Response
    {
        ...
        if (!is_dir($dataDir) && !mkdir($dataDir, 0755, true) && !is_dir($dataDir)) {
            return Response::fail('data_dir_failed', 500);
        }
        ...
        if (!$loaded) {
            return Response::fail('no_file', 400);
        }
    ...
    public function adozioniApply(Request $req): Response
    {
        ...
        $_SESSION['flash'] = [
            ...
            'message' => sprintf(
                ...
                . 'sezioni a cui e\' stato assegnato l\'indirizzo: %d · libri nel catalogo: %d.',
                $fatte['indirizzi'],
                $fatte['sezioni'],
                $fatte['materie'],
                $fatte['sistemate'],
