...
final class AdminGdprController
{
    ...
    public function dataRequestsAction(Request $req, array $params): Response
    {
        ...
        // 2026-09-22 — apre l'incidente dell'art. 33 da questa segnalazione e
        // ne lascia scritto il legame. Senza, l'unico modo di far tacere
        // l'invariante `segnalazioni` della diagnostica sarebbe chiudere la
        // segnalazione, cioè dichiarare che non era una violazione: una scelta
        // sola non è una scelta.
        if ($action === 'open_incident') {
            if ($notes === '') {
                return Response::redirect("/admin/data-requests/{$id}?error=notes_required");
            }
            return $this->incidenteDaSegnalazione('dpo', $id, $notes, "/admin/data-requests/{$id}");
        }
    ...
    /** Apre l'incidente e porta l'amministratore sulla sua pagina. Il perché sta nel servizio. */
    private function incidenteDaSegnalazione(string $fonte, int $id, string $note, string $dove): Response
    {
        try {
            ...
            error_log('[gdpr] apertura incidente da segnalazione fallita: ' . $e->getMessage());
            return Response::redirect($dove . '?error=' . urlencode('Apertura incidente fallita'));
        }

        return isset($esito['id'])
            ? Response::redirect('/admin/data-breach/' . $esito['id'])
            : Response::redirect($dove . '?error=' . urlencode((string)$esito['errore']));
    }
    ...
    /**
     * GET /admin/data-breach/piano — il piano per le violazioni, da scaricare.
     *
     * PERCHE' (2026-09-22)
     *   `docs/privacy/data_breach_runbook.md` esisteva ma non era raggiungibile
     *   da nessuna pagina: stava nel repository, e basta. Chi ne ha bisogno lo
     *   cerca in un momento pessimo — con le settantadue ore dell'art. 33 gia'
     *   partite — e dovrebbe sapere che esiste e dove. Un piano che si trova
     *   solo se si sa gia' dove guardare non e' un piano.
     *
     *   Si serve come allegato e non come pagina resa: il convertitore
     *   markdown vive dentro TrustPagesController con quattro metodi privati, e
     *   quelle pagine sono confrontate a pixel. Duplicarlo o estrarlo per una
     *   pagina d'emergenza sarebbe un rischio sproporzionato. I passi che
     *   servono subito stanno comunque in chiaro nella pagina del registro:
     *   in emergenza non si scarica niente, si legge.
     */
    public function dataBreachPiano(Request $req): Response
    {
        $file = \dirname(__DIR__, 3) . '/docs/privacy/data_breach_runbook.md';
        if (!is_file($file)) {
            return Response::html('<h1>404 — piano non trovato</h1>', 404);
        }

        $r = Response::html((string)file_get_contents($file));
        $r->headers['Content-Type'] = 'text/markdown; charset=UTF-8';
        $r->headers['Content-Disposition'] = 'attachment; filename="piano-violazioni-pantedu.md"';
        // Non si mette in cache: se il piano cambia, chi lo scarica deve avere
        // quello di adesso.
        ...
        try {
            switch ($action) {
                ...
                case 'notify_users':
                    ...
                    break;
                case 'notify_controller':
                    // L'Istituto titolare dei dati (R20): è l'unico che possa
                    // adempiere verso i propri interessati, e lo sa solo da noi.
                    $repo->setControllerNotified($id, trim((string)($req->post['method'] ?? 'email')) ?: 'email');
                    break;
                default:
    ...
    public function teacherContentSearch(Request $req): Response
    {
        ...
        if ($teacherId <= 0) {
            return Response::fail('teacher_id required', 400);
        }
