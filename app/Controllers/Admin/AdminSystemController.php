...
final class AdminSystemController
{
    ...
    public function deploymentPage(Request $req): Response
    {
        ...
        $body = $view->render('admin/system/deployment', [
            ...
            // ADR-032 — lo scenario e' la sorgente di verita'; il modo legacy
            // ($snap) resta come asse derivato, allineato a ogni cambio.
            'scenario'          => DeploymentScenario::snapshot(),
            'scenario_docs'     => DeploymentScenario::legalDocuments(),
            'scenario_blockers' => DeploymentScenario::blockers($activeUsers),
            ...
            // Phase 25.P — stato del gate ToS/AUP + quanti verrebbero bloccati.
            'tos_enforce'       => TosEnforcement::snapshot(),
            'twofactor_enforce' => TwoFactorEnforcement::snapshot(),
            'twofactor_missing' => $this->twoFactorPendingCount(),
            'tos_pending'       => $this->tosPendingCount(),
     ...
     *
     * ADR-032 — si cambia SCENARIO (1 personale, 2 colleghi, 3 Istituto), non
     * piu' il modo legacy: quello viene allineato da DeploymentScenario::persist.
     * La motivazione e' obbligatoria e finisce nel log degli accessi
     * privilegiati: cambiare scenario cambia chi e' il titolare, chi puo'
     * iscriversi e quali documenti valgono. Lo scenario 3 richiede inoltre
     * che l'istanza sia dichiarata su infrastruttura qualificata ACN (.env)
     * e che chi preme il pulsante lo confermi.
     */
    public function deploymentSwitch(Request $req): Response
    {
        ...
        try {
            switch ($action) {
                case 'set_scenario':
                    $target = (string)($req->post['scenario'] ?? '');
                    $reason = trim((string)($req->post['_audit_reason'] ?? ''));
                    // Stessa soglia del middleware `audit_reason`: vedi tosEnforceSet().
                    if (mb_strlen($reason) < 10) {
                        return $this->redirectFlash('scenario_no_reason', 'error');
                    }
                    if (mb_strlen($reason) > 200) {
                        $reason = mb_substr($reason, 0, 200);
                    }
                    $institute = [
                        'institute_owner_email' => trim((string)($req->post['institute_owner_email'] ?? '')),
                        'institute_legal_name'  => trim((string)($req->post['institute_legal_name']  ?? '')),
                    ];
                    if ($target === DeploymentScenario::INSTITUTE && empty($req->post['acn_ack'])) {
                        return $this->redirectFlash('institute_requires_ack', 'error');
                    }
                    $blocker = DeploymentScenario::activationBlocker($target, $institute, $this->activeUserCount());
                    if ($blocker !== null) {
                        return $this->redirectFlash($blocker, 'error');
                    }
                    $before = DeploymentScenario::current();
                    if ($before === $target) {
                        return $this->redirectFlash('scenario_unchanged', 'info');
                    }
                    DeploymentScenario::persist($target, (string)$actor, $reason, $institute);
                    $details = "motivazione: $reason";
                    if ($target === DeploymentScenario::INSTITUTE) {
                        $details .= "; titolare={$institute['institute_legal_name']}; dpo={$institute['institute_owner_email']}";
                    }
                    $this->auditSwitch($actor, "scenario: {$before}→{$target}", $details, 'deployment_scenario_switch');
                    return $this->redirectFlash('scenario_' . $target, 'ok');

                case 'reset_to_env':
                    // Rimuove gli override runtime (scenario + modo legacy):
                    // si ricasca su .env.
                    DeploymentScenario::clearRuntime();
                    $path = (string) \App\Core\Config::get('app.paths.storage') . '/config/deployment.json';
                    if (is_file($path)) {
                        @unlink($path);
                    }
                    DeploymentMode::resetCache();
                    $this->auditSwitch($actor, 'reset_to_env', 'runtime override removed (scenario + mode)');
    ...
    /**
     * POST /admin/system/tos-enforce — accende/spegne il gate ToS/AUP.
     *
     * Impedire l'accesso al servizio è una decisione, non una preferenza:
     * la motivazione è obbligatoria (middleware `audit_reason`) e finisce in
     * privileged_access_log insieme a chi l'ha presa. Prima il flag stava solo
     * in .env e si girava via SSH, senza che restasse traccia di nulla.
     */
    public function tosEnforceSet(Request $req): Response
    {
        $action = (string)($req->post['tos_action'] ?? '');
        $actor  = (string)(Auth::user()['username'] ?? '?');
        $reason = trim((string)($req->post['_audit_reason'] ?? ''));

        // Il `required` sul form è client-side e si aggira con una POST
        // diretta, quindi la motivazione si valida anche qui. La soglia è la
        // stessa del middleware `audit_reason` (dieci caratteri): finché era
        // otto, una motivazione di otto o nove caratteri passava di qui e si
        // prendeva un 400 JSON dal middleware — su una POST di form, cioè una
        // pagina di errore grezza al posto del messaggio.
        if (mb_strlen($reason) < 10) {
            return $this->redirectFlash('tos_enforce_no_reason', 'error');
        }
        if (mb_strlen($reason) > 200) {
            $reason = mb_substr($reason, 0, 200);
        }
        ...
        try {
            $before = TosEnforcement::isEnabled() ? 'on' : 'off';

            switch ($action) {
                case 'enable':
                    // Quante persone si stanno chiudendo fuori: va scritto nel
                    // log, altrimenti fra sei mesi nessuno sa quanto pesò.
                    $blocked = $this->tosPendingCount();
                    TosEnforcement::persistRuntime(true, $actor, $reason);
                    $this->auditSwitch(
                        $actor,
                        "tos_enforce: {$before}→on",
                        "utenti da far passare dal form: $blocked; motivazione: $reason",
                        'tos_enforce_switch',
                    );
                    return $this->redirectFlash('tos_enforce_on', 'ok');

                case 'disable':
                    TosEnforcement::persistRuntime(false, $actor, $reason);
                    $this->auditSwitch(
                        $actor,
                        "tos_enforce: {$before}→off",
                        "motivazione: $reason",
                        'tos_enforce_switch',
                    );
                    return $this->redirectFlash('tos_enforce_off', 'ok');

                case 'reset':
                    TosEnforcement::clearRuntime();
                    $after = TosEnforcement::isEnabled() ? 'on' : 'off';
                    $this->auditSwitch(
                        $actor,
                        "tos_enforce: {$before}→{$after} (reset)",
                        "override rimosso, torna a valere .env; motivazione: $reason",
                        'tos_enforce_switch',
                    );
                    return $this->redirectFlash('tos_enforce_reset', 'ok');

                default:
                    return $this->redirectFlash('tos_enforce_bad_action', 'error');
            }
        } catch (\Throwable $e) {
            error_log('[admin/system/tos-enforce] ' . $e->getMessage());
            return $this->redirectFlash('exception', 'error');
        }
    }

    /**
     * POST /admin/system/2fa-enforce — obbligo di verifica in due passaggi.
     *
     * Tre modalita' (off | admins | all) invece di un booleano: "obbligatoria"
     * senza dire per chi non e' una decisione, e' meta' decisione.
     */
    public function twoFactorEnforceSet(Request $req): Response
    {
        $mode   = (string)($req->post['twofactor_action'] ?? '');
        $reason = trim((string)($req->post['_audit_reason'] ?? ''));
        $actor  = (string)(\App\Core\Auth::user()['username'] ?? 'unknown');

        // Stessa soglia del middleware `audit_reason`: vedi tosEnforceSet().
        if (mb_strlen($reason) < 10) {
            return $this->redirectFlash('twofactor_enforce_no_reason', 'error');
        }
        if (mb_strlen($reason) > 200) {
            $reason = mb_substr($reason, 0, 200);
        }
        if (!in_array($mode, ['off', 'admins', 'all', 'reset'], true)) {
            return $this->redirectFlash('twofactor_enforce_bad_action', 'error');
        }

        try {
            $before = TwoFactorEnforcement::mode();

            if ($mode === 'reset') {
                TwoFactorEnforcement::clearRuntime();
                $after = TwoFactorEnforcement::mode();
                $this->auditSwitch(
                    $actor,
                    "2fa_enforce: {$before}→{$after} (reset)",
                    "override rimosso, torna a valere .env; motivazione: $reason",
                    'twofactor_enforce_switch',
                );
                return $this->redirectFlash('twofactor_enforce_reset', 'ok');
            }

            // Quante persone dovranno passare dall'iscrizione prima di poter
            // usare il resto: va scritto, altrimenti fra sei mesi nessuno sa
            // quanto pesò la decisione.
            $pending = $this->twoFactorPendingCount($mode);
            TwoFactorEnforcement::persistRuntime($mode, $actor, $reason);
            $this->auditSwitch(
                $actor,
                "2fa_enforce: {$before}→{$mode}",
                "utenti da accompagnare all'iscrizione: $pending; motivazione: $reason",
                'twofactor_enforce_switch',
            );
            return $this->redirectFlash('twofactor_enforce_' . $mode, 'ok');
        } catch (\Throwable $e) {
            error_log('[admin/system/2fa-enforce] ' . $e->getMessage());
            return $this->redirectFlash('exception', 'error');
        }
    }

    /**
     * Quanti utenti dei ruoli obbligati non hanno ancora attivato la 2FA.
     * Con $mode null si guarda la modalita' in vigore adesso.
     */
    private function twoFactorPendingCount(?string $mode = null): int
    {
        $roles = $mode === null
            ? TwoFactorEnforcement::requiredRoles()
            : (['off' => [], 'admins' => ['super_admin', 'administrator'],
                'all' => ['super_admin', 'administrator', 'teacher']][$mode] ?? []);
        if ($roles === []) {
            return 0;
        }
        try {
            ...
            $st = \App\Core\Database::connection()->prepare(
                "SELECT COUNT(*) FROM users
                  WHERE active = 1 AND role IN ($in)
                    AND (totp_enabled IS NULL OR totp_enabled = 0)"
            );
            $st->execute($roles);
    ...
    /** Quanti teacher/admin non hanno accettato la versione vigente. */
    private function tosPendingCount(): int
    {
        try {
            if (!\App\Core\Config::get('database.enabled') || !Database::isAvailable()) {
                return 0;
            }
            $svc = new \App\Services\Gdpr\TosAcceptanceService();
            $rows = Database::connection()
                // ADR-040: `admin` non esisteva; il super-amministratore è esente.
                ->query("SELECT id FROM users WHERE role IN ('teacher','administrator','institute_admin') AND is_super_admin = 0")
                ->fetchAll(\PDO::FETCH_COLUMN);
            $n = 0;
            foreach ($rows ?: [] as $id) {
                if (!$svc->hasAccepted((int)$id)) {
                    $n++;
                }
            }
            return $n;
    ...
    /** @return list<array<string,mixed>> docenti attivi + profilo assegnato */
        ...
        try {
            $stmt = Database::connection()->query(
                "SELECT id, username, role, capability_profile_id,
                 ...
                 FROM users
                 WHERE role = 'teacher' AND active=1 AND deleted_at IS NULL
    ...
    private function auditSwitch(
        string $actor,
        string $transition,
        string $details,
        string $action = 'deployment_mode_switch',
    ): void {
        try {
            if (!\App\Core\Config::get('database.enabled')) {
                return;
            ...
            // Il default 'super_admin' era una speranza: se Auth::user() c'era,
            // vinceva sempre users.role ('teacher') e il flag super-admin non
            // compariva mai. actorRole() guarda is_super_admin.
            $role     = Auth::actorRole();
            // Hash, non in chiaro (2026-09-03): vedi RequestFingerprint.
            $ip       = \App\Services\Audit\RequestFingerprint::ipHash();
            Database::connection()->prepare(
                'INSERT INTO privileged_access_log
                    (user_id, actor_name, actor_role, action,
                     resource_type, resource_id, reason, outcome, ip_hash, created_at)
                 VALUES (?, ?, ?, ?, ?, NULL, ?, ?, ?, NOW())'
            )->execute([
                $userId,
                $actor,
