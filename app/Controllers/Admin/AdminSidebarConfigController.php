...
final class AdminSidebarConfigController
{
    ...
    public function page(Request $req): Response
    {
        ...
        $body = $view->render('admin/sidebar-config', [
            ...
            'pubblica_in_rete' => [
                'scelto'    => PublicContentPolicy::sceltoPerLaRete(),
                'effettivo' => PublicContentPolicy::proprietarioPubblico(),
                'docenti'   => $this->docentiAttivi(),
            ],
        ...
        try {
            $rows = \App\Core\Database::connection()->query(
                "SELECT id, username, first_name, last_name FROM users
                 WHERE role = 'teacher' OR is_super_admin = 1
    ...
    /**
     * POST /admin/sidebar-config/pubblica-in-rete — il docente i cui contenuti
     * pubblicati sono visibili senza login, nelle sezioni «Pubblica in rete».
     * Vuoto toglie la scelta: senza login resta solo l'accesso.
     * Rende pubblici i contenuti di un docente: per questo chiede il motivo.
     */
    public function pubblicaInRete(Request $req): Response
    {
        $valore = trim((string)($req->post['docente'] ?? ''));
        if ($valore !== '' && !ctype_digit($valore)) {
            return $this->back('Scelta non valida: la scelta di prima resta.', 'error');
        }
        $docente = $valore === '' ? null : (int)$valore;
        try {
            PublicContentPolicy::scegliPerLaRete($docente);
        } catch (\InvalidArgumentException) {
            return $this->back('Non è un docente attivo: la scelta di prima resta.', 'error');
        }
        return $docente === null
            ? $this->back('Nessun docente in rete: senza login la barra mostra solo l\'accesso.', 'success')
            : $this->back('Docente per la rete salvato.', 'success');
    }

    /** I docenti attivi, fra cui scegliere chi pubblica in rete. @return list<array{id:int,label:string}> */
    private function docentiAttivi(): array
    {
        if (!\App\Core\Database::isAvailable()) {
            return [];
        }
        try {
            ...
            return array_map(static function ($r) {
                $name = trim((string)($r['last_name'] ?? '') . ' ' . (string)($r['first_name'] ?? ''));
                $utente = (string)$r['username'];
                return ['id' => (int)$r['id'], 'label' => $name !== '' ? "$name ($utente)" : $utente];
            }, $rows);
        } catch (\Throwable) {
            return [];
        }
