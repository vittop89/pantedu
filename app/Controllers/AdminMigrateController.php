...
final class AdminMigrateController
{
    ...
    public function page(Request $req): Response
    {
        ...
        // Il nonce della CSP sullo script del modulo (23/9/2026, A-16).
...
<form id="runForm">
    <p><label>Motivazione (obbligatoria, finisce nel registro degli accessi privilegiati):<br>
        <input type="text" name="_audit_reason" required minlength="10" maxlength="255" style="width:100%"
               placeholder="es. rilascio del … senza accesso SSH: applico le migrazioni in attesa"></label></p>
...
document.getElementById('runForm').addEventListener('submit', async (ev) => {
    ev.preventDefault();
    // 23/9/2026 (A-69) — la motivazione va nel corpo (`_audit_reason`), non in
    // un'intestazione: un'intestazione accetta solo ISO-8859-1, e una
    // lineetta lunga scritta a mano farebbe fallire la richiesta nel browser.
    const motivo = ev.target.elements['_audit_reason'].value.trim();
    ...
    try {
        const r = await fetch('/admin/migrate/run', {
            ...
            body: new URLSearchParams({ _audit_reason: motivo }).toString(),
        });
...
HTML;
    ...
    public function run(Request $req): Response
    {
        ...
        if (!Config::get('database.enabled')) {
            return Response::fail('db_disabled', 503);
        }

        try {
            @set_time_limit(120);
            // La connessione delle migrazioni, non quella dell'applicazione.
            //
            // Le migrazioni fanno DDL: CREATE, ALTER, DROP. Usando qui
            // `Database::connection()` quei permessi dovevano stare addosso a
            // `pantedu_app`, cioe' all'utente con cui il sito serve **ogni**
            // richiesta — e infatti ce li aveva. Un'iniezione SQL in una
            // qualunque pagina avrebbe potuto cancellare tabelle.
            //
            // `Database::migrationConnection()` usa `pantedu_migrator`, che
            // esiste apposta. Se `DB_MIGRATOR_USER` non e' configurata ricade
            // sulla connessione ordinaria, quindi il pannello continua a
            // funzionare dove quella separazione non c'e'.
            $migrator = new Migrator(Database::migrationConnection(), \dirname(__DIR__, 2) . '/database/migrations');
    ...
    private function getStatus(): array
    {
        ...
        // Anche qui la connessione delle migrazioni: `ensureTrackingTable()`
        // fa un `CREATE TABLE IF NOT EXISTS`, che e' DDL come il resto.
        $migrator = new Migrator(Database::migrationConnection(), \dirname(__DIR__, 2) . '/database/migrations');
