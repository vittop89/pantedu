...
{
    /** Attivazione non salvata: il messaggio del database resta nel registro. */
    private const NON_SALVATA = 'Non sono riuscito a salvare l\'attivazione: la verifica in due passaggi '
        . 'non e\' attiva. Riprova fra qualche minuto.';
    ...
    public function enable(Request $req): Response
    {
        ...
        $svc  = new TotpService();
        // 23/9/2026 — il passo del codice, non solo «giusto o sbagliato»: il
        // codice usato per l'iscrizione e' consumato come quelli del login
        // (TwoFactorPolicy::verifyTotp). Senza, chi lo vedeva sullo schermo
        // entrava con quello stesso codice per un altro minuto e mezzo.
        $passo = $svc->matchingCounter($pending['secret'], $code);
        if ($passo === null) {
        ...
        try {
            ...
            // 23/9/2026 — il messaggio del database va nel registro, non sulla
            // pagina: puo' nominare tabelle, colonne e vincoli, e all'utente
            // non dice niente che possa usare (rilievo A-74).
            error_log('[TotpController::enable] ' . $e->getMessage());
            $_SESSION['totp_flash'] = ['type' => 'error', 'msg' => self::NON_SALVATA];
            return Response::redirect('/me/2fa');
        }
    ...
    public function enableEmail(Request $req): Response
    {
        ...
        try {
            ...
            // Come in enable(): il dettaglio nel registro, non nel flash.
            error_log('[TotpController::enableEmail] ' . $e->getMessage());
            $_SESSION['totp_flash'] = ['type' => 'error', 'msg' => self::NON_SALVATA];
            return Response::redirect('/me/2fa');
        }
    ...
    public function disable(Request $req): Response
    {
        ...
        // 23/9/2026 — un errore del database si dice (rilievo A-74). Prima
        // l'eccezione si ingoiava e la pagina diceva «2FA disabilitato»
        // comunque: l'utente toglieva la voce dall'app di autenticazione, il
        // secondo fattore restava attivo, e al login successivo restava fuori.
        try {
            $stmt = Database::connection()->prepare(
            ...
            error_log('[TotpController::disable] ' . $e->getMessage());
            $_SESSION['totp_flash'] = ['type' => 'error',
                'msg' => 'Non sono riuscito a disattivare la verifica in due passaggi: e\' ancora attiva. '
                    . 'Non togliere la voce dall\'app di autenticazione, e riprova fra qualche minuto.'];
            return Response::redirect('/me/2fa');
        }
