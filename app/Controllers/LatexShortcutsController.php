...
final class LatexShortcutsController
{
    ...
    {
        $tid = Auth::check() ? TeacherContextResolver::currentTeacherId() : 0;
        try {
    ...
    public function save(Request $req): Response
    {
        ...
        if (isset($body['snippet'])) {
            ...
            if (strlen($snip) > 8192) {
                return Response::fail('snippet_too_large', 413);
            }
            $fields['snippet'] = $snip;
        }
        ...
        if (empty($fields)) {
            return Response::fail('nothing_to_save', 400);
        }
        try {
    ...
    public function adminSave(Request $req): Response
    {
        ...
        if (!is_array($groups)) {
            return Response::fail('invalid_groups', 400);
        }
        $clean = $this->sanitizeGroups($groups);
        if ($clean === null) {
            return Response::fail('invalid_shape', 422);
        }
        try {
