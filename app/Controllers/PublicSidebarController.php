...
final class PublicSidebarController
{
    public function section(Request $req, array $params): Response
    {
        ...
        // Senza un docente che pubblica in rete nessuna sezione è pubblica (la
        // barra dei visitatori mostra solo l'accesso): stessa risposta.
        $sec = PublicContentPolicy::proprietarioPubblico() > 0
            ? (new SidebarSectionRepository())->publicSectionByKey($key)
            : null;
        if (!$sec) {
