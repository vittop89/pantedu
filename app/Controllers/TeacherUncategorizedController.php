...
final class TeacherUncategorizedController
{
    ...
    {
        // ADR-035 — le voci che il docente ha spuntato (curriculum_teacher),
        // con l'etichetta che ha scelto lui se ne ha una.
        $st = \App\Core\Database::connection()->prepare(
            'SELECT c.id, c.code, COALESCE(ct.label_override, c.label) AS label,
              ...
              WHERE c.kind = ? AND ct.user_id = ? AND ct.active = 1 AND c.active = 1
