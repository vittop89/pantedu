...
final class AdminController
{
    ...
    public function debugLog(Request $req): Response
    {
        try {
            ...
            return Response::fail('internal_error', 400);
        }
    }
...
<main style="max-width:720px;margin:2rem auto;padding:1.5rem;background:#fff;border-radius:8px;box-shadow:0 2px 8px rgba(0,0,0,.08)">
  ...
  <p class="fm-muted">Hash bcrypt sicuro. Cost 10–14.</p>
...
HTML;
    ...
    /** GET /admin/tools — vecchio indirizzo degli strumenti, che ora sono /admin (2026-09-04). */
    ...
    public function dashboard(Request $req): Response
    {
        ...
        $counts = [
            ...
            // Dal registro degli accessi, non più dalle statistiche, che dal
            // 24/9/2026 non tengono i nomi utente (AccessLogger::updateStats).
            'today'     => $logger->utentiDiOggi(),
            'indirizzi' => count($curr->listActive('indirizzi')),
            'materie'   => count($curr->listActive('materie')),
            'pending'   => count($pending),
        ];
        // Piano classi-credenziali-scenari, B — i riquadri che dipendono dallo
        // scenario si calcolano solo quando l'area e' attiva: negli scenari 1
        // e 2 la porta degli studenti e' la credenziale di classe, nel 3 sono
        // gli account con la sezione.
        if (\App\Support\AdminAreas::isActive('class_credentials')) {
            $counts['class_credentials'] = (new \App\Repositories\TeacherCredentialRepository())->countActive();
        }
        if (\App\Support\AdminAreas::isActive('students_without_section')) {
            $counts['students_without_section'] = (new \App\Services\TeacherSectionService())
                ->countStudentsWithoutSection(Auth::currentInstitute());
        }
    ...
    {
        // Dati d'istanza: i registri li scrivono AccessLogger e LogRotator
        // sotto `app.paths.logs`, cioè la cartella dei dati. `app.paths.base`
        // è sempre la radice del repository, che nel container è l'immagine:
        // l'elenco puntava a tre file che lì dentro non esistono.
