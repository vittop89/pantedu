...
final class CurriculumController
{
    ...
    public function index(Request $req): Response
    {
        ...
        return Response::json([
            ...
            'institute_id' => $instituteId,
            'curriculum' => $cur,
            // Vocabolario degli indirizzi della SCUOLA (righe owner NULL). Sono
            // esattamente quelli che CurriculumService::add() accetta da un
            // docente: l'editor ci popola una tendina invece di un campo
            // libero, cosi' il rifiuto non arriva dopo aver digitato una sigla
            // che non poteva funzionare.
            'institute_indirizzi' => array_values(array_map(
                static fn(array $r): array => [
                    'code'  => $r['code'],
                    'label' => $r['label'],
                    'group' => $r['group'] ?? null,
                ],
                $this->svc->all($instituteId, null)['indirizzi']
            )),
        ]);
    }
