...
final class VerificaCompileController
{
    ...
    public function compilePdf(Request $req, array $params): Response
    {
        try {
            ...
            if ($texOverride !== '' && \strlen($texOverride) > 5 * 1024 * 1024) {
                return Response::fail('tex_override_too_large', 413);
            }
            ...
            // 20/9/2026 — `tempo_scaduto` esclude il caso che non è transitorio:
            // la connessione c'era e la compilazione ha superato il tetto del
            // client. Il servizio sta ancora lavorando su quel documento;
            // rimandarglielo altre due volte gli raddoppia il carico e al
            // docente fa aspettare il triplo per la stessa risposta.
            $isTransientNetwork = static fn(array $r): bool =>
                empty($r['tempo_scaduto'])
            ...
            if (!$result['ok']) {
                // Tre esiti diversi, e vanno detti diversi (20/9/2026): un
                // documento troppo lungo non è un guasto di rete, e 504 è il
                // codice che lo dice. Prima finiva in `tex_compile_network`
                // (502), cioè «il servizio non si raggiunge», che mandava a
                // guardare firewall e indirizzi.
                $troppoLunga = !empty($result['tempo_scaduto']);
                $isNetwork = !$troppoLunga
                    && (($result['http_status'] ?? 0) === 0
                        || ($result['http_status'] ?? 0) >= 500);
                $errCode   = $troppoLunga
                    ? 'tex_compile_timeout'
                    : ($isNetwork ? 'tex_compile_network' : 'tex_compile_failed');
                $httpCode  = $troppoLunga ? 504 : ($isNetwork ? 502 : 422);
    ...
    public function getJob(Request $req, array $params): Response
    {
        try {
            ...
            if (!$job) {
                return Response::fail('job_not_found', 404);
            }
    ...
    public function synctexEdit(Request $req, array $params): Response
    {
        try {
            ...
            // Vuoto → empty_payload, non JSON → invalid_json (400), troppo
            // grande → 413: il lettore comune (ADR-034, A-52), mappato da
            // statusFor().
            $body = $req->jsonObbligatorio();
            if (!isset($body['synctex_gz_b64'], $body['page'], $body['x'], $body['y'])) {
            ...
            if ($synctexGz === false || $synctexGz === '') {
                return Response::fail('invalid_synctex_b64', 400);
            }
            $page = (int)$body['page'];
            $x = (float)$body['x'];
            $y = (float)$body['y'];
