 ...
 *   - conferma = nuova
 * Aggiorna password_hash nel database.
 *
 * 2026-09-24 — fino a quel giorno leggeva e aggiornava anche `users.json`, la
 * copia degli account approvati che `RegistrationService` teneva accanto al
 * database: se la riga del database non aveva un hash, valeva quello della
 * copia. Il login non legge quella copia dalla Phase 18 (`UserRepository` sta
 * sul database), quindi non serviva a nessuno che fosse entrato; poteva solo
 * far valere come «password attuale» quella di un account a cui il database
 * l'aveva tolta. La copia non si scrive più, e il lavoro notturno la cancella
...
/* best-effort */
    ...
    /**
     * L'hash della password attuale, dal database e basta. Senza database non
     * c'è una password da verificare: lo si dice (`database_unavailable`)
     * invece di rispondere «Password attuale errata» a chi l'ha scritta
     * giusta.
     */
    private function currentHash(string $username): ?string
    {
        ...
        try {
            ...
            // Il testo del database va nel registro, non nell'indirizzo della
            // pagina (changePassword() mette il codice nel `?error=`).
            error_log('[UserProfileController] lettura della password fallita: ' . $e->getMessage());
            throw new RuntimeException('database_unavailable');
        }
    ...
    private function persistHash(string $username, string $newHash): void
    {
        ...
        try {
            ...
            error_log('[UserProfileController] scrittura della password fallita: ' . $e->getMessage());
            throw new RuntimeException('persist_failed');
        }
        if ($stmt->rowCount() === 0) {
            throw new RuntimeException('persist_failed');
    ...
    private function errorMessage(string $code): ?string
    {
        ...
        return match ($code) {
            ...
            'database_unavailable' => 'Il servizio non è disponibile in questo momento: la password non è cambiata. Riprova più tardi.',
            'persist_failed'   => 'Non è stato possibile salvare la nuova password: quella attuale resta valida. Riprova.',
            default            => 'Errore: ' . $code,
        };
    }
}
