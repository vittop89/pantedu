...
{
    /**
     * Dove stanno le immagini sostituite (override del docente e baseline
     * istituzionale): dati d'istanza sotto PANTEDU_DATA_PATH, non nella radice
     * del repository. Nel container quella radice è l'immagine, e un'immagine
     * caricata lì sparisce al rilascio lasciando in `risdoc_teacher_overrides`
     * un `image_hash` che punta al nulla. In git, sotto `storage/overrides/`,
     * c'è solo `.gitkeep`: nessuna base versionata, quindi niente cascata.
     */
    private static function radiceOverride(): string
    {
        return \App\Support\PercorsiDati::base(dirname(__DIR__, 3)) . '/storage/overrides';
    }
...
// Phase 24.56 — no cache: invalidato istantaneamente dopo edit admin.
    ...
    public function tex(Request $req, array $params): Response
    {
        ...
        $data = ['fields' => [], 'state' => []];
        // Le compilazioni sono cifrate (migration 101): si leggono solo dal
        // repository, che decifra. Niente piu' SELECT data_json diretti.
        $compRepo = new \App\Repositories\Risdoc\CompilationRepository();
        if ($cid > 0) {
            $c = $compRepo->find($tid, $cid);
            if ($c) {
                $data = json_decode((string)$c['data_json'], true) ?: $data;
            }
        } else {
            $data = $compRepo->latestData($tid, $id) ?: $data;
        }
    ...
    public function file(Request $req, array $params): Response
    {
        ...
        // Solo i kind che il client chiede (risdoc-editor.js e i due gestori
        // delle immagini). `schema` ha la sua rotta, con il percorso preso dal
        // modello e non dalla richiesta (23/9/2026, revisione Risdoc A1).
        if (!self::kindAmmesso($kind)) {
            return Response::json(['error' => 'kind_not_allowed'], 400);
        }
...
// Phase 24.28 — texCommon: bypass resolver, lookup diretto
        ...
        if ($kind === 'image') {
            ...
            if ($result) {
                ...
                if (($result['source'] ?? '') === 'override' && ($result['image_hash'] ?? null)) {
                    $absolute = self::radiceOverride() . '/teacher_' . $tid . '/' . $result['image_hash'];
                }
            }
    ...
    public function legacyPath(Request $req, array $params): Response
    {
        ...
        // In CASCATA: prima i dati d'istanza, poi l'immagine. È la rotta che
        // usa il fetcher delle opzioni in modalità `file`
        // (js/components/risdoc/_options-fetcher.js): dal 20/9/2026 il
        // pannello dell'amministratore salva nei dati, e leggendo dalla sola
        // immagine questa rotta avrebbe servito per sempre la versione
        // vecchia mentre il pannello mostrava quella nuova.
        $abs = \App\Support\PercorsiDati::inCascata(
            'storage/templates/risdoc/' . str_replace(['\\'], '/', $rel),
            dirname(__DIR__, 3),
        );
        // 2026-09-04 — il logo della scuola non e' un asset del codice: e' il
        // file dell'istanza per l'Istituto del docente, se caricato. Senza,
        // 404: l'anteprima mostra la cella vuota, come il PDF.
        if ($rel === 'images/logo_scuola.png') {
            $logo = \App\Services\Risdoc\InstituteAssets::logoPathFor($tid);
            if ($logo !== null) {
                return $this->mimeResponse($rel, (string)@file_get_contents($logo));
            }
            return Response::json(['error' => 'not_found'], 404);
        }
// Se match override per-teacher (JSON o asset), cercalo
        if (preg_match('/\.json$/i', $rel)) {
            ...
            if ($tmplId !== null) {
                $ov = (new \App\Repositories\Risdoc\OverrideRepository())->find($tid, $tmplId, 'json', $rel);
                if ($ov && $ov['body'] !== null) {
    ...
    public function optionsSources(Request $req): Response
    {
        ...
        $files = [];
        $folders = [];
        // Entrambe le radici, i dati per primi: la base versionata viaggia
        // nell'immagine, gli scostamenti dell'amministratore stanno nei dati
        // (RisdocAdminController, dal 20/9/2026). Guardando la sola immagine,
        // un file NUOVO creato dal pannello non sarebbe mai comparso in
        // questo catalogo, e il docente non avrebbe potuto sceglierlo.
        $visti = [];
        $viste = [];
// Scan ricorsivo, max depth 4, solo directory non-legacy
        $skip = ['images', 'texCommon', 'MODELLI', 'RISORSE'];
        foreach (\App\Support\PercorsiDati::radiciInCascata('storage/templates/risdoc', dirname(__DIR__, 3)) as $root) {
            if (!is_dir($root)) {
                continue;
            }
            ...
            foreach ($it as $entry) {
                ...
                if ($entry->isFile() && strtolower($entry->getExtension()) === 'json' && !isset($visti[$rel])) {
                    $visti[$rel] = true;
                    $files[] = [
                        'path'  => $rel,
                ...
                if ($entry->isDir() && !isset($viste[$rel])) {
     ...
     */
    /** I kind che GET /api/risdoc/templates/{id}/file serve. */
    public const KIND_DEI_FILE = ['html', 'tex', 'css', 'json', 'image', 'texCommon'];

    public static function kindAmmesso(string $kind): bool
    {
        return \in_array($kind, self::KIND_DEI_FILE, true);
    }
...
// Phase 24.58 — instance_key + instance_label optional
        ...
        // image: upload multipart via $_FILES['file']
                ...
                $dir = self::radiceOverride() . '/teacher_' . $tid;
    ...
    public function institutionalOverrideSave(Request $req, array $params): Response
    {
        ...
        try {
            ...
            if ($kind === 'image') {
                ...
                $hash = \hash_file('sha256', $tmp);
                $base = self::radiceOverride() . '/institutional';
