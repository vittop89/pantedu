...
// Phase 24.58 — multi-instance: query ?instance=KEY oppure default ''
        ...
        // chip visivo ADMIN/COLLAB + ✕ Esci.
        // 23/9/2026 (revisione architetturale A-19) — «✕ Esci» aveva un
        // onclick in linea (window.close(), poi /admin/templates#risdoc), che
        // la CSP rigorosa blocca: in produzione il bottone non faceva niente.
        // Ora è data-fm-chiudi-scheda, gestito da js/modules/core/declarative.js.
        ...
        $adminSection = $isAdminEdit
            ? '<span class="fm-doc-topbar__chip fm-doc-topbar__chip--admin"'
                ...
                . ' data-fm-chiudi-scheda="/admin/templates#risdoc"'
        ...
        $enterAdminBtn = (!$isAdminEdit && $canEditStruct)
            ...
            : '';
        // 2026-09-04 — i componenti passano dal bundle Vite (entry
        // risdoc-components e pt-document): Lit viene da npm, non da un CDN,
        // e il suo specifier bare si risolve solo nel bundle. ViteManifest
        // ricade sul path sorgente se il manifest manca (dev senza build).
        $pageHead = '<link rel="stylesheet" href="/css/risdoc-tokens.css">' . "\n"
                  . \App\Support\ViteManifest::script('js/components/risdoc/index.js');
        $ptDocumentScript = \App\Support\ViteManifest::script('js/components/pt-document/fm-pt-document.js');
        // Il nonce della CSP sugli script scritti qui (23/9/2026, A-16): prima
        // lo metteva la timbratura del middleware su ogni <script> del corpo.
        ...
        if ($useUnified) {
            ...
            // 2026-09-22 — sui modelli istituzionali il divieto di scrivere
            // dati personali degli studenti si legge QUI, mentre si compila,
            // e non solo nei Termini al momento dell'iscrizione. Vedi
            // AvvisoCompilazione per il perché e per le due varianti.
            $siSalva = \App\Services\Risdoc\CompilationStoragePolicy::allowedFor($tid, $tmpl);
            $avviso = \App\Services\Risdoc\AvvisoCompilazione::perModello(
                \App\Services\Risdoc\CompilationStoragePolicy::isInstitutional($tmpl),
                \App\Support\DeploymentScenario::studentAccountsEnabled(),
                $siSalva
            );
            // 2026-09-22, ADR-046 — e quanto dura la bozza. Frase separata
            // perché vale per qualunque compilazione salvata, non solo per i
            // modelli istituzionali: il docente deve saperlo mentre scrive,
            // non quando gli arriva l'avviso dell'ultimo momento.
            $scadenza = \App\Services\Risdoc\AvvisoCompilazione::scadenza($siSalva);
            $avvisoHtml = '';
            foreach ([$avviso, $scadenza] as $riga) {
                if ($riga !== null) {
                    $avvisoHtml .= '<p class="fm-risdoc-avviso" role="note">'
                        . htmlspecialchars($riga, ENT_QUOTES) . '</p>';
                }
            }
            $pageContent = <<<HTML
...
{$ptDocumentScript}
<script{$csp} src="/js/components/risdoc/fm-risdoc-section-navigator.js"></script>
HTML;
    ...
    /**
     * Il modello ha un corpo Portable Text proprio (`risdoc_templates.body_pt`,
     * una lista non vuota di blocchi)? È la fonte del documento quando manca
     * lo schema: la shell la carica dal master (RisdocTemplateAdapter).
     *
     * @param array<string, mixed> $tmpl
     */
    private static function haUnCorpo(array $tmpl): bool
    {
        $grezzo = $tmpl['body_pt'] ?? null;
        if (!\is_string($grezzo) || \trim($grezzo) === '') {
            return false;
        }
        $corpo = json_decode($grezzo, true);
    ...
    public function show(Request $req, array $params): Response
    {
        ...
        // Prima l'esistenza, poi il permesso: un id inesistente è un 404, non un
        // 403 (2026-09-05: rispondeva «non hai accesso» anche per modelli mai esistiti).
        ...
        // Se il template ha schema_path o un body_pt → shell <fm-pt-document
        // source="risdoc-template"> (onepath ADR-026 #3). Altrimenti
        // fallback legacy html_file PHP.
        //
        // 23/9/2026 — anche il body_pt. I modelli creati dal pannello
        // («crea modello», RisdocAdminController::createTemplate) nascono
        // con il solo body_pt e senza schema_path: guardando soltanto lo
        // schema cadevano nel ripiego legacy, che cercava un file HTML che
        // per un modello nato nel database non c'è, e rispondevano 404
        // (revisione architetturale del 23/9/2026, A-23; prova
        // tests/Integration/CreaModelloSiApreTest.php).
        $schemaPath = (string)($tmpl['schema_path'] ?? '');
        if ($schemaPath !== '' || self::haUnCorpo($tmpl)) {
            return $this->renderWebComponent($req, $tmpl, $id, $tid);
        }

        $headExtra = '';

        // 2026-09-08 — qui c'era un blocco dentro `if (false)`: rendeva il
        // modulo dallo schema con `FormRenderer`, precompilando i campi
        // dall'ultima compilazione del docente. Era spento fin dall'import
        // iniziale del repository — non spento dopo un incidente: nato spento,
        // pezzo di una modernizzazione («Plan A», dice il suo docblock) che non
        // è mai partita. Nessun test lo copriva, e riattivarlo avrebbe
        // significato mandare in produzione codice mai esercitato.
        //
        // Tolto insieme a `FormRenderer`, a `lookupLastCompilation()` e ai
        // quattordici partial che servivano solo a lui. Se la modernizzazione
        // riparte, riparte da un disegno aggiornato: questo sta nella storia
        // di git. Voce 83 del debito.
        //
        // 2026-09-23 — tolto anche il residuo di quel blocco: un
        // `if ($htmlBody === '')` subito dopo `$htmlBody = ''`, sempre vero
        // (PHPStan: identical.alwaysTrue, fino a oggi taciuto da un'eccezione
        // in phpstan.neon). Aveva senso solo quando il blocco spento poteva
        // riempire `$htmlBody`; ora il file HTML si carica sempre, come
        // succedeva già. Revisione architetturale del 23/9/2026, A-6.

        // Carica html_file PHP e strip head/body tags.
        ...
        // (raggiunto solo se schema_path e body_pt sono tutti e due vuoti),
        // che storicamente caricava jQuery/MathJax per i template "risdoc".
        // 23/9/2026 — resta perché non è dimostrabile che sia irraggiungibile:
        // un super-amministratore svuota il body_pt di un modello senza schema
        // con `POST /api/risdoc/templates/{id}/body-pt` (`body_pt=`), e in
        // produzione i modelli senza schema non sono stati contati. Voce 127.
        $category    = htmlspecialchars((string)$tmpl['category'], ENT_QUOTES);
        $numArg      = htmlspecialchars((string)$tmpl['num_arg'], ENT_QUOTES);
        $title       = htmlspecialchars(str_replace('_', ' ', (string)$tmpl['argomento']), ENT_QUOTES);
...
// ── Build $pageHead (asset esclusivi della pagina risdoc) ──
        $pageHead = '';
        // Il nonce della CSP sui due script scritti qui (23/9/2026, A-16). Gli
        // <script> del file HTML del modello ($headExtra, $htmlBody) non lo
        // hanno: sono contenuto, e con la CSP rigorosa non partono (prima li
        // timbrava il middleware). Qui arriva solo un modello senza
        // schema_path e senza body_pt (voce 127 del debito).
        $csp = \App\Support\Csp::attributo();
        if (true) {
        // history.replaceState → URL legacy-compatibile PRIMA di risdoc.js
            $pageHead .= '<script' . $csp . '>if (location.pathname.startsWith("/risdoc/view/")) { try { history.replaceState(null, "", '
            ...
            // 2026-09-04: via anche MathJax 2.7.7 e jszip da cdnjs — jszip non
            // era usato da nessun modulo e le formule le rende il MathJax 4
            // locale caricato dal layout (revisione 2026-09, rilievo A4).
        }
        if (!empty($cssBody)) {
