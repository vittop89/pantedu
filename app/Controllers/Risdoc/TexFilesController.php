 ...
 *
 * Riusa ExportController::buildFiles per coerenza con il flusso ZIP.
 *
...
final class TexFilesController
{
    ...
    public function getFiles(Request $req, array $params): Response
    {
        ...
        $files = [
            ...
            // 2026-09-04 — dati dell'Istituto per l'intestazione: generati dal
            // profilo, o override del docente/dell'istanza (vedi ExportController).
            ['path' => 'texCommon/risdoc-istituto.tex',     'content' => $built['istitutoBody'] ?? '', 'editable' => true, 'kind' => 'texCommon', 'rel' => 'risdoc-istituto.tex'],
        ];
     ...
     * SOLO per i 4 file editable (main.tex, risdoc.sty, intestaLAteX_IIS.tex,
     * risdoc-istituto.tex).
    ...
    public function saveFiles(Request $req, array $params): Response
    {
        ...
        $allowed = [
            ...
            'texCommon/risdoc-istituto.tex'  => 'risdoc-istituto.tex',
        ];

        $repo = new OverrideRepository();
        $saved = [];
        $skipped = [];
        // 21/9/2026 — la versione di partenza della copia è l'impronta del
        // modello ADESSO, non una data.
        //
        // Prima qui si scriveva `'manual-' . date('Y-m-d')`, che non coincide
        // con nessuna impronta: `source_version != source_hash` è la
        // definizione stessa di «copia disallineata», quindi ogni file salvato
        // da questo riquadro nasceva già segnalato come da riallineare, e
        // restava così per sempre. Misurato in produzione il 21/9/2026: sul
        // «Piano annuale» otto copie disallineate su otto, e quattro erano
        // state scritte quello stesso giorno, dalla versione corrente.
        $impronta = (string)($this->resolver->findTemplate($id)['source_hash'] ?? '');
    ...
    public function compilePdf(Request $req, array $params): Response
    {
        ...
        $bundle = [
            ...
            'texCommon/risdoc-istituto.tex'     => (string)($built['istitutoBody'] ?? ''),
        ];
        // Immagini (stemma in intestaLAteX_IIS via \includegraphics{images/…}).
        // Senza, il compile della preview falliva (l'export ZIP le includeva già).
        // Il logo della scuola non e' nel repo: e' il file dell'istanza per
        // l'Istituto del docente (InstituteAssets); se manca, il template lo salta.
        ...
        if (!empty($built['logoPath']) && is_file((string)$built['logoPath'])) {
            $bundle['images/logo_scuola.png'] = (string)file_get_contents((string)$built['logoPath']);
        }
