...
final class TikzRenderController
{
    ...
    public function lookup(Request $req): Response
    {
        ...
        if ($this->service === null) {
            ...
            return Response::fail('service_unavailable', 503);
        }
    ...
    public function render(Request $req): Response
    {
        ...
        // Il client JS POSTa application/json; il form resta accettato. JSON
        // dichiarato, altrimenti il form, altrimenti un corpo che sembra JSON:
        // è la regola di Request::body() (ADR-034, che legge il Content-Type
        // da $_SERVER['CONTENT_TYPE']), ricopiata qui fino al 23/9/2026 (A-52).
        $body = $req->body();
        ...
        if ($tikz === '' || \strlen($tikz) > TikzRenderService::MAX_SOURCE_BYTES) {
            return Response::fail('tikz_size_invalid', 400);
        }
        if (!\in_array($scope, [TikzRenderService::SCOPE_PUBLIC, TikzRenderService::SCOPE_TEACHER], true)) {
            return Response::fail('invalid_scope', 400);
        }
        ...
        // - SCOPE_PUBLIC: chi ha un account, e — dal 21/9/2026 — anche l'ospite
        //   entrato con la credenziale di classe del docente. La cache e'
        ...
        //   limiting + middleware route gia' protegge da abuse.
        //
        //   Prima bastava `Auth::check()`, e chi studiava con la credenziale
        //   vedeva «[TikZ render error] unauthenticated» al posto di ogni
        //   figura non ancora in cache: la pagina gliela serviamo, ma il pezzo
        //   che la disegna glielo negavamo. La credenziale e' l'autorizzazione
        //   del docente, ed e' riverificata sul database a ogni richiesta
        //   (StudioMiddleware).
        $teacherId = 0;
        if ($scope === TikzRenderService::SCOPE_TEACHER) {
            $teacherId = $this->currentTeacherId();
        ...
        } elseif (!Auth::check() && !\App\Support\ClassAccessGrant::isActive()) {
            return Response::fail('auth_required', 401);
        }
        ...
        try {
            ...
            // `errors` (24/9/2026): gli errori di pdflatex. Il client, se ci
            // sono, non ritenta — è il disegno a essere sbagliato, non un
            // intoppo del servizio — e li mostra.
            return Response::json([
                'ok'     => false,
    ...
    private function currentTeacherId(): int
    {
        ...
        // public; l'admin scrive solo public).
