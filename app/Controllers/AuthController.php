...
final class AuthController
{
    ...
    public function showLogin(Request $req): Response
    {
        ...
        // 23/9/2026 — qui la sezione del redirect finiva in sessione come
        // `selectedIIS` e `selectedCLS`, che nessun codice lato server legge
        // (le chiavi omonime del front-end stanno in sessionStorage):
        // tolte (revisione architetturale del 23/9/2026, A-59).
    ...
    public function login(Request $req): Response
    {
        ...
        // 23/9/2026 — l'IP di EdgeContext, come il limitatore e il WAF. Qui si
        // prendeva `Client-IP`, poi il primo elemento di `X-Forwarded-For`:
        // due header che sceglie il client. Il blocco dell'IP per sezione si
        // aggirava mandandone uno inventato, e con quello di un altro si
        // poteva far scattare il suo (revisione architetturale 2026-09, A-63).
        ...
        if (!$user) {
            ...
            if ($reason !== Auth::REASON_RATE_LIMITED) {
                (new \App\Services\Waf\WafBruteforceGuard())->registerFailure($ip, $username);
            }
    ...
    private function landingFor(object $user, string $redirect): string
    {
        ...
        // ADR-040 — l'amministratore di istituto non ha altre zone: la sua pagina.
        if ($noPreference && (string)($user->role ?? '') === \App\Domain\Role::INSTITUTE_ADMIN->value) {
            return '/istituto';
        }
        return $safe;
    }
    ...
    public function show2fa(Request $req): Response
    {
        ...
        $metodo   = (string)($pending['method'] ?? 'app');
    ...
    {
        // 23/9/2026 — la rotta non ha `auth`: la sessione revocata si chiude
        // qui, prima di rispondere. Senza, un account disattivato o cambiato
        // riceveva un'ultima volta «autenticato», e la sessione si chiudeva
        // solo dopo, alla domanda sul flag di super-admin.
        if (!Auth::check() || Auth::chiudiSeRevocata()) {
    ...
    private function errorMessage(?string $code): string
    {
        return match ($code) {
            ...
            Auth::REASON_SCENARIO     => 'Questo account amministra un istituto, e si usa solo quando la piattaforma è adottata da un Istituto.',
    ...
    /** POST /accesso-classe/esci — una credenziale (credential_id) o tutto il portachiavi. */
    public function classAccessLogout(Request $req): Response
    {
        ...
        if (\App\Support\ClassAccessGrant::all() === []) {
            \App\Support\ClassKeychainCookie::clear();
        }
        return Response::redirect($cid > 0 ? '/accesso-classe' : '/', 303);
    }
    ...
    public function showClassAccess(Request $req): Response
    {
        ...
        $body = $view->render('auth/class_access', [
            'csrf'             => Csrf::token(),
            'scenario'         => \App\Support\DeploymentScenario::snapshot(),
            // Piano classi, C — il portachiavi corrente, gli avvisi e se il
            // dispositivo puo' essere ricordato.
            ...
            'errore'           => (string)($req->query['errore'] ?? ''),
        ]);
