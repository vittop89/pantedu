...
final class TeacherDrawioLibraryController
{
    ...
    public function upload(Request $req): Response
    {
        ...
        try {
            ...
            if ($size <= 0 || $size > self::MAX_BYTES) {
                return Response::fail('file_size_invalid', 400);
            }
            $name = (string)($_FILES['file']['name'] ?? '');
            if (!preg_match(self::ALLOWED_NAME, $name)) {
                return Response::fail('invalid_filename', 400);
            }
            if (!str_ends_with(strtolower($name), '.xml')) {
                return Response::fail('must_be_xml', 400);
            }
            ...
            if (stripos($content, '<mxlibrary') === false) {
                return Response::fail('not_a_drawio_library', 400);
            }

            $teacherDir = $this->teacherDir($tid);
            if (!is_dir($teacherDir)) {
                if (!@mkdir($teacherDir, 0775, true) && !is_dir($teacherDir)) {
                    return Response::fail('mkdir_failed', 500);
                }
            }
    ...
    public function delete(Request $req): Response
    {
        ...
        try {
            ...
            if (!preg_match(self::ALLOWED_NAME, $name)) {
                return Response::fail('invalid_name', 400);
            }
            ...
            if (!@unlink($target)) {
                return Response::fail('unlink_failed', 500);
            }
            return Response::ok();
        } catch (Throwable $e) {
            return Response::json(['ok' => false, 'error' => $e->getMessage()], 500);
    ...
    public function saveContent(Request $req): Response
    {
        ...
        try {
            ...
            if (strlen($xml) === 0 || strlen($xml) > self::MAX_BYTES) {
                return Response::fail('xml_size_invalid', 400);
            }
    ...
    /**
     * Le librerie drawio sono dati d'istanza e non hanno nessun riscontro nel
     * database: il file è l'unica copia. Stanno sotto PANTEDU_DATA_PATH, non
     * nella radice del repository — che nel container è l'immagine. Di questo
     * albero in git non c'è niente (`git ls-files storage/templates/drawio` è
     * vuoto): nessuna base versionata da leggere dall'immagine, quindi niente
     * cascata.
     */
    private function radiceDrawio(): string
    {
        return \App\Support\PercorsiDati::base(dirname(__DIR__, 2)) . '/storage/templates/drawio';
    }

    private function defaultDir(): string
    {
        return $this->radiceDrawio() . '/_default';
    }

    private function teacherDir(int $tid): string
    {
        return $this->radiceDrawio() . "/teachers/{$tid}";
    }
}
