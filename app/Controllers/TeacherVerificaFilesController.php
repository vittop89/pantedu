...
final class TeacherVerificaFilesController
{
    public function listFiles(Request $req): Response
    {
        ...
        foreach ([TemplateFileStore::SCOPE_DEFAULT, $instituteCode, $teacherScope] as $sc) {
            ...
            // Due radici: i dati d'istanza (dove si scrive) e il codice, che
            // porta `_default/griglie` versionato dentro l'immagine.
            foreach (TemplateFileStore::cartelleDiLettura($sc, 'griglie') as $dir) {
                if (!is_dir($dir)) {
                    continue;
                }
        ...
        foreach ($allPaths as $rel) {
            $tFile = TemplateFileStore::percorsoDiLettura($teacherScope, $rel);
            $iFile = $instituteCode ? TemplateFileStore::percorsoDiLettura($instituteCode, $rel) : null;
            $dFile = TemplateFileStore::percorsoDiLettura(TemplateFileStore::SCOPE_DEFAULT, $rel);

            $isMine     = $tFile !== null && is_file($tFile);
            $hasInstitute = $iFile !== null && is_file($iFile);
            $hasDefault = $dFile !== null && is_file($dFile);
            ...
            $size = $isMine ? (int)@filesize((string)$tFile)
                  : ($hasInstitute ? (int)@filesize((string)$iFile)
                  : ($hasDefault ? (int)@filesize((string)$dFile) : 0));
    ...
    public function previewPdf(Request $req): Response
    {
        ...
        if ($path === '') {
            return Response::fail('path_required', 400);
        }
        ...
        if ($content === null) {
            ...
            $content = TemplateFileStore::readRaw($teacherScope, $path)
                    ...
                    ?? '';
            if ($content === '') {
                return Response::fail('file_not_found', 404);
            }
        }
    ...
    public function copyFromBase(Request $req): Response
    {
        ...
        try {
            ...
            $content = $instituteCode
                ...
                : TemplateFileStore::read(TemplateFileStore::SCOPE_DEFAULT, $path);
            if ($content === null) {
                return Response::fail('base_missing', 404);
            }
            TemplateFileStore::write(TemplateFileStore::teacherScope($tid), $path, $content);
            return Response::json(['ok' => true, 'bytes' => strlen($content)]);
        } catch (Throwable $e) {
