...
final class SidebarConfigController
{
    public function config(Request $req): Response
    {
        ...
        // ADR-040 — l'amministratore di istituto non ha le sezioni dello studio:
        // la sua barra è vuota (e senza, lo si sarebbe trattato da studente).
        if (Auth::role() === \App\Domain\Role::INSTITUTE_ADMIN->value) {
            return Response::json(['ok' => true, 'sections' => []]);
        }
        $u = Auth::user();
        $role = $this->role();
    ...
    private function instituteId(?array $u, string $role): int
    {
        $uid = (int)($u['id'] ?? 0);
        if ($uid <= 0) {
            // 14/9/2026 — l'ospite con la credenziale di classe raggiunge questa
            // rotta dal gruppo dello studio: le sezioni sono quelle dell'istituto
            // della credenziale, come la barra disegnata dal server
            // (views/partials/sidebar.php). Senza credenziale, nessun istituto.
            return !Auth::check() ? \App\Support\ClassAccessGrant::instituteId() : 0;
        }
        try {
            if ($role === 'student') {
