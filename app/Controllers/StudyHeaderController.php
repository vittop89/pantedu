 ...
 * Helper condivisi duplicati: resolveUserId.
 *
 * L'intestazione è una per scuola (23/9/2026, A-7): il docente la legge e la
 * scrive nell'istituto attivo, gli studenti la leggono nella propria scuola,
 * l'ospite in quella della credenziale. Tutti e tre guardano lo stesso file,
 * `institutes/{scuola}/private/{docente}/header_page.json`. Fino a oggi il
 * docente scriveva sempre nella prima scuola, e gli studenti della seconda
 * vedevano l'intestazione predefinita. Il perché della scelta sta nel
 * glossario (wiki/glossary.md, «intestazione di studio»).
 */
final class StudyHeaderController
{
    ...
    public function headerPageJson(Request $req): Response
    {
        ...
        $iid = \App\Support\TeacherContextResolver::activeInstituteId($tid);
        if (!$iid) {
    ...
    public function headerPageStudentJson(Request $req): Response
    {
        $u = Auth::user();
        if (!$u) {
            // ADR-032 — ospite con credenziale di classe: l'intestazione e'
            // quella del docente della credenziale, nel suo istituto.
            $grant = \App\Support\ClassAccessGrant::current();
            if ($grant !== null) {
                $iid = \App\Support\ClassAccessGrant::instituteId();
                return Response::json($iid > 0
                    ? $this->loadHeaderPage($iid, $grant['teacher_id'])
                    : $this->defaultHeaderPage());
            }
            return Response::json(['error' => 'unauthorized'], 401);
        }
        $userId = (int)($u['id'] ?? 0);
