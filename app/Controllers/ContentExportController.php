...
final class ContentExportController
{
    ...
    public function export(Request $req, array $params): Response
    {
        ...
        // Portarsi via un contenuto e' un'operazione, e va a registro come le
        // altre: `content_exported` esisteva come costante ma nessuno la
        // scriveva, cosi' un export non lasciava traccia. Conta soprattutto
        // quando chi esporta non e' il proprietario.
        \App\Services\Audit\ContentActionLogger::log(
            \App\Services\Audit\ContentActionLogger::ACTION_EXPORTED,
            (int)$row['teacher_id'],
            $id,
            (string)($row['content_type'] ?? 'unknown'),
            [
                'format'        => 'tex_bundle',
                'exported_by'   => $tid,
                'own_content'   => (int)$row['teacher_id'] === $tid,
            ]
        );
        ...
        // disco. Questi `content-*.zip` erano anche quelli che nessuna pulizia
        // del codice guardava: `cleanupOld` del gemello filtrava `doc-*.zip`, e
        // a toglierli restava la sola unità oraria di sistema.
        try {
            $byte = \App\Support\PacchettoZip::byte(
                function (\ZipArchive $zip) use ($mainFinal, $docName, $docBody, $styBody, $headBody, $root): void {
        ...
        // Il nome che il docente si ritrova nella cartella dei download è il
        // titolo del suo documento, non un numero.
        $nome = (string)($row['title'] ?? ('documento-' . $id));
    ...
    private function buildTexBundle(array $row, int $tid): ?array
    {
        ...
        $context = [
            ...
            'state'  => \App\Services\Risdoc\StatoDelDocumento::arricchito([
                ...
                'sezione'    => '',
            ], \App\Services\Risdoc\StatoDelDocumento::perDocente(
                // Il nome che va nel documento è quello di chi l'ha
                // scritto, non di chi lo sta esportando.
                (int)($row['teacher_id'] ?? $tid),
            )),
        ];
    ...
    public function texFiles(Request $req, array $params): Response
    {
        ...
        $b = $this->buildTexBundle($row, $tid);
        if (!$b) {
            return Response::fail('no_body_pt', 400);
        }
        $defs = [
    ...
    public function compilePdf(Request $req, array $params): Response
    {
        ...
        try {
            ...
            return Response::fail('compile_failed', 500);
        }
        if (!($result['ok'] ?? false)) {
     ...
     * (stile ProgettoPrecedente): <!doctype html> + CSS page-doc inline + render
    ...
    {
        return \App\Domain\ViewerContext::forTeacher($teacherId, $this->privateFilesInstituteId($teacherId));
    }
