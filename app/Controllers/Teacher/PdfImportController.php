...
final class PdfImportController
{
    ...
    private PdfImportSessionRepository $sessions;
    private ProviderRouter $router;

    public function __construct(
        PdfImportSessionRepository $sessions = new PdfImportSessionRepository(),
    ...
    public function createSession(Request $req): Response
    {
        try {
            ...
            $instituteId = TeacherContextResolver::privateFilesInstituteId($teacherId);
            if ($instituteId <= 0) {
    ...
    public function previewRow(Request $req, array $params): Response
    {
        try {
            ...
            $iid = TeacherContextResolver::privateFilesInstituteId($teacherId);
    ...
    public function editCell(Request $req, array $params): Response
    {
        try {
            ...
            return Response::ok()->withNoCache();
        } catch (Throwable $e) {
            return Response::json(['ok' => false, 'error' => $e->getMessage()], $this->pdfStatusFor($e));
        }
    }
    ...
    /* providerModels() + openRouterVisionModels() RIMOSSI il 2026-08-26
       insieme al provider OpenRouter: l'endpoint /provider-models serviva
       solo a elencare i modelli vision di OpenRouter e per ogni altro
       provider restituiva gia' lista vuota. Il client JS non lo chiamava
       piu'. Vedi ProviderRouter::CLIENTS. */
    ...
    public function saveProviderOperation(Request $req): Response
    {
        try {
            ...
            if ($provider !== '' && !in_array($provider, ['anthropic', 'openai', 'ollama'], true)) {
    ...
    public function saveProviderKey(Request $req): Response
    {
        try {
            ...
            $key = trim((string)($body['key'] ?? ''));
            $model = trim((string)($body['model'] ?? ''));
            if (!in_array($provider, ['anthropic', 'openai', 'ollama'], true)) {
                throw new \RuntimeException('invalid_provider');
            }
            $store = new ProviderKeyStore();
    ...
    {
        if ($e instanceof \App\Core\CorpoNonValido) {
            return $e->stato();
        }
