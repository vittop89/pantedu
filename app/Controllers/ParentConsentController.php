...
final class ParentConsentController
{
    ...
    public function preview(Request $req, array $params): Response
    {
        ...
        // Richiesta + nome dello studente. La ricerca sta nel servizio dal
        // 23/9/2026: nel database c'e' l'hash del gettone, e l'hash si
        // calcola in un posto solo (ParentConsentService::anteprima).
        $row = $this->service->anteprima($token);
    ...
    public function confirm(Request $req, array $params): Response
    {
        ...
        // 23/9/2026 — la casella dei diritti è quella dell'istanza
        // (`mail.dpo_email`), non quella di produzione scritta qui; senza, il
        // modulo di contatto, che c'è sempre.
        $dpo = trim((string)\App\Core\Config::get('mail.dpo_email', ''));
        $revoca = $dpo !== ''
            ? '<a href="mailto:' . htmlspecialchars($dpo, ENT_QUOTES) . '?subject=Revoca consenso parentale">'
              . htmlspecialchars($dpo) . '</a>'
            : 'il modulo <a href="/dpo-contact">per le richieste privacy</a>';
        return Response::html($this->renderPage(
            'Consenso confermato',
