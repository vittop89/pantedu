 ...
 * (`<dati>/storage/security/blocked_*.json`, in `app/Config/auth.php`) restano
 * sincronizzati DB→JSON come read-cache per AuthCode legacy che ancora li
 * legge (back-compat).
 *
...
final class SecurityAdminController
{
    ...
    public function __construct(
        ...
        // Dati d'istanza, non `app.paths.base`: quello è SEMPRE la radice del
        // repository (app/Config/app.php), che nel container è l'immagine. Le
        // soglie salvate dal pannello finivano nello strato scrivibile e
        // sparivano al rilascio.
        //
        // Sotto `storage/`, e non accanto: `<dati>` in produzione è di root e
        // solo `storage/` è di www-data, nel salvataggio notturno e nei
        // controlli d'avvio. Il 20/9/2026 si è misurato che con `<dati>` non
        // scrivibile il pannello rispondeva 200 «salvato» e il file non
        // nasceva.
        //
        // Il registro degli accessi lo si chiede a chi lo scrive
        // (`AccessLogger`): cercarlo per conto proprio in `log/data/` voleva
        // dire leggere un file che nessuno scrive più.
        $base = \App\Support\PercorsiDati::base(dirname(__DIR__, 2));
        $this->accessLogPath   = $accessLogPath   ?? \App\Core\AccessLogger::percorsoDelRegistro($base);
        $this->alertConfigPath = $alertConfigPath ?? ($base . '/storage/security/alerts/config.json');
        $this->sec             = $sec;
    }
    ...
    public function setConfig(Request $req): Response
    {
        ...
        // L'esito si GUARDA, tutte e tre le volte. Il 20/9/2026, con la
        // cartella dei dati non scrivibile, questa funzione rispondeva
        // `{"ok":true}` con 200 e il file non esisteva: `file_put_contents`
        // tornava `false` (un Warning nel registro di PHP, che nessuno legge)
        // e `@rename` falliva in silenzio. Il pannello diceva «salvato» e le
        // soglie restavano quelle di prima.
        try {
            $dir = dirname($this->alertConfigPath);
            if (!is_dir($dir) && !@mkdir($dir, 0775, true) && !is_dir($dir)) {
                return $this->nonSalvato('mkdir', $dir);
            }
            ...
            if ($scritti === false) {
                return $this->nonSalvato('file_put_contents', $tmp);
            }
            if (!@rename($tmp, $this->alertConfigPath)) {
                @unlink($tmp);
                return $this->nonSalvato('rename', $this->alertConfigPath);
            }
            return Response::json(['ok' => true, 'config' => $cfg]);
        } catch (Throwable $e) {
    ...
    /**
     * Una scrittura fallita è un 500, non un «salvato».
     *
     * Il percorso entra nella risposta perché è l'informazione che serve a
     * chi deve rimediare (il pannello lo mostra all'amministratore, che è
     * super-admin): quasi sempre è un permesso sbagliato sulla cartella dei
     * dati.
     */
    private function nonSalvato(string $passo, string $percorso): Response
    {
        error_log("[security] soglie non salvate: {$passo} su «{$percorso}»");

        return Response::json([
            'error'  => 'persist_failed',
            'detail' => "{$passo} non riuscita su «{$percorso}»: controllare i permessi della cartella dei dati",
        ], 500);
    }
