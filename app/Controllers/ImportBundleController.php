 ...
 *     senza database né chiave master (TeacherRecoveryService::
 *     verifyManifestHmac). Prova che il manifest, e con le sue impronte i
 *     file, non l'ha cambiato chi non conosce la R di chi l'ha esportato;
 *     NON prova chi l'ha esportato rispetto al server: R non si confronta
 *     con la chiave del docente `exporter_user_id` né con quelle salvate su
 *     questa istanza, e chi ha una R qualunque firma un manifest suo. Così
 *     si importa anche da un'altra istanza e dopo la perdita della chiave
 *     master (23/9/2026; il perché nel docblock del servizio).
 *   - Owner-only: import scrive SOLO nel teacher autenticato (no
 *     impersonate via exporter_user_id; quel field è solo informativo,
 *     come exporter_username: dichiarazioni del pacchetto, non verificate)
...
final class ImportBundleController
{
    ...
    private function execute(Request $req, bool $apply): Response
    {
        try {
            ...
            // Il corpo lo dà la Request (revisione 2026-09, P4), che lo tiene
            // fino alla fine: prima si liberava subito dopo json_decode, ma il
            // picco era già lì (testo e array decodificato insieme), e nginx
            // lo ferma a 128 MB. Con la Request le prove passano il corpo
            // senza php://input, che da riga di comando è vuoto.
            ...
            if (!in_array($strategy, ['skip', 'rename'], true)) {
                return Response::fail('invalid_conflict_strategy', 422);
            }
            if ($recoveryCode === '' || !$manifestIn) {
                return Response::fail('missing_recovery_or_manifest', 422);
            }
            if (count($files) > self::MAX_FILES) {
                return Response::fail('too_many_files', 413);
            }
            ...
            // Ferma un pacchetto vecchio così com'è, non chi ha la R: la
            // verifica non guarda il database, quindi revocare R non
            // invalida i bundle già firmati, e `exported_at` sta fra i campi
            // firmati ma lo sceglie chi firma (23/9/2026: prima questo
            // commento diceva che la revoca li invalidava).
            ...
            if (!$valid) {
                return Response::fail('invalid_recovery_code_or_manifest', 403);
            }
            ...
            if ($totalBytes > self::MAX_TOTAL_BYTES) {
                return Response::fail('bundle_too_large', 413);
            }
            ...
            foreach ($files as $f) {
                ...
                // 23/9/2026 (revisione architetturale A-3) — l'import è la
                // seconda strada da cui entra una mappa, e fino a oggi non
                // guardava il contenuto: un .html diventava una mappa text/html,
                // un .pdf una application/pdf. Lo stesso controllo di «Carica
                // file» (FileDrawio): una mappa che non è un drawio non si crea,
                // né in anteprima né nell'applicazione, e il riepilogo lo dice
                // fra gli errori.
                if ($parsed['type'] === 'mappa') {
                    $motivo = FileDrawio::motivoDelRifiuto($bin);
                    if ($motivo !== null) {
                        $report['errors'][] = ['path' => $path, 'reason' => 'drawio_non_valido', 'motivo' => $motivo];
                        continue;
                    }
                }
    ...
    private function parseEntryPath(string $type, string $path): ?array
    {
        ...
        if ($type === 'mappa') {
            ...
            // Le estensioni sono quelle che BundlePathBuilder::mapPath dà alle
            // mappe esportate. Il tipo non viene più dall'estensione: una mappa
            // entra solo se è un drawio (FileDrawio, in execute) e si salva
            // come application/xml (insertMappa).
     ...
     *
     * Il contenuto l'ha già controllato execute (FileDrawio): è un drawio, e
     * si salva come application/xml, come «Carica file».
     */
    private function insertMappa(int $teacherId, array $parsed, string $title, string $plaintext): void
    {
        $pdo = Database::connection();
        // La transazione si apre solo se chi chiama non ne ha già una (le
        // prove), come in MapsController::create.
        ...
        try {
            ...
            $clsId  = !empty($parsed['classe'])       ? $L::idFromCodeForTeacher('classi', (string)$parsed['classe'], $teacherId, !empty($parsed['indirizzo']) ? (string)$parsed['indirizzo'] : null) : null;
            ...
            $stmt = $pdo->prepare(
                'INSERT INTO teacher_content_data
                    (teacher_id, content_subtype, topic, title, metadata_json, visibility)
                 VALUES (?, "mappa", ?, ?, ?, "draft")'
            );
            $stmt->execute([
                $teacherId,
            ...
            // ADR-037, fase 4c — la principale la scrive l'applicazione.
            \App\Support\PostoPrincipale::contenuto($pdo, $contentId, $indId, $clsId, $subjId);
            ...
            $upd = $pdo->prepare(
                'UPDATE teacher_content_data
                    SET map_blob_path = ?, map_mime = "application/xml", map_size = ?,
                        map_origin = "upload", map_version = 1
                  WHERE id = ? AND teacher_id = ?'
            );
            $upd->execute([$blobPath, strlen($plaintext), $contentId, $teacherId]);
    ...
    private function insertEsercizio(int $teacherId, array $parsed, string $title, string $jsonBundle): void
    {
        ...
        try {
            ...
            $eSubj = $L::idFromCodeForTeacher('materie', $eSubjRaw, $teacherId);
            $eInd  = $eIndRaw !== null ? $L::idFromCodeForTeacher('indirizzi', (string)$eIndRaw, $teacherId) : null;
            $eCls  = $eClsRaw !== null ? $L::idFromCodeForTeacher('classi', (string)$eClsRaw, $teacherId, $eIndRaw !== null ? (string)$eIndRaw : null) : null;
            $ins->execute([
                $teacherId,
                (string)($row['topic'] ?? ''),
                $title,
            ...
            // ADR-037, fase 4c — la principale la scrive l'applicazione.
            \App\Support\PostoPrincipale::contenuto($db, (int)$db->lastInsertId(), $eInd, $eCls, $eSubj);

            // Scrivi .contract.json sul filesystem se il bundle lo include
            if (is_array($w['contract'] ?? null)) {
                // Dati d'istanza: sotto PANTEDU_DATA_PATH, non nella radice del
                // repository (nel container è l'immagine, e il contratto sparisce
                // al rilascio lasciando la riga orfana del suo contract_key).
                $objects = \App\Support\PercorsiDati::base(dirname(__DIR__, 2)) . '/storage/objects';
                $absDir = $objects . '/' . dirname($newRel);
                if (!is_dir($absDir) && !mkdir($absDir, 0775, true) && !is_dir($absDir)) {
                    throw new \RuntimeException('contract_mkdir_failed');
                }
                $absFile = $objects . '/' . $newRel;
    ...
    private function insertDocumento(int $teacherId, array $parsed, string $title, string $jsonBundle): void
    {
        ...
        $ins = $db->prepare(
            'INSERT INTO teacher_content_data
                (teacher_id, content_subtype, topic, title, body_html, metadata_json, visibility)
        ...
        $dSubj = $L::idFromCodeForTeacher('materie', $dSubjRaw, $teacherId);
        $dInd  = $dIndRaw !== null ? $L::idFromCodeForTeacher('indirizzi', (string)$dIndRaw, $teacherId) : null;
        $dCls  = $dClsRaw !== null ? $L::idFromCodeForTeacher('classi', (string)$dClsRaw, $teacherId, $dIndRaw !== null ? (string)$dIndRaw : null) : null;
        $ins->execute([
            $teacherId,
            (string)($row['topic'] ?? ''),
            $title,
        ...
        // ADR-037, fase 4c — la principale la scrive l'applicazione.
        \App\Support\PostoPrincipale::contenuto($db, (int)$db->lastInsertId(), $dInd, $dCls, $dSubj);
    }
    ...
    private function updateVerificaMetadata(
        ...
        $clsId = $classe    !== '' ? $L::idFromCodeForTeacher('classi', $classe, $teacherId, $indirizzo !== '' ? $indirizzo : null) : null;
        $upd = $db->prepare(
            'UPDATE verifica_documents_data
                SET variant = ?, batch_id = ?
              WHERE id = ? AND teacher_id = ?'
        );
        $upd->execute([
        ...
        // ADR-037, fase 4c — la principale segue: indirizzo e classe nuovi, la
        // materia quella con cui la variante è stata creata.
        // Anche quando la riga non cambia (le etichette non stanno più nella
        // riga): basta che la variante sia del docente.
        $mia = $db->prepare('SELECT 1 FROM verifica_documents_data WHERE id = ? AND teacher_id = ? LIMIT 1');
        $mia->execute([$docId, $teacherId]);
        if ($mia->fetchColumn()) {
            $materiaId = \App\Support\PostoPrincipale::dove($db, \App\Support\PostoPrincipale::VERIFICA, $docId)['materia'];
            \App\Support\PostoPrincipale::verifica($db, $docId, $indId, $clsId, $materiaId);
        }
    }
