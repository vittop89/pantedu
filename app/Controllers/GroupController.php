 ...
 * Helper condivisi duplicati: teacherId, dbReady, privateFilesInstituteId.
 */
final class GroupController
{
    ...
    public function groupAdd(Request $req, array $params): Response
    {
        ...
        try {
            ...
            // Oggi i due chiamanti di /group/add non mandano mai un `intro`
            // libero — viene da TemplateDefaults::introForType(), lato server —
            // quindi con l'input di adesso la guardia qui non scatta. Ma se un
            // domani lo mandassero, il rifiuto usciva come 500 `persist_failed`,
            // cioè «errore del server» per una richiesta che il server ha
            // capito benissimo e rifiutato apposta.
            return Response::json([
                'error'   => 'rendered_text',
            ...
            // 23/9/2026 (A-76) — save() rifiuta un gruppo fuori schema (per
            // esempio un `type` che nessun renderer conosce): è la richiesta
            // a essere sbagliata, non il server.
            return Response::json([
        ...
        foreach ($agg->groups() as $g) {
            if (($g['id'] ?? '') === $groupId) {
                $renderer = \App\Services\ContractRenderer::loadSourcesFor(
                    $this->privateFilesInstituteId($tid) ?? 0,
                    $tid,
                );
    ...
    public function groupDelete(Request $req, array $params): Response
    {
        ...
        $expected = $this->readExpectedVersion($req);
        try {
            $agg = \App\Repositories\Contract\ContractRepository::default()
    ...
    public function groupPatch(Request $req, array $params): Response
    {
        ...
        try {
            ...
            // Come in QuesitoController: l'intro del gruppo passa dallo stesso
            // serializzatore dell'editor (vedi TestoDiPaginaResa).
            return Response::json([
                'error'   => 'rendered_text',
                'message' => $e->getMessage(),
                'where'   => array_column($e->trovati, 'percorso'),
            ...
            // Come in groupAdd (23/9/2026, A-76): `type` e `intro` arrivano dal client.
            return Response::json([
                'error'   => 'contract_schema',
                'message' => $e->getMessage(),
                'errors'  => $e->errori,
