 ...
 *   POST /segnalazione-contenuti       → submit (rate:takedown,3 → 3/h per IP)
 *
 * Vedi:
...
{
    /** Casella presidiata che riceve le segnalazioni. */
     ...
     * sul percorso Resend (SPF su send.pantedu.eu + DKIM resend._domainkey),
     * quindi una mail uscita dal PHP del VPS non è allineata DMARC.
     */
    private function notifyAbuseEmail(int $requestId, array $data): void
    {
        ...
        try {
            ...
            $body = "Nuova segnalazione ricevuta su /segnalazione-contenuti.\n\n"
                  ...
                  . "Descrizione:\n" . ($data['description'] ?? '—') . "\n\n"
                  . "Dettaglio + azione: https://pantedu.eu/admin/takedown/{$requestId}\n";
            ...
            $this->mailer->send(self::ABUSE_MAILBOX, $subject, $body, $replyTo);
        } catch (\Throwable $e) {
