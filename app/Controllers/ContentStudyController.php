...
final class ContentStudyController
{
    ...
    public function contentSingleJson(Request $req, array $params): Response
    {
        ...
        if (!$u) {
            ...
            } elseif ((int)($row['teacher_id'] ?? 0) !== $grant['teacher_id']) {
                return Response::json(['error' => 'forbidden'], 403);
            }
        }
    ...
    private function viewerContext(): \App\Domain\ViewerContext
    {
        ...
        // della credenziale (filtro teacher_id in scopedFilters) e, se la
        ...
        if (!$u) {
            $grant = \App\Support\ClassAccessGrant::current();
            if ($grant !== null) {
                return \App\Domain\ViewerContext::forStudent(0, null, $grant['indirizzo'], $grant['classe']);
            }
        }
    ...
    private function scopedFilters(array $params, string $type): array
    {
        ...
        // ADR-032 — la credenziale di classe vale per UN docente: il filtro
        // teacher_id e' esplicito, cosi' un ospite non vede i contenuti
        // pubblicati da altri docenti per la stessa classe.
        if (!Auth::user()) {
            $grantTeacher = \App\Support\ClassAccessGrant::teacherId();
            if ($grantTeacher > 0) {
