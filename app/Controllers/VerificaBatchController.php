...
final class VerificaBatchController
{
    ...
    public function batchFiles(Request $req, array $params): Response
    {
        try {
            ...
            if (!$docs) {
                return Response::fail('verifica_batch_empty', 404);
            }
        ...
        foreach ($docs as $doc) {
            ...
            foreach ($build->files as $f) {
                ...
                if (str_starts_with($src, 'texCommon/')) {
                    ...
                    $dst = "$grigliaDir/" . $g[1];
                } elseif (preg_match('#^versioni/[^/]+\.tex$#', $src)) {
                    // main_*, esercizi_*, ma anche tikz_preamble.tex e fonti_SOL.tex:
                    // main_*.tex li include con \input relativo alla PROPRIA cartella,
                    // quindi vanno tutti nella cartella della versione (fino al
                    // 2026-09-05 tikz_preamble restava in versioni/ e pdflatex
                    // falliva con «File tikz_preamble.tex not found»).
