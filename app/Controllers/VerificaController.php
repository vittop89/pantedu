...
final class VerificaController
{
    ...
    public function saveTex(Request $req): Response
    {
        try {
            ...
            $doc = $this->svc->saveTex([
                ...
                'tex'              => $tex,
                'exercise_ids'     => $excIds,
                'problems'         => (array)($payload['problems'] ?? []), // fonte (copyright) dai quesiti
     ...
     */
    /**
     * Phase 25.Q.15 — GET /api/study/verifica/list — le verifiche che vede chi
     * studia, solo metadati (niente TEX né PDF nella risposta).
     *
     * ADR-037, fase 3 (2026-09-13) — agli studenti arriva ciò che il docente ha
     * PUBBLICATO, non ciò che ha condiviso con i colleghi. Prima bastava
     * `shared_with_pool`, per sigle, con la scuola del primo collegamento e
     * senza incarichi: condividere e pubblicare erano la stessa casella, il
     * punto che ADR-032 teneva aperto e che la DPIA dichiara distinto. Ora le
     * regole sono quelle dei contenuti, dalla stessa politica
     * ({@see \App\Domain\ContentVisibilityPolicy::studyListFilters()}):
     *   - ospite con credenziale di classe: le verifiche pubblicate del docente
     *     della credenziale, nella sua scuola, per la classe coperta;
     *   - studente con account: pubblicate nella sua scuola, per la sua
     *     classe o il suo anno, da un docente con un incarico nella sua sezione;
     *   - docente e amministratore: le verifiche della scuola attiva, poi l'ACL
     *     fra colleghi riga per riga (le proprie sempre, quelle degli altri se
     *     condivise o con un grant).
     */
    public function listForStudent(Request $req): Response
    {
        try {
            ...
            if (!$u) {
                ...
                $ctx = \App\Domain\ViewerContext::forKeychain($grants);
                return $this->elencoDiStudio($repo->listForStudy(
                    $policy->studyListFilters($ctx, [], $classe !== '' ? $classe : null, $indirizzo !== '' ? $indirizzo : null)
                ));
            }
            $userId = (int)($u['id'] ?? 0);
            if ($userId <= 0) {
                return Response::fail('invalid_user', 401);
            }
            if ((string)\App\Core\Auth::role() === 'student') {
                // Scuola, indirizzo e classe dal profilo, non dall'URL: la classe
                // chiesta conta solo se è un anno passato ammesso (archivio).
                $profili = new \App\Services\Student\StudentProfileService();
                $scope   = $profili->scopeForUser($userId) ?? [];
                $ctx = \App\Domain\ViewerContext::forStudent(
                    $userId,
                    $scope['institute_id'] ?? null,
                    $scope['indirizzo'] ?? null,
                    $scope['classe'] ?? null,
                    $profili->storicoForUser($userId)
                );
                return $this->elencoDiStudio($repo->listForStudy(
                    $policy->studyListFilters($ctx, [], $classe !== '' ? $classe : null, $indirizzo !== '' ? $indirizzo : null)
                ));
            ...
            $scuola = (int)(\App\Support\CurriculumLookup::instituteForTeacher($userId) ?? 0);
            if ($scuola <= 0) {
                return $this->elencoDiStudio([]);
            }
            $rows = $repo->listForStudy(
                ['pub_institute_id' => $scuola, 'pub_actor_id' => $userId],
                $indirizzo !== '' ? $indirizzo : null,
                $classe !== '' ? $classe : null,
            );
            $acl = new \App\Services\Sharing\SharedContentPolicy();
            $rows = array_values(array_filter($rows, static fn(array $r): bool => $acl->canReadContent(
                $userId,
                'verifica_documents',
                (int)$r['id'],
                (int)$r['teacher_id'],
                !empty($r['shared_with_pool'])
            )));
            return $this->elencoDiStudio($rows);
        } catch (Throwable $e) {
            return Response::json(['ok' => false, 'error' => $e->getMessage()], $this->statusFor($e));
        }
    }
    ...
    private function elencoDiStudio(array $rows): Response
    {
    ...
    public function listForTeacher(Request $req): Response
    {
        try {
            ...
            $indirizzo = trim((string)($req->query['indirizzo'] ?? ''));
            $classe    = trim((string)($req->query['classe'] ?? ''));
            // ADR-037, fase 3 — la barra nella scuola attiva (la stessa in cui
            // si salvano le verifiche nuove): le sigle si confrontano sulle
            // pubblicazioni in quella scuola.
            $scuola = (int)(\App\Support\CurriculumLookup::instituteForTeacher($teacherId) ?? 0);
            $rows = $this->svc->listForTeacher(
                $teacherId,
                $materia   !== '' ? $materia   : null,
                $indirizzo !== '' ? $indirizzo : null,
                $classe    !== '' ? $classe    : null,
                $scuola > 0 ? $scuola : null,
            );
            if ($section !== '') {
    ...
    public function sharePool(Request $req, array $params): Response
    {
        try {
            ...
            // 14/9/2026 — vale per tutte le varianti e le versioni della verifica.
            return Response::json(['ok' => true, 'id' => $id, 'shared_with_pool' => $enabled, 'varianti' => (int)($result['righe'] ?? 1)]);
        } catch (Throwable $e) {
    ...
    public function zipExport(Request $req, array $params): Response
    {
        try {
            ...
            if (!$doc) {
                return Response::fail('verifica_not_found', 404);
            }
            $pdf = null;
    ...
    public function updateTexFiles(Request $req, array $params): Response
    {
        try {
            ...
            // Il lettore comune (ADR-034, A-52): vuoto o non JSON 400, troppo
            // grande 413, mappati da statusFor().
            $body = $req->jsonObbligatorio();
            if (!isset($body['files']) || !is_array($body['files'])) {
                return Response::fail('invalid_payload', 400);
            }
    ...
    public function geogebraAttach(Request $req, array $params): Response
    {
        try {
            ...
            $body = $req->jsonObbligatorio();
            if (!isset($body['svg_b64'])) {
                return Response::fail('invalid_payload', 400);
            }
            $svgB64 = (string)$body['svg_b64'];
            $label  = (string)($body['label'] ?? '');
            if ($svgB64 === '') {
                return Response::fail('svg_missing', 400);
            }
            if (strlen($svgB64) > 4 * 1024 * 1024) {
                return Response::fail('svg_too_large', 413);
            }
