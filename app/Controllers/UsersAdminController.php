...
final class UsersAdminController
{
    public function index(Request $req): Response
    {
        ...
        if ($status !== '') {
            ...
            // Gli account cancellati restano come segnaposto `anon-<id>`
            // (CancellazioneDellAccount): nell'elenco solo se li si chiede.
    ...
    public function setRole(Request $req, array $params): Response
    {
        ...
        if (!in_array($role, \App\Support\RuoliNelPannelloUtenti::NOTI, true) || $role === 'institute_admin') {
            return Response::json(['error' => 'invalid_role'], 400);
        }
        // 15/9/2026 — negli scenari senza account studente il ruolo «student»
        // non si assegna: la pagina non lo offre, e il server non lo accetta.
        $assegnabili = \App\Support\RuoliNelPannelloUtenti::assegnabili(\App\Support\DeploymentScenario::studentAccountsEnabled());
        if (!in_array($role, $assegnabili, true)) {
            return Response::json(['error' => 'ruolo_non_previsto_nello_scenario'], 409);
        }
        if ($id === $this->currentUserId() && $role !== 'administrator') {
            return Response::json(['error' => 'cannot_demote_self'], 403);
        }
        // ADR-040 — un amministratore di istituto non cambia ruolo da qui: il
        // suo istituto resterebbe appeso, e il database lo rifiuterebbe
        // (migrazione 125). Si gestisce dalla pagina degli istituti.
        $attuale = Database::connection()->prepare('SELECT role FROM users WHERE id = ? LIMIT 1');
        $attuale->execute([$id]);
        if ((string)$attuale->fetchColumn() === \App\Domain\Role::INSTITUTE_ADMIN->value) {
            return Response::json(['error' => 'institute_admin_role_is_managed_by_institute'], 409);
        }
        try {
    ...
    public function delete(Request $req, array $params): Response
    {
        ...
        // 24/9/2026 — la stessa cancellazione dell'art. 17 e dei 730 giorni
        // (CancellazioneDellAccount). Prima la riga di users si cancellava e basta: le
        // cascate cancellavano i registri che vanno tenuti (richieste di
        // cancellazione, verbale dei Termini, audit del recupero), la chiave
        // spariva senza la riga «shred» nel registro della cifratura, e tutti
        // i file del docente, contratti in chiaro compresi, restavano sul
        // disco senza più un proprietario.
        $esiste = Database::connection()->prepare('SELECT 1 FROM users WHERE id = ?');
        $esiste->execute([$id]);
        if ($esiste->fetchColumn() === false) {
            return Response::json(['ok' => true, 'removed' => false]);
        }
        try {
            (new \App\Services\Gdpr\CancellazioneDellAccount())->esegui($id, 'admin_panel_deletion');
