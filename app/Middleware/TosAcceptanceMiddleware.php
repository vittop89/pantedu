 ...
 * rotte protette. Non ha un alias di rotta: i test lo istanziano direttamente.
 */
final class TosAcceptanceMiddleware
{
    ...
    public function handle(Request $req, callable $next): Response
    {
        ...
        // Check accettazione. Il try avvolge solo il controllo, non `$next`
        // (23/9/2026): quando c'era dentro, un'eccezione del controller finiva
        // nel catch pensato per il database, e il controller girava una
        // seconda volta (revisione architetturale 2026-09, A-1).
        try {
            $userId = (int) (Auth::user()['id'] ?? 0);
            $passa = $userId <= 0 || $this->service->hasAccepted($userId);
        } catch (Throwable $e) {
