 ...
 *   - monitor:      osserva senza bloccare. Un blocco (IP a mano, threat
 *                   intel, CrowdSec, regola, punteggio) si registra con un
 *                   esito `monitor_*` e la richiesta passa; l'honeypot scrive
 *                   la sua riga `honeypot_trap` e lascia passare; le sfide di
 *                   threat intel, CrowdSec e punteggio `soft` si registrano
 *                   soltanto. La sfida invisibile alla PRIMA visita invece
 *                   resta, di proposito: è lei che raccoglie l'impronta e il
 *                   punteggio che la modalità serve a osservare. Restano anche
 *                   le sfide che l'amministratore chiede in modo esplicito,
 *                   con una regola o l'honeypot in `challenge`. (23/9/2026,
 *                   A-71: prima un blocco in monitor rispondeva 200 con il
 *                   corpo vuoto, anche alle richieste JSON.)
...
final class WafMiddleware
{
    ...
    public function handle(Request $req, callable $next): Response
    {
        ...
        // La decisione (pass/blocco/challenge) è presa QUI dentro; il controller a
        // valle gira FUORI dal try, così un suo errore arriva al Kernel come 500
        // loggato e non viene travestito da «security_check_required» (era così
        // fino al 2026-09-05: un import mancante in GroupController rispondeva
        // 403 waf_challenge e nessuno vedeva l'eccezione).
        //
        // 23/9/2026 — fuori anche dal try del ripiego qui sotto. Lì `$next`
        // era ancora dentro: con il WAF guasto in modo monitor (o spento, o su
        // un percorso escluso) un'eccezione del controller finiva nel catch
        // interno e il controller girava una seconda volta (revisione
        // architetturale 2026-09, A-1). Ora ogni ramo produce solo la
        // decisione, e `$next` si chiama in un punto solo, qui in fondo.
        try {
            $decision = $this->evaluate($req);
        } catch (\Throwable $e) {
            ...
            try {
                ...
                $lascia = !$cfg->getBool('enabled', false) || $cfg->get('mode', 'monitor') === 'off'
                    || $cfg->get('mode', 'monitor') === 'monitor' || $this->shouldBypass($req->path);
                $decision = $lascia ? null : $this->respondChallenge('invisible', $cfg);
            } catch (\Throwable) {
                $decision = null;
            }
        }
        return $decision ?? $next($req);
    }

    private function evaluate(Request $req): ?Response
    {
        ...
        if ($config->getBool('honeypot_enabled', true) && $this->isHoneypotPath($req->path)) {
            ...
            if (!$isSuperAdmin) {
                ...
                if ($action === 'block') {
                    ...
                    // In monitor enforceBlock lascia passare: la riga
                    // `honeypot_trap` qui sopra è già il registro.
        ...
        if (GeoIpService::ipInCidrList($ip, $rules->blacklistCidrs())) {
            return $this->blocca(
                $mode,
                $logSvc,
                $baseLog + ['outcome' => 'blocked_manual'],
                $baseLog + ['outcome' => 'monitor_manual'],
                'Manual IP block',
            );
        }
            ...
            if ($hit !== null) {
                ...
                if ($action === 'block') {
                    return $this->blocca(
                        $mode,
                        $logSvc,
                        $baseLog + ['outcome' => 'blocked_threat_intel'],
                        $baseLog + ['outcome' => 'monitor_threat_intel'],
                        'Threat intel: ' . $hit['source'],
                    );
                }
                if ($action === 'challenge') {
            ...
            if ($csHit !== null) {
                ...
                if ($csHit['action'] === 'block') {
                    return $this->blocca(
                        $mode,
                        $logSvc,
                        $baseLog + ['outcome' => 'blocked_crowdsec'],
                        $baseLog + ['outcome' => 'monitor_crowdsec'],
                        'CrowdSec: ' . $csHit['scenario'],
                    );
                }
        ...
        if ($ruleHit !== null) {
            ...
            if ($action === 'block') {
                return $this->blocca(
                    $mode,
                    $logSvc,
                    $baseLog + ['outcome' => 'blocked_rule'],
                    $baseLog + ['outcome' => 'monitor_rule'],
                    'Blocked by rule: ' . $ruleHit['rule_name'],
                );
            }
            if ($action === 'challenge') {
        ...
        if ($challenge === 'block') {
            // In monitor l'API della sfida non assegna più `block`
            // (WafApiController): qui arriva un cookie avuto in `enforce`
            // prima che la modalità cambiasse.
            return $this->blocca(
                $mode,
                $logSvc,
                $baseLog + ['outcome' => 'blocked_score'],
                $baseLog + ['outcome' => 'monitor_score'],
                'Risk score too high',
            );
        }
    ...
    private function shouldBypass(string $path): bool
    {
        ...
        // 2026-08-31 — la lista diceva `/healthz`, ma la rotta reale e' `/health`
        // (routes/web.php:41). Nessun path combaciava, quindi /health riceveva la
        // challenge PoW e rispondeva 200 con la pagina "Verifica…" invece del JSON
        // di stato: un monitor HTTP vedeva "tutto ok" anche con il DB giu',
        // vanificando il 503 che HealthController emette apposta. Stesso problema
        // per /version, che il deploy-verify usa al posto di SSH+git log.
        // 2026-09-19 — /health/tex: la chiedono il rilascio e la diagnostica
        // cercando `"tex":` nel corpo; con la sfida non lo troverebbero mai.
        if (
            $path === '/health' || $path === '/healthz' || $path === '/version'
            || $path === '/health/backup' || $path === '/health/tex'
            || $path === '/metrics'
    ...
    /**
     * Blocca, oppure — in `monitor` — registra che avrebbe bloccato e lascia
     * passare. Le due righe del registro le scrive chi chiama, con l'esito
     * letterale: la prova degli esiti (FiltroDelleRichiesteTest,
     * EsitiDelWafStannoNellaColonnaTest) li legge dal sorgente.
     *
     * @param array<string,mixed> $seBlocca  la riga quando blocca (`blocked_*`)
     * @param array<string,mixed> $seOsserva la riga quando, in monitor, lascia passare (`monitor_*`)
     */
    private function blocca(
        string $mode,
        WafLogService $registro,
        array $seBlocca,
        array $seOsserva,
        string $motivo,
    ): ?Response {
        $registro->log($mode === 'monitor' ? $seOsserva : $seBlocca);
        return $this->enforceBlock($mode, $motivo);
    }

    /**
     * Il blocco: 403, in HTML o in JSON. In `monitor` null, cioè la richiesta
     * passa.
     *
     * 23/9/2026 — in monitor rispondeva `new Response('', 200)`: una pagina
     * bianca per la navigazione e un corpo vuoto per le fetch, che poi
     * fallivano su `.json()`. Il commento diceva «ripiego alla sfida
     * invisibile», ma nessuna sfida partiva: la modalità pensata per
     * osservare senza disturbare bloccava in modo muto (revisione
     * architetturale 2026-09, A-71; prove in WafInMonitorTest).
     */
    ...
    /**
     * L'indirizzo dello script della challenge, con la sua versione.
     *
     * 22/9/2026 — misurato con il browser in produzione: dopo il rilascio che
     * riduceva l'impronta del dispositivo, i visitatori continuavano a
     * ricevere lo script di PRIMA. `cf-cache-status: HIT`, `age: 9861`,
     * `cache-control: max-age=86400`; la stessa richiesta con un parametro
     * qualunque in coda restituiva il file nuovo. L'origine era aggiornata e
     * la cache di bordo no.
     *
     * Non è un dettaglio di prestazioni: l'informativa dichiara che da quella
     * data certi valori non si leggono più, e per ventiquattro ore non era
     * vero per chi aveva la copia in cache. Una correzione sulla privacy che
     * non arriva a chi si vuole proteggere non è una correzione.
     *
     * Stessa ricetta dei fogli di stile (`views/partials/head.php`): il tempo
     * di modifica del file in coda all'indirizzo. Cambia a ogni rilascio che
     * tocca lo script, e solo allora.
     */
    private static function indirizzoDelloScript(): string
    {
        $percorso = '/js/waf/fingerprint.js';
        $file = dirname(__DIR__, 2) . $percorso;
        $mtime = @filemtime($file);

        return $mtime === false ? $percorso : $percorso . '?v=' . $mtime;
    }
    ...
    private function respondChallenge(string $mode, WafConfigRepository $config): Response
    {
        ...
        $scriptUrl = self::indirizzoDelloScript();
        ...
        // Il nonce della CSP sullo script della sfida (23/9/2026, A-16): senza,
        // con la CSP rigorosa lo script non parte e la sfida non si chiude mai.
        // Prima lo metteva la timbratura del middleware delle intestazioni.
...
<body><script{$csp} src="$scriptUrl" data-waf-mode="invisible" data-waf-reload="1"$powAttr></script>
...
<script{$csp} src="$scriptUrl" data-waf-mode="$modeAttr" data-waf-reload="1"$powAttr></script>
</body></html>
HTML;
        return new Response($body, 200, ['Content-Type' => 'text/html; charset=UTF-8']);
    }
