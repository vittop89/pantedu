...
return [
    ...
    /**
     * Da dove viene `hmac_secret` (24/9/2026): vero se è `WAF_HMAC_SECRET`
     * dell'ambiente, falso se il valore di sopra è la chiave del file in
     * `storage/keys` o una generata sul posto. Letto come lo legge il valore
     * di sopra, che con la variabile lunga abbastanza la restituisce così
     * com'è. Lo guarda `App\Support\ImprontaIp`, che in produzione segnala
     * una chiave che non viene dall'ambiente; l'ambiente si legge solo qui
     * (ADR-049).
     */
    'hmac_secret_dall_ambiente' => strlen(
        (string)($_ENV['WAF_HMAC_SECRET'] ?? (getenv('WAF_HMAC_SECRET') ?: ''))
    ) >= 32,
     ...
     *
     * Setup VPS (tools/dev/setup_crowdsec.sh, sull'host):
     ...
     *   6. .env.local: CROWDSEC_LAPI_KEY=<key> e CROWDSEC_LAPI_URL=<url>
     *
     * Fail-open se irraggiungibile.
     *
     * 23/9/2026 — l'URL non ha più un valore predefinito: era
     * `http://127.0.0.1:8080`, la porta di serie della LAPI sull'host. Dall'8/9
     * l'applicazione gira nel container, dove quell'indirizzo è il nginx
     * dell'applicazione (docker/nginx.conf): con la sola chiave impostata il
     * bouncer interrogava il sito e, fail-open, non bloccava niente (A-15).
     * Adesso servono tutte e due, e l'URL deve valere nel container e sull'host,
     * dove gira la diagnostica: il gateway del bridge Docker, come per il TeX
     * (docs/ops/diagnostica.md, «crowdsec»). Se è giusto lo dice la diagnostica
     * (`crowdsec`), che riconosce la risposta della LAPI
     * (WafCrowdSecBouncerService::riconosci).
     */
    'crowdsec_lapi_url' => $_ENV['CROWDSEC_LAPI_URL'] ?? '',
