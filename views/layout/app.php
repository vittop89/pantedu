...
if ($_currUserRole === 'student' && $_currInstituteId !== null) {
    ...
    // ADR-032 (2026-09-04) — ospite con credenziale di classe: e' uno
    // studente senza account, e vede l'offerta formativa dell'istituto della
    // credenziale (o del docente che l'ha creata), come uno studente iscritto.
    $_fmGrantIid = \App\Support\ClassAccessGrant::instituteId();
    if ($_fmGrantIid > 0) {
        $_currInstituteId = $_fmGrantIid;
        $curriculum = $_curriculumSvc->allActiveForInstitute($_fmGrantIid);
    } else {
