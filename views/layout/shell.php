...
<head>
    <meta charset="utf-8">
    <?php /* Una pagina con un gettone nell'indirizzo non lo manda come
            Referer, neanche ai fogli di stile e agli script qui sotto: il meta
            viene prima di loro. Vedi App\Support\PaginaConGettone. */ ?>
    <?php if (!empty($senzaReferer)): ?>
    <meta name="referrer" content="no-referrer">
    <?php endif; ?>
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <title><?= e($title ?? 'Pantedu') ?></title>
    <?php /* Il tema all'avvio, prima dei fogli di stile: la stessa politica
            di head.php, da js/tema-iniziale.js (23/9/2026, A-50). Prima
            queste pagine lo ricevevano solo da bootstrap.js, a caricamento
            finito. */ ?>
    ...
    <!-- Dark-mode rules (body.fm-dark) live in layout.css; carichiamo
         ...
         analytics/registration ecc.) — la classe body.fm-dark la mette
         js/tema-iniziale.js, in linea qui sopra. -->
    ...
    if (class_exists(\App\Support\ViteManifest::class)
        ...
        ?>
    <script<?= \App\Support\Csp::attributo() ?> type="module" src="/js/modules/<?= $_jsEntry ?><?= is_file($_jsAbs) ? '?v=' . filemtime($_jsAbs) : '' ?>"></script><?php
    endif; ?>
</head>
