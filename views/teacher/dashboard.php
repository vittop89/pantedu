...
/** @var array{istanza: string, collegato: bool} $drive */
...
<?php /* ADR-041 — sopra le schede: vale qualunque scheda sia aperta. */ ?>
...
<div class="fm-dash-shell">
    ...
    <section class="fm-dash-panel fm-card" data-dash-panel="panoramica" hidden id="fm-ex-search-section">
        ...
        <p class="fm-muted fm-text-13 fm-m-0 fm-mb-3" >
            ...
            nella scuola che scegli. Click su un risultato per aprirlo in studio.
        </p>
        <form id="fm-ex-form" class="fm-ex-filters">
            <?php /* 2026-09-14 — la scuola della ricerca: compare solo a chi ne ha più d'una
                     (teacher-dashboard.js), parte da quella attiva, e carica indirizzi,
                     classi e materie di quella scuola. */ ?>
            <select name="institute_id" class="fm-input fm-input--sm" aria-label="Scuola" hidden
                    data-scuola-attiva="<?= (int)(\App\Support\CurriculumLookup::instituteForTeacher((int)($user['id'] ?? 0)) ?? 0) ?>"></select>
            <select name="type" class="fm-input fm-input--sm" aria-label="Tipo di contenuto">
            ...
            <select name="indirizzo" class="fm-input fm-input--sm" data-fm-curriculum="indirizzi" aria-label="Indirizzo">
                <option value="">— indirizzo —</option>
            </select>
            <select name="classe" class="fm-input fm-input--sm" data-fm-curriculum="classi" aria-label="Classe">
                <option value="">— classe —</option>
            </select>
            <select name="subject" class="fm-input fm-input--sm" data-fm-curriculum="materie" aria-label="Materia">
                <option value="">— materia —</option>
            </select>
            <input type="text" name="q" placeholder="ricerca testuale (titolo/topic)" class="fm-input fm-input--sm fm-flex-1-grow fm-min-w-40" aria-label="Ricerca testuale nel titolo o nell'argomento" >
    ...
    <?php /* Phase G1.a — Google Drive integration status pill.
            ADR-038: con Drive spento la sezione c'è solo per chi ha un
            collegamento di prima, che può sempre scollegare. */ ?>
    <?php if (($drive['istanza'] ?? 'spento') !== 'spento' || !empty($drive['collegato'])): ?>
    ...
    <section class="fm-dash-panel fm-card" data-dash-panel="pool" hidden id="fm-share-groups-section">
        ...
        <div class="fm-d-flex fm-gap-2 fm-items-center fm-mb-3 fm-flex-wrap">
            <input id="fm-sg-name" type="text" placeholder="Nome gruppo" maxlength="120"
                   aria-label="Nome del gruppo di condivisione"
            ...
            <input id="fm-sg-desc" type="text" placeholder="Descrizione (opzionale)" maxlength="500"
                   aria-label="Descrizione del gruppo di condivisione (facoltativa)"
...
<?php /* 2026-09-05 — entry Vite (era js/teacher-dashboard.js, script classico
         re-iniettato da fm-router): il modulo si re-inizializza su fm:navigated. */ ?>
<?= \App\Support\ViteManifest::script('js/entries/teacher-dashboard.js') ?>
