...
<?php foreach ($diag_results as $ip => $r): ?>
    <tr>
        <td><code><?= $dh($ip) ?></code><?php if (!empty($r['nota'])): ?> <small>(<?= $dh($r['nota']) ?>)</small><?php endif; ?></td>
...
<?php
// 23/9/2026 — «raggiungibile» vuol dire che ha risposto la LAPI, non un
// codice qualunque fra 200 e 499: prima il 404 del nginx dell'applicazione
// (la loopback del container) risultava «✓ YES» (A-15). La risposta la
// classifica WafCrowdSecBouncerService::riconosci().
$csRisposta = match ($diag_csStatus['risposta'] ?? null) {
    'lapi'             => '✓ risponde la LAPI, con la chiave accettata',
    'chiave_rifiutata' => '✗ risponde la LAPI, ma rifiuta la chiave: nessuna decisione arriva',
    'non_lapi'         => '✗ risponde qualcosa che non è la LAPI: il bouncer non riceve decisioni',
    'irraggiungibile'  => '✗ nessuna risposta: il bouncer non riceve decisioni',
    default            => '— (non interrogata: il bouncer è spento)',
};
$csMancano = (array)($diag_csStatus['mancano'] ?? []);
?>
<h3 class="fm-text-base fm-mt-4">🐝 CrowdSec Bouncer</h3>
<table class="fm-waf-table">
<tr><th scope="col">Field</th><th scope="col">Value</th></tr>
<tr><td>Configurato</td><td><?= $diag_csStatus['configured']
    ? '✓ sì'
    : $dh('✗ no: manca ' . implode(' e ', $csMancano)) ?></td></tr>
<?php if (($diag_csStatus['servizio'] ?? '') !== ''): ?>
    <tr><td>LAPI</td><td><code><?= $dh($diag_csStatus['servizio']) ?></code></td></tr>
<?php endif; ?>
<tr><td>Risposta</td><td><?= $dh($csRisposta) ?>
    <?php if ((int)($diag_csStatus['http'] ?? 0) > 0): ?>
        <small>(HTTP <?= (int)$diag_csStatus['http'] ?><?= ($diag_csStatus['tipo'] ?? '') !== ''
            ? ', ' . $dh($diag_csStatus['tipo']) : '' ?>)</small>
    <?php endif; ?>
</td></tr>
...
<?php if (!$diag_csStatus['configured']): ?>
    <p class="fm-muted fm-text-em-md" >
        Setup: <code>bash tools/dev/setup_crowdsec.sh</code> sull'host; l'URL si imposta a mano,
        raggiungibile dal container. La diagnostica lo verifica: <code>--solo=crowdsec</code>.
    </p>
<?php endif; ?>
