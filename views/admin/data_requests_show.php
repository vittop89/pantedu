...
<div class="fm-card fm-mt-6" >
    ...
    <form method="POST" action="/admin/data-requests/<?= (int)$id ?>/action" class="fm-d-flex fm-flex-col fm-gap-4">
        ...
        <input type="hidden" name="_audit_reason" value="Richiesta dell'interessato #<?= (int)$id ?>: esito registrato dal pannello GDPR">
        ...
        <div class="fm-d-flex fm-gap-3 fm-flex-wrap">
            ...
            <button type="submit" name="action" value="open_incident"     class="fm-btn fm-btn--warn" title="Apre un incidente art. 33 collegato a questa segnalazione: le 72 ore decorrono da adesso">🚨 Apri incidente (art. 33)</button>
        </div>
    </form>
</div>
