...
$breadcrumb = [];  // Root admin: nessun crumb dopo "Admin"
$back_href  = '/?home=1';
...
?>
    ...
    <section class="fm-admin-kpi">
        ...
        <div class="fm-grid fm-grid--3">
            <?php
            // Piano classi-credenziali-scenari, B — i riquadri seguono lo
            // scenario (AdminAreas). Le registrazioni contano dove le
            // iscrizioni sono aperte; una richiesta arrivata lo stesso non si
            // nasconde mai. Credenziali di classe negli scenari 1 e 2, studenti
            // senza sezione nel 3: il controller li calcola solo se attivi.
            $_pending = (int)($counts['pending'] ?? 0);
            ?>
            <?php if (\App\Support\AdminAreas::isActive('registrations') || $_pending > 0): ?>
            <div class="fm-tile<?= $_pending > 0 ? ' fm-tile--alert' : '' ?>">
                <h3>Registrazioni in attesa</h3>
                <div class="fm-big"><?= $_pending ?></div>
                <div class="fm-muted fm-text-em-md" ><?= \App\Support\AdminAreas::isHighlighted('registrations')
                    ? 'Scenario 2: i colleghi entrano solo con la tua approvazione'
                    : 'Approvazione manuale' ?></div>
            </div>
            <?php endif; ?>
            <?php if (isset($counts['class_credentials'])): ?>
            <div class="fm-tile" id="credenziali-di-classe">
                <h3>Credenziali di classe</h3>
                <div class="fm-big"><?= (int)$counts['class_credentials']['active'] ?></div>
                <div class="fm-muted fm-text-em-md" >attive, di <?= (int)$counts['class_credentials']['teachers'] ?> docent<?= (int)$counts['class_credentials']['teachers'] === 1 ? 'e' : 'i' ?>: la porta degli studenti in questo scenario</div>
            </div>
            <?php endif; ?>
            <?php if (isset($counts['students_without_section'])): ?>
            <a href="/admin/sections" class="fm-tile fm-link-reset<?= (int)$counts['students_without_section'] > 0 ? ' fm-tile--alert' : '' ?>">
                <h3>Studenti senza sezione</h3>
                <div class="fm-big"><?= (int)$counts['students_without_section'] ?></div>
                <div class="fm-muted fm-text-em-md" >Nessun incarico li raggiunge finché non hanno una sezione</div>
            </a>
            <?php endif; ?>
            <?php if ($isSuperAdmin): ?>
...
<div id="fm-page-config" hidden data-csrf="<?= htmlspecialchars((string)$csrf, ENT_QUOTES) ?>"></div>
<?= \App\Support\ViteManifest::script('js/entries/admin-dashboard.js') ?>
