 ...
 * @var string|null         $drillLast
 * @var int|null            $drillDaysAgo
 * @var bool                $drillStale
 * @var string|null         $drillDueDate
...
<?php if ($drillStale): ?>
    <div class="fm-alert fm-alert--warning fm-mb-4">
        <strong>Drill di custodia da eseguire.</strong>
        <?php if ($drillDaysAgo === null): ?>
            Non risulta nessuna verifica delle copie di <code>KMS_MASTER_KEY</code>.
        <?php else: ?>
            L'ultima risale a <?= (int)$drillDaysAgo ?> giorni fa (<?= $h(substr((string)$drillLast, 0, 10)) ?>).
        <?php endif; ?>
        <br>
        Apri le tre copie in custodia e confronta la chiave con quella attiva in
        produzione, poi registra qui sotto un evento <code>kms_backup_verified</code>.
        La verifica non richiede di trascrivere la chiave: bastano i primi e gli
        ultimi caratteri.
    </div>
<?php endif; ?>
...
<section class="fm-grid fm-grid--3 fm-mb-6" >
    ...
    <div class="fm-tile<?= $drillStale ? ' fm-tile--alert' : '' ?>">
        <h3>Drill custodia <?= $drillStale ? '⚠️' : '' ?></h3>
        <div class="fm-big">
            <?php if ($drillDaysAgo === null): ?>
                mai eseguito
            <?php elseif ($drillDaysAgo === 0): ?>
                oggi
            <?php else: ?>
                <?= (int)$drillDaysAgo ?>g fa
            <?php endif; ?>
        </div>
        <p class="fm-muted fm-text-em-md fm-mt-1 fm-mb-0" >
            <?php if ($drillDueDate !== null): ?>
                Prossimo entro il <?= $h($drillDueDate) ?>.
            <?php endif; ?>
            Verifica semestrale delle copie di <code>KMS_MASTER_KEY</code>
            contro il valore in produzione (evento <code>kms_backup_verified</code>).
        </p>
    </div>
</section>
...
<section class="fm-card fm-mb-6" >
    ...
    <form method="POST" action="/admin/crypto-status/event" class="fm-form-grid">
        ...
        <input type="hidden" name="_audit_reason" value="Registrazione di un evento della cifratura dal pannello Stato cifratura">
            ...
            <tbody>
                <?php foreach ($recent as $e): ?>
                    ...
                    <tr class="fm-row-summary">
                        <td>
                            <button type="button" class="fm-row-toggle" data-fm-row-toggle
                                    aria-expanded="false"
