...
<?php
// Piano classi-credenziali-scenari, B — le aree che in questo scenario non
// hanno effetto stanno fuori dal menu, ma non sparite: sono elencate qui, con
// il motivo e il collegamento, cosi' chi cerca «Sezioni» sa dove sono.
$inertAreas = \App\Support\AdminAreas::inert($scenarioKey);
?>
<?php if ($inertAreas !== []): ?>
<section class="fm-section fm-mb-6" id="aree-inattive">
    <h2>⏸️ Aree del pannello inattive in questo scenario</h2>
    <p class="fm-muted">Fuori dal menu perché qui non cambiano nulla; restano raggiungibili dal collegamento, con l'avviso in testa.</p>
    <table class="fm-table fm-max-w-640">
        <thead><tr><th scope="col">Area</th><th scope="col">Perché tace</th><th scope="col">Si attiva con</th></tr></thead>
        <tbody>
        <?php foreach ($inertAreas as $a): ?>
            <tr>
                <td><a href="<?= $h($a['href']) ?>"><?= $h($a['label']) ?></a></td>
                <td class="fm-muted"><?= $h($a['why']) ?></td>
                <td><?= $h($a['activates_with']) ?></td>
            </tr>
        <?php endforeach; ?>
        </tbody>
    </table>
    ...
    <details class="fm-mb-4">
        ...
        <?php else: ?>
            <form method="post" action="/admin/system/deployment/switch" class="fm-form fm-max-w-640">
                ...
                <label class="fm-label">
                    ...
                    <input type="text" name="_audit_reason" class="fm-input" required minlength="10" maxlength="200"
                           placeholder="<?= $h($s === 'colleagues'
                               ? 'es. DPIA adottata il …: apro le iscrizioni ai colleghi'
                ...
                <button data-fm-confirm="Confermi il cambio di scenario? Pagine di accesso, registrazione e documenti di riferimento cambiano subito." type="submit" class="fm-btn <?= $s === 'personal' ? 'fm-btn--danger' : 'fm-btn--primary' ?>">
...
<details class="fm-section fm-mt-8" >
    ...
    <div class="fm-p-4">
        ...
        <form method="post" action="/admin/system/deployment/switch" class="fm-form">
            <input type="hidden" name="_csrf" value="<?= $h($csrf) ?>">
            <input type="hidden" name="_audit_reason" value="Rimozione dell'override dello scenario: si torna al valore del file .env">
            <input type="hidden" name="action" value="reset_to_env">
            <button data-fm-confirm="Rimuovo runtime override? Tornerà attivo il valore in .env." type="submit" class="fm-btn fm-btn--ghost">
                Reset a .env default
            </button>
        </form>
    </div>
...
$capForm = function (array $p) use ($h, $csrf, $cap_doc_types, $cap_sections) {
    ...
    <form method="post" action="/admin/system/capability/profile/save" class="fm-form" style="border:1px solid var(--fm-c-border,#e2e8f0);padding:12px;border-radius:6px;margin:8px 0">
        <input type="hidden" name="_csrf" value="<?= $h($csrf) ?>">
        <input type="hidden" name="_audit_reason" value="Salvataggio di un profilo di capacità dei docenti dal pannello Sistema">
...
?>
<section class="fm-section fm-mt-8" id="registrazione-studenti">
    <?php $inert_area = 'student_registration'; include __DIR__ . '/../_partials/inert_area.php'; ?>
    ...
    <form method="post" action="/admin/system/registration-mode" class="fm-stack">
        <input type="hidden" name="_csrf" value="<?= $h($csrf) ?>">
        <input type="hidden" name="_audit_reason" value="Cambio della modalità di iscrizione degli studenti dal pannello Sistema">
...
<section class="fm-section fm-mt-8">
        ...
        <details<?= !empty($p['is_default']) ? ' open' : '' ?>>
            ...
            <?php if (empty($p['is_default'])): ?>
                <form method="post" action="/admin/system/capability/profile/delete" class="fm-inline">
                    <input type="hidden" name="_csrf" value="<?= $h($csrf) ?>">
                    <input type="hidden" name="_audit_reason" value="Eliminazione del profilo di capacità #<?= (int)$p['id'] ?>, confermata nel pannello">
                    <input type="hidden" name="id" value="<?= (int)$p['id'] ?>">
                    <button data-fm-confirm="Elimino il profilo? I docenti assegnati torneranno al default." type="submit" class="fm-btn fm-btn--ghost fm-btn--sm">Elimina profilo</button>
                </form>
            <?php endif; ?>
        </details>
    <?php endforeach; ?>
    ...
    <?php if (empty($cap_teachers)): ?>
        <p class="fm-muted"><em>Nessun docente registrato.</em></p>
    <?php else: ?>
        <table class="fm-table fm-max-w-640">
                ...
                <tr>
                    ...
                    <td>
                        <form method="post" action="/admin/system/capability/assign" class="fm-inline">
                            <input type="hidden" name="_csrf" value="<?= $h($csrf) ?>">
                            <input type="hidden" name="_audit_reason" value="Profilo di capacità assegnato al docente #<?= (int)$t['id'] ?> dal pannello Sistema">
                            <input type="hidden" name="user_id" value="<?= (int)$t['id'] ?>">
                            <select data-fm-autosubmit name="profile_id" class="fm-input">
...
<?php /* L'entry va caricato in ogni modo, non solo nel ramo istituto in cui
         stava il primo <script>: i blocchi controllano da se' che i loro
         elementi esistano. */ ?>
<?= \App\Support\ViteManifest::script('js/entries/admin-system-deployment.js') ?>

<?php /* Piano classi-credenziali-scenari, B — la nota «Nessun account studente
         in questo scenario» e' sostituita dall'avviso di area inerte sopra i
         due blocchi della registrazione studenti: una voce sola. */ ?>

<!-- ═══════════════════════════════════════════════════════
     ADR-028 Fase 1 — CLASSI AMMESSE ALLA REGISTRAZIONE (trasversale)
     Sempre visibile; negli scenari 1 e 2 con l'avviso di area inerte.
     ═══════════════════════════════════════════════════════ -->
<section class="fm-section fm-mt-8" id="classi-ammesse">
    <?php $inert_area = 'student_registration'; include __DIR__ . '/../_partials/inert_area.php'; ?>
                ...
                <tr>
                    ...
                    <td>
                        <form method="post" action="/admin/system/registration-classes/remove" class="fm-inline">
                            <input type="hidden" name="_csrf" value="<?= $h($csrf) ?>">
                            <input type="hidden" name="_audit_reason" value="Rimozione di una classe dalle ammesse all'iscrizione, confermata nel pannello">
                            <input type="hidden" name="id" value="<?= (int)$c['id'] ?>">
                            <button data-fm-confirm="Rimuovo questa classe dalle ammesse?" type="submit" class="fm-btn fm-btn--ghost fm-btn--sm">Rimuovi</button>
                        </form>
                    </td>
                </tr>
            <?php endforeach; ?>
    ...
    <form method="post" action="/admin/system/registration-classes/add" class="fm-form fm-max-w-640 fm-mt-4">
        <input type="hidden" name="_csrf" value="<?= $h($csrf) ?>">
        <input type="hidden" name="_audit_reason" value="Aggiunta di una classe alle ammesse all'iscrizione dal pannello Sistema">
        <div class="fm-rc-row">
