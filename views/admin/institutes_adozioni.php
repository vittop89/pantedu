...
$stats  = $plan['stats'] ?? ['indirizzi' => 0, 'sezioni' => 0, 'sistemate' => 0, 'illeggibili' => 0, 'libri' => 0];
$totale = (int)$stats['indirizzi'] + (int)$stats['sezioni']
        + (int)($stats['materie'] ?? 0) + (int)$stats['sistemate'] + (int)($stats['libri'] ?? 0);
...
<section class="fm-mt-4">
    <div class="fm-alert fm-alert--<?= $totale > 0 ? 'info' : 'success' ?>">
        <strong>
            ...
            Libri nuovi nel catalogo delle adozioni: <?= (int)($stats['libri'] ?? 0) ?>
        </strong>
...
<section class="fm-mt-8">
    ...
    <div class="fm-d-flex fm-gap-2 fm-items-center">
        <form method="post" action="/admin/institutes/adozioni/apply">
            ...
            <input type="hidden" name="_audit_reason" value="Applicazione delle adozioni MIUR confermate a indirizzi e sezioni dell'istituto">
