...
<details>
    ...
    <div class="fm-card fm-mt-2" >
        ...
        <form method="POST" action="/admin/backup/b2-verified" class="fm-form-grid">
            ...
            <input type="hidden" name="_audit_reason" value="Registrazione della verifica del backup su B2 dal pannello Backup">
...
<details<?= $coldBackupStale ? ' open' : '' ?>>
    ...
    <div class="fm-card fm-mt-2" >
        ...
        <ul class="fm-mt-2 fm-mb-4 fm-pl-em-lg fm-lh-loose">
            ...
            <li><strong>Repo pantedu clonato</strong> in una cartella locale (serve per leggere il commit HEAD nel manifest)</li>
            <li><strong>SSH via Cloudflare Tunnel</strong>: alias <code>pantedu-tunnel</code> in <code>~/.ssh/config</code> con la chiave <code>id_ed25519</code>. L'alias <code>pantedu-vps</code> non funziona pi&ugrave;: sshd ascolta solo su <code>127.0.0.1:2222</code>.</li>
            <li><strong>rclone CLI</strong> con remote <code>b2-pantedu</code> configurato (application key Backblaze B2 — conservata nel password manager)</li>
        ...
        <p class="fm-info-banner fm-mt-4" >
            ...
            <a href="https://github.com/vittop89/pantedu/blob/main/tools/admin/cold_backup.ps1"
        ...
        <ol class="fm-mt-2 fm-mb-4 fm-pl-em-lg fm-lh-loose">
            <li>
                Connetti l'HDD esterno alla postazione di sviluppo. Annota la <strong>lettera del drive</strong>
                (es. <code>E:</code>, <code>F:</code>) o il <strong>label fisico</strong>.
            </li>
            <li>
                <strong>Prepara l'accesso al VPS.</strong> Dalla cartella del repo:
                <pre class="fm-row-detail__pre fm-my-1 fm-select-all" >./pantedu.exe up</pre>
                Chiede la passphrase della chiave SSH. Va fatto <em>prima</em>: dentro
                lo script non c'&egrave; modo di digitarla.
            </li>
            <li>
                Sempre dalla cartella del repo, avvia il backup
                <small class="fm-muted">(clic sul riquadro per selezionarlo tutto)</small>:
                <pre class="fm-row-detail__pre fm-my-1 fm-select-all" >powershell -ExecutionPolicy Bypass -File tools/admin/cold_backup.ps1</pre>
            </li>
            <li>
                Lo script aprirà <strong>Esplora Risorse</strong> per scegliere la cartella
                Seleziona la <strong>radice del drive esterno</strong> (es. <code>E:\</code>):
                la sottocartella <code>pantedu-backups\AAAA-MM\</code> la crea lo script.
            </li>
            <li>
        ...
        <form method="POST" action="/admin/backup/cold-completed" class="fm-form-grid">
            <input type="hidden" name="_csrf" value="<?= $h($csrf) ?>">
            <input type="hidden" name="_audit_reason" value="Registrazione della copia fredda completata dal pannello Backup">
            <label>
