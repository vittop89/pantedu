...
// `$form`: id del <form> della riga. L'HTML non permette un <form> che avvolga
// delle <td>, quindi il form e' una riga a parte e l'attributo `form` lega i
// select (fino al 2026-09-05 lo metteva uno script inline).
$tendina = static function (string $nome, string $id, array $voci, string $form = '') use ($h): string {
    if ($voci === []) {
    ...
    <?php else: ?>
        <section class="fm-card">
                        ...
                        <tr>
                            ...
                            <td>
                                <?php if (in_array('indirizzo', $manca, true)): ?>
                                    ...
                                    <?= $tendina('indirizzo_id', 'ind-' . $i, $indirizzi, 'cat-' . $i) ?>
                                <?php else: ?>
                            ...
                            <td>
                                <?php if (in_array('classe', $manca, true)): ?>
                                    ...
                                    <?= $tendina('classe_id', 'cls-' . $i, $classi, 'cat-' . $i) ?>
                                <?php else: ?>
                            ...
                            <td>
                                <?php if (in_array('materia', $manca, true)): ?>
                                    ...
                                    <?= $tendina('subject_id', 'mat-' . $i, $materie, 'cat-' . $i) ?>
                                <?php else: ?>
            ...
            <p class="fm-text-sm fm-text-muted fm-mt-3 fm-mb-0">
                ...
                serve a ritrovarlo. Per cambiare la classe a contenuti che ce l'hanno già, a
                blocchi: <a href="/area-docente/sposta-di-classe">Sposta di classe</a>.
            </p>
        </section>
    <?php endif; ?>
</main>
