...
<script type="module">
    ...
    // Vocabolario indirizzi della scuola (righe owner NULL), dal server:
    // popola la tendina dell'editor invece del campo libero.
    let instituteIndirizzi = [];
    ...
    async function loadCurriculumAndPivot({ skipEditor = false } = {}) {
        ...
        instituteIndirizzi = cat.institute_indirizzi || [];
    ...
    function renderCatalogPanel(kind, entries, panelEl) {
        ...
        const codeHint = {
            indirizzi: "codice di un indirizzo che la scuola ha già (es. SCI, ART)",
        ...
        // Gli indirizzi li definisce la scuola: il docente sceglie fra quelli,
        // non ne inventa. Con un campo libero il server rifiuterebbe la sigla
        // sbagliata DOPO averla digitata; qui non gliela lasciamo scrivere.
        // Vengono esclusi quelli che ha gia': ripeterli darebbe duplicate_code.
        const isIndirizzi = kind === "indirizzi";
        const giaPresi = new Set(ownEntries.map(e => e.code));
        const disponibili = (instituteIndirizzi || []).filter(i => !giaPresi.has(i.code));
        let addFormHtml;
        if (isIndirizzi && !instituteIndirizzi.length) {
            addFormHtml = `<p class="fm-muted fm-text-xs">Questo istituto non ha ancora indirizzi definiti.
                Gli indirizzi li stabilisce la scuola: chiedi a un amministratore di aggiungerli.</p>`;
        } else if (isIndirizzi && !disponibili.length) {
            addFormHtml = `<p class="fm-muted fm-text-xs">Hai gia' tutti gli indirizzi di questo istituto.</p>`;
        } else if (isIndirizzi) {
            addFormHtml = `
            <div class="fm-curr-form" data-add-form data-kind="${kind}">
                <select name="code" data-indirizzo-select required>
                    <option value="">— Scegli un indirizzo della scuola —</option>
                    ${disponibili.map(i => `<option value="${escapeHtml(i.code)}"
                        data-label="${escapeHtml(i.label || i.code)}"
                        data-group="${escapeHtml(i.group || "")}">${escapeHtml(i.label || i.code)} (${escapeHtml(i.code)})</option>`).join("")}
                </select>
                <input type="text" name="label" placeholder="Etichetta visibile" maxlength="120" required>
                <input type="text" name="group" placeholder="Gruppo (opzionale)" maxlength="60">
        ...
        `;
        const addBtn = panelEl.querySelector("[data-add-btn]");
        if (addBtn) { addBtn.addEventListener("click", () => addEntry(kind, panelEl)); }
        // Scelto il codice, etichetta e gruppo arrivano dalla scuola: sono
        // modificabili, ma il default e' quello giusto.
        const indSelect = panelEl.querySelector("[data-indirizzo-select]");
        if (indSelect) {
            indSelect.addEventListener("change", () => {
                const opt = indSelect.selectedOptions[0];
                if (!opt || !opt.value) { return; }
                const form = indSelect.closest("[data-add-form]");
                form.querySelector("[name=label]").value = opt.dataset.label || opt.value;
                form.querySelector("[name=group]").value = opt.dataset.group || "";
            });
        }
