...
// Chi include il partial fuori dal layout (ricerca esercizi, sidebar pubblica)
// non definisce le variabili d'ingresso: ripiego calcolato qui (2026-09-05,
// prima: «Undefined variable $isTeacher/$isAdmin» a ogni resa).
$isAdmin   = $isAdmin   ?? (Auth::check() && Auth::hasAccess('admin'));
$isTeacher = $isTeacher ?? (Auth::check() && Auth::role() === 'teacher');
// Il layout lo mette a true quando ha ristretto le materie a quelle con
// materiali per chi studia (19/9/2026); fuori dal layout il selettore e' quello di sempre.
$_fmMaterieDiChiStudia = $_fmMaterieDiChiStudia ?? false;
...
// Uno STUDENTE non sceglie indirizzo e classe: sono quelli della sua
// iscrizione, e il filtro dei contenuti li usa comunque dal DB
// (StudentProfileService::scopeForUser). Offrire dei selettori suggeriva
// una liberta' che non ha e che non avrebbe effetto.
$studentScope = null;
if (!$isTeacher && !$isAdmin && !empty($authedUser['id'])) {
    try {
        $studentScope = (new \App\Services\Student\StudentProfileService())
            ->scopeForUser((int)$authedUser['id']);
    } catch (\Throwable) {
        $studentScope = null;
    }
    if (empty($studentScope['indirizzo']) || empty($studentScope['classe'])) {
        $studentScope = null; // profilo incompleto: restano i selettori
    }
}
// ADR-032 (2026-09-04) — ospite con credenziale di classe: e' uno studente
// senza account. La sidebar lo tratta come tale (sezioni visibili agli
// studenti, endpoint /api/study/*), e indirizzo/classe vengono dalla
// credenziale quando e' delimitata. Prima era un guest puro e non vedeva nulla.
$_fmGrant  = Auth::check() ? null : \App\Support\ClassAccessGrant::current();
// Piano classi, C — il portachiavi: piu' credenziali, una per docente. Lo
// scope della sidebar e' quello della prima credenziale delimitata.
$_fmGrants = $_fmGrant !== null ? \App\Support\ClassAccessGrant::all() : [];
if ($_fmGrant !== null && $studentScope === null) {
    foreach ($_fmGrants as $_g) {
        if ($_g['indirizzo'] !== null && $_g['classe'] !== null) {
            $studentScope = ['indirizzo' => $_g['indirizzo'], 'classe' => $_g['classe']];
            break;
        }
    }
}
// Piano classi-credenziali-scenari, D — classi frequentate: lo studente puo'
// guardare anche gli anni gia' fatti (al livello di anno, o la sezione se il
// profilo la ricorda). Il selettore Classe torna visibile con quelle voci;
// l'indirizzo resta fisso. Con una voce sola resta tutto com'era.
$studentClassi = [];
if ($studentScope !== null && $_fmGrants !== []) {
    // Portachiavi (C): l'unione delle classi frequentate di ogni credenziale
    // delimitata. Una sezione nota rende superfluo l'anno secco dello stesso
    // anno («3A» comprende i «3»), cosi' chi ha credenziali «3A» e «3» vede
    // una voce sola per la terza.
    $_byCode = [];
    foreach ($_fmGrants as $_g) {
        if ($_g['classe'] === null) {
            continue;
        }
        foreach (\App\Domain\ClassiFrequentate::per((string)$_g['classe']) as $_c) {
            $_byCode[strtoupper((string)$_c['classe'])] ??= $_c;
        }
    }
    foreach ($_byCode as $_k => $_c) {
        if (\App\Domain\ClassCode::isSezione($_c['classe'])) {
            continue;
        }
        foreach ($_byCode as $_c2) {
            if (\App\Domain\ClassCode::isSezione($_c2['classe']) && $_c2['anno'] === $_c['anno']) {
                unset($_byCode[$_k]);
                break;
            }
        }
    }
    $studentClassi = array_values($_byCode);
    usort($studentClassi, static fn(array $a, array $b): int =>
        [(int)$b['anno'], (int)$b['corrente']] <=> [(int)$a['anno'], (int)$a['corrente']]);
} elseif ($studentScope !== null) {
    $_storico = [];
    if (!$isTeacher && !$isAdmin && !empty($authedUser['id'])) {
        try {
            $_storico = (new \App\Services\Student\StudentProfileService())->storicoForUser((int)$authedUser['id']);
        } catch (\Throwable) {
            $_storico = [];
        }
    }
    $studentClassi = \App\Domain\ClassiFrequentate::per((string)$studentScope['classe'], $_storico);
}
$scopeLabel = static function (array $rows, ?string $code): string {
    foreach ($rows as $r) {
        if ((string)($r['code'] ?? '') === (string)$code) {
            return (string)($r['label'] ?? $code);
        }
    }
    return (string)$code;
};
...
$_fmGuest = !Auth::check() && $_fmGrant === null;
...
// Senza un docente che pubblica in rete le sezioni pubbliche sarebbero vuote:
// PublicContentPolicy non ne restituisce, e al visitatore resta solo l'accesso.
$_publicSections = [];
if ($_fmGuest) {
    try { $_publicSections = \App\Services\Study\PublicContentPolicy::sezioniPubbliche(); } catch (\Throwable $_) {}
}
...
<nav class="sidebar fm-sidebar"
    ...
    <div class="sel-wrapper" data-curriculum-endpoint="/curriculum">
        ...
        <?php endif; ?>
        <?php if ($studentScope !== null): ?>
            <?php /* Badge al posto dei selettori. I <select> restano nel DOM,
                     nascosti e con un'unica opzione: fm-pt-document.js e
                     fm-risdoc-section-header.js leggono #sel-iis/#sel-cls per
                     valore, e toglierli spegnerebbe la navigazione. */ ?>
            <?php /* Il badge con la classe sta nel banner di sessione, piu'
                     in basso: e' un'informazione sullo studente, come il nome
                     e i suoi dati, e teneva un riquadro tutto suo per dire una
                     riga. Qui restano i soli <select> nascosti. */ ?>
            <select id="sel-iis" hidden class="fm-d-none" aria-hidden="true" tabindex="-1">
                <option value="<?= htmlspecialchars((string)$studentScope['indirizzo'], ENT_QUOTES) ?>" selected></option>
            </select>
        <?php else: ?>
        <label class="fm-sb-lab" for="sel-iis">Indirizzo: </label>
        <?php /* I tre selettori si scelgono in ordine — indirizzo, classe, materia —
                 e nessuno sceglie da solo: il segnaposto ha valore vuoto e resta
                 selezionato finché l'utente non decide (js/modules/core/sidebar-cascade.js
                 chiude i selettori a valle finché quello a monte è vuoto). */ ?>
        ...
        <?php if ($studentScope !== null && count($studentClassi) > 1): ?>
            <?php /* Piano classi, D — le classi frequentate: la propria in testa,
                     poi gli anni gia' fatti. fm-url-state.js seleziona quella
                     dell'URL; db-sidepage.js naviga al cambio come per tutti. */ ?>
            <label class="fm-sb-lab" for="sel-cls">Classe:</label>
            <select class="fm-sb-sel" id="sel-cls" data-fm-classi-frequentate="1">
                <?php foreach ($studentClassi as $c): ?>
                    <option value="<?= htmlspecialchars((string)$c['classe'], ENT_QUOTES) ?>" <?= $c['corrente'] ? 'selected' : '' ?>>
                        <?= htmlspecialchars((string)$c['label']) ?>
                    </option>
                <?php endforeach; ?>
            </select><br>
        <?php elseif ($studentScope !== null): ?>
            <select id="sel-cls" hidden class="fm-d-none" aria-hidden="true" tabindex="-1">
                <option value="<?= htmlspecialchars((string)$studentScope['classe'], ENT_QUOTES) ?>" selected></option>
            </select>
        <?php else: ?>
        <label class="fm-sb-lab" for="sel-cls">Classe:</label>
        <select class="fm-sb-sel" id="sel-cls">
            ...
            <?php foreach ($clsGroups as $groupName => $rows): ?>
                <optgroup label="<?= htmlspecialchars($groupName ?: 'Altri', ENT_QUOTES) ?>">
                    <?php foreach ($rows as $row): ?>
                        <?php /* Il corso della sezione (migrazione 100): la cascata
                                 filtra le classi per indirizzo già al primo disegno,
                                 senza aspettare la rete. Vuoto = vale per tutti. */ ?>
                        <option value="<?= htmlspecialchars($row['code'], ENT_QUOTES) ?>"
                                data-indirizzo="<?= htmlspecialchars((string)($row['indirizzo'] ?? ''), ENT_QUOTES) ?>">
                            <?= htmlspecialchars($row['label']) ?>
                        </option>
                    <?php endforeach; ?>
                </optgroup>
        ...
        <?php /* 19/9/2026 — per chi studia le materie sono solo quelle con
                 materiali che puo' vedere (views/layout/app.php). Senza
                 nessuna, al posto del selettore c'e' un avviso; il <select>
                 resta nel DOM, nascosto, perche' altri moduli lo leggono, e il
                 segno data-fm-materie-di-chi-studia dice che le sue voci sono
                 tutte e sole le materie da chiedere (db-sidepage.js) e che al
                 cambio di classe vanno ricalcolate (materie-di-chi-studia.js). */ ?>
        <?php if ($_fmMaterieDiChiStudia): ?>
            <p class="fm-sb-avviso" id="fm-materie-avviso" role="status"<?= $materie === [] ? '' : ' hidden' ?>>Non ci sono ancora materiali pubblicati per la tua classe.</p>
        <?php endif; ?>
        <label class="fm-sb-lab" for="sel-mater"<?= $_fmMaterieDiChiStudia && $materie === [] ? ' hidden' : '' ?>>Materia:</label>
        <select class="fm-sb-sel" id="sel-mater"<?php if ($_fmMaterieDiChiStudia): ?> data-fm-materie-di-chi-studia="1"<?= $materie === [] ? ' hidden' : '' ?><?php endif; ?>>
        ...
        <div class="sel-wrapper-actions">
            ...
            <?php elseif (!Auth::check()): ?>
                ...
                <?php if ($_fmGrant !== null): ?>
                    <?php /* ADR-032 — chiude il grant di classe (POST con CSRF, niente JS inline). */ ?>
                    <form method="post" action="/accesso-classe/esci" class="fm-inline">
                        <input type="hidden" name="_csrf" value="<?= htmlspecialchars(Csrf::token(), ENT_QUOTES) ?>">
                        <button type="submit" class="sel-action-link sel-action-link--danger" title="Esci dalla classe"
                                style="background:none;border:0;padding:0;cursor:pointer;font:inherit">Esci</button>
                    </form>
                <?php else: ?>
                    <?php /* ADR-032 — link secondo lo scenario: iscrizione docenti
                             solo se aperta; credenziale di classe se non ci sono
                             account studente. */ ?>
                    <?php if (\App\Support\DeploymentScenario::teacherSelfSignupOpen()): ?>
                        <a class="sel-action-link" href="/register">Registrati</a>
                    <?php endif; ?>
                    <?php if (!\App\Support\DeploymentScenario::studentAccountsEnabled()): ?>
                        <a class="sel-action-link" href="/accesso-classe" title="Studenti: entra con la credenziale della classe">🎓 Classe</a>
                    <?php endif; ?>
                <?php endif; ?>
            <?php else: ?>
    ...
    <?php /* Phase 20 — layout 2-row: user+badges top centrato, links row bottom
              ...
              ai lati). */ ?>
    <?php if ($isTeacher): ?>
        <?php /* data-fm-drive: i comandi Drive nella barra ci sono solo con Drive acceso (ADR-038). */ ?>
        <div class="sel-session-banner fm-session-banner fm-session-banner--teacher" data-fm-drive="<?= htmlspecialchars(\App\Services\Drive\StatoDiDrive::attuale(), ENT_QUOTES) ?>">
    ...
    <?php elseif ($_fmGrant !== null): ?>
        <?php /* ADR-032 — banner della credenziale di classe: stessa classe
                 CSS del banner studente (student-resource-auth.js la usa come
                 marker per non mostrare il suo widget). Nessun nome: la
                 sessione non e' associata a una persona. */ ?>
        <div class="sel-session-banner fm-session-banner fm-session-banner--student">
            <?php if ($studentScope !== null): ?>
                <div class="fm-sb-scope">
                    <span class="fm-sb-scope__lab fm-sr-only">Classe</span>
                    <span class="fm-sb-scope__cls"><?= htmlspecialchars((string)$studentScope['classe']) ?></span>
                    <span class="fm-sb-scope__ind"><?= htmlspecialchars($scopeLabel($curriculum['indirizzi'] ?? [], $studentScope['indirizzo'])) ?></span>
                </div>
            <?php endif; ?>
            <div class="fm-session-ident">
                <?php /* Piano classi, C (2026-09-13) — il portachiavi lo disegna
                         il server, qui: una riga per credenziale, con l'uscita
                         dalla singola quando ce n'e' piu' d'una (per una sola
                         basta l'«Esci» della riga dei comandi). Il riquadro
                         #fm-resource-auth di student-resource-auth.js non puo'
                         comparire davanti a questo banner (voce 100 del debito). */ ?>
                <ul class="fm-session-keychain fm-pl-0 fm-m-0" style="list-style:none" aria-label="Le tue credenziali di classe">
                    <?php foreach ($_fmGrants as $_g): ?>
                        <li class="fm-d-flex fm-items-center fm-gap-1">
                            <span class="fm-session-user">🎓 <strong<?php if (!empty($_g['materie_nomi'])): ?> title="Materie: <?= htmlspecialchars((string)$_g['materie_nomi'], ENT_QUOTES) ?>"<?php endif; ?>><?= htmlspecialchars($_g['label'] !== '' ? $_g['label'] : 'Accesso classe', ENT_QUOTES) ?></strong><?php
                                if ($_g['classe'] !== null): ?> <span class="fm-muted fm-text-11">· <?= htmlspecialchars((string)$_g['classe'], ENT_QUOTES) ?></span><?php endif; ?></span>
                            <?php if ($_g['credential_id'] !== null && \count($_fmGrants) > 1): ?>
                                <form method="post" action="/accesso-classe/esci" class="fm-inline fm-ml-auto">
                                    <input type="hidden" name="_csrf" value="<?= htmlspecialchars(Csrf::token(), ENT_QUOTES) ?>">
                                    <input type="hidden" name="credential_id" value="<?= (int)$_g['credential_id'] ?>">
                                    <button type="submit" class="sel-action-link fm-text-11" title="Togli solo questa credenziale"
                                            style="background:none;border:0;padding:0;cursor:pointer;font:inherit">Esci</button>
                                </form>
                            <?php endif; ?>
                        </li>
                ...
                <div class="fm-session-links">
                    <?php /* La scritta resta in ogni scenario: e' l'unico segno
                             che si e' entrati senza account. Il link per la
                             credenziale di un altro docente solo dove altri
                             docenti ci sono (19/9/2026): nello scenario 1 c'e'
                             solo l'autore (ADR-032, informativa §2). */ ?>
                    <span class="fm-muted fm-text-11">Credenziale del docente, nessun account</span>
                    <?php if (\App\Support\DeploymentScenario::consentePiuDocenti()): ?>
                        <a href="/accesso-classe" title="Inserisci la credenziale di un altro docente">➕ Aggiungi</a>
                    <?php endif; ?>
                </div>
            </div>
        </div>
    <?php elseif (Auth::check() && Auth::role() === 'student'): ?>
        ...
        <div class="sel-session-banner fm-session-banner fm-session-banner--student">
            <?php if ($studentScope !== null): ?>
                <?php /* Classe a sinistra, identita' a destra. In colonna il
                         banner diventava alto il doppio per dire due cose
                         corte, e la sidebar ha larghezza da vendere. */ ?>
                <div class="fm-sb-scope">
                    <?php /* L'etichetta resta per chi legge con lo screen
                             reader — "1A" da solo non dice di cosa e' il
                             numero — ma a video e' rumore: la pastiglia e il
                             nome del corso si spiegano da soli. */ ?>
                    <span class="fm-sb-scope__lab fm-sr-only">La tua classe</span>
                    <span class="fm-sb-scope__cls"><?= htmlspecialchars((string)$studentScope['classe']) ?></span>
                    <span class="fm-sb-scope__ind"><?= htmlspecialchars($scopeLabel($curriculum['indirizzi'] ?? [], $studentScope['indirizzo'])) ?></span>
                </div>
            <?php endif; ?>
