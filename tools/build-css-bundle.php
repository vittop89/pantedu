...
function expandImports(string $content, string $sourceFile, array &$visited): string {
    // Due sintassi, entrambe valide in CSS e entrambe usate nel progetto:
    //
    //     @import url('/css/x.css') layer(y);     <- forma con url()
    //     @import '/css/x.css'      layer(y);     <- stringa nuda
    //
    // Il regex accettava solo la prima. main.css usa la seconda per tutti i
    // suoi import, quindi non ne combaciava nemmeno uno: il bundle usciva con
    // "main.css + 0 @import nested" e i ~70 moduli restavano @import da
    // risolvere a runtime. Il difetto non si vedeva — il CSS arrivava lo
    // stesso, un file alla volta — finche' non e' emerso da dove doveva:
    // main.bundle.css porta il cache-bust `?v=<mtime>`, i moduli importati no,
    // e con `Cache-Control: max-age=604800` una modifica al CSS restava
    // invisibile ai browser di ritorno fino a SETTE GIORNI.
    return preg_replace_callback(
        '#@import\s+(?:url\s*\(\s*[\'"]?(?P<u1>/css/[^\'")]+\.css)[\'"]?\s*\)'
        . '|[\'"](?P<u2>/css/[^\'"]+\.css)[\'"])(?P<rest>[^;]*);#',
        function (array $m) use ($sourceFile, &$visited): string {
            // Con l'alternanza entrambi i gruppi esistono sempre: quello del
            // ramo non imboccato arriva vuoto, non assente.
            $url    = $m['u1'] !== '' ? $m['u1'] : $m['u2'];
            // Con i gruppi nominati $m porta sia chiavi numeriche sia
            // nominate: pescare "l'ultimo indice" restituirebbe null.
            $layer  = trim($m['rest']);
