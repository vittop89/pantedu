...
# DENTRO WSL LO SCRIPT NON GIRA (2026-09-22). pandoc e i font (Calibri,
# Consolas) stanno su Windows. Col bash di WSL pandoc mancava e lo script
# suggeriva "winget install", che lo installa su Windows, dove questo bash non
# lo cerca: un consiglio sbagliato. Ci si ferma qui, prima di cercare qualunque
# programma e di creare file, e si rimanda alla guida. Esce 3: ne' un documento
# fallito (1) ne' un errore d'uso (2).
#
# Il modo giusto non si blocca: il bash di Git per Windows lanciato dalla
# sessione in WSL non riceve WSL_DISTRO_NAME, perche' WSLENV e' vuota
# (misurato il 2026-09-22). Prova: tests/ops/build-pdf-wsl.test.sh.
if [[ -n "${WSL_DISTRO_NAME:-}" ]]; then
    {
        echo "build_pdf.sh: questa shell e' dentro WSL (WSL_DISTRO_NAME=$WSL_DISTRO_NAME),"
        echo "  dove non ci sono pandoc ne' i font dei PDF. Si lancia con il bash di Git"
        echo "  per Windows, dalla sessione in WSL: docs/dev/sviluppo-in-wsl.md,"
        echo "  sezione «Che cosa si fa ancora da Windows»."
    } >&2
    exit 3
fi
...
ALL_DOCS=(
    ...
    docs/privacy/informativa-personale.md
