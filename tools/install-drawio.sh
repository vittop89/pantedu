...
#
# Lo chiama la costruzione dell'immagine (docker/Dockerfile, stadio `drawio`):
# è la copia che il container serve. Il webhook deploy.sh lo chiama ancora
# anche sull'host, dove il container non la legge.
# In dev: bash tools/install-drawio.sh per (re)installare.

set -euo pipefail

DRAWIO_VERSION="${DRAWIO_VERSION:-v29.7.12}"
# L'impronta di draw.war per la versione qui sopra: quella che GitHub pubblica
# per l'allegato della release (campo `digest` dell'API, letto il 15/9/2026).
# Un file diverso non si installa. Chi cambia versione cambia anche questa.
DRAWIO_SHA256="${DRAWIO_SHA256:-22876d0cf22216bb32c3b306ee6b80a467716c15c01d9e64a4b3746592324b61}"
# Da dove si scarica: si cambia solo nella prova (tests/ops/install-drawio.test.sh).
...
AVUTA=$(sha256sum "$TMP_DIR/draw.war" 2>/dev/null | cut -d' ' -f1 || true)
[ -n "$AVUTA" ] || AVUTA=$(shasum -a 256 "$TMP_DIR/draw.war" | cut -d' ' -f1)
if [ "$AVUTA" != "$DRAWIO_SHA256" ]; then
    echo "ERROR: sha256 di draw.war $AVUTA, attesa $DRAWIO_SHA256: non si installa" >&2
    exit 1
fi
echo "    OK: $AVUTA"
