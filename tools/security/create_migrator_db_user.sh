...
# 23/9/2026 (A-86) — password da un file di opzioni temporaneo, non da `-p`;
# con --defaults-file, perché da root /root/.my.cnf avrebbe sostituito le
# credenziali e la prova sarebbe passata come root (vedi opzioni-mysql.sh).
con_credenziali_mysql "$MIG_USER" "$PW" mysql -N -e "SELECT 1;" "$DB_NAME" >/dev/null 2>&1 \
