...
/**
 * Chiude un blocco: piega gli hash sull'impronta corrente e descrive il blocco.
 * Funzione pura al posto della closure con catture per riferimento: PHPStan
 * non riusciva a seguirne lo stato (2026-09-04) e il codice era meno leggibile.
 *
 * @param list<string> $hashes
 * @return array{from_id:int,to_id:int,count:int,prev_head:string,head:string}
 */
function closeBlock(string $prevHead, array $hashes, int $from, int $to): array
{
    return [
        'from_id'   => $from,
        'to_id'     => $to,
        'count'     => count($hashes),
        'prev_head' => $prevHead,
        'head'      => AuditChain::fold($prevHead, $hashes),
    ];
}
...
foreach (TABLES as $table) {
    ...
    while (($row = $st->fetch(PDO::FETCH_ASSOC)) !== false) {
        ...
        if (count($hashes) >= BLOCK) {
            $blocks[] = closeBlock($head, $hashes, $from, $to);
            $head     = $blocks[array_key_last($blocks)]['head'];
            $total   += count($hashes);
            $hashes   = [];
            $from     = 0;
        }
    ...
    $newLast = $blocks === [] ? $lastId : (int)$blocks[array_key_last($blocks)]['to_id'];
