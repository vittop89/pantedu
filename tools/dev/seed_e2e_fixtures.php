 ...
 *   4. Registra l'accettazione ToS/AUP per gli utenti E2E: con TOS_ENFORCE=true
 *      il gate redirige al form chiunque non l'abbia, e ogni spec fallirebbe.
 *      Sono fixture (User-Agent 'e2e-fixture'), non prove di consenso.
 *
...
try {
    // ADR-037, fase 4c-2: il posto sta nella principale (la vista lo dice), e lo
    // stato della principale lo porta l'applicazione.
    ...
    $pubblica = $pdo->prepare("UPDATE teacher_content_data SET visibility='published' WHERE id = ?");
    foreach ($ids as $id) {
        $pubblica->execute([$id]);
        \App\Support\PostoPrincipale::statoDelContenuto($pdo, $id);
    }
    echo "[2b] Esercizi SCI/2/MAT pubblicati: " . count($ids) . " ✓\n";
} catch (\Throwable $e) {
...
// ── 4. Accettazione ToS/AUP per gli utenti E2E ────────────────────────────
// Con TOS_ENFORCE=true il middleware redirige a /tos-acceptance chiunque non
// abbia accettato la versione vigente: senza questa riga ogni spec che fa
// login finirebbe sul muro invece che sulla pagina in prova.
//
// Queste righe sono FIXTURE DI TEST su DB locale, non prove di consenso: lo
// dichiara l'User-Agent 'e2e-fixture'. In produzione l'accettazione si
// raccoglie solo dal form, con IP e User-Agent reali.
echo "\n[4] Accettazione ToS/AUP per gli utenti E2E:\n";
try {
    $svc = new \App\Services\Gdpr\TosAcceptanceService($pdo);
    $eff = $svc->effectiveVersions();
    echo "    versioni vigenti: ToS {$eff['tos']} / AUP {$eff['aup']}\n";

    $e2eUsers = array_values(array_filter([
        $adminUser,
        (string)($_ENV['E2E_TEACHER_USER'] ?? ''),
    ], static fn(string $u) => $u !== ''));

    foreach ($e2eUsers as $username) {
        $q = $pdo->prepare('SELECT id FROM users WHERE username = ? LIMIT 1');
        $q->execute([$username]);
        $uid = (int)($q->fetchColumn() ?: 0);
        if ($uid <= 0) {
            echo "    - $username: ⚠ utente non trovato\n";
            continue;
        }
        if ($svc->hasAccepted($uid)) {
            echo "    - $username (id $uid): già in regola ✓\n";
            continue;
        }
        $svc->recordAcceptance($uid, '127.0.0.1', 'e2e-fixture');
        echo "    - $username (id $uid): accettazione seedata ✓\n";
    }
} catch (\Throwable $e) {
    echo "    ⚠ impossibile seedare l'accettazione: {$e->getMessage()}\n";
    $ok = false;
}
