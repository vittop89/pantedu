...
mysqldump --single-transaction --quick "$DB_NAME" \
    ...
    risdoc_compilations teacher_access_credentials \
...
# Il nome del servizio segue la versione di PHP (8.3 -> 8.4 il 2026-07): va
# rilevato, non inchiodato. Prima era "systemctl reload php8.3-fpm", che dopo
# l'aggiornamento ricaricava un'unita' inesistente stampando comunque "OK":
# l'opcache restava stale e nessuno se ne accorgeva.
echo "[9] PHP-FPM reload..."
FPM_UNIT="$(systemctl list-units --type=service --state=running --no-legend 'php*-fpm.service' \
            | awk '{print $1}' | head -1)"
if [ -z "$FPM_UNIT" ]; then
    echo "  ✗ nessun php*-fpm in esecuzione — opcache NON ricaricata" >&2
    exit 1
fi
if systemctl reload "$FPM_UNIT"; then
    echo "  ✓ $FPM_UNIT ricaricato"
else
    echo "  ✗ reload di $FPM_UNIT fallito" >&2
    exit 1
fi
echo ""
...
echo "  curl -I https://pantedu.eu/login    # 200 expected (tex.pantedu.eu: DNS rimosso 2026-05-24)"
