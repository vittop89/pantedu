<?php
// G22.S20 v2.C2 — Full E2E real test: docente1 (485 file) → docente2
...
echo "═══ FULL E2E test: docente1 (all) → docente2 (v2.C2) ═══\n\n";

// ── 1. Setup: cleanup docente2 + rigenera recovery docente1
echo "[1] Setup…\n";
$pdo->beginTransaction();
$pdo->prepare('DELETE FROM verifica_documents_data WHERE teacher_id=?')->execute([$docente2Id]);
$pdo->prepare('DELETE FROM teacher_content_data WHERE teacher_id=? AND content_type IN ("mappa","esercizio","documento")')->execute([$docente2Id]);
$pdo->commit();

$svc = new TeacherRecoveryService();
$svc->revoke($docente1Id);
$pdo->prepare('DELETE FROM teacher_recovery_keys WHERE user_id=?')->execute([$docente1Id]);
$pdo->prepare('UPDATE teacher_keys SET kek_recovery_wrapped=NULL, recovery_wrap_kv=NULL WHERE teacher_id=?')->execute([$docente1Id]);
$gen = $svc->generate($docente1Id, '127.0.0.1', 'cli-full-test');
$rHex = $gen['recovery_hex'];
echo "  ✓ docente2 cleaned, docente1 recovery key regenerated\n";

// ── 2. Manifest signed (full bundle)
echo "\n[2] Operatore: manifest signed (full bundle)…\n";
auth('docente.uno', $docente1Id);
...
$entries = $ctrl->buildLocalBundleManifest($docente1Id);
$byPath = [];
...
foreach ($manifest['files'] as $i => $sel) {
    ...
    try {
        $mat = $ctrl->materializeBundleEntry($docente1Id, $entry);
...
// ── 4. Docente2: chiama PREVIEW (full manifest, no files = dry-run)
echo "\n[4] Docente2: PREVIEW dry-run…\n";
auth('docente.due', $docente2Id);
