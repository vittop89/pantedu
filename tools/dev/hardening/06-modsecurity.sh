...
NGINX_SITE="/etc/nginx/sites-enabled/pantedu.eu"
...
# ──────────────────────────────────────────────────────────────
C "=== [5/5] Smoke test ==="
HTTP_CODE=$(curl -sS -o /dev/null -w '%{http_code}' -m 10 https://pantedu.eu/ 2>&1 || echo "000")
G "  ✓ HTTPS app HTTP $HTTP_CODE (atteso 200/302)"

# Test ModSecurity attivo: SQL injection nell URL → check log
TEST_URL="https://pantedu.eu/?id=1%27%20OR%20%271%27=%271"
