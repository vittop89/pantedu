...
cat <<'EOF'
    ...
    location /grafana/ {
        # IP whitelist (modifica per i tuoi IP). Segnaposto dal 23/9/2026:
        # qui c'era un indirizzo di casa del manutentore, non più attuale
        # (A-44). Finché resta il segnaposto `nginx -t` rifiuta il blocco,
        # invece di ammettere un indirizzo che nessuno ha scelto.
        allow IP_DA_AMMETTERE;
...
echo "       sed -i 's|^;\\?root_url.*|root_url = https://pantedu.eu/grafana/|' /etc/grafana/grafana.ini"
