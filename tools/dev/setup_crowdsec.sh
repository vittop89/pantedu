...
if sudo cscli bouncers list 2>/dev/null | grep -q "${BOUNCER_NAME}"; then
    ...
    # 23/9/2026 — l'URL non si scrive più da qui. Era `http://127.0.0.1:8080`,
    # giusto quando PHP girava sull'host; dall'8/9/2026 gira nel container, dove
    # quella porta è il nginx dell'applicazione (docker/nginx.conf) e il bouncer
    # interrogava il sito invece della LAPI (A-15). L'URL deve essere uno che il
    # container raggiunge, e la LAPI deve ascoltare lì: è una scelta sull'host,
    # non un valore predefinito. Se è giusto lo dice la diagnostica, `--solo=crowdsec`.
    if ! grep -q "^CROWDSEC_LAPI_URL=" "${ENV_FILE}" 2>/dev/null; then
        R "  CROWDSEC_LAPI_URL non è in ${ENV_FILE}: senza, il bouncer resta spento."
        R "  Va messo a mano: il gateway del bridge Docker, valido nel container e sull'host"
        R "  (non 127.0.0.1, non host.docker.internal): docs/ops/diagnostica.md, «crowdsec»."
    fi
...
echo "  curl -H 'X-Forwarded-For: 1.2.3.4' https://pantedu.eu/  # → 403"
