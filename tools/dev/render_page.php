...
if (isset($opt['user'])) {
    $st = App\Core\Database::connection()->prepare(
        'SELECT id, username, role, is_super_admin FROM users WHERE username = ? LIMIT 1'
    );
    $st->execute([$opt['user']]);
    ...
    // Claims appena letti (Auth::CLAIMS_TTL_SECONDS): senza, AuthMiddleware li
    // rilegge, e con --role diverso dal database chiude la sessione come per
    // un ruolo cambiato (23/9/2026).
    Session::put('is_super_admin', (bool)$u['is_super_admin']);
    Session::put('active', true);
    Session::put('claims_at', \time());
