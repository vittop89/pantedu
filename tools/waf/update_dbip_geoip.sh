...
# ---------------------------------------------------------------------------
# Dove finiscono i database. Il predefinito segue `PANTEDU_DATA_PATH`, cioè
# l'alberatura dei dati, che è dove il WAF li legge davvero.
#
# Il predefinito precedente era `/var/www/pantedu/storage/geoip`, cioè dentro
# il repository. Sono due alberature diverse, e la differenza non è teorica: la
# copia installata sul VPS aveva ancora il nome vecchio del progetto
# (la cartella del nome vecchio del progetto), quindi ogni primo del mese
# scaricava i database
# in una cartella che nessuno legge — uscendo **zero**, senza un errore,
# lasciando due righe nel giornale che sembravano un aggiornamento riuscito.
# Il WAF ha filtrato per indirizzo geografico su dati fermi al 22 maggio fino
# al 9 settembre 2026: centodieci giorni.
GEOIP_DIR="${WAF_GEOIP_DIR:-${PANTEDU_DATA_PATH:-/var/www/pantedu}/storage/geoip}"
...
update_db() {
    ...
    for url in "${url_current}" "${url_previous}"; do
        ...
        if curl --fail --silent --show-error --location \
                --max-time 120 \
                --user-agent "pantedu-geoip-updater/1.0 (https://pantedu.eu)" \
                --output "${tmp}" \
                "${url}"; then
            fetched=1
            break
