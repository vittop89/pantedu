 ...
 *
 * USO
 *   php tools/contenuti/etichetta.php --teacher=docente.uno --ids=89 \
 *       --indirizzo=SCI --classe=1
 *   php tools/contenuti/etichetta.php --teacher=docente.uno \
...
$voce = static function (string $kind, string $code) use ($pdo, $uid): ?array {
    ...
    $st = $pdo->prepare(
        'SELECT c.id, c.label, COALESCE(i.name, i.code) AS istituto
           ...
           JOIN curriculum_teacher ct ON ct.curriculum_id = c.id AND ct.active = 1
          WHERE c.kind = ? AND c.code = ? AND ct.user_id = ? AND c.active = 1
          LIMIT 1'
    );
...
try {
    $campi = 0;
    // ADR-037, fase 4c-2: le etichette sono il posto della pubblicazione
    // principale, e «solo dove e' vuoto» si guarda li'. La riga cambia solo
    // updated_at.
    $tocca = $pdo->prepare('UPDATE teacher_content_data SET updated_at = NOW() WHERE id = ? AND teacher_id = ?');
    foreach ($ids as $id) {
        $ora = \App\Support\PostoPrincipale::dove($pdo, \App\Support\PostoPrincipale::CONTENUTO, $id);
        $nuovo = $ora;
        foreach ([['indirizzo', $ind], ['classe', $cls], ['materia', $mat]] as [$chiave, $v]) {
            if ($v !== null && $ora[$chiave] === null) {
                $nuovo[$chiave] = (int)$v['id'];
                $campi++;
            }
        }
        if ($nuovo !== $ora) {
            \App\Support\PostoPrincipale::contenuto($pdo, $id, $nuovo['indirizzo'], $nuovo['classe'], $nuovo['materia']);
            $tocca->execute([$id, $uid]);
        }
    }
    if ($sez !== null) {
        $upd = $pdo->prepare(
