#!/usr/bin/env python3
"""Smoke test: invia il TikZ schema modulare dell'utente al /render-tikz locale."""
import base64
import hashlib
import hmac
import json
...
# TikZ inviato dall'utente (post-decode entita HTML).
TIKZ = r"""
\usepackage{amssymb}
\usepackage{amsmath}
\usepackage{tikz}
...
try:
    with urllib.request.urlopen(req, timeout=60) as r:
        ...
        body = r.read()
        print(f"HTTP {r.status} | {ct} | {len(body)} bytes")
        print(f"X-Compile-Duration-Ms: {r.headers.get('X-Compile-Duration-Ms')}")
        print(f"X-Compile-Engine: {r.headers.get('X-Compile-Engine')}")
        if "image/svg" in ct:
            out = "smoke_schema_out.svg"
            with open(out, "wb") as f:
                f.write(body)
            print(f"OK SVG salvato in {out} ({len(body)} bytes)")
        else:
            print("--- BODY ---")
