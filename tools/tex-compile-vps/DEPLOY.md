...
> **Assetto di oggi (19/9/2026).** Questa procedura descrive il servizio su un
> VPS suo, con un dominio e nginx davanti: l'assetto fino al 24/5/2026. Oggi il
> servizio sta sulla stessa macchina dell'applicazione, che gira in un
> container Docker, e ascolta sul **gateway del bridge Docker** (non su
> 127.0.0.1, che dal container è il container stesso). Dove ascolta, la regola
> del firewall, l'endpoint dell'applicazione, i controlli e il ritorno
> indietro: `docs/ops/tex-dal-container.md` nel repository dell'applicazione.
> Il codice del servizio lo aggiorna il rilascio (`deploy-container.sh`, passo
> 7-bis).
