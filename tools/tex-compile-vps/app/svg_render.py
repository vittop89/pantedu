...
def _svg_to_pdf_svglib(svg_file: Path, pdf_file: Path) -> bytes | None:
    """Ripiego senza librsvg: svglib (parser SVG) + reportlab (PDF vettoriale).
    Ritorna None se i pacchetti non sono installati o il rendering fallisce."""
    try:
    ...
    except Exception as exc:  # noqa: BLE001 — qualunque errore = ripiego non riuscito
        logger.warning("svglib fallback failed: %s", exc)
        return None
...
async def svg_to_pdf(svg_source: str, *, timeout: int = DEFAULT_TIMEOUT, dpi: int = DEFAULT_DPI) -> SvgToPdfResult:
    ...
    with tempfile.TemporaryDirectory(dir=workdir, prefix="svg_") as tmp:
        ...
        try:
            ...
            # 2026-09-05 — sviluppo locale senza librsvg (Windows): ripiego in
            # puro Python con svglib + reportlab (app/requirements-local.txt).
            # In produzione rsvg-convert c'e' sempre e questo ramo non gira.
            pdf_bytes = await asyncio.to_thread(_svg_to_pdf_svglib, svg_file, pdf_file)
            duration_ms = int((loop.time() - started) * 1000)
            if pdf_bytes is None:
                raise SvgToPdfError(f"rsvg-convert non trovato e svglib non disponibile: {e}")
            logger.info("svg-to-pdf ok (svglib fallback) duration_ms=%d svg_bytes=%d pdf_bytes=%d",
