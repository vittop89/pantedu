...
# include anche usepackage/RequirePackage/LoadClass: i pacchetti con path
# (es. \usepackage{//verifica.sty}) leggono file dal disco esattamente
# come \input — il guard deve coprirli (fix FND-011 bypass via usepackage).
_INCLUDE_RE = re.compile(
    r"\\(?:input|include|InputIfFileExists|openin|import|subimport"
    r"|subfile|lstinputlisting|verbatiminput|includegraphics"
    r"|usepackage|RequirePackage|LoadClass)\b"
    r"\s*(?:\[[^\]]*\])?\s*\{?\s*([^}\s{][^}\s]*)"
)
...
def _rewrite_root_relative_includes(content: bytes, bundle_root: Path, file_rel: str) -> bytes:
    """Convenzione kpathsea: `\\input{/nome}` (o `//nome`) = cerca `nome`
    lungo i root TEXINPUTS, NON nel filesystem (stile openin_any=p).
    pdflatex gira con openin_any=a → risolverebbe /nome come path di
    filesystem ASSOLUTO. Riscrive i target root-relative in nome BARE
    (strip degli slash iniziali): la risoluzione dentro la sandbox è
    affidata alla TEXINPUTS ricorsiva impostata in compile_bundle.
    I target con `..` restano al guard FND-011 che li valida/rifiuta."""
    try:
    ...
    def _repl(m: "re.Match[str]") -> str:
        ...
        if not t.startswith("/"):
            return m.group(0)
        # Un solo slash O doppio slash: entrambi sono root-relative kpathsea.
        name = t.lstrip("/")
        if not name:
            return m.group(0)  # lascia al guard FND-011 di rifiutarlo
        return m.group(0).replace(t, name, 1)

    return re.sub(_INCLUDE_RE, _repl, text).encode("utf-8")
    ...
    *,
    ...
    with tempfile.TemporaryDirectory(dir=WORKDIR, prefix="bundle_") as tmp:
        ...
        for f in files:
            ...
            # Root-relative includes (/intestazione) → path relativi reali
            # PRIMA della scrittura: il guard FND-011 e pdflatex vedono il
            # contenuto riscritto (openin_any=a risolverebbe /x come assoluto).
            target.write_bytes(_rewrite_root_relative_includes(f.content, tmp_path, rel))
        ...
        # pdflatex risolve i path dalla cwd = main_dir. Si leggono i byte DA
        # DISCO: includono la riscrittura root-relative e latexindent.
        for _f in files:
            if _f.path.lower().endswith((".tex", ".sty", ".cls")):
                _assert_safe_tex_includes(
                    (tmp_path / _f.path.lstrip("/")).read_bytes(),
                    tmp_path,
                    main_dir,
                )
        ...
        # G22.16 — TEXINPUTS ricorsiva sulla sandbox: i target root-relative
        # riscritti in nome bare (es. /verifica.sty → verifica.sty) vengono
        # risolti da kpathsea dentro il bundle; "." preserva la risoluzione
        # cwd-relative dei `..` (main in versioni/ → ../texCommon/...).
        # Il `:` FINALE è obbligatorio: kpathsea espande l'elemento vuoto
        # col default di texmf.cnf. Senza, l'albero TeX Live sparisce dalla
        # ricerca e anche `article.cls` non viene trovato.
        compile_env = {
            **os.environ,
            "openin_any": "a",
            "openout_any": "p",
            "TEXINPUTS": f"{tmp_path}//:.:",
        }
