...
from .errori_tex import estratto, trova_errori
...
class TikzRenderResult:
    ...
    log: str
    duration_ms: int
    # Gli errori di pdflatex (vedi errori_tex.trova_errori): vuota se la
    # compilazione è pulita o se è fallita per un altro motivo.
...
async def render_tikz(
    ...
    *,
    ...
    with tempfile.TemporaryDirectory(dir=WORKDIR, prefix="tikz_") as tmp:
        ...
        # Un errore di pdflatex è un errore anche se il PDF esce: in
        # nonstopmode TeX rimedia come può e il disegno viene a metà, senza
        # che nessuno lo sappia (errori_tex.py, 24/9/2026).
        errori = trova_errori(last_log)
        pdf_path = tmp_path / "doc.pdf"
        if errori or not pdf_path.exists():
            return TikzRenderResult(
                ok=False, svg=None,
                log=estratto(last_log, errori) if errori else _truncate_log(last_log),
                duration_ms=int((loop.time() - started) * 1000),
                errors=errori,
            )
