...
Avvio prod (gestito da systemd, systemd/tex-compile.service):
  uvicorn app.main:app --host <gateway del bridge Docker> --port 8001 --workers 2

  Non 127.0.0.1 (19/9/2026): l'applicazione gira in un container, e dal
  container 127.0.0.1 è il container stesso. Il gateway del bridge
  (`docker network inspect bridge`) vale sia dall'host sia dal container; lo
  legge l'unità da TEX_COMPILE_HOST in /opt/tex-compile/.env. Perché, passi a
  mano e ritorno indietro: docs/ops/tex-dal-container.md.
"""
...
async def render_tikz_endpoint(payload: TikzRenderRequest) -> Response:
    ...
    if not result.ok:
        logger.warning(
            "tikz render failed doc_id=%s duration_ms=%d tex_errors=%d",
            payload.doc_id, result.duration_ms, len(result.errors),
        )
        return Response(
            content=json.dumps({
                "ok": False,
                "log": result.log,
                # Vuota se il fallimento non viene da pdflatex (dvisvgm,
                # tempo scaduto): il client distingue così un errore del
                # disegno, da mostrare, da un intoppo, da ritentare.
...
class CompileBundleRequest(BaseModel):
    files: list[BundleFilePayload] = Field(..., min_length=1, max_length=256,
        description="Lista file del bundle (max 256 per evitare abusi)")
...
def _json_error_payload(result: CompileResult, with_artifacts: bool) -> bytes:
    ...
    if with_artifacts:
        ...
        payload["errors"] = parse_latex_errors(result.log)
