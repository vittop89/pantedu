 ...
 *
 * COME
 *   Per ogni incarico spunta per il docente la voce indirizzo e la voce classe
 *   del vocabolario dell'istituto (curriculum_teacher, ADR-035), come farebbe
 *   l'attivazione normale. Non inventa niente:
 *   se la voce non c'e', quella riga resta scoperta e viene elencata a parte
...
// Cosa ha gia' spuntato ogni docente, per non riproporre spunte che ci sono.
// ADR-042 — un anno si cerca nel corso dell'incarico (`corso_anno`).
$possiede = static function (int $uid, int $iid, string $kind, string $code, string $corso) use ($pdo): bool {
    $q = $pdo->prepare(
        'SELECT 1 FROM curriculum_entries c
           JOIN curriculum_teacher ct ON ct.curriculum_id = c.id
          WHERE c.kind = ? AND c.code = ? AND c.institute_id = ? AND c.corso_anno = ? AND ct.user_id = ? LIMIT 1'
    );
    $q->execute([$kind, $code, $iid, $corso, $uid]);
    return (bool)$q->fetchColumn();
};
$ancora = static function (int $iid, string $kind, string $code, string $corso) use ($pdo): bool {
    $q = $pdo->prepare(
        'SELECT 1 FROM curriculum_entries
          WHERE kind = ? AND code = ? AND institute_id = ? AND corso_anno = ? LIMIT 1'
    );
    $q->execute([$kind, $code, $iid, $corso]);
    return (bool)$q->fetchColumn();
};
...
foreach ($incarichi as $r) {
    ...
    foreach ([['indirizzi', (string)$r['indirizzo']], ['classi', (string)$r['classe']]] as [$kind, $code]) {
        $corso = $kind === 'classi' && \App\Domain\ClassCode::isAnno($code) ? strtoupper((string)$r['indirizzo']) : '';
        if ($possiede($uid, $iid, $kind, $code, $corso)) {
            continue;
        }
        if (!$ancora($iid, $kind, $code, $corso)) {
            $scoperti[] = sprintf('%s @ %s — %s %s non e\' in catalogo', $r['username'], $r['istituto'], $kind, $code);
            continue;
        }
        $daFare[] = ['uid' => $uid, 'iid' => $iid, 'kind' => $kind, 'code' => $code, 'indirizzo' => (string)$r['indirizzo'],
...
$fatte = 0;
foreach ($daFare as $d) {
    if (CurriculumLookup::ensureEntryForTeacher($d['kind'], $d['uid'], $d['code'], $d['iid'], $d['indirizzo']) !== null) {
        $fatte++;
    }
}
