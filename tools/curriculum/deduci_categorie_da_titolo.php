 ...
 *
 * USO
 *   php tools/curriculum/deduci_categorie_da_titolo.php --teacher=docente.uno
 *   php tools/curriculum/deduci_categorie_da_titolo.php --teacher=docente.uno --apply
 */
...
try {
    ...
    // principale. «Solo se vuoto», guardato li', e' cio' che rende sicuro
    // rilanciarlo; la riga cambia solo updated_at.
    ...
    foreach ($daScrivere as $p) {
        ...
        $nuovo = $ora;
        foreach (['indirizzo' => 'indirizzo_id', 'classe' => 'classe_id', 'materia' => 'subject_id'] as $chiave => $colonna) {
            $valore = $p['g'][$colonna];
            if ($valore !== null && $ora[$chiave] === null) {
                $nuovo[$chiave] = (int)$valore;
                $campi++;
            }
        }
        if ($nuovo !== $ora) {
