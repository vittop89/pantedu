...
#   2. git fetch + reset --hard origin/main (idempotente, pulisce drift)
...
#   9. verifica dopo il rilascio: quattro rotte pubbliche devono rispondere
#      (tools/ops/smoke_after_deploy.php, Kernel in-process, niente HTTP)

set -euo pipefail

# 2026-09-02 — esito complessivo del deploy: i passi che falliscono senza
# poter abortire (migrations) lo marcano qui, e si esce non-zero alla fine.
DEPLOY_FAILED=0
DEPLOY_FAILURES=()
...
# shellcheck disable=SC2034  # usata dentro il `trap` qui sotto, che e' fra
...
# NB3: include docs/ — PHP ci legge dentro sul serio: i markdown legali
# (TrustPagesController) e docs/curriculum/miur_alias.json (import adozioni).
# Finora docs/ stava solo nella rete di sicurezza §3.ter, che era rotta: due
# incidenti diversi, stessa causa. Meglio coprirla anche qui.
...
#
# 2026-09-02 — questa rete era rotta da quando e' stata scritta, e in silenzio.
# `git ls-files` girava come root su un repo di $GIT_USER: git rifiuta
# ("detected dubious ownership"), l'errore finiva in /dev/null, la pipeline
# usciva vuota e `xargs -r` non eseguiva niente. Era l'unica invocazione di git
# nello script a non passare da git_as_owner. Sintomo: docs/curriculum/
# miur_alias.json non leggibile da www-data → l'import delle adozioni si
# fermava su "registro alias non leggibile".
#
# Ora passa dal proprietario come tutte le altre, e se l'elenco esce vuoto lo
# DICE invece di proseguire come se avesse lavorato: una rete di sicurezza che
# fallisce di nascosto e' peggio che non averla, perche' fa smettere di cercare.
TRACKED_COUNT=$(git_as_owner ls-files | wc -l)
if [[ "$TRACKED_COUNT" -eq 0 ]]; then
    warn "safety-net permessi: 'git ls-files' non ha restituito nulla — i file"
    warn "  tracciati NON sono stati resi leggibili. Controlla proprietario del"
    warn "  repo e safe.directory. A valle: 500/Permission denied a runtime."
else
    git_as_owner ls-files -z | xargs -0 -r chmod a+r 2>/dev/null || true
    log "safety-net permessi: a+r su $TRACKED_COUNT file tracciati"
fi
...
if [[ -r "$MYSQL_DEFAULTS_FILE" ]]; then
    if mysqldump --defaults-file="$MYSQL_DEFAULTS_FILE" --single-transaction --quick --routines --triggers --events pantedu 2>/dev/null | gzip > "$SNAP_FILE"; then
        ...
        # 2026-09-07 — non piu' solo un warning: senza questa copia, se una
        # migrazione distruttiva va male non c'e' modo di tornare indietro
        # (il codice si, lo schema no). Il deploy prosegue — fermarsi qui
        # lascerebbe il sito peggio di com'e' — ma l'unita' systemd risulta
        # failed, che e' un segnale che si vede.
        warn "pre-deploy DB snapshot FALLITO (auth?): questo deploy non ha rete di sicurezza sul database."
        rm -f "$SNAP_FILE"
        DEPLOY_FAILED=1
        DEPLOY_FAILURES+=("db-snapshot")
    fi
else
    warn "pre-deploy DB snapshot SALTATO: $MYSQL_DEFAULTS_FILE non leggibile: nessuna rete di sicurezza sul database (resta lo snapshot Hetzner, che e' piu' grosso e piu' lento)."
    rm -f "$SNAP_FILE"
    DEPLOY_FAILED=1
    DEPLOY_FAILURES+=("db-snapshot")
fi
...
# migrations_log).
#
# 2026-09-02 — il fallimento non e' piu' silenzioso. Prima produceva un
# `warn` nel log e il deploy usciva comunque 0: nessuno se ne accorgeva, e
# lo schema restava indietro rispetto al codice appena messo in produzione.
# Ora viene marcato e il deploy termina con exit 1 — l'unita' systemd
# risulta `failed`, che e' un segnale che si vede.
#
# Perche' non abortire subito: il reset --hard e' gia' avvenuto, quindi il
# codice nuovo e' sul disco. Fermarsi qui lascerebbe il sito senza asset
# ricostruiti e senza permessi riallineati, cioe' peggio. Si completa il
# deploy e si dichiara il fallimento alla fine.
...
if [[ -f "$REPO_DIR/tools/migrate.php" ]]; then
    ...
    if sudo -u "$GIT_USER" php "$REPO_DIR/tools/migrate.php" 2>&1 | sed 's/^/[migrate] /'; then
        ...
        DEPLOY_FAILED=1
        DEPLOY_FAILURES+=("migrations")
    fi
fi

# 2026-09-01 — registro versioni ToS/AUP allineato a docs/legal/versions.json.
# Il preavviso e la data di entrata in vigore vivono nella tabella
# legal_document_versions, non nel markdown: senza questo passo il documento
# pubblicato annuncia "in vigore dal <data>" mentre il DB non sa nemmeno che
# quella versione esiste, il banner di preavviso non compare e la versione non
# diventa mai vincolante. Idempotente (confronta e aggiorna solo cio' che
# diverge), come migrate.php. Non-fatale.
if [[ -f "$REPO_DIR/tools/legal/sync_versions.php" ]]; then
    log "run php tools/legal/sync_versions.php --apply (idempotente)"
    if sudo -u "$GIT_USER" php "$REPO_DIR/tools/legal/sync_versions.php" --apply 2>&1 | sed 's/^/[legal] /'; then
        log "registro versioni legali allineato"
    else
        warn "sync_versions.php exit non-zero — verifica con tools/legal/tos_gate_preview.php"
    fi
fi
...
if [[ -f "$REPO_DIR/package.json" ]] && command -v npm >/dev/null 2>&1; then
    if grep -q '"build"' "$REPO_DIR/package.json"; then
        ...
        if cd "$REPO_DIR" && sudo -u "$GIT_USER" npm ci --no-audit --no-fund --prefer-offline 2>&1 | tail -5 | sed 's/^/[npm-ci] /'; then
            ...
            # 2026-08-31 — backup del bundle precedente PRIMA del build.
            # vite.config.js ha emptyOutDir=true: la cartella viene svuotata
            # all'inizio, quindi un build che fallisce non lascia il bundle
            # vecchio, lo DISTRUGGE. E il fallback documentato sotto
            # ("bootstrap.dist.js raw") copre solo i template PHP, che leggono
            # il manifest da disco via ViteManifest: i modal lazy-loaded lo
            # chiedono via HTTP e senza fallback muoiono con 404
            # ("Caricamento bundle fallito"). Colpiti anteprima verifiche,
            # editor PT risdoc, template TikZ, GeoGebra, dialog TeX.
            BUILD_BAK=""
            if [[ -f "$REPO_DIR/public/build/manifest.json" ]]; then
                BUILD_BAK=$(mktemp -d)
                cp -a "$REPO_DIR/public/build/." "$BUILD_BAK/" 2>/dev/null || BUILD_BAK=""
            fi
            # Il manifest DEVE esistere a fine build: l'exit code da solo non
            # basta come prova di successo.
            if sudo -u "$GIT_USER" npm run build 2>&1 | tail -20 | sed 's/^/[vite] /' \
               && [[ -f "$REPO_DIR/public/build/manifest.json" ]]; then
                [[ -n "$BUILD_BAK" ]] && rm -rf "$BUILD_BAK"
                chgrp -R www-data "$REPO_DIR/public/build" 2>/dev/null || true
                # Il prebuild (tools/build/vendor-assets.mjs) ricrea public/vendor/{mathjax,@mathjax}
                # col gruppo dell'utente di deploy: senza www-data nginx risponde 404 (2026-09-04).
                chgrp -R www-data "$REPO_DIR/public/vendor" 2>/dev/null || true
                log "vite build OK"
                ...
                if [[ -n "$BUILD_BAK" ]]; then
                    rm -rf "$REPO_DIR/public/build"
                    mkdir -p "$REPO_DIR/public/build"
                    cp -a "$BUILD_BAK/." "$REPO_DIR/public/build/" 2>/dev/null || true
                    chown -R "$GIT_USER":www-data "$REPO_DIR/public/build" 2>/dev/null || true
                    rm -rf "$BUILD_BAK"
                    warn "vite build FALLITO — ripristinato il bundle precedente (i modal restano funzionanti, ma servono asset VECCHI: ricostruisci appena possibile)"
                else
                    warn "vite build FALLITO e nessun bundle precedente da ripristinare — i modal lazy-loaded (anteprima verifiche, editor PT risdoc, TikZ, GeoGebra) risponderanno 'Caricamento bundle fallito'"
                fi
            fi
        else
...
if [[ -f "$REPO_DIR/tools/install-drawio.sh" ]]; then
    ...
    # shellcheck disable=SC2024  # la redirezione avviene come root, ed e' voluta:
    # il file sta in /tmp e lo rilegge root due righe piu' sotto.
...
ensure_geoip_db() {
    ...
    if [[ -f "$dest" ]]; then
        ...
        # Dichiarazione e assegnazione separate: con `local x=$(...)` il
        # codice d'uscita del comando viene mangiato da `local`.
        local ym ym_prev
        ym=$(date -u +%Y-%m)
...
SYSTEMD_NEW_COUNT=0
# 2026-08-31 — aggiunto *.path al glob. Prima le .path unit erano allineate
# SOLO dallo step 7b, che e' diff-gated su OLD_HEAD..NEW_HEAD: se un deploy
# passava oltre il commit che le tocca (o il file installato divergeva a mano),
# restavano disallineate per sempre. E' lo stesso difetto gia' corretto qui il
# 2026-05-24 passando da "diff-gated" a "always-idempotent" — vedi commento
# sopra. Caso reale: /etc/systemd/system/pantedu-deploy.path conteneva DUE
# volte `After=multi-user.target` (assente nel repo), causando un ciclo di
# dipendenze che ha tenuto il sito giu' 3 giorni e 19 ore dal 27 agosto.
for unit in "$REPO_DIR/$APP_SYSTEMD_SRC_REL"/*.service "$REPO_DIR/$APP_SYSTEMD_SRC_REL"/*.timer "$REPO_DIR/$APP_SYSTEMD_SRC_REL"/*.path; do
    [[ -f "$unit" ]] || continue
...
# 2026-09-08 — aggiunti gli script della finestra oraria all'elenco: senza,
# cambiarli non faceva ripartire l'installatore e /usr/local/bin restava
# indietro. E' successo il giorno stesso: il timer del rilascio differito ha
# trovato uno script che non c'era ancora.
AUTO_DEPLOY_CHANGES=$(git_as_owner diff --name-only "$OLD_HEAD" "$NEW_HEAD" | grep -E "^(tools/systemd/(pantedu-deploy\.(path|service)|pantedu-deploy-trigger\.sh|pantedu-deploy-differito\.sh|pantedu-finestra-rilascio\.sh|php8\.4-fpm\.service\.d/pantedu-auto-deploy\.conf)|tools/webhook/install_auto_deploy\.sh)$" || true)
...
# ── Step 9: il sito risponde? ──────────────────────────────────────────────
# 2026-09-07 — fino a oggi un deploy poteva finire "OK" con il sito che
# rispondeva 500: nessuno lo guardava fino alla prima visita. Qui si
# dispaccia il Kernel dentro un processo PHP — niente HTTP, quindi niente
# 403 del WAF, che ai client automatici risponde cosi' — e si chiede a quattro
# rotte pubbliche di rispondere. Verifica che il sito stia in piedi, non che
# funzioni: quello lo dice la suite.
if [[ -f "$REPO_DIR/tools/ops/smoke_after_deploy.php" ]]; then
    log "verifica dopo il rilascio (tools/ops/smoke_after_deploy.php)"
    if sudo -u "$GIT_USER" php "$REPO_DIR/tools/ops/smoke_after_deploy.php" 2>&1 | sed 's/^/[smoke] /'; then
        log "  il sito risponde"
    else
        warn "il sito NON risponde come dovrebbe: guarda le righe [smoke] qui sopra."
        warn "  per tornare indietro: git -C $REPO_DIR reset --hard $OLD_HEAD && systemctl restart $PHP_FPM_SERVICE"
        DEPLOY_FAILED=1
        DEPLOY_FAILURES+=("smoke")
    fi
fi

# 2026-09-09 — la verifica qui sopra dice che il sito sta in piedi. Non dice
# se sta in piedi **nello stato che dichiara**: se il commit servito e' quello
# giusto, se i file GeoIP configurati esistono davvero, se www-data legge
# ancora `.env` dopo il `git reset --hard`, se le migrazioni sul disco sono
# tutte applicate. Sono le cose che quel giorno erano rotte mentre il sito
# rispondeva 200.
#
# Gira come **www-data**, non come $GIT_USER: e' la lezione dei due disservizi
# dell'8 settembre. Un controllo dei permessi fatto da un utente che quei
# permessi ce li ha comunque non controlla niente.
#
# Non fa fallire il rilascio: il codice e' gia' in produzione e tornare
# indietro per un database GeoIP scaduto sarebbe sproporzionato. Lascia un
# avviso nel registro del rilascio e una riga nel registro delle anomalie, che
# il timer della diagnostica ritrovera' e trasformera' in una mail.
if [[ -f "$REPO_DIR/tools/ops/diagnostica.php" ]]; then
    log "diagnostica degli invarianti (come www-data)"
    if sudo -u www-data php "$REPO_DIR/tools/ops/diagnostica.php" 2>&1 | sed 's/^/[diag] /'; then
        log "  gli invarianti reggono"
    else
        warn "un invariante non regge: guarda le righe [diag] qui sopra."
        warn "  il sito e' su e risponde, ma qualcosa non e' nello stato che dichiara."
    fi
fi

if [[ "${DEPLOY_FAILED:-0}" -eq 1 ]]; then
    log "=== deploy end (deployed $NEW_HEAD) — CON ERRORI: ${DEPLOY_FAILURES[*]} ==="
    warn "Passi falliti: ${DEPLOY_FAILURES[*]}. Il codice e' in produzione ma qualcosa non e' allineato."
    exit 1
fi
