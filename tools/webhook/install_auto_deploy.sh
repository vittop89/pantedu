...
for unit in pantedu-deploy.path pantedu-deploy.service pantedu-avviso@.service; do
...
# 2026-09-08 — gli script della finestra oraria stanno accanto al trigger:
# quello li carica con `source`, quindi devono trovarsi in /usr/local/bin/.
echo "==> 2a-bis. Install script della finestra oraria"
for s in pantedu-finestra-rilascio.sh pantedu-deploy-differito.sh; do
    SRC="$REPO_DIR/tools/systemd/$s"
    DST="/usr/local/bin/$s"
    if [ -f "$SRC" ]; then
        install -m 755 -o root -g root "$SRC" "$DST"
        echo "    OK: $s installato"
    fi
done
