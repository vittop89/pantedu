...
> [!warning] Pagina in parte superata (nota del 23/9/2026)
> Dall'8/9/2026 il rilascio è a container (`deploy-container.sh`, scambio
> blu/verde), non più il `reset --hard` in-place di `deploy.sh` che i disegni
> qui sotto descrivono. Il rilascio **non** installa più unità systemd né
> `install_auto_deploy.sh`: si reinstallano a mano. Che cosa fa il rilascio, e
> che cosa resta a mano: [ci-cd](../../docs/dev/ci-cd.md), in particolare
> «Che cosa il rilascio non installa». Il webhook e il trigger descritti qui
> sono ancora quelli in servizio.

**Aggiornato 2026-09-04**: auto-deploy ATTIVO via systemd Path unit.
Push su `main` → deploy automatico entro ~15s. Nessun comando manuale richiesto.
(Il ramo si chiamava `master_vps` fino a giugno 2026: `github.php` filtra
`refs/heads/main` e `deploy.sh` fa `reset --hard origin/main`; i disegni
sotto usano ancora il vecchio nome.)
...
(Lo eseguiva `deploy.sh` quando cambiavano `tools/systemd/*` o `install_auto_deploy.sh`; il rilascio a container non lo fa: si lancia a mano, vedi [ci-cd](../../docs/dev/ci-cd.md), «Che cosa il rilascio non installa».)
...
CF_API_TOKEN=<il token dal pannello Cloudflare>
CF_ZONE_ID=<lo Zone ID della zona pantedu.eu, dal pannello Cloudflare>
EOF
...
- **Phase 25.R.22** (2026-05-23): hetzner_snapshot.sh path fix `/etc/progetto-precedente/` → `/etc/pantedu/` + commit script in repo + auto-install via deploy.sh.
