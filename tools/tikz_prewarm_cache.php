...
$opts = [
    ...
    'limit'   => null,
    'delay'   => 3,
    // 21/9/2026 — di default si scaldano TUTTE E DUE le dispense.
    //
    // Il giro notturno girava senza `--scope`, quindi sul valore di prima
    // («teacher»), e riempiva la cache cifrata del docente: 464 figure. Le
    // pagine di studio, pero', chiedono le figure in ambito «public» (nessuno
    // scrive `data-tikz-scope="teacher"`), e li ce n'erano 19 — quelle che il
    // docente aveva compilato navigando. Chi entra con la credenziale di
    // classe non puo' compilare a vista: vedeva «[TikZ render error]» su tutto
    // il resto. Un lavoro riuscito ogni notte che riempiva la dispensa
    // sbagliata.
    'scope'   => 'public,teacher',
];
foreach ($argv as $a) {
    if (preg_match('/^--(\w+)=(.+)$/', $a, $m)) {
...
$scopes    = array_values(array_filter(array_map('trim', explode(',', (string)$opts['scope']))));
foreach ($scopes as $s_) {
    if (!in_array($s_, ['public', 'teacher'], true)) {
        fwrite(STDERR, "scope sconosciuto: $s_ (ammessi: public, teacher)
");
        exit(1);
    }
}
            ...
            foreach ($g['items'] as $it) {
                foreach (['question','options','solution','justification'] as $k) {
                    ...
                    foreach ($it[$k] as $b) {
                        ...
                        // Una dispensa per volta: la stessa figura va scaldata
                        // sia dove la cerca chi studia sia dove la cerca il
                        // docente, e le due cache hanno chiavi diverse.
