 ...
 *     dove esito ∈ {OK, NO_DRAWIO, MULTIPLE_MATCH, ENCRYPT_FAIL, SKIP, LOSS_SIGNATURE}
 *
 *   LOSS_SIGNATURE (dal 19/9/2026): il sorgente ha le lettere accentate
 *   diventate «??» (RipristinoCaratteri::firmaDiPerdita) e non si ricifra.
 */
...
// ── Process rows ───────────────────────────────────────────
$stats = ['ok' => 0, 'no_drawio' => 0, 'multiple' => 0, 'encrypt_fail' => 0, 'skip_orphan' => 0, 'firma_di_perdita' => 0];
$reportLines = [];
...
foreach ($rows as $r) {
    ...
    // 2026-09-19 — i drawio legacy di storage/objects sono in buona parte le
    // copie rovinate dal vecchio sito nel 2025: le lettere accentate sono
    // diventate «??». Ricifrarli rimetterebbe nelle mappe il testo rotto, dopo
    // averlo corretto (wiki/domains/mappe/mappe-overview.md, «Caratteri persi»).
    // Un sorgente con la firma della perdita si rifiuta, anche a secco: il
    // testo buono sta negli originali del Drive del docente.
    if (RipristinoCaratteri::firmaDiPerdita($plaintext)) {
        $stats['firma_di_perdita']++;
        $reportLines[] = sprintf("id=%-4d | LOSS_SIGNATURE | %s | %s: «??» e nessun byte non ASCII, rifiutato",
            $id, $title, basename($drawioPath));
        continue;
    }
