 ...
 * progetto-precedente-teacher-kek-v1 -> pantedu-teacher-kek-v1.
 *
 ...
 *
 * Out-of-scope (kept as legacy progetto-precedente-* due to user-file dependency):
 *   - TeacherRecoveryService::HKDF_INFO ('progetto-precedente-recovery-key-v1')
 *     = HMAC label for user-downloaded recovery manifests
 *   - AdminCryptoStatusController 'progetto-precedente-export-signing-v1'
...
// 23/9/2026 (A-36) — la chiave si legge e si giudica in ChiaveMadre, come
// nell'app e negli altri strumenti di tools/crypto. Qui c'era una regola
// propria, con `$`, che lasciava passare un a capo finale. Si carica la sola
// classe: lo strumento non passa dal bootstrap né dall'autoloader.
require_once dirname(__DIR__, 2) . '/app/Services/Crypto/ChiaveMadre.php';
$chiave = \App\Services\Crypto\ChiaveMadre::dallAmbiente();
if (!$chiave->valida()) {
    fwrite(STDERR, 'ERROR: KMS_MASTER_KEY ' . $chiave->stato() . " (expected 64 hex chars)\n");
    exit(2);
}
$kms = $chiave->byte();
...
foreach ($rows as $row) {
    ...
    $tkek_old = hash_hkdf('sha256', $kms, 32, (string)$kv, "progetto-precedente-teacher-kek-v1|$tid");
