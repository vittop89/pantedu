...
.NOTES
    Prerequisiti sulla postazione di sviluppo:
      - PowerShell 5.1+ o pwsh 7+
      - rclone con un remote Backblaze B2 configurato (vedi $B2Remote nel codice)
      - SSH via Cloudflare Tunnel: alias 'pantedu-tunnel' in ~/.ssh/config
        NB: sshd ascolta solo su 127.0.0.1:2222, quindi il vecchio alias
        'pantedu-vps' (porta 2222 diretta) non funziona piu'.
        Lo ssh di Windows non vede l'agent di Git Bash: la passphrase verra'
        chiesta a ogni connessione. Per evitarlo prepara l'accesso con
        'pantedu up' e lancia questo script da Git Bash.
      - Repo pantedu clonato (per leggere git HEAD): la copia in WSL, lanciando
        lo script da \\wsl.localhost\Ubuntu\home\<utente>\pantedu\tools\admin\
...
try {
    # La copia su Windows e' in dismissione dal 22/9/2026: eliminazione decisa,
    # la fa l'utente. Lo script si lancia dalla
    # cartella WSL (\\wsl.localhost\...), dove il git di Windows si ferma per
    # «dubious ownership» e qui si finiva nel catch con HEAD «(unknown)»
    # (misurato: esito 128). L'eccezione vale per questi due comandi di sola
    # lettura, con -c: niente safe.directory nella configurazione globale, che
    # farebbe vedere ~150 cambi di modo fasulli a ogni altro comando
    # (docs/dev/sviluppo-in-wsl.md, «Che cosa si fa ancora da Windows»).
    $gitHead   = (git -c 'safe.directory=*' rev-parse HEAD 2>$null).Trim()
    $gitBranch = (git -c 'safe.directory=*' rev-parse --abbrev-ref HEAD 2>$null).Trim()
...
# ============================================================
# Il nome del remote e' cambiato col rebranding (b2-progetto-precedente -> b2-pantedu)
# e non e' detto che la macchina corrente abbia quello giusto configurato.
# Verifichiamo prima: fallire subito con un messaggio chiaro e' meglio che
# scoprirlo a meta' sync con un "didn't find section in config file".

# ── I passi che non riescono ──────────────────────────────────────────────
#
# Finiscono in questa lista, e da qui nel manifest e nel riepilogo finale.
#
# Prima ognuno stampava il suo avviso giallo e basta. Il riepilogo usciva
# comunque «Backup completato» in verde, con le istruzioni per registrarlo su
# /admin/backup identiche a quelle di un giro riuscito. Un cold backup a cui
# manca meta' roba, registrato come completo, e' peggio di un cold backup non
# fatto: nel secondo caso lo sai.
#
# Cinque punti possono degradare senza fermare lo script — remote non
# configurato, sync da B2, .env.local, tar di /etc/pantedu, SSH — e adesso
# tutti e cinque scrivono qui.
$problemi = @()

$B2Remote = 'b2-pantedu'
# 2026-09-09 — migrato. Il bucket aveva ancora il nome del brand precedente,
# e su B2 i nomi sono unici a livello globale e non rinominabili: si e' creato
# `pantedu-backup-vps` e copiati i 28 oggetti (259 MiB), verificati con
# `rclone check` — 0 differenze.
#
# Attenzione se leggi questo dopo aver clonato su una macchina nuova: la
# chiave d'applicazione e' ristretta a **un solo secchio**. Il remote
# `b2-pantedu` di questa macchina va rifatto con la chiave nuova, altrimenti
# qui sotto arriva un 401 e il download non avviene.
$B2Bucket = 'pantedu-backup-vps'
$B2Source = "${B2Remote}:${B2Bucket}/"

Write-Step "Download backup da Backblaze B2 (può richiedere 5-20 min)"
Write-Host "    Source: $B2Source"
Write-Host "    Target: $b2Dir"
Write-Host ""

$remotes = @()
try { $remotes = @(& rclone listremotes 2>$null) } catch { }
if ($remotes -notcontains "${B2Remote}:") {
    Write-Warn "remote rclone '$B2Remote' non configurato su questa macchina"
    if ($remotes.Count -gt 0) {
        Write-Host "    remote disponibili: $($remotes -join ' ')"
    } else {
        Write-Host "    nessun remote configurato (rclone installato?)"
    }
    Write-Host "    configuralo con: rclone config   (application key nel password manager)"
    $problemi += "remote rclone '$B2Remote' non configurato su questa macchina"
}
...
if ($rcloneOk) {
    ...
    $problemi += "download da B2 ($B2Source) fallito o incompleto"
}
...
# Lo ssh di Windows non vede l'agent di Git Bash: socket Unix contro named
# pipe. Chiederebbe la passphrase a ogni connessione, due volte per run.
# Quello di Git la usa, purche' SSH_AUTH_SOCK punti al socket. Verificato
# funzionante da PowerShell il 2026-08-31.
# Prepara l'accesso con `pantedu up` PRIMA di lanciare questo script: qui
# non c'e' modo di digitare la passphrase.
$SshExe = 'ssh'
$gitSsh = Join-Path $env:ProgramFiles 'Git\usr\bin\ssh.exe'
if (Test-Path $gitSsh) {
    $SshExe = $gitSsh
    $env:SSH_AUTH_SOCK = Join-Path $env:USERPROFILE '.ssh\agent\sock'
    Write-Ok "ssh di Git Bash + agent"
} else {
    Write-Warn "ssh di Git non trovato: uso quello di Windows, chiedera' la passphrase"
}
...
try {
    ...
    & $SshExe pantedu-tunnel 'cat /var/www/pantedu/.env.local' > $envLocalFile 2>$null
    if ($LASTEXITCODE -ne 0 -or -not (Test-Path $envLocalFile) -or (Get-Item $envLocalFile).Length -eq 0) {
        ...
        $problemi += ".env.local del VPS non scaricato (o vuoto)"
        $sshOk = $false
    } else {
    ...
    # Tar di /etc/pantedu/
    & $SshExe pantedu-tunnel 'sudo tar czf - -C /etc pantedu 2>/dev/null' > $etcTarFile 2>$null
...
# I passi non riusciti finiscono NEL manifest, non solo a schermo.
#
# Il riepilogo a video lo leggi una volta, mentre l'HDD lo apri fra sei mesi,
# staccato dalla rete, magari non tu. Il manifest e' l'unica cosa che viaggia
# con la copia: se manca qualcosa deve dirlo lui.
$manifest += ""
$manifest += "## Esito dei passi"
if ($problemi.Count -eq 0) {
    $manifest += "Tutti i passi riusciti."
} else {
    $manifest += "ATTENZIONE — questa copia e' INCOMPLETA. Passi non riusciti:"
    foreach ($p in $problemi) { $manifest += "  - $p" }
}
...
if ($problemi.Count -gt 0) {
    Write-Host "  QUESTA COPIA E' INCOMPLETA — $($problemi.Count) passo/i non riusciti:" -ForegroundColor Red
    foreach ($p in $problemi) { Write-Host "    - $p" -ForegroundColor Red }
    Write-Host ""
    Write-Host "  NON registrarla su /admin/backup come completata." -ForegroundColor Red
    Write-Host "  Risolvi e rilancia: una copia parziale registrata come buona" -ForegroundColor Red
    Write-Host "  e' peggio di nessuna copia, perche' il registro dice che c'e'." -ForegroundColor Red
    Write-Host ""
    Write-Host "  (i passi non riusciti sono scritti anche nel manifest)" -ForegroundColor Yellow
    Write-Host ""
    exit 1
}
...
Write-Host "    3. Vai su https://pantedu.eu/admin/backup"
