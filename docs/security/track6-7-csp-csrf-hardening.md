...
- `stampScriptNonce()`: aggiungeva `nonce="…"` a ogni `<script>` delle risposte
  HTML quando strict è attivo (relaxed non muta il body → zero overhead).
  **Tolto il 23/9/2026** (revisione architetturale A-16): timbrava anche gli
  script arrivati dal contenuto. Il nonce ora è di `App\Support\Csp` e lo
  scrive chi scrive lo script; la regola corrente sta in
  `wiki/security-notes.md`, «Security header e CSP».
...
- `docente.uno` ha ruolo admin → utile per E2E delle pagine admin.
