...
**Il job CI di questo scanner è sospeso dal 2026-09-07** (dettagli sotto,
«CI integration»): il gate reale dell'accessibilità in
[`.github/workflows/a11y.yml`](../../../.github/workflows/a11y.yml) è il job
`axe-baseline` (`npm run e2e:a11y`, axe-core su Playwright), che gira ed è
verde. L'installazione locale descritta qui resta comunque utile per gli
agenti Claude Code e il server MCP durante lo sviluppo.
...
```powershell
cd C:/percorso/locale/pantedu
git worktree add C:/percorso/locale\sandbox\pantedu-a11y-test main
cd C:/percorso/locale\sandbox\pantedu-a11y-test
...
## CI integration (sospesa dal 2026-09-07)

Il job `agents-scan`, che avrebbe fatto girare
`accessibility-agents/action@v5.4.0` sulle pull request e confrontato il
SARIF con `a11y-baseline.sarif`, non ha mai funzionato: muore in «Set up job»
perché il pacchetto a monte contiene symlink che GitHub Actions non riesce a
materializzare (stesso esito su v5.4.0 e v6.0.0, non è una configurazione
nostra da correggere). È commentato in
[`.github/workflows/a11y.yml`](../../../.github/workflows/a11y.yml) dal
2026-09-07 e non gira. `a11y-baseline.sarif` resta nel repository per quando
l'azione tornerà utilizzabile a monte. Registrato come voce 74 del
[registro del debito tecnico](../../../wiki/technical-debt.md).

Il gate reale dell'accessibilità in CI è il job `axe-baseline`
(`npm run e2e:a11y`, axe-core su Playwright): gira su ogni pull request e non
richiede questa installazione locale.
