...
Totale: **182** phase distinte, **1067** occorrenze in `app/` + `js/` + `views/`.
...
| **2** | transazione DB atomica. | 4 | `app/Services/BlockList.php`, `app/Services/Verifica/VerificaDocumentService.php`, `app/Support/ViteManifest.php` |
...
| **9** | Admin Files API per editor template verifiche. | 29 | `app/Controllers/Admin/VerificaFilesAdminController.php`, `app/Services/TikzElementsService.php`, `js/modules/bootstrap.js` +25 |
| **12** | Check endpoints (check_password). | 4 | `app/Controllers/CheckController.php`, `app/Controllers/VerificheController.php`, `app/Services/PrintInfoService.php` |
| **13.5** | studente seleziona indirizzo + classe per stats docenti | 5 | `app/Controllers/AdminAnalyticsController.php`, `app/Controllers/RegistrationController.php`, `app/Services/AdminAnalyticsService.php` +1 |
| **13** | il pulsante ATTIVA viene iniettato dentro OGNI .sidepage | 22 | `app/Controllers/AdminController.php`, `app/Controllers/AdminToolsController.php`, `app/Controllers/ContentStudyController.php` +17 |
...
| **15** | passa teacher_id + institute_id al renderer per source registry lookup | 23 | `app/Controllers/ContentExportController.php`, `app/Controllers/ContentStudyController.php`, `app/Controllers/ExerciseStudyController.php` +13 |
| **16** | gestione buttons a LIVELLO .fm-collapsible (.fm-checkmod .fm-edit-eser): | 82 | `app/Controllers/ContentExportController.php`, `app/Controllers/ContentStudyController.php`, `app/Controllers/HomeController.php` +24 |
| **17** | storage put con retry esponenziale (3 tentativi: 0ms, 50ms, 200ms). | 34 | `app/Controllers/GroupController.php`, `app/Controllers/QuesitoController.php`, `app/Controllers/TeacherContentController.php` +16 |
| **18** | formato 'map': NO exercise-context (evita padding-top:95px | 55 | `app/Config/auth.php`, `app/Config/filesystem.php`, `app/Controllers/ContentExportController.php` +25 |
| **19** | probe endpoint no-op per health check + test e2e CSRF/rate. | 19 | `app/Controllers/ContentExportController.php`, `app/Controllers/ContentStudyController.php`, `app/Controllers/CsrfProbeController.php` +15 |
| **20** | href con ?ids=<id> (server carica solo quel content). class=linkref | 54 | `app/Controllers/ContentStudyController.php`, `app/Controllers/ContentTemplateController.php`, `app/Controllers/GroupController.php` +24 |
| **21** | `fm-has-upbar` identifica route CON upbar (esercizio/verifica), | 18 | `app/Controllers/Admin/RisdocAdminController.php`, `app/Controllers/ContentStudyController.php`, `app/Controllers/Risdoc/ExportController.php` +13 |
...
| **23.4** | — | 3 | `js/components/risdoc/_pt-loader.js`, `js/components/risdoc/fm-risdoc-checkbox-group.js` |
...
| **24.9** | se il field dichiara options_source e abbiamo options | 3 | `js/components/risdoc/_options-fetcher.js`, `js/components/risdoc/fm-risdoc-pt-section.js`, `js/modules/risdoc/pt/section-to-pt.js` |
...
| **24.29** | skip header vuoto (era duplicato col title del parent) | 9 | `app/Services/Risdoc/Pt/PtToTex.php`, `app/Services/Risdoc/TexBuilder.php` |
...
| **24.32** | fill <select> con <optgroup> raggruppando per opt.group. | 11 | `app/Services/Risdoc/Pt/PtToTex.php`, `js/components/risdoc/fm-risdoc-pt-editor.js`, `js/modules/risdoc/pt/pm-schema.js` |
...
| **24.35** | Estrai PT AST da row.metadata.body_pt. La query base | 3 | `app/Services/Risdoc/Pt/PtToHtml.php`, `app/Services/Study/StudyPageRenderer.php` |
| **24.36** | POST /api/teacher/content/{id}/export | 1 | `app/Controllers/ContentExportController.php` |
...
| **24.45** | legge metadata.layout (exercises\|custom) con full lookup fallback. | 3 | `app/Services/Study/StudyPageRenderer.php` |
...
| **24.51** | overlay editor PT seed per template istituzionale (DEPRECATED 2026-05-28). | 2 | `js/modules/features/admin-risdoc.js` |
| **24.55** | Institutional override (admin-edited baseline) ─────── | 4 | `app/Controllers/Risdoc/TemplateController.php`, `app/Repositories/Risdoc/InstitutionalOverrideRepository.php`, `app/Services/Risdoc/TemplateResolver.php` |
| **24.56** | no cache: invalidato istantaneamente dopo edit admin. | 3 | `app/Controllers/Risdoc/TemplateController.php`, `app/Services/Risdoc/TemplateResolver.php` |
| **24.57** | crea un nuovo template. POST /api/admin/risdoc/templates/create. | 8 | `app/Controllers/Admin/RisdocAdminController.php`, `app/Repositories/Risdoc/RisdocTemplateRepository.php`, `js/modules/features/admin-risdoc.js` +1 |
...
| **24.73** | override etichetta PER-DOCENTE trasversale (store condiviso), | 8 | `js/entries/admin-sidebar-config.js`, `js/modules/features/db-sidepage.js`, `js/modules/features/risdoc-sidepage.js` +3 |
...
| **24.77** | Stepper custom ▲▼ (BEM .fm-stepper__btn) generico per input | 13 | `app/Services/ContractRenderer.php`, `app/Services/Study/StudyPageRenderer.php`, `js/modules/core/utilities.js` +2 |
...
| **25** | editor categorie predefinite per sezione (dinamico su group_mode). | 34 | `app/Controllers/Admin/AdminSidebarConfigController.php`, `app/Controllers/LatexShortcutsController.php`, `app/Controllers/TeacherContentController.php` +16 |
| **25.A** | nonce + strict-dynamic: gli script iniziali (incl. il | 1 | `app/Middleware/SecurityHeadersMiddleware.php` |
...
| **25.B1** | — | 2 | `js/modules/core/config.js`, `js/modules/integrations/google-apps.js` |
| **25.B2** | race-safe: usa INSERT ... ON DUPLICATE KEY UPDATE per | 3 | `app/Controllers/Risdoc/TemplateController.php`, `app/Repositories/Risdoc/OverrideRepository.php`, `app/Services/Crypto/TeacherCryptoService.php` |
...
| **25.B5** | bucket scoped: bucketKey distingue endpoint sensibili | 4 | `app/Config/security.php`, `app/Middleware/RateLimitMiddleware.php` |
| **25.B6** | Security headers obbligatori per tutte le response HTML/JSON. | 2 | `app/Core/Kernel.php`, `app/Middleware/SecurityHeadersMiddleware.php` |
| **25.B7** | — | 1 | `app/Middleware/SecurityHeadersMiddleware.php` |
| **25.C.2** | — | 1 | `app/Config/waf.php` |
| **25.C** | Self-service GDPR endpoints per data subjects (Art. 7, 16, 17, 20). | 4 | `app/Config/waf.php`, `app/Controllers/SelfServiceController.php`, `app/Core/Kernel.php` +1 |
| **25.C13** | DPO contact form pubblico (no auth richiesto). | 1 | `app/Controllers/DpoContactController.php` |
| **25.C2** | TOS + privacy disclosure obbligatoria (Art. 7 + 13). | 10 | `app/Controllers/RegistrationController.php`, `app/Services/PassiDopoLApprovazione.php`, `app/Services/RegistrationService.php` +2 |
...
| **25.C8** | Wrapper Mailer per workflow parent consent (Art. 8 GDPR). | 4 | `app/Jobs/SendMailJob.php`, `app/Services/ParentConsentMailer.php`, `app/Services/PassiDopoLApprovazione.php` |
| **25.D** | CSS estratto in /css/admin.css (auto-load da layout/shell). */ ?> | 10 | `app/Config/crypto.php`, `app/Controllers/MetricsController.php`, `app/Services/Crypto/TeacherCryptoService.php` +7 |
| **25.D3** | true se le read devono usare i ciphertext (crypto.read_from). | 9 | `app/Repositories/TeacherContentRepository.php` |
...
| **25.E17** | Cron / localhost-only entrypoints. | 2 | `app/Controllers/AdminPartialController.php`, `app/Controllers/CronController.php` |
...
| **25.E4** | Request ID correlation: imposta X-Request-ID prima | 4 | `app/Core/Kernel.php`, `app/Core/Telemetry.php`, `app/Middleware/RequestIdMiddleware.php` |
...
| **25.F** | — | 2 | `app/Controllers/SecurityAdminController.php`, `app/Repositories/Waf/WafSecurityRepository.php` |
| **25.G** | persistere in `waf_anomalies` table. | 3 | `app/Controllers/Admin/WafAdminController.php`, `views/admin/_partials/page_head.php`, `views/admin/waf/_layout_head.php` |
...
| **25.J.4** | TotpService, iscrizione | 2 | `app/Controllers/TotpController.php`, `app/Services/Security/TwoFactorPolicy.php` |
| **25.J** | bouncer free (no CrowdSec Service API a pagamento). | 12 | `app/Config/security.php`, `app/Config/waf.php`, `app/Core/AccessLogger.php` +4 |
| **25.P.1** | Verifica che un contenuto sia ELIGIBLE alla condivisione | 3 | `app/Services/Sharing/SharedContentPolicy.php` |
| **25.P.3** | sincronizza anche `teacher_content_data.source_type` (cache | 8 | `app/Controllers/ContentPublishController.php`, `app/Repositories/Contract/ContractRepository.php`, `app/Repositories/VerificaDocumentRepository.php` +3 |
| **25.P** | gate ToS/AUP. Il preavviso (banner + email) resta attivo comunque: | 10 | `app/Config/multitenancy.php`, `app/Controllers/Admin/AdminSystemController.php`, `app/Controllers/Admin/AdminTakedownController.php` +7 |
| **25.Q.4** | Quill self-hosted in /vendor/quill/1.3.6/ per | 2 | `views/partials/_exercise_assets.php`, `views/partials/head.php` |
| **25.Q.5** | risolve institute_id da codice MIUR (institutes.code). | 4 | `app/Controllers/CurriculumController.php`, `js/entries/auth-register.js` |
| **25.Q.7** | reset claim cached del precedente utente: | 1 | `app/Core/Auth.php` |
| **25.Q.8** | studente non vede checkmod (GIUST/SOL toggle + edit/move). | 11 | `app/Controllers/ContentStudyController.php`, `app/Controllers/PublicStudyController.php`, `app/Services/ContractRenderer.php` +1 |
...
| **25.Q.15** | GET /api/study/verifica/list — le verifiche che vede chi | 2 | `app/Controllers/VerificaController.php`, `js/modules/features/verifica-documents-sidepage.js` |
| **25.Q.16** | GET /api/study/header-page.json — endpoint read-only | 3 | `app/Controllers/StudyHeaderController.php`, `js/modules/features/upbar-controls.js`, `js/modules/features/verifica-documents-sidepage.js` |
| **25.Q** | RisDoc riservato a teacher/admin; nascosto a student/guest. */ ?> | 18 | `app/Controllers/Admin/AdminInstitutesController.php`, `app/Controllers/Admin/AdminTosLogController.php`, `app/Controllers/Public/PublicTakedownController.php` +9 |
...
| **25.R.23.2** | include anche risdoc_templates creati/forkati dal docente | 4 | `app/Services/Gdpr/Export/Exporters/TeacherContentExporter.php`, `app/Services/Gdpr/Export/Exporters/TemplatesExporter.php` |
| **25.R.23** | Bundle ZIP esteso con contenuti decifrati via UserDataExportService. | 19 | `app/Controllers/Admin/AdminCryptoStatusController.php`, `app/Controllers/Admin/AdminGdprController.php`, `app/Controllers/SelfServiceController.php` +14 |
| **25.R.24** | Scope contenuti unificato dentro Step ② (GDPR minimizzazione perimetro) | 7 | `app/Controllers/Admin/AdminCryptoStatusController.php`, `app/Services/Crypto/ShamirSecretSharing.php`, `app/Services/Gdpr/Export/Exporters/TeacherContentExporter.php` +2 |
| **25.R.25** | Pre-fetch state attuale per detection visibility/share transition | 14 | `app/Controllers/Admin/AdminLogsController.php`, `app/Controllers/AdminController.php`, `app/Repositories/Risdoc/OverrideRepository.php` +7 |
| **25.R.30** | raggruppamento rivisto + menu a tendina per gruppo. | 2 | `js/entries/admin.js`, `views/admin/_partials/page_head.php` |
...
| **26** | — | 2 | `app/Controllers/ExerciseController.php`, `js/modules/core/endpoints.js` |
