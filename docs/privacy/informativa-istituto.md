...
**Versione:** 1.3 — modello per lo Scenario 3 (adozione da parte di un Istituto). 1.3 (22 settembre 2026): la comunicazione delle modifiche è per email, non «con avviso nell'applicativo» — quell'avviso non esiste per l'informativa. 1.2 (22 settembre 2026): le bozze di compilazione dei modelli si cancellano 15 giorni dopo lo scaricamento del documento, e comunque a fine anno scolastico se restano ferme (§5, ADR-046); corretto il termine del log del filtro di sicurezza, che il documento dichiarava a 90 giorni mentre il codice ne applica 30 (`tools/audit/tabelle_da_purgare.php`). 1.1 (6 settembre 2026): conservazione dei backup portata a un anno, come in produzione; cookie del portachiavi nella modalità Anonima.
**Istanza:** {{APP_URL}}
...
> valgono invece le informative del gestore (`informativa-personale` per
> l'uso personale, `informativa` per i colleghi iscritti).
>
...
| **Anonima** | nessun account studente | accesso con la credenziale del docente, non nominativa: nessun dato personale dello studente. A scelta dello studente, il cookie tecnico `fm_keychain` («Ricorda su questo dispositivo») conserva i riferimenti firmati alle credenziali usate, senza dati personali |
...
Per ogni accesso autenticato vengono registrati indirizzo IP e User-Agent **in forma di hash SHA-256**, timestamp e operazione, nei registri di audit dell'istanza. Restano in chiaro, per la sola sicurezza e a conservazione breve, nel log del filtro di sicurezza (30 giorni), nei contatori anti-abuso e nelle sessioni attive, e nel verbale di accettazione dei Termini di Servizio quale prova dell'accettazione.
...
| Bozze di compilazione dei modelli salvate sul server | **15 giorni** dall'ultima fra lo scaricamento del documento e l'ultima modifica; e comunque il **31 agosto**, per quelle ferme dal 1° giugno precedente. Avviso con almeno 7 giorni di anticipo; ogni modifica fa ripartire il conto; si cancella la compilazione, non il modello |
...
Le modifiche a questa informativa sono comunicate agli utenti registrati **per email**; la versione vigente è sempre quella pubblicata su `/privacy/informativa`, con la propria tabella delle revisioni.

*Fino al 22 settembre 2026 questa riga diceva «con avviso nell'applicativo». Quell'avviso non esiste per l'informativa: è un banner costruito per i Termini e per l'AUP, dove è un impegno contrattuale. Dichiarare una misura che non si applica è quello che l'art. 5 §2 non consente.*
