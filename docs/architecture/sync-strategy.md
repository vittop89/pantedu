...
**Com'è implementato (23/9/2026).** L'export firma il manifest (versione,
data, esportatore, istituto e l'impronta SHA-256 e la dimensione di ogni file)
con HMAC-SHA256 su una chiave derivata dalla Recovery Key R del docente, che il
server rilegge da `teacher_recovery_keys` con la chiave master
(`TeacherRecoveryService::signManifestForExporter`). L'import
(`ImportBundleController`) chiede il codice R dal PDF dell'esportatore e
ricalcola l'HMAC con quello (`TeacherRecoveryService::verifyManifestHmac`),
senza database né chiave master; poi confronta ogni file caricato con la sua
impronta nel manifest.

- **Che cosa prova la firma:** che il pacchetto non l'ha cambiato chi non
  conosce la R di chi l'ha esportato. Con un manifest cambiato dopo la firma, o
  con un codice diverso da quello con cui è stato firmato, l'import risponde
  403 `invalid_recovery_code_or_manifest`.
- **Che cosa non prova:** chi l'ha esportato, rispetto al server. Il server non
  controlla che R sia la chiave del docente `exporter_user_id`, né che sia una
  chiave registrata o attiva su questa istanza, né che il manifest sia uscito
  dall'export: chiunque abbia una R, anche la propria, firma un manifest suo
  con l'esportatore, il nome e la data che vuole. «Esportato da» nell'anteprima
  è una dichiarazione del pacchetto. Revocare o rigenerare la chiave non
  invalida i pacchetti già firmati; la scadenza a 365 giorni si calcola su
  `exported_at`, che è firmato ma lo sceglie chi firma.
- **Perché così:** la verifica non dipende dallo stato del server, e servono
  tutti e due gli usi per cui esiste: lo scenario B qui sopra (i propri
  contenuti su un'altra istanza) e il reimport dopo la perdita o la rotazione
  della chiave master, che la Recovery Key esiste per sopravvivere («Contesto
  sicurezza» qui sopra). L'import scrive solo nell'account di chi importa,
  quindi un pacchetto di origine non provata tocca solo chi sceglie di
  importarlo. Il 23/9/2026 si è valutato di verificare con la chiave
  dell'esportatore salvata sul server: toglieva i due usi e non fermava un
  docente che firma con una chiave sua, e si è lasciato com'era. Le prove:
  `tests/Unit/Services/Crypto/FirmaDelManifestoTest.php` e
  `tests/Integration/FirmaDelPacchettoTest.php`.
