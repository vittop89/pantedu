...
> **Triggered by:** richiesta "aggiungere modello documento per costruire pagine web tipo ProgettoPrecedente senza scrivere HTML"
...
- https://www.progetto-precedente.example/strcomp_bes_altro/ALTRO/0.0_SBA-Legislazione-ALTRO.php
- https://www.progetto-precedente.example/strcomp_bes_altro/ALTRO/0.1_SBA-Glossario-ALTRO.php
- https://www.progetto-precedente.example/strcomp_bes_altro/ALTRO/1.0_SBA-Verifiche_e_Recuperi-ALTRO.php

oggi sono **file PHP/HTML standalone** in `storage/templates/strcomp/ALTRO/` (su repo progetto-precedente master), scritti a mano con CSS embedded. Per produrre nuove pagine simili docenti devono conoscere HTML/CSS. Inaccettabile su Pantedu (target: docenti scuole italiane, anche rurali).
...
| Stub | Section type richiesto | Mappato a pagina ProgettoPrecedente |
|---|---|---|
...
- **Seed da repo progetto-precedente master** (branch master, file in `storage/templates/strcomp/ALTRO/`).
...
## 4 — Seed extraction from progetto-precedente master

### 4.1 Source files

Path repo: `C:/percorso/locale/progetto-precedente/`, branch master, file:
...
`tools/scripts/extract-seed-from-progetto-precedente.php` (one-shot, da git-ignored):
...
| File | Operazione |
|---|---|
| `schemas/risdoc/_pt/seeds/glossario.pt.json` | NEW (estratto da progetto-precedente) |
...
| `tools/scripts/extract-seed-from-progetto-precedente.php` | NEW (one-shot, gitignored o `tools/scripts/` committato) |
...
| `tests/e2e/page_doc_seed_roundtrip.spec.js` | NEW — verifica che i 4 seed renderizzano output HTML equivalente alle pagine ProgettoPrecedente target |
...
2. **Day 7:** estrazione seed completi (3 pagine ProgettoPrecedente) + roundtrip E2E + ADR finale + merge.
