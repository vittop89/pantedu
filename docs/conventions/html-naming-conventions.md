...
Accoppiate a `app/Services/PhpContentParser.php` (XPath) e
`js/modules/print/verifiche-print-ui.js`.
...
4. **Verifica** — eseguire Playwright (`npm run e2e -- <spec>`) oppure, se
   l'ambiente non è montato, grep negativo del vecchio nome con report nel PR.
...
```bash
# 1. Build frontend (Vite). Obbligatorio: la suite rifiuta di girare su un
#    pacchetto più vecchio dei sorgenti.
cd ~/pantedu
npm run build

# 2. Server e database di sviluppo attivi (vedi docs/dev/sviluppo-in-wsl.md)
bash tools/dev/wsl/database.sh
bash tools/dev/wsl/server.sh
curl -sSI http://127.0.0.1:8765/ | head -3

# 3. Eseguire SOLO gli spec dello scope del tuo unit
npm run e2e -- tests/e2e/<spec-file>.spec.js

# 4. Fallback grep negativo (se l'ambiente non è montato)
grep -rn "<old-selector>" views/ css/ js/ app/ tests/ || true
# Report PR: "e2e skipped — ambiente non montato, grep pulito"
```
...
- Plan completo: `C:/percorso/locale/.claude/plans/eventual-doodling-starlight.md`
