...
- **sostituisce** i token PII con segnaposto neutri (nome istituto, città,
  provincia, foro competente e comuni vicini, ogni codice meccanografico con
  la sigla della provincia, indirizzo, telefono/fax, C.F., email e domini
  reali, IP del server, nome del Dirigente e del consulente DPO, nome utente
  della postazione, nomi dei docenti di prova: gli account diventano
  `docente.uno` e `docente.due`, il nome dentro identificatori e commenti
  `docente1` e `docente2`. Nella copia pubblica non ci sono nomi di docenti);
- **cancella** i file/cartelle interni non destinati al pubblico (handoff di
  ...
  stesso) e i due workflow che nel clone pubblico non possono girare
  (`e2e.yml` e `immagine.yml`: vogliono il runner di casa, l'immagine del
  rilascio e `database/schema.sql`, che lì non c'è). Gli altri — A11y,
  compliance, lighthouse, CI — restano. Gli strumenti d'accesso al server
  (la console `pantedu.exe` con il suo avvio e il sorgente, la cassetta
  `tools/postazione`) stanno nello script fra i segni `>>> strumenti
  d'accesso` e `<<< strumenti d'accesso`: dal 23/9/2026 quelle voci devono
  essere escluse anche dall'immagine del container, in `.dockerignore`, e lo
  pretende `tests/ops/strumenti-accesso-fuori-immagine.test.sh`. Uno
  strumento d'accesso nuovo va fra i segni, e in `.dockerignore`;
