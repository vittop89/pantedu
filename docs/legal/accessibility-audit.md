...
- **Ambito** (pagine campione, tema chiaro e scuro):
  ...
  - **Pagine autenticate verificate con axe** (login reale docente + super-admin)
    tramite la spec di allora `a11y_authenticated.spec.js` (oggi
    `tests/e2e/qualita/accessibilita-riservata.spec.js`): profilo, dashboard docente,
...
- **Automazione/CI**: vedi §6. Gli asset (`css/`, `js/`, `build/`) sono symlinkati in `public/` nel job così il test gira sulla **pagina realmente stilizzata** (in prod serviti da nginx), non su HTML privo di CSS.
...
dichiarazione accessibilità, sicurezza, modulo del recapito privacy (`/dpo-contact`), segnalazione contenuti,
...
- email `{{OPERATORE_EMAIL}}` (e il recapito privacy `{{OPERATORE_EMAIL}}` per i diritti GDPR collegati).

Risposta entro 30 giorni (vedi Dichiarazione). Procedura di rinvio ad AgID in caso di risposta insoddisfacente.

## 6. Monitoraggio continuo

Com'è la catena al 25 settembre 2026:

- **`a11y.yml`** (GitHub Actions), a ogni pull request verso `main` e una
  volta alla settimana (domenica): esegue
  `tests/e2e/qualita/accessibilita-pubblica.spec.js` con axe-core, regole
  WCAG 2.0/2.1/2.2 A e AA più le buone pratiche, contro un server PHP senza
  database. Sono 13 prove: le 11 pagine pubbliche dell'elenco
  `PAGINE_PUBBLICHE` (`tests/e2e/support/pagine-accessibilita.ts`), il
  collegamento «salta al contenuto» e il comando del tema scuro. Una
  violazione serious/critical fa fallire il workflow, ma **non è un controllo
  obbligatorio** per unire una pull request: il rosso resta visibile a chi
  rivede, e non blocca.
- **`tests/e2e/qualita/accessibilita-riservata.spec.js`**: le pagine
  riservate del docente e dell'amministrazione, con axe-core. Ha bisogno del
  database, quindi non gira in `a11y.yml`: gira nella suite end-to-end, dopo
  ogni unione su `main` e la domenica.
- **`lighthouse.yml`**, una volta alla settimana (domenica): Lighthouse CI su
  tre pagine (`lighthouserc.json`), con soglia di accessibilità 0,95.
  Nella suite end-to-end, `tests/e2e/qualita/punteggio-di-accessibilita.spec.js`
  controlla che cinque pagine pubbliche restino sopra 90 su 100.
- **`tests/e2e/qualita/regressione-visiva.spec.js`**: il confronto a pixel
  delle pagine, anche con il testo ingrandito al doppio (criterio 1.4.4).
...
_Redatto: 2026-06-21, rivisto il 2026-09-25 (§2 e §6: la catena dei controlli automatici com'è oggi) — Autovalutazione del soggetto erogatore. Standard: WCAG 2.2 AA, EN 301 549._
