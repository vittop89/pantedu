...
| `<pec operatore>` | PEC per atti formali | **non pubblicata** — `takedown_procedure.md` §1.3 è ancora `TBD` | provider PEC |
...
7. [ ] **DPA**:
   ...
   - Sottoscrizione formale: il Titolare del trattamento e il legale rappresentante dell'Istituto (il Dirigente Scolastico pro tempore, che questo documento non nomina)
...
10. [ ] **DPIA (Data Protection Impact Assessment)**:
    - La redige e la adotta il Titolare del trattamento, che in questo scenario
      è l'Istituto: il suo DPO vi partecipa secondo l'art. 39 §1 lett. c), e
      non è una persona che questo documento debba nominare. Fino al 22
      settembre 2026 qui c'era il nome di un professionista, e diceva una cosa
      non vera: che la valutazione fosse «coordinata con» lui. Nessun parere è
      stato chiesto né reso.
...
```bash
# In WSL xelatex c'è, mancano pandoc e i font (Calibri, Consolas), che stanno
# su Windows con lo xelatex di MiKTeX. Il modo che regge è
# tools/legal/build_pdf.sh; come si lancia: docs/dev/sviluppo-in-wsl.md,
# «Che cosa si fa ancora da Windows». Il comando qui sotto ha due trappole
# che lo script evita (vedi il suo commento).
...
| 2026-09-01 | Indirizzi di contatto pubblici normalizzati | Via da pagine e documenti pubblici il Gmail personale e l'indirizzo di posta della scuola (vedi tabella "Indirizzi di contatto"). Il secondo confondeva il ruolo di docente con quello di operatore e sarebbe morto al primo trasferimento |
