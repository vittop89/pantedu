...
> [`accessibility-audit.md`](https://github.com/vittop89/pantedu/blob/main/docs/legal/accessibility-audit.md).
...
**Pantedu è parzialmente conforme** ai requisiti previsti dalle linee
guida AgID sull'accessibilità degli strumenti informatici (Legge 4/2004,
Direttiva UE 2016/2102, EN 301 549 v3.2.1 / WCAG 2.1 livello AA). Le pagine
web sono conformi; non lo sono i **documenti PDF che la piattaforma
genera** (verifiche, esercizi), elencati alla lettera a) della sezione
"Contenuti non accessibili". Restano le esenzioni per i **contenuti di terze
parti** non sviluppati, finanziati né controllati dal soggetto erogatore,
documentate alla lettera c).
...
- **Documenti PDF generati dalla piattaforma** (verifiche, esercizi): sono
  prodotti con LaTeX senza struttura accessibile (PDF non marcati: niente
  tag, ordine di lettura né testo alternativo per le figure), e uno screen
  reader non li legge in modo affidabile. Sono contenuto di prima parte, e
  per questo stanno qui e non fra le esenzioni. Il contenuto di un esercizio
  resta consultabile in HTML nella pagina di studio.

Non si rilevano altri contenuti di **prima parte** (sviluppati dal soggetto
...
| Campo | Valore |
|---|---|
| Data di redazione | 2026-05-23 (rev. 2026-06-21: risoluzione barriere; rev. 2026-09-25: controlli automatici descritti come sono, PDF generati dichiarati non conformi) |
...
- **Email accessibilità**: [{{OPERATORE_EMAIL}}](mailto:{{OPERATORE_EMAIL}})
- **Recapito privacy** (per esercizio diritti GDPR collegati): [{{OPERATORE_EMAIL}}](mailto:{{OPERATORE_EMAIL}})
- **Modulo segnalazione**: [pantedu.eu/segnalazione-contenuti](https://pantedu.eu/segnalazione-contenuti)
...
[form.agid.gov.it](https://form.agid.gov.it/view/eb1f4528-bcb4-4f0b-a40a-4c629051ac21)
...
| Conformità ai sensi della Legge 4/2004 | Parzialmente conforme: pagine web conformi, PDF generati non conformi (lettera a); esenzioni per contenuti di terze parti (lettera c) |
...
I pulsanti "Entra con SPID" e "Entra con CIE" compaiono, **disabilitati**,
solo sulla pagina di accesso di un'istanza dello Scenario 3 (adozione da
parte di un Istituto); su pantedu.eu non compaiono. Nessuna istanza è
registrata come Service Provider presso AgID.
...
"PA tenant pilota", contattare <{{OPERATORE_EMAIL}}>.
...
| D.2 | WCAG 2.2 AA upgrade (target size 2.5.8, dragging 2.5.7, focus 2.4.11, auth 3.3.8, help 3.2.6, redundant entry 3.3.7) | ✅ Completata (criteri implementati in `a11y.css`; audit per criterio in `accessibility-audit.md`; controllo axe-core delle pagine pubbliche verde il 2026-06-21, 14 prove su 14) | 2026-06-21 |
| D.3 | EN 301 549 v3.3 readiness assessment + audit esterno indipendente | 🔜 Pianificata | 2027-Q2 |

---

_Ultimo aggiornamento: 2026-09-25 (parzialmente conforme: pagine web conformi, PDF generati non conformi; controlli automatici descritti in `accessibility-audit.md` §6; screen reader: MathML assistivo sulle formule verificato in prod, gerarchia heading senza salti, 0 ID duplicati, h1 su ogni pagina; allineamento volontario EAA)_
