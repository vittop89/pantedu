...
> `docs/dpo/executive_summary.md` per il
...
b. **Responsabile ({{OPERATORE_NOME}})**: responsabile delle misure
   tecniche/organizzative infrastrutturali ex art. 32 GDPR e del
   corretto uso della procedura di accesso amministrativo alle chiavi;
   **NON è responsabile dei contenuti caricati dai docenti
   Autorizzati**, che non seleziona né sorveglia in via preventiva e di
   cui l'Applicativo non gli mostra il contenuto in chiaro, nei limiti
   e alle condizioni dell'art. 16 del D.Lgs. 70/2003.

   L'architettura di envelope encryption per-teacher KEK rende i
   contenuti di ciascun docente inaccessibili agli **altri docenti** in
   ogni circostanza. Non li rende invece inaccessibili al Responsabile,
   che custodisce la chiave master da cui le KEK sono derivate: ogni
   accesso amministrativo è ammesso nei soli casi tassativi previsti e
   lascia traccia immutabile nel registro `crypto_custody_events` (cfr.
   ToS §3(c) e Informativa §11-bis).
...
|-------------------|----------|------|-----|
| Hetzner Online GmbH | Hosting VPS (server applicativo, database, storage) | Norimberga (DE — UE) | DPA standard Hetzner, art. 28 GDPR |
| Cloudflare, Inc. | CDN e sicurezza di bordo: **termina il TLS**, WAF, protezione volumetrica | Rete globale; per il traffico italiano instradamento UE | DPA Cloudflare + clausole contrattuali tipo per eventuali trasferimenti (Capo V) |
| Backblaze, Inc. — bucket EU | Storage backup offsite | Amsterdam (NL — UE) | DPA standard B2. I dati sono **cifrati lato client prima dell'upload**: Backblaze non accede al contenuto |
| Google LLC | OAuth di accesso e integrazione Drive — **solo su opt-in esplicito** del singolo docente | USA | Clausole contrattuali tipo art. 46(2)(c). Nessun dato trasferito finché un docente non attiva la funzione |

> **Cloudflare termina la connessione TLS**, quindi il traffico gli transita in
> chiaro: va indicato fra i sub-Responsabili anche se non conserva contenuti.
> Elenco allineato al Registro dei trattamenti (art. 30), Sezione D — che resta
> la fonte unica: se le due liste divergono, prevale il Registro.

#### 7.1-bis Fornitori di modelli di IA (solo se PDF-Import è attivo)

La funzione **PDF-Import** invia pagine di libri di testo rasterizzate a un
fornitore di modelli linguistici. È **disattivata per impostazione
predefinita** (`PDF_IMPORT_ENABLED=false`): se non viene attivata, nessuno dei
fornitori sotto elencati riceve alcun dato e le sezioni 7.3 e 10 si applicano
senza eccezioni.

Se il Responsabile attiva la funzione, si aggiungono i seguenti
sub-Responsabili, in numero e identità dipendenti dalla configurazione scelta:

| Sub-Responsabile | Servizio | Sede | Base del trasferimento (art. 44 e ss.) |
|---|---|---|---|
| Anthropic PBC | Modello vision/testo | USA | Clausole contrattuali tipo art. 46(2)(c) — DPA del fornitore |
| OpenAI, L.L.C. | Modello vision/testo | USA | Clausole contrattuali tipo art. 46(2)(c) — DPA del fornitore |
| Ollama (istanza locale) | Modello vision in locale | Stessa infrastruttura del Responsabile | **Nessun trasferimento** — non è un sub-Responsabile |

**Opzione senza trasferimenti**: configurando `PDF_IMPORT_DEFAULT_PROVIDER=ollama`
con un'istanza locale, la funzione opera senza alcun trasferimento verso terzi.
È la configurazione da preferire quando il Titolare non intende autorizzare
trasferimenti extra-SEE.

**Misure applicate prima dell'invio**: redazione automatica di codice fiscale e
indirizzi email (`PiiMasker`), incapsulamento del testo non fidato
(`PromptGuard`), cifratura delle chiavi API con la KEK del docente,
cancellazione degli artefatti di sessione dopo `PDF_IMPORT_RETENTION_DAYS`.
**Limite dichiarato**: la redazione automatica non riconosce i nomi propri.
Il materiale caricato deve essere costituito da libri di testo, non da
elaborati di studenti — vedi [`ai-literacy.md`](/legal/ai-literacy).
...
**I dati a riposo restano in UE.** Server applicativo, database e storage sono
su Hetzner (Norimberga, DE); i backup su Backblaze bucket europeo, cifrati lato
client prima dell'upload. Nessun contenuto dei docenti è conservato fuori
dall'Unione.

Tre eccezioni riguardano il **transito** o funzioni facoltative, e vanno
dichiarate per intero:

| Caso | Quando | Base del trasferimento |
|---|---|---|
| **Cloudflare** | sempre — termina il TLS in ingresso | Clausole contrattuali tipo, Capo V. Non conserva contenuti; per il traffico italiano l'instradamento è UE |
| **Google (OAuth + Drive)** | solo se un docente attiva l'opt-in | Clausole contrattuali tipo art. 46(2)(c). Nessun dato finché non attivato |
| **Fornitori di modelli di IA** | solo con `PDF_IMPORT_ENABLED=true` e provider cloud | § 7.1-bis. Con Ollama in locale **nessun trasferimento** |

Le ultime due sono **disattivate per impostazione predefinita**: in
configurazione standard l'unico soggetto extra-UE coinvolto è Cloudflare, e
solo in transito. Prima di attivare l'una o l'altra, il Responsabile ne dà
notizia al Titolare ex § 7.2.
...
**Nessun dato è conservato fuori dallo Spazio Economico Europeo.** Hosting,
database, storage e backup risiedono in UE (Hetzner DE, Backblaze bucket
europeo), e i backup sono cifrati lato client prima di lasciare il server.

I trasferimenti che possono verificarsi riguardano il **transito** o funzioni
facoltative, e sono elencati per esteso al § 7.3: Cloudflare (sempre, in
transito, termina il TLS), Google (solo su opt-in del docente) e i fornitori di
modelli di IA (solo con PDF-Import attivo e provider cloud). Ciascuno è coperto
da clausole contrattuali tipo ex art. 46(2)(c).
...
| Identificazione | Studenti pseudonimi default (codici random); 2FA TOTP facoltativa per i docenti, verificata al login, con obbligo imponibile per ruolo dal Titolare |
