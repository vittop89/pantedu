...
import { escHtml as escapeHtml } from "../../core/dom-utils.js";
...
function renderGlossaryTable(block) {
    ...
    return `<div class="pt-glossary-table"${nameAttr}>${search}<table><caption class="pt-glossary-caption">Glossario (${entries.length} voci)</caption>${thead}<tbody>${tbody}</tbody>`
        + `</table>`
        + `</div>`;
}
...
function renderStaticContent(block) {
    ...
    return `<section class="pt-static-content" data-level="${level}">${heading}${bodyHtml}${nested
         }</section>`;
}
...
function renderLinkListItem(item) {
    ...
    const subsHtml = subs.length
        ...
        : "";
    return `<li>`
        + `<a href="${escapeHtml(href)}" class="pt-link-list-pdf__link"${targetAttr}>${escapeHtml(label)}${iconHtml}</a>${desc}${subsHtml
         }</li>`;
}
