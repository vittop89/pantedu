 ...
 */
// 2026-09-05 — SortableJS dal pacchetto npm, risolto dal bundle Vite (come Lit
// dal P3): la copia vendorizzata in js/vendor e' stata tolta.
...
export const EventHendler = {
  handleInsElement (element, checkboxClass, elementSelector, sez, newElement) {
  ...
  UploadDynamicId (dynamicId, index) {
  ...
  aggiornaMultipleOrigineCollex ($collexItems, selectedValue) {
   ...
   */
  aggiornaMultipleColorCollex ($collexItems, selectedColor) {
  ...
  moveCollexItemToPosition (collexItem, targetPosition) {
    const collexEl = asElement(collexItem);
    if (!collexEl) return;
    const problemEl = collexEl.closest(".fm-groupcollex");
    this.normalizeOriginalIndices(problemEl);
   ...
   */
  normalizeOriginalIndices (problem) {
    const problemEl = asElement(problem);
   ...
   */
  reorderSingleContainer (container, withMathJax = true) {
   ...
   */
  initializeDefPositionImpHandlers () {
    const self = this;
