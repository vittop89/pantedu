...
export function createDropdownView(deps) {
    ...
    function buildTexDropdownGlobal(getPanel) {
        ...
        // Escape chiude, **da dovunque arrivi**.
        //
        // Stava solo su `menu`, quindi funzionava soltanto se il fuoco era già
        // dentro il menu. Da quando il fuoco ci entra solo con la tastiera
        // (vedi sotto), un menu aperto col mouse non si sarebbe più chiuso con
        // Escape: un menu aperto deve rispondere a Escape comunque, e chi lo
        // preme si aspetta che si chiuda.
        const suEscape = (e) => {
        ...
        menu.addEventListener("keydown", suEscape);
        document.addEventListener("keydown", suEscape);

        // Tab-out del menu (focus esce e non torna al bottone) → chiudi.
        //
        // `relatedTarget === null` NON è un tab-out: vuol dire che il fuoco non
        // è andato da nessuna parte — l'elemento che lo teneva è stato tolto dal
        // DOM, o la finestra ha perso il fuoco. Chiudere il menu in quel caso lo
        // fa sparire sotto le mani di chi lo sta usando.
        ...
        btn.addEventListener("click", async (e) => {
            ...
            if (menu.style.display === "none") {
                ...
                // Il fuoco entra nel menu **solo se il menu è stato aperto da
                // tastiera**. `detail === 0` distingue Invio/Spazio su un
                // bottone da un clic vero, che vale 1 o più.
                //
                // Serve per chi naviga da tastiera: il menu vive in
                // `document.body`, fuori dall'ordine di tabulazione, quindi
                // senza questo salto non ci si arriverebbe.
                //
                // Col mouse invece è **dannoso**, per due motivi. Il primo si
                // vede subito: stai scrivendo in un campo, apri il menu TeX, e
                // il cursore sparisce da dove stavi scrivendo.
                //
                // Il secondo è quello che ha tenuto rossa la suite notturna dal
                // passaggio al runner di casa. Con il fuoco dentro il menu basta
                // che qualcuno lo sposti perché `focusout` scatti e il menu si
                // chiuda da solo. E qualcuno lo sposta: il campo dell'editor se
                // lo riprende quando finisce di prepararsi. Provato in una
                // riproduzione locale, strumentando `focusout`:
                //
                //     [focusout] verso: DIV .fm-editor-field
                //
                // Sui computer di GitHub quel recupero arriva **prima** che il
                // menu si apra, quindi non succede niente; su una macchina più
                // lenta arriva dopo, e il menu sparisce sotto le mani.
                //
                // Non prendendo il fuoco col mouse, la condizione che innesca
                // la corsa non esiste più: non è una schermatura del sintomo, è
                // il comportamento giusto in entrambi i casi.
                if (e.detail === 0) {
