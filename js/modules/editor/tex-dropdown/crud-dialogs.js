...
/** Carica l'entry del dialogo finché non popola `window.FM[fmKey]`. */
async function loadFmDialogEntry(entryKey, fmKey) {
    await caricaEntry(entryKey, { pronto: () => !!window.FM?.[fmKey] });
}
        ...
        return new Promise((resolve) => {
            window.FM.openTexElementEditor({
                ...
                onSave: async ({ type, label, code, groupName, newGroup }) => {
                    // 23/9/2026 (A-69) — sono i modelli globali, di tutti i
                    // docenti: le due rotte vogliono la motivazione
                    // (X-Audit-Reason). Un indirizzo per chiamata, scritto
                    // nella chiamata: la guardia sui chiamanti lo riconosce.
                    let res;
                    if (mode === "new") {
                        res = await apiPost("/tikz/save-new-element", {
                            groupName:     newGroup || "",
                            existingGroup: !newGroup ? groupName : "",
                            elementType:   type,
                            label, code,
                        }, { motivo: auditReason("Nuovo modello TikZ globale dall'editor", `${label} nel gruppo ${newGroup || groupName}`) });
                    } else {
                        res = await apiPost("/tikz/edit-element", {
                            groupName: groupKey,
                            elementLabel,
                            elementType: type,
                            label, code,
                        }, { motivo: auditReason("Modifica di un modello TikZ globale dall'editor", `${elementLabel} nel gruppo ${groupKey}`) });
                    }
                    if (res?.success === true || res?.ok === true) {
