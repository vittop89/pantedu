...
export function createBuildSectionView(deps) {
    ...
    function buildSection(label, initialValue, fieldKey = null) {
        ...
        // `type` non è per forza il primo attributo (vedi inline-blocks-markers.js):
        // il filtro si accontenta di trovarlo dentro un tag `<script …>`.
        ta.addEventListener("input", () => {
            if (!/<script\b[^>]*\btype=["']text\/tikz["']/i.test(ta.value)) return;
