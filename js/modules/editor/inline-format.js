...
export function handleInlineBoxExit(field, key) {
    ...
    while (span && span !== field) {
        // Classi legacy (dots/AddTextDSA) e BEM (fm-dots/fm-add-text-dsa): l'editor
        // emette da tempo `fm-add-text-dsa` e senza questo ramo il caret restava
        // intrappolato nel box (2026-09-05).
        if (span.nodeType === Node.ELEMENT_NODE
            && span.tagName === "SPAN"
            && (span.classList?.contains("dots") || span.classList?.contains("fm-dots")
                || span.classList?.contains("AddTextDSA") || span.classList?.contains("fm-add-text-dsa"))) {
            break;
        }
