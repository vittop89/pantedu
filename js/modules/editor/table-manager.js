...
/**
 * Il clic sulla casella della risposta (`.fm-checkbox-rm`) non risale alla
 * cella: la cella ha i suoi gestori (selezione, editor), e la casella deve
 * solo cambiare stato.
 *
 * 23/9/2026 (revisione architetturale A-19) — era
 * `onclick="event.stopPropagation();"` scritto nell'HTML delle tre celle qui
 * sotto; con la CSP rigorosa un gestore in linea non parte, e in produzione il
 * clic risaliva. Adesso è un ascoltatore vero, messo da
 * `_setupCheckboxPositionButtons`, da cui passano le celle appena scritte e
 * quelle delle tabelle già salvate. La funzione è una sola, quindi aggiungerla
 * due volte alla stessa casella non la fa girare due volte. Prova:
 * tests/js-unit/gestori-senza-onclick.test.js.
 */
function fermaAllaCasella(e) {
    e.stopPropagation();
}

/** @param {ParentNode|null|undefined} radice */
export function fermaIClicDelleCaselle(radice) {
    radice?.querySelectorAll?.(".fm-checkbox-rm").forEach((c) => c.addEventListener("click", fermaAllaCasella));
}
...
export const TableManager = {
     ...
     */
    _processCellContent (cell, checksol, typeCell) {
    ...
    _processCheckboxCell (cellEl, checkboxRM, text, checksol) {
    ...
    _processRegularCell (text, typeCell) {
    ...
    _processTableRows (table, elements, checksol, typeCell) {
    ...
    _processTableColumn (table, elements, checksol, typeCell, NColumn) {
        const tableEl = asElement(table);
        if (!tableEl) return "";
    ...
    _processTableMixed (table, elements, checksol, typeCell) {
        const tableEl = asElement(table);
        if (!tableEl) return "";
    ...
    _processTableNoMix (table, elements, checksol, typeCell) {
        const tableEl = asElement(table);
        if (!tableEl) return "";
    ...
    _processTable (table, tableIndex, tableArray, elements, checksol) {
        const tableEl = asElement(table);
        if (!tableEl) return "";
    ...
    _processGiustSol (table, checksol) {
        const checksolEl = asElement(checksol);
    ...
    printTabelle (elements, checksol, _chekgiust) {
     ...
     */
    gestioneRigaColonna (table, numTab) {
        const tableEl = asElement(table);
        if (!tableEl) return;
    ...
    updateCheckboxState (cell, isChecked) {
        const cellEl = asElement(cell);
        if (!cellEl) return;
    ...
    _setupCheckboxPositionButtons (cell) {
        const cellEl = asElement(cell);
        if (!cellEl) return;
        const self = this;
        // Tutte le strade che creano o ritrovano una casella passano di qui:
        // le tre celle scritte con innerHTML e le tabelle già salvate.
        fermaIClicDelleCaselle(cellEl);
    ...
    _changeCheckboxPosition (cell, position) {
        const cellEl = asElement(cell);
        if (!cellEl) return;
        const wrapCheckCell = cellEl.querySelector(".fm-wrap-check-cell");
    ...
    _saveCheckboxPosition (cell, position) {
        const cellEl = asElement(cell);
        if (!cellEl) return;
    ...
    ajaxUpdateTableAttribute (table, key, newValue) {
        const tableEl = asElement(table);
        if (!tableEl) return;
        const parent = tableEl.closest(".fm-groupcollex");
        const path = PathManager.extractPath(parent);
    ...
    batchSaveCheckboxRMState (filePath, user) {
    ...
    debounceBatchSave (filePath, user) {
    ...
    initializeTableEditor (element, parent, index) {
        const self = this;
    ...
    _setupTableForEditing (table, btnC, btnR1, btnR2) {
        const tableEl = asElement(table);
        if (!tableEl) return;
    ...
    _addRowControls (table, btnR1, btnR2) {
        const tableEl = asElement(table);
    ...
    _createTableEditInterface (table) {
        const tableEl = asElement(table);
        if (!tableEl) return;
    ...
    _createInputGroup (key, value, table) {
    ...
    _createCheckboxInput (key, value) {
    ...
    _createTypeCellInput (key, value, table) {
    ...
    _createTextInput (key, value, table) {
        const wrapper = document.createElement("div");
        wrapper.className = `input-${key}`;
        wrapper.innerHTML = `
            <label>${key}</label>
    ...
    _updateTypeCellAttribute (table, key, container) {
    ...
    _updateTableAttribute (table, key, newValue) {
        const tableEl = asElement(table);
        if (!tableEl) return;
        elData(tableEl, key, newValue);
        tableEl.setAttribute(`data-${key}`, newValue);
    ...
    _rebuildCheckboxStructure (cell) {
        const cellEl = asElement(cell);
        if (!cellEl) return;
        const checkboxRM = cellEl.querySelector(".fm-checkbox-rm");
            ...
            const newWrapHtml = `
                <div class="${wrapClass}" style="${wrapStyle}">
                    <div class="fm-checkbox-wrapper">
                        <button class="fm-btn-check-pos fm-btn-check-top" title="Sposta checkbox sopra">↑</button>
                        <button class="fm-btn-check-pos fm-btn-check-left" title="Sposta checkbox a sinistra">←</button>
                        <input type="checkbox" class="checkbox fm-checkbox-rm" ${isCheckboxRMChecked ? "checked" : ""}>
                        <button class="fm-btn-check-pos fm-btn-check-right" title="Sposta checkbox a destra">→</button>
                        <button class="fm-btn-check-pos fm-btn-check-bottom" title="Sposta checkbox sotto">↓</button>
                    </div>
