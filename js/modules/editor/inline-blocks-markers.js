 ...
 *
 * Funzioni pure, nessuna dipendenza DOM; l'escape degli attributi è quello
 * di core/dom-utils.js (23/9/2026, A-21).
 */

import { escAttr } from "../core/dom-utils.js";

// 18 settembre 2026 — l'espressione di prima pretendeva `type` come PRIMO
// attributo (`<script\s+type=...`). In produzione non lo è mai: la CSP è
// rigida e SecurityHeadersMiddleware::stampScriptNonce infila `nonce="…"`
// subito dopo `<script`, quindi il tag vero è
// `<script nonce="…" type="text/tikz" …>`. Nessun riscontro → la figura non
// diventava un marcatore, restava nel campo come HTML e al salvataggio
// finiva nel contratto come TESTO (verifica 75, esercizio 14: il contratto
// si è ritrovato dentro `<script nonce="" type="text/tikz" …>\usepackage…`
// e la figura era persa). Ora `type` si cerca ovunque dentro il tag.
// 23 settembre 2026 (A-16) — il middleware non timbra più il corpo: le figure
// TikZ del contenuto arrivano senza nonce. Il nonce resta possibile nell'HTML
// salvato prima di allora, e la regola qui sotto lo regge ancora.
const _TIKZ_SCRIPT_RE = /(<script\b(?=[^>]*\btype=["']text\/tikz["'])[^>]*>)([\s\S]*?)(<\/script>)/gi;
// Il `nonce` è di quella risposta e di quella sola: non deve entrare né nel
// valore del campo né, tramite `data-tikz-tagopen`, nel contratto. Il browser
// lo nasconde già nel DOM (`nonce=""`), ma il valore vero c'è nell'HTML che
// arriva via fetch, ed è quello che l'editor legge quando apre un pannello
// caricato a parte.
export function senzaNonce(html) {
    // Solo il PRIMO tag: `html` può essere l'outerHTML di uno <script>, e il
    // corpo TeX non si tocca.
    return String(html || "").replace(
        /^(\s*<[a-z][^>]*>)/i,
        (tag) => tag.replace(/\s+nonce=(["'])[^"']*\1/gi, ""),
    );
}
...
export function collapseTikzBlocks(value) {
    ...
    while ((m = _TIKZ_SVG_RE.exec(value)) !== null) {
        ...
        blocks.push({ tagOpen: senzaNonce(tagOpen), body: `\n${body}\n`, tagClose: '</' + 'script>' });
    ...
    while ((m = _TIKZ_SCRIPT_RE.exec(collapsed)) !== null) {
        ...
        blocks.push({ tagOpen: senzaNonce(m[1]), body: m[2], tagClose: m[3] });
