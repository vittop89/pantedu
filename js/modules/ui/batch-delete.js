...
/** `motivo`: il testo per X-Audit-Reason, che /files/delete pretende dal
 *  23/9/2026 (A-69). */
async function postForm(url, fields, motivo = "") {
...
export const BatchDeleteManager = {
    ...
    async showDeleteConfirmation (problems) {
    ...
    async executeBatchDelete (problems) {
            ...
            try {
                ...
                if (svgToDelete.length > 0) {
                    const svgDeletePromises = svgToDelete.map((svgFileName) =>
                        ...
                        }, auditReason("Eliminazione in blocco: SVG di un problema cancellato", svgFileName))
                            .then((delResponse) => {
