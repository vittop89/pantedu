 ...
 */
import { isVFGroup } from "../core/dom-utils.js";
...
export const StateManager = {
    ...
    saveCompleteState () {
        ...
        document.querySelectorAll(".fm-checkbox-ain").forEach((el, index) => {
            ...
            const problemId = el.closest(".fm-groupcollex")?.id;
            if (problemId) {
                // 2026-09-04 — era `state.fm-checkbox-ain.push(...)`: un rinomina
                // delle classi CSS aveva riscritto anche la chiave dell'oggetto,
                // che JS leggeva come `state.fm - checkbox - ain` (ReferenceError
                // alla prima checkbox): lo stato A/in non veniva mai salvato.
                state.checkboxAin.push({ problemId, collexIndex, index, checked: el.checked });
            }
        });
        ...
        document.querySelectorAll(".fm-checkbox-bin").forEach((el, index) => {
            ...
            const problemId = el.closest(".fm-groupcollex")?.id;
            if (problemId) {
                state.checkboxBin.push({ problemId, collexIndex, index, checked: el.checked });
            }
        });
        ...
        document.querySelectorAll(".fm-groupcollex").forEach((problem, index) => {
            ...
            if (!problemId || !isVFGroup(problem)) return;
    ...
    loadCompleteState () {
        ...
        try {
            ...
            state.checkboxAin?.forEach((item) => {
                const problem = problemById(item.problemId);
                if (!problem) return;
            ...
            state.checkboxBin?.forEach((item) => {
                const problem = problemById(item.problemId);
                if (!problem) return;
                const collexItem = problem.querySelectorAll(".fm-collection__item")[item.collexIndex];
                if (!collexItem) return;
    ...
    recalculatePointsAfterStateRestore () {
        setTimeout(() => {
    ...
    debugCurrentState () {
        ...
        if (savedState) {
            ...
            console.log("  - Checkbox Ain:", state.checkboxAin ? state.checkboxAin.length : 0);
            console.log("  - Checkbox Bin:", state.checkboxBin ? state.checkboxBin.length : 0);
