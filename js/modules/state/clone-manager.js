...
import { asElement, riempiSelect } from "../core/dom-utils.js";
...
export const CloneManager = {
     ...
     * @returns {string}
     */
    replaceTextAfterArrayBlock (latexContent) {
        const regexParts = {
     ...
     */
    async cloneCollexItem (collexItemArg, modifiedHtml = null) {
        try {
    ...
    _getExercisePathFromVerification (verificationPath) {
     ...
     */
    async _updateDomAfterClone (response, exercisePath, _problemId, _collexItemIndex) {
        try {
            ...
            if (response.action === "appended") {
                ...
                for (const problem of problems) {
                    ...
                    // `cloneNode` invece di rileggere `outerHTML` e farlo
                    // ri-analizzare al parser: stesso risultato, senza far
                    // passare per HTML un nodo che abbiamo già in mano.
                    collexItems.forEach((item) => {
                        collexList.append(item.cloneNode(true));
                    });
    ...
    _postCloneSetup (problem, _appendedOnly) {
