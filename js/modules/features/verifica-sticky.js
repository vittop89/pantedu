...
function isInScope() {
    // 2026-09-05 — anche i docenti in modalita' verifica (body.fm-verifica-mode), non solo admin.
    return (document.body.classList.contains("fm-admin-access") || document.body.classList.contains("fm-verifica-mode"))
...
function sectionScopeSelector(tag) {
    ...
    return `body:is(.fm-admin-access, .fm-verifica-mode) [id^="type_verAll"] ${tag},`
        + ` body:is(.fm-admin-access, .fm-verifica-mode) .fm-contract-wrap[data-kind="verifica"] ${tag},`
        + ` body:is(.fm-admin-access, .fm-verifica-mode) .fm-contract-wrap[data-kind="esercizio"] ${tag}`;
}
 ...
 *    (body.fm-info-open). NON considera le classi display:none
...
function computeBaseTop() {
    ...
    const si = document.getElementById("scrollbarInfo");
    // 2026-09-07 — la classe `upbar-hidden` non esiste più (voce 40 del
    // debito): resta il solo controllo su `display`.
    const upbarVisible = upbar && getComputedStyle(upbar).display !== "none";
...
export function bindVerificaStickyObservers() {
    ...
    // MutationObserver: reagisce a scrollbarInfo-hidden e a .active su
    // .fm-collapsible (open/close problem).
