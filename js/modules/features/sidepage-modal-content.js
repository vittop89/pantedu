 ...
 *   - extractFields(form, type, existingMeta?, mode?)
 *   - patchDeiMetadati(prima, dopo)
 *   - createContent(type, fields, preSubject?) → risposta del server
 *   - updateContent(id, fields) → risposta del server
 *   - deleteContent(id)
...
import { montaDoveVale } from "./dove-vale.js";
import { eLinkDrive, messaggioErroreLink } from "./mappa-da-link.js";
import { srcEditorDrawio, origineDellEditor } from "./drawio-app.js";
...
export function openModal({ type, mode, sidepage: _sidepage, row, preCategory, preSubject, sectionKey = "", allowedTypes = [], templateOrigin = "", templateGroups = [] }) {
    ...
    // ADR-037, fase 2 — «Dove vale» e «Duplica in…» nella modifica di un
    // contenuto (non dei modelli d'istituto): al posto della lista di classi
    // di «più classi», le pubblicazioni con una scuola e uno stato ciascuna.
    const doveVale = modal.querySelector(".fm-dove-vale-box");
    if (doveVale && mode !== "create" && row?.id && !row?._isTemplate) {
        const spKey = _sidepage?.dataset?.sidepage || (_sidepage?.id || "").replace(/^fm-sp-/, "");
        montaDoveVale(doveVale, row.id, {
            onDuplicato: () => {
                if (spKey) refreshSidepageByKey(spKey); else refreshSidepage(type);
            },
            // ADR-037, fase 4c — i posti pubblicati di un contenuto in bozza si
            // mostrano sospesi, e il pannello segue la visibilità mentre cambia.
            selettoreStato: modal.querySelector('select[name="visibility"]'),
        });
    }
    ...
    if (drawioEditBtn) {
        drawioEditBtn.addEventListener("click", async () => {
            ...
            if (!cid) return;
            try {
                // Il modulo lo carica bootstrap.js solo nel contesto degli
                // esercizi: altrove si carica adesso, invece di chiedere di
                // ricaricare una pagina che non l'avrebbe caricato comunque.
                const openDrawioEditor = window.FM?.DrawioEditor?.openDrawioEditor
                    || (await import("./drawio-editor.js")).openDrawioEditor;
                const result = await openDrawioEditor({
                    contentId: cid, mode: "edit"
                });
                if (result?.saved) {
                    closeModal();
                    // La sidepage in cui sta il contenuto, non quella del tipo:
                    // una mappa può stare nel Laboratorio.
    ...
    form?.addEventListener("submit", async (e) => {
        ...
        try {
            ...
            // Un link a un drawio su Drive si importa allo stesso modo (mappa-da-link.js).
            const linkDaImportare = mode === "create" && docModeSel === "link" && eLinkDrive(form.elements.href?.value || "");
            if (mode === "create" && (docModeSel === "upload" || docModeSel === "drawio_native" || linkDaImportare)) {
                await createMappaWithBlob(form, linkDaImportare ? "link" : docModeSel, preSubject || "", sectionKey);
                closeModal();
            ...
            const fields = extractFields(form, effType, mode === "edit" ? safeMeta(row || {}) : {}, mode);
            ...
            // Il 📥 e i ruoli D/C/R della voce li decide il server, con la
            // regola del caricamento della barra (App\Support\RigheDellaBarra):
            // calcolati qui, da ciò che si mandava, il 📥 compariva dopo un
            // salvataggio e spariva ricaricando (analisi C-export-bodypt).

            if (mode === "create") {
                const creato = await createContent(effType, fields, preSubject || "", sectionKey);
                const newId = creato.id;
                closeModal();
                ...
                const rowIns = { id: newId, topic: fields.topic, title: fields.title,
                                 has_body_pt: creato.has_body_pt === true,
                                 doc_roles: typeof creato.doc_roles === "string" ? creato.doc_roles : "",
                ...
                const salvato = await updateContent(row.id, fields);
                closeModal();
                ...
                if (canSurg && fields.visibility === "archived") {
                    // archiviato → sparisce dalla sidepage
                        ...
                        // Senza i due campi nella risposta la voce tiene i suoi.
                        row: { id: row.id, topic: fields.topic, title: fields.title,
                               has_body_pt: salvato?.has_body_pt, doc_roles: salvato?.doc_roles,
                               subject_code: row.subject_code || row.subject || "",
                               // Bozza ↔ pubblicato cambia il segno della voce.
                               visibility: fields.visibility },
                    });
                }
                if (!done) {
...
export function buildModalHtml({ type, mode, row, preCategory }) {
    ...
    // ADR-037, fase 2 — «più classi» non si sceglie più: si pubblica in altri
    // posti da «Dove vale». Resta solo per i contenuti che lo avevano, e dice
    // che cosa vuol dire adesso.
    const scopeOptionsHtml = [
        ["class",   "La classe del documento (e i posti di «Dove vale»)"],
        ...(publishScope === "classes"
            ? [["classes", "Solo i posti di «Dove vale», non la classe del documento"]]
            : []),
        ["general", "Tutti gli studenti della stessa materia, dove è pubblicato"],
    ...
    // PDF, ecc.), required SOLO quando doc_mode=link. In creazione sta subito
    // sotto la sua spunta, come la zona di caricamento sotto «Carica file»
    // (15/9/2026, richiesta dell'utente: prima stava sopra, sempre visibile).
    const hrefLabel       = "Link esterno (opzionale)";
    const hrefPlaceholder = "https://... (drawio, Drive, sito, PDF, ecc.)";
    const hrefInputHtml   = `<input type="url" name="href" value="${hrefDefault}" placeholder="${hrefPlaceholder}">`;
    ...
    // Il modello salvato, se c'è. Le mappe e gli esercizi a contratto non ne
    // hanno uno: in modifica non se ne inventa uno (19/9/2026, sotto).
    const layoutSalvato = (meta.layout === "exercises" || meta.layout === "custom") ? meta.layout : "";
    const currentLayout = layoutSalvato || "exercises";
    ...
    const mappaCreateOpts = isCreate ? `
                    <label class="fm-modal-layout__opt">
                        ...
                        <span>🔗 <strong>Link esterno</strong> — drawio, Drive, sito, PDF, ecc.</span>
                    </label>
                    ${isTemplate ? "" : `<div class="fm-modal-doc-section" data-fm-doc-mode="link" hidden>
                        <label>Link
                            ${hrefInputHtml}
                        </label>
                        <p style="font-size:12px;color:#475569;margin:6px 0">
                            Un file drawio su Google Drive, con il link pubblico, si importa: la mappa si vede nella pagina e si modifica qui.
                        </p>
                    </div>`}

                    <label class="fm-modal-layout__opt">
                        <input type="radio" name="doc_mode" value="upload" ${defaultDocMode === "upload" ? "checked" : ""}>
                        <span>⬆ <strong>Carica file</strong> — un file drawio (.drawio o .xml, max 50MB)</span>
                    </label>
...
` : "";
    ...
    // RUOLO (D/C/R) per i documenti personalizzabili.
    //
    // 19/9/2026 — il campo nascosto `doc_mode` c'è solo se il contenuto ha un
    // modello salvato. Prima c'era sempre, con «exercises» di ripiego: una
    // mappa o un esercizio a contratto ricevevano a ogni «Salva»
    // `layout: exercises` e un body_pt d'esempio, e il 📥 compariva su una
    // mappa (analisi C-export-bodypt). In modifica il modale non scrive più il
    // modello: il campo resta solo come descrizione del contenuto.
    ...
    const docModeHtml = !showDocMode ? "" : (isCreate ? `
                ...
                <fieldset class="fm-modal-layout fm-modal-layout--ro">
                    ...
                    ${layoutSalvato ? `<input type="hidden" name="doc_mode" value="${escAttr(layoutSalvato)}">` : ""}
                    ${roleEditRO}
                </fieldset>`);
    ...
    // In creazione il campo sta sotto la spunta «🔗 Link esterno» (mappaCreateOpts);
    // qui resta solo per la modifica di una mappa, dove le spunte non ci sono.
    ...
    const audienceHtml = `
                ...
                ${!isCreate && r.id ? `
                <section class="fm-modal-dove-vale" aria-label="Dove vale">
                    <p style="margin:6px 0 2px 0"><strong>Dove vale</strong></p>
                    <p class="fm-muted" style="font-size:11px;margin:0 0 4px 0;line-height:1.4">
                        Lo stesso contenuto può valere in più classi, anche di un'altra tua scuola:
                        lo correggi una volta. Ogni posto ha il suo stato.
                    </p>
                    <div class="fm-dove-vale-box" data-content-id="${Number(r.id)}"></div>
                </section>` : ""}
                ${type === "verifica" ? `
                <label class="fm-modal-archive" style="display:block;margin:4px 0 8px 0;font-size:13px">
                    <input type="checkbox" name="archive_visible" value="1" ${Number(r.archive_visible) === 1 ? "checked" : ""}>
                    Visibile anche dopo l'anno — resta nell'archivio degli anni passati degli studenti.
                    Per default le verifiche ne restano fuori, perché si riusano con la classe più giovane.
                </label>` : ""}
                ${sharePoolHtml}`;
...
/**
 * I campi del modulo per la creazione o la modifica.
 *
 * In creazione `fields.metadata` sono i metadati del contenuto nuovo, con il
 * `body_pt` iniziale del modello scelto.
 *
 * In modifica (19/9/2026) il modale non ricostruisce più i metadati: manda
 * `fields.metadata_patch`, le sole chiavi che l'utente ha cambiato nel modulo
 * (`null` = togli), e il server le fonde con quelle salvate
 * (App\Domain\MetadatiDelContenuto). Prima li ricostruiva da zero e il server li
 * sostituiva: ogni «Salva» dava a una mappa `layout: exercises` e un body_pt
 * d'esempio, le toglieva `mappa.href_hide` e `mappa.drawio_id`, e a un
 * esercizio toglieva `contract_key`, cioè i suoi quesiti (analisi
 * C-export-bodypt e F-metadati-modifica-mappa).
 *
 * @param {HTMLFormElement} form
 * @param {string} type
 * @param {object} [existingMeta] i metadati salvati (in modifica)
 * @param {"create"|"edit"} [mode]
 */
export function extractFields(form, type, existingMeta = {}, mode = "create") {
    ...
    if (mode === "edit") {
        const fields = { title, topic, visibility, publish_scope: (fd.get("publish_scope") || "class").trim() };
        if (type === "verifica") fields.archive_visible = fd.get("archive_visible") ? "1" : "0";
        const patch = patchDeiMetadati(existingMeta, metadatiModificati(form, type, existingMeta));
        if (Object.keys(patch).length > 0) fields.metadata_patch = patch;
        return fields;
    }
    ...
    // Piano classi, D — «visibile anche dopo l'anno»: solo le verifiche lo
    // chiedono; "0" esplicito, così deselezionare la casella salva davvero.
    if (type === "verifica") fields.archive_visible = fd.get("archive_visible") ? "1" : "0";
    // ADR-037, fase 2 — i bersagli non si mandano più: i posti in più sono
    // pubblicazioni, e si gestiscono da «Dove vale».
    const meta = {};
...
/**
 * I metadati salvati con le sole modifiche fatte nel modulo. Un campo conta
 * solo se l'utente l'ha cambiato rispetto a come il modale l'ha aperto: così
 * un valore mostrato per comodità (il link di una mappa letto da `url`, la
 * categoria nascosta) non si riscrive dove non c'era.
 *
 * @param {HTMLFormElement} form
 * @param {string} type
 ...
 * @returns {object}
 */
function metadatiModificati(form, type, esistenti) {
    const meta = JSON.parse(JSON.stringify(esistenti && typeof esistenti === "object" ? esistenti : {}));

    // Ruolo D/C/R (solo i documenti Personalizzabili hanno la scelta).
    const ruolo = form.querySelector('input[name="doc_roles"]:checked');
    if (ruolo && !ruolo.defaultChecked) {
        const scelto = String(ruolo.value || "").toUpperCase();
        if (["D", "C", "R"].includes(scelto)) meta.doc_roles = [scelto];
        else delete meta.doc_roles;
    }

    // Il link (in modifica c'è solo per le mappe).
    const campoLink = form.querySelector('input[name="href"]');
    if (campoLink && campoLink.value.trim() !== campoLink.defaultValue.trim()) {
        const href = campoLink.value.trim();
        if (type === "mappa") {
            const mappa = (meta.mappa && typeof meta.mappa === "object" && !Array.isArray(meta.mappa))
                ? { ...meta.mappa } : null;
            if (href) {
                meta.mappa = { ...(mappa || { display: "show" }), href };
            } else if (mappa) {
                delete mappa.href;
                meta.mappa = mappa;
            }
    ...
    // La categoria in modifica è un campo nascosto che nasce con quella
    // salvata: si confronta con quella salvata, non con il `defaultValue` del
    // campo. Sono la stessa stringa, quindi il confronto con `defaultValue`
    // era sempre falso e il ramo non poteva scattare (revisione della PR
    // #145, 20/9/2026): il giorno che qualcosa scrivesse qui una categoria
    // nuova — un «sposta di categoria» dal modale, o `preCategory` passato
    // anche in modifica — la patch non l'avrebbe portata. Un campo vuoto vuol
    // dire «il modale non la conosce», non «toglila».
    const campoCategoria = form.querySelector('input[name="category"]');
    if (campoCategoria) {
        const categoria = campoCategoria.value.trim();
        const salvata = typeof meta.category === "string" ? meta.category.trim() : "";
        if (categoria && categoria !== salvata) meta.category = categoria;
    }
    return meta;
}

/**
 * Le chiavi di primo livello che cambiano fra `prima` e `dopo`: il valore
 * nuovo, o `null` per una chiave tolta. È la `metadata_patch` che il server
 * fonde con i metadati salvati.
 *
 * @param {object} prima
 * @param {object} dopo
 * @returns {object}
 */
export function patchDeiMetadati(prima, dopo) {
    const a = prima && typeof prima === "object" ? prima : {};
    const b = dopo && typeof dopo === "object" ? dopo : {};
    const patch = {};
    for (const chiave of Object.keys(b)) {
        if (JSON.stringify(b[chiave]) !== JSON.stringify(a[chiave])) patch[chiave] = b[chiave];
    }
    for (const chiave of Object.keys(a)) {
        if (!(chiave in b)) patch[chiave] = null;
    }
    return patch;
}
...
export async function createContent(type, fields, preSubject = "", sectionKey = "") {
    ...
    if (fields.archive_visible !== undefined) fd.set("archive_visible", fields.archive_visible);
    ...
    if (!r.ok || !j.ok) throw new Error(messaggioDiErrore(j, r.status));
    ...
    // La risposta intera: con l'id ci sono has_body_pt e doc_roles della riga.
    return j;
}

/**
 * Il messaggio da mostrare per una risposta di errore del server. Il rifiuto
 * di «più classi» (ADR-037, fase 2) dice dove si fa adesso.
 */
function messaggioDiErrore(j, status) {
    if (j && j.error === "usa_dove_vale") {
        return "«Più classi» ora si fa da «Dove vale»: crea il contenuto, poi riaprilo e aggiungi i posti.";
    }
...
export async function updateContent(id, fields) {
    ...
    if (fields.publish_scope) fd.set("publish_scope", fields.publish_scope);
    if (fields.archive_visible !== undefined) fd.set("archive_visible", fields.archive_visible);
    // Il modale manda solo la patch (extractFields in modifica); `metadata`
    // sostituirebbe i metadati salvati per intero.
    if (fields.metadata_patch) fd.set("metadata_patch", JSON.stringify(fields.metadata_patch));
    else if (fields.metadata) fd.set("metadata", JSON.stringify(fields.metadata));
    ...
    // Phase 25.A4 — invalida ETag cache su mutate.
    window.FM?.clearTeacherContentCache?.();
    // Con has_body_pt e doc_roles della riga salvata.
    return j;
}
    ...
    const r = await fetch(`/api/admin/risdoc/templates/${id}/meta`, {
        ...
        headers: {
            ...
            "X-Audit-Reason": auditReason(
                `Modifica scheda template #${id}`,
                [num_arg, argomento].filter(Boolean).join(" ")
            ),
        },
        body: fd.toString(),
    });
...
function wireMappaDrawioEmbed(modal) {
    ...
    openBtn.addEventListener("click", () => {
        // Lo stesso editor delle mappe esistenti, ospitato qui (drawio-app.js).
        // Fino al 15/9/2026 si apriva `embed.diagrams.net`, e il riquadro
        // restava bianco: il messaggio «carica» andava alla nostra origine.
        const overlay = document.createElement("div");
        overlay.className = "fm-drawio-overlay";
        overlay.innerHTML = `
            <iframe class="fm-drawio-iframe" allow="fullscreen" src="${srcEditorDrawio()}"></iframe>
        ...
        // I messaggi di drawio (proto=json):
        ...
        function onMsg(e) {
            ...
            if (data?.event === "init") {
                ...
                iframe.contentWindow.postMessage(JSON.stringify({ action: "load", xml: "" }), origineDellEditor(iframe));
...
async function createMappaWithBlob(form, mappaMode, preSubject, sectionKey = "") {
    ...
    if (mappaMode === "upload") {
        ...
        fd.set("xml", xml);
    } else if (mappaMode === "link") {
        fd.set("href", (form.elements.href?.value || "").trim());
    }
    ...
    const j = await r.json();
    if (!r.ok || !j.ok) throw new Error(messaggioErroreLink(j.error) || j.error || `HTTP ${r.status}`);
