...
import { CLASSE_BOZZA, eBozza, segnoBozzaHtml } from "./segno-bozza.js";
...
function init() {
    ...
    const popolaTutte = (giaRidisegnati = new Set()) => {
        for (const def of dbLoaderDefs()) {
            if (giaRidisegnati instanceof Set && giaRidisegnati.has(def.key)) continue;
            const sidepage = sidepageOf(def.key);
    ...
    //
    // 19/9/2026 — e ridisegna i pannelli già disegnati con un raggruppamento
    // diverso da quello della configurazione. Il pannello Verifiche partiva
    // dalla base del registro (per categoria, da /api/teacher/content) quando il
    // primo disegno arrivava prima della configurazione (per materia), e restava
    // così, perché popolaTutte salta i pannelli già popolati: niente 📥 su un
    // documento che l'aveva (analisi C-export-bodypt, misurato con la
    // configurazione ritardata).
    document.addEventListener("fm:sidebar-config-hydrated", () => {
        const ridisegnati = new Set();
        for (const def of pannelliDaRidisegnare(dbLoaderDefs(), sidepageOf, isVisible)) {
            ridisegnati.add(def.key);
            loadDbContent(def.key, def.type);
        }
        popolaTutte(ridisegnati);
    });
}

/**
 * I pannelli visibili già disegnati con un raggruppamento diverso da quello
 * che la configurazione chiede (`def.group`). Il raggruppamento disegnato sta
 * in `ul.fm-db-block[data-section-kind]`.
 *
 * @param {Array<{key: string, type: string, group: string}>} defs
 * @param {(key: string) => HTMLElement|null} pannelloDi
 * @param {(el: HTMLElement) => boolean} visibile
 * @returns {Array<{key: string, type: string, group: string}>}
 */
export function pannelliDaRidisegnare(defs, pannelloDi, visibile) {
    return defs.filter((def) => {
        const pannello = pannelloDi(def.key);
        if (!pannello || !visibile(pannello)) return false;
        const disegnato = pannello.querySelector("ul.fm-db-block[data-section-kind]")?.dataset?.sectionKind;
        return !!disegnato && disegnato !== def.group;
    });
}
...
async function loadDbContentBySubject(sidepageKey, type) {
    ...
    if (!ind || !cls) {
        // 20/9/2026 — si svuota il pannello, non lo si rade al suolo: dentro
        // c'è il bottone .js-edit-section, il segno di permesso che mette il
        // server ed è l'unica cosa da cui nascono la ✎ e il «+». Con
        // `innerHTML = ""` se ne andava anche quello, e al primo caricamento —
        // con i menu ancora vuoti — il pannello restava senza comandi finché
        // non si ricaricava la pagina. Il disegnatore per categoria lo
        // conservava già (vedi loadDbContentByCategory).
    ...
    // Il visitatore senza login chiede sempre per sezione: la configurazione
    // delle sezioni (allowedTypes) non la riceve, e per tipo gli arrivavano
    // anche le mappe del Laboratorio nella sidepage Mappe (16/9/2026).
    const _pubSb = isGuestSidebar();
    const _perSezione = _multiType || _pubSb;

    await Promise.all(subjs.map(async (subj) => {
        if (sidepage.__fmToken !== token) return;
        try {
            const qs  = new URLSearchParams(_perSezione
...
async function loadDbContentByCategory(def) {
    ...
    let rows = [];
    try {
        // ADR-027 — multi-tipo → per `section`; mono-tipo → per `type`. Il
        // visitatore senza login sempre per sezione (vedi il loader per materia).
        const _pubSb = isGuestSidebar();
        const _multiType = !!(def.allowedTypes && def.allowedTypes.length > 1) || _pubSb;
        ...
        // risposta è identica a quella che abbiamo già, e non manda il corpo.
        //
        // 2026-09-10 — questa cache la invalidiamo noi a ogni modifica, ed è
        // il motivo per cui funziona. Sotto ce n'era un'altra che non
        // potevamo invalidare: `Cache-Control: max-age` sulle stesse risposte
        // faceva rispondere il browser da sé, quindi l'`If-None-Match` qui
        // sopra non partiva nemmeno. Adesso quelle degli indirizzi del
        // docente dicono `max-age=0`.
...
function collectActiveSubjects() {
    ...
    // 19/9/2026 — per chi studia le voci sono tutte e sole le materie con
    // materiali (views/layout/app.php): anche un elenco vuoto è la risposta,
    // e ripiegare sul catalogo intero vorrebbe dire una richiesta per ogni
    // materia della scuola, tutte vuote.
    if (sel.dataset.fmMaterieDiChiStudia === "1") return unique;
 ...
 */
// Esportata per le prove: il riquadro dell'argomento e la chiave d'ordinamento
// sono due cose che si possono misurare solo guardando il markup prodotto.
export function itemLiHtml(type, ind, cls, subj, r, opts = {}) {
    ...
    // Il riquadro davanti al titolo mostra il `topic` della riga, che però non
    // vuol dire la stessa cosa per tutti i tipi: per un esercizio è il numero
    // dell'argomento («3.0»), per una verifica è il TITOLO DELL'ESERCIZIO da cui
    // nasce — la chiave con cui il sito ritrova «le verifiche di questo
    // argomento» (ContentStudyController::relatedVerificaHtml). Finché la
    // verifica non viene rinominata, topic e titolo coincidono e la barra
    // stampava la stessa frase due volte: il riquadro e poi il titolo a capo
    // (misurato il 20/9/2026 sulle verifiche 80, 81, 93, 99, 106).
    //
    // Regola: il riquadro si mostra solo quando dice qualcosa in più del
    // titolo. Una verifica rinominata continua quindi a dichiarare da quale
    // argomento viene, e l'esercizio continua a mostrare il suo numero.
    const topic = r.topic ? String(r.topic) : "";
    const titolo = String(r.title || r.topic || "");
    // 20/9/2026 — una verifica nasce da un esercizio e ne porta il numero: il
    // server lo risolve per titolo dentro la stessa classe (RigheDellaBarra) e
    // lo manda in `numero_esercizio`. Così le due voci parlano la stessa
    // lingua: «4.0» sull'esercizio «Derivate» e «4.0» sulla verifica che ne
    // viene. Dove il numero non c'è — l'esercizio cancellato, o due esercizi
    // con lo stesso titolo — vale la regola di prima: il riquadro si mostra
    // solo se dice qualcosa in più del titolo.
    const numero = r.numero_esercizio ? String(r.numero_esercizio) : "";
    const etichetta = numero || (topic !== titolo ? topic : "");
    const spiega = numero ? `Dall'esercizio «${topic}»` : `Argomento: ${topic}`;
    const num = topic && etichetta
        ? `<span class="fm-numarg" title="${escAttr(spiega)}">${esc(etichetta)}</span> `
        : "";
    ...
    // Una bozza si riconosce: stile diverso e occhio barrato (segno-bozza.js).
    const bozza = eBozza(r);
    const liCls = bozza ? ` class="${CLASSE_BOZZA}"` : "";
    const segno = bozza ? segnoBozzaHtml() : "";
    // `data-topic` c'è sempre, anche quando il riquadro non si vede: è la chiave
    // con cui sortBlockItems rimette in ordine dopo un inserimento o una
    // modifica. Prima quell'ordine si leggeva dal testo del riquadro, quindi
    // nasconderlo avrebbe voluto dire perderlo.
    return `<li${liCls} data-content-id="${r.id}" data-topic="${escAttr(topic)}" data-has-body-pt="${hasPt}"${rolesAttr}>${num}<a${aCls} href="${href}">${segno}${esc(titolo)}${subjSuffix}</a></li>`;
}
...
function topicOfLi(li) {
    // Prima l'attributo, che c'è anche quando il riquadro non si vede (una
    // verifica il cui topic ripete il titolo). Il testo del riquadro resta come
    // ripiego per il DOM disegnato dal server, che non porta `data-topic`.
    const attr = li.dataset?.topic;
...
export function sortBlockItems(ul) {
...
function surgicalUpdateItem({ sidepageKey, type, row }) {
    ...
    // Il 📥 e i ruoli li dice il server dopo il salvataggio; se la risposta
    // non li porta, la voce tiene quelli che aveva.
    const conservati = {
        has_body_pt: row.has_body_pt === undefined ? old.dataset.hasBodyPt === "1" : row.has_body_pt,
        doc_roles: row.doc_roles === undefined ? (old.dataset.docRoles || "") : row.doc_roles,
    };
    const fresh = liFromHtml(itemLiHtml(type, readSelect("sel-iis"), readSelect("sel-cls"), subj, { ...row, ...conservati }, opts));
...
/** La barra laterale dei visitatori senza login (sezioni pubbliche). */
function isGuestSidebar() {
