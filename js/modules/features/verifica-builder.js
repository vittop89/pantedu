...
function toast(msg, kind = "ok") {
    if (window.FM?.ToastManager?.show) {
        // show(tipo, titolo, testo): «ok»/«warn»/«err» li riconosce ToastManager.
        window.FM.ToastManager.show(kind, "Verifica", msg);
        return;
    }
    ...
    el.style.cssText =
        ...
        `font:14px/1.4 system-ui;max-width:360px;box-shadow:0 6px 18px rgba(0,0,0,.35);${kind === "err" ? "background:#c02a2a" : kind === "warn" ? "background:#c78a2a" : "background:#1d7a3a"}`;
