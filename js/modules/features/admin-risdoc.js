...
import {
    AMBITI, ETICHETTE_AMBITO, ambitoInBreve, pastiglieDellaRiga, chiLoVede, righeDeiPermessi,
    riassuntoDeiPermessi, filtraDocenti,
} from "./template-chi-lo-vede.js";
...
async function renderPending() {
    ...
    try {
        ...
        // apre il tab (Docente2 potrebbe aver fatto nuovi save dal precedente
    ...
    // doveva cliccare il tab in/out per refreshare la lista quando Docente2
    // pushava nuovi save.
    const refreshHeader = `
    ...
    try {
        ...
        const r = await fetch(`/api/admin/risdoc/pending/${pid}/approve`, {
            ...
            headers: {
                ...
                "X-Audit-Reason": auditReason(`Approvazione revisione #${pid}`, note),
            },
            body: fd.toString(),
        });
        const j = await r.json();
...
async function rejectPending(pid, card) {
    ...
    try {
        ...
        const r = await fetch(`/api/admin/risdoc/pending/${pid}/reject`, {
            ...
            headers: {
                "Content-Type": "application/x-www-form-urlencoded",
                "X-Audit-Reason": auditReason(`Rifiuto revisione #${pid}`, note),
            },
            body: fd.toString(),
        });
        const j = await r.json();
...
function renderTemplates() {
    ...
    const sections = Object.keys(byCat).sort().map((category) => {
        ...
        const rows = list.map((t) => {
            // 21/9/2026 — la colonna diceva «0 visib · 0 collab · 0 override ·
            // ok» con le parole della tabella del database, tutto dello stesso
            // azzurro. Adesso: si mostra quello che c'è, si tace quello che non
            // c'è, si colora solo ciò che chiede di essere guardato, e ogni
            // pastiglia porta la frase che la spiega (template-chi-lo-vede.js).
            const pastiglie = pastiglieDellaRiga(t).map((x) =>
                `<span class="fm-ar-pill${x.tono === "avviso" ? " fm-ar-pill--warn" : ""}"`
                + ` title="${esc(x.titolo)}">${esc(x.testo)}</span>`).join("");
            return `
                <tr data-template-id="${t.id}">
                    ...
                    <td class="c-stats">
                        <div class="fm-ar-inline-list">${pastiglie}</div>
                    </td>
                    <td class="c-act">
                ...
                <table class="fm-ar-tbl fm-ar-tbl--unified">
                    <thead>
                        <tr>
                            ...
                            <th class="c-stats" style="width:14em">Chi lo vede</th>
...
async function saveTemplateMeta(btn) {
    ...
    try {
        ...
        const r = await fetch(`/api/admin/risdoc/templates/${id}/meta`, {
            ...
            headers: {
                "Content-Type": "application/x-www-form-urlencoded",
                "X-Audit-Reason": auditReason(`Modifica scheda template #${id}`, `${num} ${name} [${cat}]`),
            },
            body: fd.toString(),
        });
...
async function renamePartition(btn) {
    ...
    try {
        ...
        const r = await fetch("/api/admin/risdoc/templates/rename-group", {
            ...
            headers: {
                "Content-Type": "application/x-www-form-urlencoded",
                "X-Audit-Reason": auditReason("Rinomina gruppo template", `da "${from}" a "${to}"`),
            },
            body: fd.toString(),
        });
        const j = await r.json().catch(() => ({}));
...
async function createTemplate(form) {
    ...
    try {
        ...
        const r = await fetch("/api/admin/risdoc/templates/create", {
            ...
            headers: {
                "Content-Type": "application/x-www-form-urlencoded",
                "X-Audit-Reason": auditReason("Creazione template istituzionale", `${category} ${numArg} ${argomento}`),
            },
            body: fd.toString(),
        });
        const j = await r.json().catch(() => ({}));
...
/**
 * Riscrive la pastiglia «chi lo vede» nella riga del modello, con quello che
 * il server ha appena risposto. Serve perche' ricaricare l'elenco intero
 * ridisegnerebbe la tabella e porterebbe via il pannello aperto.
 */
function aggiornaPastigliaDellaRiga(triggerBtn, risposta) {
    const riga = triggerBtn?.closest("tr");
    const pastiglia = riga?.querySelector(".fm-ar-pill");
    if (!pastiglia) return;
    const stato = ambitoInBreve({
        visibility_scope:   risposta.visibility_scope,
        scope_institute_id: risposta.scope_institute_id,
        scope_indirizzo:    risposta.scope_indirizzo,
        scope_classe:       risposta.scope_classe,
    });
    pastiglia.textContent = stato.testo;
    pastiglia.title = stato.titolo;
    // Niente colore: chi lo vede è uno stato, non un allarme né un successo.
    // L'unica pastiglia colorata della riga è quella che aspetta una risposta.
    pastiglia.classList.remove("fm-ar-pill--warn", "fm-ar-pill--ok");
}
...
async function openDetail(id, triggerBtn) {
    ...
    if (!opened) return;
    await riempiDettaglio(panel.querySelector("#fm-ar-detail"), id, triggerBtn);
}

/**
 * Legge il dettaglio dal server e lo disegna dentro il riquadro gia' aperto.
 *
 * Sta separato da `openDetail` per una ragione misurata il 21/9/2026: dopo il
 * salvataggio dell'ambito richiamavo `openDetail`, e siccome
 * `expandInlineUnderRow` fa da interruttore, il pannello si CHIUDEVA invece di
 * ricaricarsi — chi salvava se lo vedeva sparire sotto il dito. Qui si ridisegna
 * e basta, senza toccare la riga.
 */
async function riempiDettaglio(detail, id, triggerBtn) {
    if (!detail) return;
    ...
    const modello    = j.template || {};
    const istituti   = Array.isArray(j.istituti) ? j.istituti : [];
    const righe      = righeDeiPermessi(STATE.teachers, visibleIds, collabMap);

    // 21/9/2026 — questa parte si costruisce con i nodi, non con innerHTML: i
    // nomi dei docenti e degli Istituti sono dati, e `textContent` non li
    // interpreta (la regola semgrep `js-innerhtml-with-user-data` conta, e non
    // deve salire).
    ...
    const titolo = nodo("h3", "fm-ar-detail-h");
    titolo.style.margin = "12px 0";
    titolo.append("📋 Dettaglio template ", nodo("code", null, modello.code || ""));
    detail.appendChild(titolo);

    const aiuto = document.createElement("details");
    aiuto.className = "fm-ar-detail-help";
    // 21/9/2026 — il riquadro rispondeva a una domanda diversa da quella che si
    // fa chi apre la pagina. Diceva che cosa può fare uno che è già entrato, e
    // intanto entravano tutti lo stesso: chi entra lo decide l'ambito, non
    // queste tre colonne. Adesso lo dice, e nell'ordine giusto.
    aiuto.innerHTML = `
        <summary>ℹ Come si decide chi vede un modello</summary>
        <p>A decidere è l'<strong>ambito</strong>, il menu qui sotto: finché è aperto a tutti, il
        modello lo vedono tutti i docenti, spunte o no. Le spunte vengono dopo e lavorano su una
        persona per volta: <strong>aggiungono</strong> a quello che l'ambito già concede e non
        tolgono niente, quindi per togliere l'accesso a qualcuno si stringe l'ambito.</p>
        <ul>
            <li><strong>👁 Visibile</strong> — il docente trova il modello fra i suoi e se ne può fare una copia da cambiare come vuole; l'originale resta com'è. Se l'ambito lo comprende già, questa spunta non gli aggiunge niente.</li>
            <li><strong>✎ Collaboratore</strong> — modifica l'originale, quello che usano tutti: su questo modello ha le stesse mani di un amministratore. Lo vede anche senza la spunta 👁.</li>
            <li><strong>🛡 Revisione</strong> — si attiva su un collaboratore: le sue modifiche non entrano subito, restano in attesa e sei tu ad approvarle o a rifiutarle da «Modifiche in revisione». Senza questa spunta salva direttamente.</li>
        </ul>
        <p style="margin:6px 0 0 0;font-style:italic">Queste tre spunte le mette solo un amministratore: un collaboratore non può invitarne altri né togliersi la revisione.</p>
    `;
    detail.appendChild(aiuto);

    // ── Chi lo vede adesso: l'ambito del modello, che è quello che decide ──
    const cartaAmbito = nodo("div", "fm-ar-detail-card");
    cartaAmbito.appendChild(nodo("label", "fm-ar-detail-lbl", "Chi lo vede adesso"));
    cartaAmbito.appendChild(nodo("p", "fm-ar-chi-vede", chiLoVede(modello, istituti)));

    const rigaAmbito = nodo("div", "fm-ar-ambito");
    const scelta = document.createElement("select");
    scelta.className = "fm-input";
    scelta.setAttribute("aria-label", "Ambito di visibilità");
    for (const a of AMBITI) {
        const o = document.createElement("option");
        o.value = a;
        o.textContent = ETICHETTE_AMBITO[a];
        if ((modello.visibility_scope || "public") === a) o.selected = true;
        scelta.appendChild(o);
    }
    const perIstituto = document.createElement("select");
    perIstituto.className = "fm-input";
    perIstituto.setAttribute("aria-label", "Istituto");
    for (const i of istituti) {
        const o = document.createElement("option");
        o.value = String(i.id);
        o.textContent = i.name;
        if (Number(modello.scope_institute_id) === Number(i.id)) o.selected = true;
        perIstituto.appendChild(o);
    }
    const perCodice = document.createElement("input");
    perCodice.type = "text";
    perCodice.className = "fm-input";
    perCodice.value = String(modello.scope_indirizzo || modello.scope_classe || "");
    const salvaAmbito = nodo("button", "fm-btn fm-btn--sm", "Salva ambito");
    salvaAmbito.type = "button";
    const esitoAmbito = nodo("span", "fm-ar-detail-hint", "");

    const mostraQuelloCheServe = () => {
        const a = scelta.value;
        perIstituto.hidden = a !== "institute";
        perCodice.hidden = a !== "indirizzo" && a !== "classe";
        perCodice.placeholder = a === "classe" ? "codice della classe (es. 3)" : "codice dell'indirizzo (es. SCI)";
    };
    mostraQuelloCheServe();
    scelta.addEventListener("change", mostraQuelloCheServe);

    salvaAmbito.addEventListener("click", async () => {
        esitoAmbito.textContent = "Salvataggio…";
        try {
            const csrf = await fetchCsrf();
            const fd = new URLSearchParams({ _csrf: csrf, scope: scelta.value });
            let dettaglio = ETICHETTE_AMBITO[scelta.value] || scelta.value;
            if (scelta.value === "institute") {
                fd.append("scope_institute_id", perIstituto.value || "0");
                dettaglio += `: ${perIstituto.options[perIstituto.selectedIndex]?.textContent || ""}`;
            }
            if (scelta.value === "indirizzo") {
                fd.append("scope_indirizzo", perCodice.value.trim());
                dettaglio += `: ${perCodice.value.trim()}`;
            }
            if (scelta.value === "classe") {
                fd.append("scope_classe", perCodice.value.trim());
                dettaglio += `: ${perCodice.value.trim()}`;
            }
            const r2 = await fetch(`/api/admin/risdoc/templates/${id}/visibility-scope`, {
                method: "POST", credentials: "same-origin",
                headers: {
                    "Content-Type": "application/x-www-form-urlencoded",
                    "X-CSRF-Token": csrf,
                    // Cambiare chi vede un modello è una mutazione di un
                    // super-admin: senza motivazione il middleware risponde 400
                    // (in modalità `enforce`) o registra la riga col motivo
                    // buttato via (in `warn`). Chi legge il registro fra un anno
                    // deve capire che cosa è successo.
                    "X-Audit-Reason": auditReason(`Chi vede il modello #${id}`, dettaglio),
                },
                body: fd.toString(),
            });
            const j2 = await r2.json();
            if (!r2.ok || !j2.ok) throw new Error(j2.error || `HTTP ${r2.status}`);
            // La pastiglia della riga dice la cosa nuova senza ricaricare
            // l'elenco intero (che porterebbe via il pannello aperto).
            aggiornaPastigliaDellaRiga(triggerBtn, j2);
            // E il dettaglio si rilegge dal server: la frase deve dirla il dato
            // salvato, non quello che ho appena mandato.
            await riempiDettaglio(detail, id, triggerBtn);
        } catch (e) {
            esitoAmbito.textContent = `✗ ${e.message}`;
        }
    });

    rigaAmbito.append(scelta, perIstituto, perCodice, salvaAmbito, esitoAmbito);
    cartaAmbito.appendChild(rigaAmbito);
    detail.appendChild(cartaAmbito);

    // ── La matrice per docente ──
    const carta = nodo("div", "fm-ar-detail-card");
    const etichetta = nodo("label", "fm-ar-detail-lbl", "Chi in più, uno per volta");
    carta.appendChild(etichetta);
    carta.appendChild(nodo("p", "fm-ar-spunte-spiega",
        "Qui aggiungi un docente per volta a quello che l'ambito ha già deciso:"
        + " 👁 glielo fa vedere · ✎ gli fa modificare l'originale · 🛡 fa passare da te le sue"
        + " modifiche. Togliere una spunta non toglie il modello a chi lo vede già per l'ambito."));
    carta.appendChild(nodo("p", "fm-ar-riassunto", riassuntoDeiPermessi(righe)));

    const cerca = document.createElement("input");
    cerca.type = "search";
    cerca.className = "fm-input fm-ar-cerca-docente";
    cerca.placeholder = "Cerca docente per nome o username";
    cerca.setAttribute("aria-label", "Cerca docente");
    carta.appendChild(cerca);

    const scorrevole = nodo("div", "fm-ar-tbl-scroll");
    const tabella = nodo("table", "fm-ar-tbl");
    tabella.innerHTML = `
        <thead>
            <tr>
                <th>Docente</th>
    ...
    const casella = (ruolo, r, acceso, spenta) => {
        const td = document.createElement("td");
        const cb = document.createElement("input");
        cb.type = "checkbox";
        cb.dataset.role = ruolo;
        cb.dataset.tid = String(r.id);
        cb.checked = acceso;
        if (spenta) cb.disabled = true;
        td.appendChild(cb);
        return td;
    };

    const disegnaRighe = (elenco) => {
        corpo.replaceChildren();
        if (!elenco.length) {
            const tr = document.createElement("tr");
            const td = nodo("td", null, "Nessun docente con questo nome.");
            td.colSpan = 4;
            tr.appendChild(td);
        ...
        for (const r of elenco) {
            const tr = document.createElement("tr");
            const chi = document.createElement("td");
            chi.appendChild(nodo("span", "fm-ar-docente-nome", r.nome || r.username));
            if (r.nome) chi.appendChild(nodo("span", "fm-ar-docente-user", ` ${r.username}`));
            tr.appendChild(chi);
            if (r.sempre) {
                // Un amministratore vede e modifica tutto a prescindere: tre
                // caselle vuote raccontavano il contrario.
                const td = nodo("td", "fm-ar-sempre", "sempre tutto — è un amministratore");
                td.colSpan = 3;
                tr.appendChild(td);
            } else {
                tr.appendChild(casella("visible", r, r.visibile, false));
                tr.appendChild(casella("collab", r, r.collaboratore, false));
                tr.appendChild(casella("review", r, r.revisione, !r.collaboratore));
            }
            corpo.appendChild(tr);
        }
        // La revisione si può spuntare solo per un collaboratore.
        corpo.querySelectorAll('input[data-role="collab"]').forEach(cb => {
            cb.addEventListener("change", () => {
                const rev = corpo.querySelector(`input[data-role="review"][data-tid="${cb.dataset.tid}"]`);
                if (!rev) return;
    ...
    // Il filtro ridisegna le righe: quello che si è spuntato e non ancora
    // salvato deve restare spuntato quando si cerca altro, altrimenti cercare
    // un nome cancella il lavoro fatto.
    const spunteVive = () => {
        const vive = new Map();
        corpo.querySelectorAll("input[data-role]").forEach((cb) => {
            vive.set(`${cb.dataset.role}:${cb.dataset.tid}`, cb.checked);
        });
        return vive;
    };
    cerca.addEventListener("input", () => {
        const vive = spunteVive();
        disegnaRighe(filtraDocenti(righe, cerca.value));
        corpo.querySelectorAll("input[data-role]").forEach((cb) => {
            const v = vive.get(`${cb.dataset.role}:${cb.dataset.tid}`);
            if (v !== undefined) cb.checked = v;
        });
    });

    scorrevole.appendChild(tabella);
    carta.appendChild(scorrevole);

    const salva = nodo("button", "fm-btn fm-btn--sm fm-btn--primary", "💾 Salva matrice");
    salva.type = "button";
    salva.dataset.action = "save-matrix";
    salva.dataset.id = String(id);
    salva.addEventListener("click", () => saveMatrix(id));
    carta.appendChild(salva);
    detail.appendChild(carta);
...
async function saveMatrix(id) {
    ...
    if (visOn.length) {
        ...
        await fetch(`/api/admin/risdoc/templates/${id}/visibility`, {
            ...
            headers: {
                "Content-Type": "application/x-www-form-urlencoded",
                "X-Audit-Reason": auditReason(`Template #${id} reso visibile`, `docenti: ${visOn.join(", ")}`),
            },
            body: fd.toString(),
        });
    }
    if (visOff.length) {
        ...
        await fetch(`/api/admin/risdoc/templates/${id}/visibility`, {
            ...
            headers: {
                "Content-Type": "application/x-www-form-urlencoded",
                "X-Audit-Reason": auditReason(`Template #${id} nascosto`, `docenti: ${visOff.join(", ")}`),
            },
            body: fd.toString(),
        });
    }
        ...
        await fetch(`/api/admin/risdoc/templates/${id}/collaborators`, {
            ...
            headers: {
                ...
                "X-Audit-Reason": auditReason(
                    `Collaboratori template #${id}`,
                    `aggiunti: ${colAdd.join(", ") || "nessuno"}; rimossi: ${colDel.join(", ") || "nessuno"}`
                ),
            },
            body: fdc.toString(),
        });
...
async function renderDrift() {
    STATE.panel.innerHTML = `<div>Caricamento…</div>`;
    ...
    STATE.panel.innerHTML = `
        <h3 style="margin:12px 0">⚠ Copie non allineate (${(j.drifted || []).length})</h3>
        <p style="font-size:12px; color:#64748b">Quello che un docente ha salvato nella sua copia partendo da una
        versione del modello precedente a quella di adesso: chi la usa non ha le ultime correzioni. Non si rompe
        niente, e la copia torna allineata quando il docente risalva quel pezzo.</p>
        <table class="fm-ar-tbl">
            <thead><tr><th>Modello</th><th>Docente</th><th>Tipo</th><th>File</th><th>Versione della copia</th><th>Versione di adesso</th><th>Salvata il</th></tr></thead>
            <tbody>${rows || '<tr><td colspan="7" style="text-align:center; color:#94a3b8">Nessuna copia fuori allineamento.</td></tr>'}</tbody>
        </table>
    `;
}
