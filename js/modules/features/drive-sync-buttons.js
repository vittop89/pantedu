...
import { driveAcceso, motivoDelFermo } from "./drive-stato.js";
...
function injectGlobalSync() {
    ...
    if (!driveAcceso()) {
        // ADR-038 — Drive spento o guasto sull'installazione: il comando di
        // Drive non c'è, le altre destinazioni sì.
    ...
    btn.addEventListener("click", async (e) => {
        ...
        async function drainEndpoint(endpoint, label) {
            ...
            for (let batch = 0; batch < 200 && !aborted && !btn._fmAborted; batch++) {
                ...
                // ADR-038 — il server ha fermato il lotto per una ragione che vale
                // per tutto il docente (da ricollegare, non collegato, Drive
                // spento): gli altri file fallirebbero allo stesso modo.
                if (r.fermato) {
                    const msg = motivoDelFermo(r.fermato, r.motivo);
                    logLine(`⚠ ${msg}`, "error");
                    tally.fermato = msg;
                    btn._fmAborted = true;
                    aborted = true;
        ...
        try {
            ...
            const esito = tally.fermato ? `Fermata: ${tally.fermato}` : "Fine.";
            const summary = `${esito} ${tally.ok} OK · ${tally.skip} skip · ${tally.error} errori${orphanPart}${delPart} · ${elapsed}s`;
            const andataMale = tally.error || tally.fermato;
            logLine(summary, andataMale ? "error" : "ok");
            ensureToast(`☁ ${  summary}`, andataMale ? "error" : "success");
            persistSyncLog("drive", andataMale ? "error" : "ok", summary, tally);
