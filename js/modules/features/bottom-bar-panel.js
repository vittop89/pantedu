 ...
 *
 * Qui si misura anche l'altezza della barra e la si scrive in
 * `--fm-bottom-bar-h`: il foglio di stile la usa per riservare lo spazio in
 * fondo al contenuto, così la barra non copre quel che ci finisce sotto
 * (voce 54 del debito, 2026-09-07).
 *
...
/**
 * Scrive l'altezza della barra in `--fm-bottom-bar-h`.
 *
 * Non è un numero fisso: sugli schermi stretti i link vanno a capo e la barra
 * cresce. Si rimisura quando cambia, così lo spazio riservato la segue.
 */
function misuraAltezza() {
    ...
    const scrivi = () => {
        ...
        if (altezza > 0) {
            document.documentElement.style.setProperty("--fm-bottom-bar-h", `${altezza}px`);
        }
    };
    scrivi();
    if (barra.dataset.fmBbMisurata === "1") return;   // idempotente
    barra.dataset.fmBbMisurata = "1";
...
export const BottomBarPanel = { init, misuraAltezza };
