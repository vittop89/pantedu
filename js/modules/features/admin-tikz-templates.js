import { escAttr, fetchCsrf } from "../core/dom-utils.js";
import { auditReason } from "../core/audit-reason.js";
import { caricaEntry } from "../core/carica-entry.js";
// Il dialogo di conferma scuro è uno (23/9/2026, A-21): qui ce n'era una copia.
import { confirmDialog } from "../editor/tex-dropdown-helpers.js";
/**
...
// ─────────────────────── lazy-load editors ───────────────────────
const ensureCm6Editor = () => caricaEntry("js/entries/tex-element-editor.js", {
    pronto: () => !!window.FM?.openTexElementEditor,
});

const ensureFiller = () => caricaEntry("js/entries/tikz-template-filler.js", {
    pronto: () => !!window.FM?.openTemplateFiller,
});

// ─────────────────────── API calls ───────────────────────
// `motivo` va in X-Audit-Reason: i modelli TikZ globali valgono per tutti i
// docenti, e dal 23/9/2026 (A-69) le rotte /tikz/*-element lo pretendono.
async function apiPost(url, payload, motivo) {
    // 2026-09-05 — /admin/templates non emette il meta csrf-token: senza
    // fetchCsrf() ogni salvataggio prendeva 403 csrf_invalid.
...
async function openNewElement(groupKey = "") {
    ...
    window.FM.openTexElementEditor({
        ...
        onSave: async ({ type, label, code, groupName, newGroup }) => {
            ...
            }, auditReason("Nuovo modello TikZ globale", `${label} nel gruppo ${newGroup || groupName}`));
    ...
    window.FM.openTexElementEditor({
        ...
        onSave: async ({ type, label, code }) => {
            const res = await apiPost("/tikz/edit-element", {
                ...
                elementLabel: it.label || "",
                elementType: type, label, code,
            }, auditReason("Modifica di un modello TikZ globale", `${it.label || label} nel gruppo ${groupKey}`));
            if (res?.success === true || res?.ok === true) {
...
async function deleteElement(groupKey, it) {
    ...
    }, auditReason("Eliminazione di un modello TikZ globale, confermata", `${it.label || "(senza nome)"} dal gruppo ${groupKey}`));
    if (res?.success === true || res?.ok === true) {
...
async function deleteGroup(groupKey, name, count) {
    ...
    }, auditReason("Eliminazione di un intero gruppo di modelli TikZ globali, confermata", `${name}, ${count} elementi`));
    if (res?.success === true || res?.ok === true) {
...
async function renameGroup(groupKey, items) {
        ...
        dlg.innerHTML = `
            <div style="background:#1e1e1e;color:#ddd;border:1px solid #444;border-radius:8px;box-shadow:0 12px 48px rgba(0,0,0,0.6);min-width:420px;max-width:90vw;overflow:hidden">
                ...
                <div style="padding:18px 16px;display:flex;flex-direction:column;gap:10px">
                    ...
                    <div style="font-size:11px;color:#888">Il nome resta come lo scrivi, spazi compresi; il prefisso "gruppo-" si aggiunge da solo.</div>
                </div>
    ...
    const res = await apiPost("/tikz/edit-element", {
        ...
        label: first.label || "",
        code: first.content || "",
    }, auditReason("Rinomina di un gruppo di modelli TikZ globali", `da ${currentName} a ${newName}`));
    if (res?.success === true || res?.ok === true) {
