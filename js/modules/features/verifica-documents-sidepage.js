...
import { haZona } from "../core/zone-di-accesso.js";
...
/**
 * Generazione dell'elenco mostrato: cresce a ogni ridisegno e a ogni
 * cancellazione.
 *
 * `renderInto` chiede l'elenco e lo aspetta: nel frattempo l'utente può aver
 * cancellato una verifica. Senza questo contatore la risposta partita PRIMA
 * della cancellazione arrivava DOPO e rimetteva in pagina la riga appena
 * tolta — il server aveva ragione, la pagina no. Sul portatile non si vedeva
 * quasi mai (la risposta torna in pochi millisecondi); su un server lento
 * succedeva ogni volta, ed è così che l'ha trovata la suite in integrazione
 * continua (2026-09-07, voce 73 del debito).
 *
 * Chi riprende dopo un `await` confronta la propria generazione con questa: se
 * è cambiata, quel che ha in mano è vecchio e si butta.
 */
let generazioneElenco = 0;

/**
 * Il ridisegno attualmente in corso, per poterlo aspettare.
 *
 * La generazione da sola non basta: protegge chi deve ancora riprendere dopo
 * l'attesa, non chi ha già ripreso e sta inserendo le righe in quel momento.
 * Le due operazioni — «togli la riga cancellata» e «rimetti l'elenco letto un
 * attimo fa» — finivano l'una dentro l'altra, e vinceva la seconda: la riga
 * spariva e ricompariva due millisecondi dopo. Chi cancella aspetta qui che il
 * ridisegno finisca, e solo allora toglie la riga.
 */
let ridisegnoInCorso = Promise.resolve();

/**
 * Phase 25.Q.15 — l'elenco da chiedere, secondo la zona di chi guarda:
 *   - zona «teacher» (docente, amministratore) → /api/verifica/list
 *     (le proprie, nella scuola attiva): è la zona che quella rotta richiede;
 *   - gli altri (studente, ospite) → /api/study/verifica/list (ADR-037, fase 3:
 *     le verifiche PUBBLICATE per la sua classe nella sua scuola; la
 *     condivisione fra colleghi non conta).
 *
 * 2026-09-14 — prima si confrontava il ruolo con "admin", ma in database
 * l'amministratore è `administrator`: vedeva l'elenco dello studente, con i
 * pulsanti di modifica del docente accanto.
 */
export function elencoDelleVerifiche() {
    return haZona("teacher") ? "/api/verifica/list" : "/api/study/verifica/list";
}
...
async function fetchVerifiche() {
    try {
        ...
        const base = elencoDelleVerifiche();
...
/** Ridisegna tenendo traccia dell'operazione, così chi cancella può aspettarla. */
function ridisegna(panel) {
    ridisegnoInCorso = renderInto(panel).catch(() => { /* il ridisegno non deve rompere chi lo aspetta */ });
    return ridisegnoInCorso;
}

async function renderInto(panel) {
    if (!panel) return;
    const miaGenerazione = ++generazioneElenco;
    ...
    skeleton.remove();
    if (!data) return;
    // Qualcuno ha cancellato o ha chiesto un altro ridisegno mentre aspettavamo:
    // questo elenco descrive un passato, e rimetterlo in pagina farebbe
    // ricomparire righe che non ci sono più.
    if (miaGenerazione !== generazioneElenco) return;
    ...
    // 19/9/2026 — solo a chi può modificare, come ✎/➕ in buildBlock: a chi
    // studia le verifiche pubblicate per la classe arrivano senza comandi.
    removeInlineItemActions(panel);
    if (document.body?.dataset?.fmCanEdit === "1") {
...
function onVerificaSaved() {
    const panel = document.getElementById("fm-sp-verif")
               || document.querySelector('.fm-sb-panel[data-sidepage="verif"]');
    if (panel && panel.style.display !== "none") ridisegna(panel);
}
    ...
    document.addEventListener("click", async (e) => {
        ...
        if (delBtn) {
            ...
            try {
                ...
                if (r.ok && j.ok) {
                    // Invalida le letture partite prima: la loro risposta
                    // descrive un elenco in cui questa verifica c'è ancora.
                    generazioneElenco++;
                    window.FM?.ToastManager?.show?.("success", "Verifica eliminata",
                        `${title} cancellata.`, 3000);
                    // E aspetta chi sta già disegnando: se togliessimo la riga
                    // adesso, quello finirebbe di inserire le sue — vecchie di
                    // un attimo — e la rimetterebbe in pagina.
                    await ridisegnoInCorso;
                    ...
                    //
                    // Si rilegge dal DOM per identificativo invece di usare il
                    // `li` del clic: un ridisegno appena concluso l'ha
                    // sostituito con un altro elemento, e quello vecchio non è
                    // più in pagina (togliere lui non toglierebbe niente).
                    const pannello = document.getElementById("fm-sp-verif")
                                  || document.querySelector('.fm-sb-panel[data-sidepage="verif"]')
                                  || document;
                    const righe = pannello.querySelectorAll(
                        `li[data-fm-content-kind="verifica"][data-content-id="${id}"]`);
                    const ul = (righe[0] || li).closest("ul.fm-db-block");
                    righe.forEach((n) => n.remove());
                    li.remove();
