...
const BASE_SIDEPAGES = ({
    ...
    eser:   { key: "eser",   panel: "fm-sp-eser",   loader: "db",
              type: "esercizio", group: "subject", customCategories: false },
    // Verifiche: per materia, come la configurazione del server in sviluppo e
    // in produzione (sidebar_sections, 19/9/2026). Il raggruppamento per
    // categoria (Compito in classe, Recupero, Orale, …, Phase 24.72) resta
    // disponibile quando la configurazione lo chiede: dopo l'idratazione
    // db-sidepage ridisegna il pannello se il raggruppamento è cambiato.
    // Prima la base diceva «category»: se il pannello si disegnava prima della
    // configurazione restava per categoria, letto da /api/teacher/content, e
    // perdeva il 📥 (analisi C-export-bodypt).
    verif:  { key: "verif",  panel: "fm-sp-verif",  loader: "db",
              type: "verifica",  group: "subject", customCategories: true,
