...
import { esc, fetchCsrf, wafFetch } from "../core/dom-utils.js";
...
// `meta[name="csrf-token"]` non esiste in nessuna vista: qui partiva una
// stringa vuota, e /api/latex-shortcuts/* ha il middleware csrf.
async function csrf() {
    return await fetchCsrf();
}

/** `motivo`, quando c'è, va in X-Audit-Reason: lo pretende il riferimento
 *  dell'amministratore (/api/admin/latex-shortcuts, dal 23/9/2026, A-69), non
 *  le personalizzazioni del docente. */
async function postJson(url, body, motivo = "") {
    const headers = { "Content-Type": "application/json", "X-CSRF-Token": await csrf() };
...
async function mount(container, opts = {}) {
    ...
    let groups = {};
    try {
        const res = await wafFetch(url, { credentials: "same-origin" });
        const j = await res.json().catch(() => ({}));
        groups = (j && j.groups) || {};
    } catch (e) {
        ...
        if (e.target.closest(".fm-sc-save") && row) {
            ...
            try {
                await postJson("/api/admin/latex-shortcuts", { groups: out },
                    auditReason("Salvataggio del riferimento delle scorciatoie LaTeX per tutti i docenti"));
                reloadEngine();
