...
export function hydrate() {
    ...
    _hydrating = (async () => {
        try {
            // L'endpoint è solo dei docenti: a studenti e ospiti (anche quelli
            // entrati con la credenziale di classe, che hanno una barra completa)
            // risponderebbe 401 o 403 e sporcherebbe la console (14/9/2026).
            if (!puoModificareEtichette()) return;
...
// Auto-idratazione una volta a load (deferita), solo dove c'è una sidebar
// autenticata: senza sidebar (login, shell modale) o da ospite il GET
// risponderebbe 401 e sporcherebbe la console (2026-09-05). Re-idrata a ogni
// navigazione SPA (fm:navigated) NO: basta una volta + l'evento di reload
// gestisce i pannelli aperti.
function hydrateIfSidebar() {
    const sidebar = document.querySelector("nav.sidebar");
    if (!sidebar || sidebar.dataset.fmGuest === "1" || !puoModificareEtichette()) return;
    hydrate();
}

/**
 * Chi l'endpoint lascia passare: la zona «teacher» di app/Config/roles.php, che
 * la rotta richiede. Si chiede la zona che il server scrive sul body, non il
 * ruolo: fino al 14/9/2026 qui c'era una lista di ruoli scritta a mano (vedi
 * ../core/zone-di-accesso.js).
 */
function puoModificareEtichette() {
    return haZona("teacher");
}
if (typeof document !== "undefined") {
    if (document.readyState === "loading") {
        document.addEventListener("DOMContentLoaded", hydrateIfSidebar);
    } else {
        queueMicrotask(hydrateIfSidebar);
    }
}
