...
export async function openDrawioEditor({ contentId, mode = "edit" }) {
    ...
    return new Promise((resolve, reject) => {
        ...
        // La ✕ chiude senza salvare, ma non annulla un salvataggio già fatto:
        // da quando «Salva» lascia aperto il riquadro, chiudere con la ✕ dopo
        // aver salvato è la strada normale. Dicendo `saved: false` chi ha
        // aperto l'editor non aggiornava la barra, e la voce restava vecchia.
        ...
        async function onMsg(e) {
            ...
            if (data?.event === "init") {
                // Destinatario esplicito, non `"*"` (drawio-app.js).
                iframe.contentWindow.postMessage(JSON.stringify({
                    action: "load",
                    xml: xml || "",
                    autosave: 0,
                }), origineDellEditor(iframe));
                if (mode === "view") {
                ...
                try {
                    ...
                    if (mode === "edit") {
                        ...
                        status.textContent = `✅ Salvataggio riuscito (v${result.map_version})`;
                    ...
                    saveCompleted = true;
                    // I tre pulsanti dell'editor mandano lo stesso evento
                    // «save», e solo «Salva ed esci» ci mette `exit: true`
                    // (drawio, modalità embed con saveAndExit=1). Chi preme
                    // «Salva» sta salvando a metà lavoro: vede che è andata e
                    // continua. Prima si chiudeva in tutti e due i casi.
                    if (data.exit === true) {
                        console.log("[drawio] chiusura chiesta con «Salva ed esci»");
                        teardown();
...
function buildOverlay({ mode, contentId }) {
    ...
    const src = srcEditorDrawio({ solaLettura: mode === "view" });
