...
// Phase 25.Q.16 — switch endpoint header-page in base alla zona:
//   zona «teacher» (docente, amministratore)
//                 → /api/teacher/header-page.json (proprio template)
//   gli altri (studente, ospite)
//                 → /api/study/header-page.json (template del docente
//                   di riferimento dell'istituto)
// Override fetch() per redirigere automaticamente lo studente.
//
// 2026-09-14 — la condizione confrontava il ruolo con "admin", ma in database
// l'amministratore è `administrator`: riceveva l'intestazione da studente. Ora
// si chiede la zona (vedi ../core/zone-di-accesso.js).
(function patchHeaderPageEndpointForRole() {
    if (typeof window === "undefined" || !window.fetch) return;
    const origFetch = window.fetch.bind(window);
    window.fetch = function (input, init) {
        if (!haZona("teacher") && typeof input === "string"
