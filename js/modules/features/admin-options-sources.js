...
const setStatus = (s) => { const el = byId("fm-osa-status"); if (el) el.textContent = s; };
...
async function loadFileList() {
    ...
    try {
        ...
        const dsSel = byId("fm-osa-dataset");
...
function applyFilter() {
    const ds = byId("fm-osa-dataset").value;
    const ind = (byId("fm-osa-ind").value || "").trim().toLowerCase();
    const cls = (byId("fm-osa-cls").value || "").trim().toLowerCase();
    const mat = (byId("fm-osa-mat").value || "").trim().toLowerCase();
    const q = (byId("fm-osa-search").value || "").trim().toLowerCase();
    ...
    const cnt = byId("fm-osa-count");
    if (cnt) cnt.textContent = String(matched.length);
}

async function loadFile() {
    const path = byId("fm-osa-file").value;
    if (!path) return;
    ...
    try {
        ...
        byId("fm-osa-raw").value = j.content || "";
        applyMode();
        byId("fm-osa-save").disabled = false;
        byId("fm-osa-raw-toggle").disabled = !structuredOk; // se non strutturato resta raw
...
function applyMode() {
    const struct = byId("fm-osa-structured");
    const addG = byId("fm-osa-add-group");
    const raw = byId("fm-osa-raw");
    ...
    if (rawMode) {
        ...
        byId("fm-osa-raw-toggle").textContent = "▦ Editor strutturato";
    } else {
        struct.hidden = false; addG.style.display = ""; raw.hidden = true;
        byId("fm-osa-raw-toggle").textContent = "{ } Raw JSON";
        renderStructured();
    }
}

function renderStructured() {
    const wrap = byId("fm-osa-structured");
    wrap.innerHTML = "";
...
function toggleRaw() {
    if (!rawMode) {
        ...
        byId("fm-osa-raw").value = JSON.stringify(model, null, 4);
        rawMode = true;
    } else {
        // raw → structured: prova a parsare
        try {
            const parsed = JSON.parse(byId("fm-osa-raw").value);
...
async function save() {
    let content;
    if (rawMode) {
        try { content = JSON.stringify(JSON.parse(byId("fm-osa-raw").value)); }
        catch (e) { notify(`JSON non valido: ${e.message}`, "error"); return; }
    } else {
...
export function initOptionsSourcesAdmin() {
    ...
    inited = true;
    byId("fm-osa-add-group").style.display = "none"; // mostrato solo dopo il load (editor strutturato)
    loadFileList();
    byId("fm-osa-load").addEventListener("click", loadFile);
    byId("fm-osa-raw-toggle").addEventListener("click", toggleRaw);
    byId("fm-osa-add-group").addEventListener("click", () => { model.push({ titolo: "", contenuti: [{ label: "", checked: false }] }); renderStructured(); });
    byId("fm-osa-save").addEventListener("click", save);
    ...
    byId("fm-osa-filter-reset")?.addEventListener("click", () => {
        ["fm-osa-dataset", "fm-osa-ind", "fm-osa-cls", "fm-osa-mat", "fm-osa-search"].forEach((id) => { const el = byId(id); if (el) el.value = ""; });
        applyFilter();
    });
}
