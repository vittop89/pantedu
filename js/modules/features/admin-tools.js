...
// Escape HTML: quello di core/dom-utils.js (23/9/2026, A-21).
import { esc } from "../core/dom-utils.js";

const csrf = () => document.getElementById("fm-tools-csrf")?.value || "";

/**
 * POST del pannello. `motivo` va nell'intestazione `X-Audit-Reason`: dal
 * 23/9/2026 (A-69) ogni mutazione di queste rotte la pretende, e senza
 * risponde 400 `audit_reason_required`. Solo il calcolo dell'hash, che non
 * scrive niente, la chiama senza.
 */
const post = async (url, body = {}, motivo = "") => {
...
/**
 * Il motivo che l'amministratore ha scritto per aprire l'elenco degli utenti
 * (obbligatorio, vedi searchUsers): è anche il perché delle azioni che fa
 * sulle righe di quell'elenco.
 */
const motivoDellElenco = () => (document.getElementById("fm-users-reason")?.value || "").trim();
...
// 2026-09-04 — il motivo dell'accesso e' obbligatorio: l'elenco degli utenti
// contiene dati personali e il server lo rifiuta senza (reason_required).
// Si ricorda per la sessione, cosi' non va riscritto a ogni ricerca.
const REASON_KEY = "pantedu.admin.users.reason";
(() => {
    const el = document.getElementById("fm-users-reason");
    if (!el) return;
    try { const saved = sessionStorage.getItem(REASON_KEY); if (saved && !el.value) el.value = saved; } catch { /* storage assente */ }
    el.addEventListener("keydown", (e) => { if (e.key === "Enter") searchUsers(); });
})();

async function searchUsers() {
    const out = document.getElementById("fm-users-result");
    const reasonEl = document.getElementById("fm-users-reason");
    const reason = (reasonEl?.value || "").trim();
    if (reason.length < 10) {
        out.innerHTML = `<p class="fm-alert fm-alert--error">Indica il motivo dell'accesso (almeno 10 caratteri): l'elenco degli utenti contiene dati personali e ogni consultazione finisce nel registro degli accessi privilegiati.</p>`;
        reasonEl?.focus();
        return;
    }
    try { sessionStorage.setItem(REASON_KEY, reason); } catch { /* storage assente */ }
    ...
    // 15/9/2026 — la pagina dice chi guarda e quali ruoli si assegnano nello
    // scenario (App\Support\RuoliNelPannelloUtenti). Sulla propria riga niente
    // azioni: il server le rifiuta comunque. Un ruolo che da qui non si
    // assegna (l'amministratore di istituto) si mostra, non si cambia.
    const pannello = document.querySelector('[data-panel="users"]');
    const io = Number(pannello?.dataset.io || 0);
    /** @type {string[]} */
    let assegnabili = [];
    try { assegnabili = JSON.parse(pannello?.dataset.ruoli || "[]"); } catch { assegnabili = []; }
    const cellaRuolo = (u) => (Number(u.id) === io || !assegnabili.includes(u.role))
        ? `<code>${esc(u.role)}</code>`
        : `<select class="fm-input fm-user-role" data-id="${u.id}" aria-label="Ruolo di ${esc(u.username)}" style="font-size:11px">
              ${assegnabili.map(ruolo => `<option value="${ruolo}" ${u.role === ruolo ? "selected" : ""}>${ruolo}</option>`).join("")}
           </select>`;
    const cellaAzioni = (u) => Number(u.id) === io
        ? `<span class="fm-muted" title="Non si disattiva, non si elimina e non si toglie il ruolo al proprio account">sei tu</span>`
        ...
        <table class="fm-tools-table">
          ...
          <tbody>${rows.map(u => `
            <tr data-id="${u.id}">
              ...
              <td class="fm-row-actions">${cellaAzioni(u)}</td>
            </tr>`).join("")}</tbody>
        </table>`;
    bindUserRowActions();
}
function bindUserRowActions() {
    document.querySelectorAll(".fm-user-toggle").forEach((btn) => {
        btn.addEventListener("click", async () => {
            const azione = btn.dataset.active === "1" ? "Riattivazione" : "Disattivazione";
            const r = await post(`/api/admin/users/${btn.dataset.id}/active`, { active: btn.dataset.active },
                auditReason(`${azione} dell'utente #${btn.dataset.id}`, motivoDellElenco()));
    ...
    document.querySelectorAll(".fm-user-role").forEach((sel) => {
        sel.addEventListener("change", async () => {
            const r = await post(`/api/admin/users/${sel.dataset.id}/role`, { role: sel.value },
                auditReason(`Cambio di ruolo dell'utente #${sel.dataset.id} in ${sel.value}`, motivoDellElenco()));
            if (!r.body.ok) return alert(`Errore: ${  r.body.error || r.status}`);
            window.dispatchEvent(new CustomEvent("fm:user-changed"));
        });
    });
    document.querySelectorAll(".fm-user-del").forEach((btn) => {
        btn.addEventListener("click", async () => {
            ...
            const r = await post(`/api/admin/users/${btn.dataset.id}/delete`, {},
                auditReason(`Eliminazione definitiva dell'utente #${btn.dataset.id}, confermata nel pannello`, motivoDellElenco()));
            if (!r.body.ok) return alert(`Errore: ${  r.body.error || r.status}`);
            window.dispatchEvent(new CustomEvent("fm:user-changed"));
            searchUsers();
        });
    ...
    document.querySelectorAll(".fm-reg-app").forEach((btn) => {
        btn.addEventListener("click", async () => {
            ...
            const r = await post(`/admin/registrations/${btn.dataset.id}/approve`, {},
                auditReason(`Approvazione della registrazione #${btn.dataset.id}, confermata nel pannello`));
            if (!r.body.ok) return alert(`Errore: ${  r.body.error || r.status}`);
    ...
    document.querySelectorAll(".fm-reg-rej").forEach((btn) => {
        btn.addEventListener("click", async () => {
            ...
            const r = await post(`/admin/registrations/${btn.dataset.id}/reject`, { reason },
                auditReason(`Rifiuto della registrazione #${btn.dataset.id}`, reason));
            if (!r.body.ok) return alert(`Errore: ${  r.body.error || r.status}`);
            window.dispatchEvent(new CustomEvent("fm:reg-changed"));
            loadRegistrations();
            loadNotifications();
...
function bindSecConfigForm() {
    ...
    form.addEventListener("submit", async (e) => {
        ...
        const r = await post("/api/admin/security/config", body,
            auditReason("Modifica delle soglie delle anomalie di sicurezza dal pannello Strumenti"));
        if (!r.body.ok) return alert(`Errore: ${  r.body.error || r.status}`);
...
function bindSecActions() {
    ...
    document.querySelectorAll(".fm-anom-block-ip").forEach((btn) => {
        btn.addEventListener("click", async () => {
            // Niente indirizzo nel motivo: nei registri di audit l'IP sta solo
            // come hash (RequestFingerprint). L'indirizzo bloccato lo tiene
            ...
            }, auditReason(`Blocco di un indirizzo segnalato per accessi eccessivi alla sezione ${btn.dataset.section}`));
            if (!r.body.ok) return alert(`Errore: ${  r.body.error || r.status}`);
    ...
    document.querySelectorAll(".fm-anom-block-cred").forEach((btn) => {
        btn.addEventListener("click", async () => {
            ...
            }, auditReason("Blocco di una credenziale segnalata come condivisa fra più indirizzi"));
            if (!r.body.ok) return alert(`Errore: ${  r.body.error || r.status}`);
            window.dispatchEvent(new CustomEvent("fm:sec-changed"));
            loadSecurity();
            loadNotifications();
    ...
    document.querySelectorAll(".fm-sec-unblock-cred").forEach((btn) => {
        btn.addEventListener("click", async () => {
            const r = await post("/api/admin/security/credentials/unblock", { username: btn.dataset.username },
                auditReason("Sblocco di una credenziale dal pannello Strumenti"));
            if (!r.body.ok) return alert(`Errore: ${  r.body.error || r.status}`);
            window.dispatchEvent(new CustomEvent("fm:sec-changed"));
            loadSecurity();
            loadNotifications();
    ...
    document.querySelectorAll(".fm-sec-unblock-ip").forEach((btn) => {
        btn.addEventListener("click", async () => {
            ...
            }, auditReason(`Sblocco di un indirizzo sulla sezione ${btn.dataset.section || "globale"}`));
            if (!r.body.ok) return alert(`Errore: ${  r.body.error || r.status}`);
            window.dispatchEvent(new CustomEvent("fm:sec-changed"));
            loadSecurity();
            loadNotifications();
...
// 23/9/2026 — il registro degli accessi non porta più l'id di sessione, ma
// un'impronta che raggruppa le richieste della stessa sessione senza
// permettere di adottarla (AccessLogger::improntaDellaSessione, A-64): la
// colonna «Sessione» mostra quella. Lo stesso giorno: la tabella leggeva
// `logs` e `action_type`, due campi che /admin/access-log non ha mai mandato
// (manda `recent` e `action`), e diceva «Nessun record.» anche col registro
// pieno.
...
async function loadLogs() {
    ...
    if (!r.body.ok) { out.innerHTML = `<p class="fm-alert fm-alert--error">${esc(r.body.error || "Errore")}</p>`; return; }
    const rows = r.body.recent || [];
    if (!rows.length) { out.innerHTML = `<p class="fm-muted">Nessun record.</p>`; return; }
    out.innerHTML = `
        <table class="fm-tools-table">
          <thead><tr><th>Quando</th><th>Utente</th><th>Azione</th><th>IP</th><th>Sessione</th><th>Linkref</th></tr></thead>
          <tbody>${rows.map(r => `
            <tr><td><code>${esc(r.timestamp || "")}</code></td>
                <td><code>${esc(r.username || "")}</code></td>
                <td>${esc(r.action || "")}</td>
                <td><code>${esc(r.ip_address || "")}</code></td>
                <td><code title="Impronta: raggruppa le richieste della stessa sessione">${esc(r.session_fingerprint || "")}</code></td>
                <td>${esc(r.linkref || "")}</td></tr>
          `).join("")}</tbody></table>`;
}
