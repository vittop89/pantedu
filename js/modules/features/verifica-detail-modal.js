...
function buildModal(detail) {
    ...
    const texSection = `
        <section class="fm-vd-detail-tex-versions">
            ...
            <p class="fm-muted">Carica un .tex modificato (es. da VSCode o TeXworks): le versioni
    ...
    const shareSection = `
        <section class="fm-vd-detail-share" style="margin:8px 16px;padding:10px 12px;background:rgba(99,102,241,0.08);border-left:3px solid #6366f1;border-radius:6px;display:flex;gap:10px;align-items:center;flex-wrap:wrap">
            ...
            <span class="fm-muted" style="font-size:11px;flex-basis:100%;margin-top:2px">
                Condivisione e grant valgono per tutte le varianti e le versioni di questa verifica.
            </span>
        </section>`;

    // ADR-037, fase 3 — «Dove vale»: i posti della verifica, con lo stato che
    // decide se gli studenti la vedono, e «Duplica qui». Il pannello si monta
    // dopo, con i nodi (dove-vale.js).
    const doveValeSection = `
        <section class="fm-vd-detail-dove-vale" style="margin:8px 16px;padding:10px 12px;border-left:3px solid #22c55e;border-radius:6px">
            <strong style="font-size:13px">Dove vale</strong>
            <div class="fm-dove-vale-box"></div>
        </section>`;
    ...
    const box = m.querySelector(".fm-vd-detail-dove-vale .fm-dove-vale-box");
    montaDoveVale(box, current.id, {
        fonte: "verifica",
        // La copia è una verifica nuova: la barra la deve elencare.
        onDuplicato: (nuovoId) => window.dispatchEvent(new CustomEvent("fm:verifica-saved", { detail: { id: nuovoId } })),
    });
}
    ...
    m.addEventListener("click", async (e) => {
        ...
        if (shareToggle) {
            ...
            try {
                ...
                const esito = await toggleSharePool("verifica_documents", vid, newVal);
                ...
                const n = Number(esito?.varianti || 1);
                const quante = n > 1 ? ` (${n} varianti)` : "";
                window.FM?.SyncPanel?.notify?.("Pool", "ok",
                    newVal ? `✓ Condivisa con i colleghi${quante}` : `✓ Condivisione ritirata${quante}`, 2500);
                // La barra mostra il segno della condivisione: che lo ridisegni.
                window.dispatchEvent(new CustomEvent("fm:verifica-saved", { detail: { id: vid } }));
            } catch (err) {
