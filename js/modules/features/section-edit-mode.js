...
window.addEventListener("fm:edit-section-toggled", (e) => {
    ...
    // La visibilità di .fm-item-actions e del «+» (.fm-section-add) la decide
    // il CSS su [data-edit-active] del blocco (già messo a "0" dal toggle):
    // css/modules/_sidebar-widgets.css. Per il «+» questo vale dal 19/9/2026:
    // prima la regola stava in un layer che perdeva contro .fm-btn e il «+» si
    // vedeva sempre, a chiunque; il commento lo dava per fatto. Le operazioni
    // fatte durante l'edit (✎ edit, 🗑 delete, ➕ create) aggiornano già il DOM
    // in modo chirurgico (o con refresh proprio nei loader category/risdoc).
...
/**
 * Chi guarda può modificare questo pannello? Il segno di permesso è la
 * matita .js-edit-section, che il server scrive solo per docenti e
 * amministratori (views/partials/sidebar.php); la classe fm-can-edit sul body
 * (views/layout/app.php) è la stessa decisione, e servono tutte e due.
 */
function puoModificare(sidepage) {
    return document.body.classList.contains("fm-can-edit")
        && !!sidepage.querySelector(".js-edit-section");
}
...
const onSidepageRendered = (e) => {
    ...
    // sono installati al render; la visibilità la decide il CSS.
    removeInlineItemActions(sidepage);
    // 19/9/2026 — ma solo a chi può modificare: il segno di permesso
    // .js-edit-section lo scrive il server per docenti e amministratori. Prima
    // si installavano per chiunque, e a chi studia restavano nel DOM
    // (✎🗑📥 in ogni voce) e il «+» apriva la finestra di creazione.
    if (type && puoModificare(sidepage)) {
