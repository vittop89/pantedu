...
import { escHtml as escapeHtml, escAttr as escapeAttr } from "../core/dom-utils.js";
(function () {
    "use strict";
    ...
    async function saveCurrent() {
        ...
        const r = await fetch("/api/admin/verifica/files/write", {
            ...
            headers: {
                ...
                "X-CSRF-Token": csrf,
                "X-Audit-Reason": `Modifica modello verifiche ${currentPath} (ambito ${currentScope})`,
            },
    ...
    async function deleteCurrent() {
        ...
        const r = await fetch("/api/admin/verifica/files/delete", {
            ...
            headers: {
                "Content-Type": "application/json",
                "X-CSRF-Token": csrf,
                "X-Audit-Reason": `Rimozione personalizzazione ${currentPath} (ambito ${currentScope}): torna al modello comune`,
            },
    ...
    async function copyFromDefault() {
        ...
        const r = await fetch("/api/admin/verifica/files/copy-from-default", {
            ...
            headers: {
                "Content-Type": "application/json",
                "X-CSRF-Token": csrf,
                "X-Audit-Reason": `Copia del modello comune ${currentPath} nell'ambito ${currentScope}`,
            },
            body: JSON.stringify({ scope: currentScope, path: currentPath, _csrf: csrf }),
        });
        const j = await r.json();
