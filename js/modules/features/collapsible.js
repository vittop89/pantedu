...
function onClick(e) {
    ...
    // Da qui in poi lo stato lo decide l'utente: le riapplicazioni tardive
    // dello stato di default non devono più chiudergli quel che ha aperto.
    statoDeciso = true;
...
/**
 * L'utente ha aperto o chiuso un collapsible da quando la pagina è arrivata.
 *
 * 2026-09-07 — serve perché lo stato di default veniva riapplicato anche a
 * 1,2 secondi dal caricamento (i `setTimeout` in fondo a `init`, più
 * `fm:mathjax-ready`): chi apriva un gruppo subito dopo l'apertura della
 * pagina se lo vedeva richiudere da solo. Le riapplicazioni tardive servono
 * per il contenuto che arriva in ritardo, non per sovrascrivere una scelta
 * già fatta; una navigazione vera (`fm:navigated`) azzera tutto.
 */
let statoDeciso = false;
...
function init() {
    ...
    window.addEventListener("fm:navigated", () => {
        statoDeciso = false;   // pagina nuova: lo stato torna a essere quello di default
    ...
    // Le due riapplicazioni ritardate qui sotto servono al contenuto che
    // arriva async.
    const bootApply = () => {
        // Se lo stato è dell'utente non si riapre e non si richiude niente,
        // ma l'altezza va ricalcolata lo stesso: il contenuto arrivato dopo
        // resterebbe tagliato.
        if (statoDeciso) { recompute(); return; }
        if (isVerificaPage()) openAll();
        else                  closeAll();
    };
    bootApply();
