...
import { hrefContenutoRisdoc } from "./collegamento-contenuto.js";
...
async function loadSidepage(sidepageKey, spec) {
    ...
    let rows = [];
    // WS4 — guest (sidebar pubblica): il guest vede solo i documenti published
    // del docente che pubblica in rete, via endpoint pubblico (sotto).
    const _isGuest = !!document.querySelector('nav.sidebar[data-fm-guest="1"]');
    // Il segno di permesso: il server scrive .js-edit-section solo per docenti
    // e amministratori (views/partials/sidebar.php). Modelli e istanze sono
    // del docente: a chi studia — ospite con la credenziale, studente con
    // account, visitatore — quelle chiamate rispondevano 401 a ogni apertura
    // del pannello (19/9/2026), quindi non partono.
        ...
        if (res.ok) {
            ...
            for (const t of tRows) {
                ...
                rows.push({
                    ...
                    // Il tipo decide la pagina che il collegamento apre
                    // (collegamento-contenuto.js): qui si crea qualsiasi tipo.
                    contentType: String(t.content_type || ""),
                    // Una bozza ha il suo segno (segno-bozza.js).
                    bozza: eBozza(t),
                    origin:    spec.origin,
                    ...
                    doc_roles: _docRoles,
                    // Il 📥: la regola sta nel server (App\Support\RigheDellaBarra),
                    // che lo manda sia in /api/teacher/content con i metadati sia
                    // in /api/study/content.json. Prima qui non c'era mai.
                    hasBodyPt: t.has_body_pt === true,
...
function renderCategory(sidepage, spec, category, rows) {
    ...
    // 19/9/2026 — solo con il segno di permesso, come la matita qui sopra e
    // come in db-sidepage. Prima nasceva per chiunque: da studente, da ospite
    // con la credenziale e da visitatore il «+» di BES/DSA si vedeva e apriva
    // la finestra «Crea bes» (il server rifiutava il salvataggio con 401).
    if (origEdit) {
    ...
    if (!rows.length) {
        ...
        for (const r of rows) {
            ...
            if (r.isInstance) {
                ...
                li.dataset.hasBodyPt = r.hasBodyPt ? "1" : "0";
                li.dataset.userCreated = "1";
            ...
            if (r.isInstance) {
                ...
                // Un documento apre la pagina BES/DSA o Risorse; una mappa, un
                // esercizio o una verifica creati qui la pagina del loro tipo.
                a.href = hrefContenutoRisdoc({
                    id: r.teacherContentId, tipo: r.contentType, origine: r.origin,
                    ind, cls, subj, topic: r.num_arg || r.argomento || "", ternaScoped: r.ternaScoped,
                });
            ...
            if (r.isTeacherContent && r.bozza) {
                li.classList.add(CLASSE_BOZZA);
                a.insertAdjacentHTML("afterbegin", segnoBozzaHtml());
            }
            li.append(num, " ", a);
            ul.appendChild(li);
        }
