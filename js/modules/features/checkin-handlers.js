...
// Serializzatore dei campi: DOM reso → campo → blocchi del contratto.
import {
    sorgenteDelContenitore, sorgenteSenzaEtichetta, blocchiDalCampo,
} from "../editor/campo-in-blocchi.js";
...
import {
    ...
    innermostList, listDepth, listStart, setListStart, numberAfter,
    previousOrderedList, parseListStart, listFits, itemCount,
...
// A-20 (revisione architetturale 23/9/2026) — bonifica lato client dei
// blocchi TikZ/GeoGebra, mirror di TikzScriptValidator/SvgSanitizer PHP.
import { escapeTikzScriptClosing, sanitizeGeogebraSvg } from "../security/tikz-geogebra-client-sanitize.js";
...
import { fetchCsrf, groupType as famigliaDelGruppo, VF_GROUP_SELECTOR, wafFetch } from "../core/dom-utils.js";
...
async function _formatTexViaVps(src) {
    ...
    const formattedSegs = await Promise.all(segments.map(async (s) => {
        ...
        const r = await wafFetch("/tex/format", {
            method: "POST",
...
// Serializzatore dei campi (dal DOM al campo, dal campo ai blocchi):
// spostato in `editor/campo-in-blocchi.js`. Alias con i nomi di prima.
const _extractRawWithTikz      = sorgenteDelContenitore;
const _extractRawWithoutLabel  = sorgenteSenzaEtichetta;
const _buildBlocksFromTextarea = blocchiDalCampo;
 ...
 *  Esportato per la prova di parità con la bonifica PHP (A-20): vedi
 *  tests/js-unit/blocchi-tikz-geogebra-render-sicuro.test.js. */
export const BLOCK_RENDERERS = {
    ...
    tikz: (b) => {
        ...
        // A-20 — neutralizza `</` letterale come TikzScriptValidator::sanitize
        // lato PHP: senza, un `\end{tikzpicture}</script><script>...` incollato
        // chiuderebbe in anticipo questo <script type="text/tikz">.
        const script = escapeTikzScriptClosing(b.script ?? "");
    ...
    geogebra: (b) => {
        ...
        // A-20 — bonifica come SvgSanitizer lato PHP: l'SVG del dialog GeoGebra
        // viene esportato dall'applet nel browser, senza passare dal server.
        const svg = sanitizeGeogebraSvg(b.svg ?? "");
        let attrs = "";
...
async function _openTikzBlocksManager(ta) {
    try {
        await caricaEntry("js/entries/tikz-blocks-manager.js", { pronto: () => !!window.FM?.openTikzBlocksManager });
 ...
 *       .editQ.single-modificaBtn  modifica contenuto (secondo clic = chiudi e salva)
...
function bindCheckinHandlers() {
    ...
    // (clona/modifica/elimina/aggiungi quesito + modifica/
...
async function onCheckinClick(e) {
    ...
    // Phase 16 — gestione buttons a LIVELLO .fm-collapsible (.fm-checkmod .fm-edit-eser):
    //   .fm-modifica-btn   → edit INTERO gruppo .fm-groupcollex (tutti i quesiti);
    //                        premuto di nuovo chiude e salva
    //   .fm-elimina-btn    → delete INTERO gruppo
    // 2026-09-07 — tolto `.checkmod .fm-quick-save-btn`: il markup emette
    // `.fm-checkmod`, quel ramo non è mai scattato, e il bottone era comunque
    // reso con display:none dal server (voce 53 del debito). Il salvataggio è
    // automatico e la chiusura dell'editor lo conclude.
    const problemBtn = t.closest(".fm-checkmod .fm-modifica-btn, .fm-checkmod .fm-elimina-btn, .fm-checkmod .fm-close-btn");
    if (problemBtn) {
        e.preventDefault(); e.stopPropagation();
...
function openGroupEditor(problem) {
    ...
    const groupType = famigliaDelGruppo(problem);
    const isVfOrRm = groupType === "VF" || groupType === "RM";
...
function toggleEditMode(item, on) {
    ...
    // close.svg sullo stesso btn .fm-single-modifica-btn. Il salvataggio
    // rapido separato non c'è più (2026-09-07): era redundante — la modifica
    // stessa diventa chiusura-con-salvataggio al secondo clic — ed era reso
    // con display:none, quindi non si poteva premere (voce 53 del debito).
    ...
    for (let i = 0; i < svgs.length; i++) {
        ...
        try {
            const res = await apiPost("/tikz/save-svg", {
                ...
                fileName,
                svgContent: svgXml,
            }, { motivo: auditReason("Editor: SVG TikZ di un quesito salvato su disco", fileName) });
...
async function deleteTikzSvg(filePath, fileName) {
    try {
        await apiPost("/tikz/delete-svg", { filePath, fileName },
            { motivo: auditReason("Editor: SVG TikZ di un quesito eliminato, tolto dal disco", fileName) });
    } catch (_) { /* skip */ }
}
...
function buildListSelectGlobal(getPanel) {
    ...
    // Il primo punto della lista numerata dove sta il caret (_setListNumbering).
    const numerazione = document.createElement("optgroup");
    numerazione.label = "Numerazione";
    [["num-continua", "continua"], ["num-inizia", "inizia da…"]].forEach(([v, l]) => {
        const opt = document.createElement("option"); opt.value = v; opt.textContent = l; numerazione.appendChild(opt);
    });
    sel.appendChild(numerazione);
    sel.title += " · Numerazione: continua la lista precedente o scegli il primo punto";
    sel.addEventListener("change", () => {
        const v = sel.value;
        sel.value = "";
        if (v === "num-continua" || v === "num-inizia") void _setListNumbering(getPanel(), v);
        else if (v) insertListSnippet(getPanel(), v);
    });
    return sel;
}
...
async function _openGeoGebraCatalogPicker(getPanel) {
    let items = [];
    try {
        const r = await wafFetch("/geogebra/catalog", { credentials: "same-origin", cache: "no-store" });
    ...
    items.forEach((it) => {
        ...
        loadBtn.addEventListener("click", async () => {
            try {
                const r = await wafFetch(`/geogebra/catalog/${encodeURIComponent(it.id)}`, { credentials: "same-origin" });
                if (!r.ok) throw new Error(`HTTP ${r.status}`);
                const j = await r.json();
        ...
        delBtn.addEventListener("click", async () => {
            ...
            if (!ok) return;
            const csrf = await fetchCsrf();
            const r = await wafFetch("/geogebra/catalog/delete", {
...
async function _openGeoGebraEditor(getPanel, { initialGgbBase64 = null, initialLabel = "", itemId = null } = {}) {
    try {
        await caricaEntry("js/entries/geogebra-editor.js", { pronto: () => !!window.FM?.openGeoGebraEditor });
    } catch (e) {
...
function insertListSnippet(panel, kind) {
    const ta = panel?._focusedTextarea
        ...
        || window.__fmFocusedTA;
    if (!ta) return;
    const section = _listSectionOf(ta);
...
/** Sezione DSA di una lista nuova, dal campo in cui nasce. */
function _listSectionOf(ta) {
    const fieldMap = { quesito: "question", soluzione: "solution", giustificazione: "justification" };
    return fieldMap[ta.dataset?.field] || "question";
}

/** Segno del livello di `list` come nelle righe di _OL_LEVELS: il `type`
 *  storico della lista, se lo ha, altrimenti quello del preset della lista
 *  più esterna alla profondità di `list`. */
function _listLevelCode(list, field) {
    const legacy = { 1: "D.", A: "UA.", a: "la.", I: "UR.", i: "lr." }[list.getAttribute("type") || ""];
    if (legacy) return legacy;
    const root = _outermostListInField(field, list);
    const levels = _OL_LEVELS[root?.getAttribute("data-fm-list-style") || ""] || _OL_LEVELS[""];
    return levels[Math.min(listDepth(list, field), levels.length - 1)];
}

/**
 * Gruppo «Numerazione» del menu List (24/9/2026): il primo punto di una
 * lista numerata. Un elenco interrotto da un paragrafo e poi ripreso
 * ripartiva sempre da 1. o da a., e non c'era modo di cambiarlo.
 *  - «continua»: la lista del caret riprende dal punto dopo l'ultimo della
 *    lista numerata precedente allo stesso livello; con il caret fuori da una
 *    lista, ne crea una nuova con lo stile di quella precedente.
 *  - «inizia da…»: chiede il segno del primo punto (c, iv, 3…).
 * Il valore è l'attributo `start` dell'<ol>, che salvataggio, pagina e PDF
 * conoscevano già (campo-in-blocchi, ContractRenderer, Sanitizer).
 */
async function _setListNumbering(panel, action) {
    const field = panel?._focusedTextarea || window.__fmFocusedTA;
    if (!field?.isContentEditable) {
        toast("Metti il cursore nel campo del testo, dentro l'elenco", "warn");
        return;
    }
    field.focus();
    const sel = window.getSelection();
    const range = sel?.rangeCount ? sel.getRangeAt(0) : null;
    const node = range && field.contains(range.startContainer) ? range.startContainer : null;
    if (!node) {
        toast("Metti il cursore nel campo del testo, dentro l'elenco", "warn");
        return;
    }
    let list = innermostList(field, node);
    if (list && list.tagName !== "OL") {
        toast("Un elenco puntato non ha numeri: scegli prima uno stile numerato o a lettere", "warn");
        return;
    }

    if (action === "num-continua") {
        const prev = previousOrderedList(field, list || node, list ? listDepth(list, field) : 0);
        if (!prev) {
            toast("Prima di qui non c'è un elenco numerato da continuare", "info");
            return;
        }
        const n = numberAfter(prev);
        const code = _listLevelCode(list || prev, field);
        if (!listFits(code, n, list ? itemCount(list) : 1)) {
            toast("Continuando, l'elenco andrebbe oltre la z: nel PDF le lettere si fermano lì", "err");
            return;
        }
        UndoManager.save(field);
        if (!list) {
            _wysiwygInsertList(field, "ol", prev.getAttribute("type") || "", _listSectionOf(field),
                prev.getAttribute("data-fm-list-style") || "");
            list = innermostList(field, window.getSelection().getRangeAt(0).startContainer);
        }
        if (list) setListStart(list, n);
        field.dispatchEvent(new Event("input", { bubbles: true }));
        return;
    }

    if (!list) {
        toast("Metti il cursore dentro l'elenco numerato di cui vuoi cambiare il primo punto", "warn");
        return;
    }
    const code = _listLevelCode(list, field);
    const core = code.replace(/[.)]$/, "");
    const [domanda, regola] = core === "la" || core === "UA"
        ? ["Con quale lettera comincia questo elenco? Va bene anche la posizione: 3 per la c.",
            "una lettera dalla a alla z, o la sua posizione (3 per la c)"]
        : core === "lr" || core === "UR"
            ? ["Con quale numero romano comincia questo elenco? Va bene anche la posizione: 4 per iv.",
                "un numero romano (iv) o la sua posizione (4)"]
            : ["Con quale numero comincia questo elenco?", "un numero intero da 1 a 999"];
    const saved = range.cloneRange();
    const answer = await window.FM.Dialog.prompt(
        domanda,
        _markerFromCode(code, listStart(list)).replace(/[.)]$/, ""),
        { title: "Il primo punto dell'elenco" });
    field.focus();
    sel.removeAllRanges();
    sel.addRange(saved);
    if (answer === null || !list.isConnected) return;
    const n = parseListStart(answer, code);
    if (n === null) {
        toast(`«${answer.trim()}» non può essere il primo punto di questo elenco: scrivi ${regola}`, "err");
        return;
    }
    if (!listFits(code, n, itemCount(list))) {
        toast(`Partendo da «${answer.trim()}», gli ultimi punti andrebbero oltre la z: nel PDF le lettere si fermano lì`, "err");
        return;
    }
    UndoManager.save(field);
    setListStart(list, n);
...
async function saveTeacherSources(data) {
    const res = await wafFetch("/api/teacher/sources.json", {
        method: "PUT",
        credentials: "same-origin",
        headers: { "Content-Type": "application/json", "X-CSRF-Token": await getCsrfTokenSafe() },
...
// Il gettone lo dà `/auth/csrf` tramite `fetchCsrf` (cache 60s, invalidata
// su fm:navigated). Prima si leggeva `meta[name="csrf-token"]`, che nessuna
// vista ha mai emesso: partiva sempre una stringa vuota.
...
async function rebuildBadgeForOrigin(item, origin) {
    ...
    if (s) {
        ...
        if (!registry) {
            try {
                const res = await wafFetch(`/api/teacher/sources.registry.json`, { credentials: "same-origin" });
...
async function syncItem(item) {
    ...
    try {
        const res = await wafFetch(`/api/study/content/${id}.json`, { credentials: "same-origin" });
...
/** 23/9/2026 (A-69) — `motivo`: il testo per X-Audit-Reason, dove la rotta
 *  lo pretende (i modelli TikZ globali, gli SVG su disco: rotte
 *  dell'amministratore). Le rotte del docente non lo chiedono. */
async function apiPost(url, body, { ifMatchVersion = null, _retry = false, silent = false, motivo = "" } = {}) {
    ...
    try {
        ...
        if (res.status === 422) {
            // Il server ha rifiutato perché il contenuto porta pezzi della
            // pagina disegnata invece della sorgente (TestoDiPaginaResa). Il
            // messaggio è già in italiano e dice cosa fare: si mostra, invece
            // di ingoiarlo come un errore qualsiasi.
            const j = await res.json().catch(() => ({}));
            if (j?.error === "rendered_text") {
                toast(j.message || "Salvataggio rifiutato: contenuto non valido", "err");
                return { ok: false, status: 422, renderedText: true };
            }
...
function toast(msg, kind = "ok") {
    ...
    el.style.cssText =
        ...
        `font:14px/1.4 system-ui;max-width:360px;box-shadow:0 6px 18px rgba(0,0,0,.35);${kind === "err" ? "background:#c02a2a" : kind === "warn" ? "background:#c78a2a"
...
function initVFProblemsOnPage() {
    ...
    if (!document.querySelector(VF_GROUP_SELECTOR)) return;
...
window.FM.__setListNumberingForTest = _setListNumbering;
