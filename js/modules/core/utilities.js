...
import { asElement, isVFGroup, trigger } from "./dom-utils.js";
...
export const utilities = {
     ...
     */
    sendLoginRedirectPath (linkHref) {
     ...
     */
    salvaScelte (verFilePath, versionKey = "v1", scope = null) {
    ...
    caricaScelte (verFilePath, versionKey = "v1", scope = null) {
        return fetchJson(Endpoints.verifiche.saveScelte, {
            method: "POST",
            headers: { "Content-Type": "application/x-www-form-urlencoded; charset=UTF-8" },
    ...
    verificaScelte (verFilePath, versionKey = "v1") {
        return fetchJson(Endpoints.verifiche.saveScelte, {
            method: "POST",
            headers: { "Content-Type": "application/x-www-form-urlencoded; charset=UTF-8" },
     ...
     */
    updatePointsTotal (problem) {
        const problemEl = asElement(problem);
        if (!problemEl) return;

        const isVFProblem = isVFGroup(problemEl);
    ...
    makeVFTotalPointsEditable (problem) {
        const problemEl = asElement(problem);
        if (!problemEl) return;
    ...
    initializeVFProblems () {
        document.querySelectorAll(".fm-groupcollex").forEach((problem) => {
            // Il tipo si legge da `data-type`: dalla Fase 15 l'id è un UUID.
            if (!isVFGroup(problem)) return;
            if (problem.querySelector(".fm-vf-total-points-input")) return; // già pronto
     ...
     */
    initTooltip (triggerSelector, tooltipId, offset = { x: 20, y: 10 }, useDelegation = true) {
