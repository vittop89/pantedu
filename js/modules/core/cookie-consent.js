...
export const CookieConsentManager = {
    ...
    /**
     * Informativa aggiornata → riapre il pannello dei consensi (Art. 7 §1:
     * l'interessato deve poter sapere a quale testo sta acconsentendo).
     *
     * Il server confronta la text_version dei consensi attivi con quella
     * corrente (app/Config/gdpr.php, allineata al frontmatter di
     * docs/privacy/informativa.md) e restituisce in `needs_reconfirm` i tipi
     * da riconfermare.
     *
     * NON è un gate: l'utente può chiudere il pannello e continuare a usare la
     * piattaforma. Subordinare l'accesso alla conferma renderebbe il consenso
     * non liberamente prestato (Art. 7 §4) — a differenza di ToS/AUP, che sono
     * accettazione contrattuale e hanno un gate duro (TosAcceptanceMiddleware).
     *
     * Il consenso precedente resta valido e tracciato finché l'utente non si
     * esprime di nuovo: qui non si revoca nulla, si chiede soltanto.
     */
    _checkReconfirm: async function () {
        // Solo utenti autenticati: per gli anonimi /me/consents risponde 401.
        if (!window.FM?.user?.username) return;
        try {
            const res = await fetch("/me/consents", {
                credentials: "same-origin",
                headers: { Accept: "application/json" },
            });
            if (!res.ok) return;
            const data = await res.json();
            if (!Array.isArray(data.needs_reconfirm) || data.needs_reconfirm.length === 0) return;

            // Una sola richiesta per versione: chi ha già chiuso il pannello per
            // questa revisione non se lo ritrova a ogni pagina.
            const key = "fm_consent_reconfirm_" + (data.current_version || "?");
            try {
                if (sessionStorage.getItem(key) === "1") return;
                sessionStorage.setItem(key, "1");
            } catch { /* sessionStorage non disponibile: mostra comunque */ }

            if (this.elements.warningMessage) {
                this.elements.warningMessage.textContent =
                    "L'informativa privacy è stata aggiornata alla versione "
                    + (data.current_version || "") + ". Conferma le tue preferenze.";
            }
            this._openModal();
        } catch { /* rete assente: non bloccare la pagina */ }
    },
