 ...
 */
import { getProviderKeys, saveProviderKey, clearProviderKey } from "./api.js";
...
export async function openKeyModal(onSaved) {
    ...
    // Parti dal provider GIÀ configurato; in mancanza Ollama locale, che
    // è il default anche lato server (nessun trasferimento verso terzi).
    const configured = PROVIDERS.find((p) => status[p.id]?.configured);
    select.value = configured ? configured.id : "ollama";
    ...
    const refresh = () => {
        ...
        input.placeholder = status[p.id]?.configured
            ...
            : (p.localUrl ? "http://127.0.0.1:11434" : "Incolla la chiave API");
        const g = GUIDES[p.id] || {};
