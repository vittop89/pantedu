 ...
 * - Il backend (il controller della pagina + views/layout/app.php) ritorna
...
(function () {
    ...
    // Se entriamo qui via SPA da una pagina senza asset,
    ...
    function targetNeedsExerciseAssets(pathname) {
        // Phase G7 — /studio/mappa/ non richiede MathJax
    ...
    async function navigate(url, push = true) {
        ...
        // e non sono ancora caricati, full reload così
        ...
        window.scrollTo(0, 0);
        rewireInjectedScripts(target, nonceDellaRisposta(res));
    ...
    /** Il nonce della CSP rigorosa della risposta, o null (CSP rilassata o
     *  solo in report-only). Una fetch della stessa origine legge le
     *  intestazioni della risposta. */
    function nonceDellaRisposta(res) {
        const csp = (res && res.headers && res.headers.get('Content-Security-Policy')) || '';
        const m = csp.match(/'nonce-([A-Za-z0-9+/=]+)'/);
        return m ? m[1] : null;
    }

    /** Isole di dati: il browser non le esegue, e il nonce non lo portano. */
    const TIPI_DI_DATI = /^(?:application\/(?:ld\+)?json|text\/tikz)$/i;

    /** Node.cloneNode preserves <script> but the browser does NOT
     *  execute them once injected via innerHTML. Re-create them.
     *
     *  23/9/2026 (revisione architetturale A-16) — con la CSP rigorosa uno
     *  script creato da qui gira sempre: 'strict-dynamic' si fida di ciò che
     *  crea uno script fidato, nonce o no. Ricrearli tutti voleva dire
     *  eseguire anche uno <script> arrivato dal contenuto della pagina, che al
     *  primo caricamento il browser avrebbe bloccato. Adesso, se la risposta
     *  ha la CSP rigorosa, si ricreano solo gli script che portano il suo
     *  nonce — quelli che il server ha scritto con Csp::attributo() — e le
     *  isole di dati; gli altri restano come li ha messi innerHTML, inerti.
     *  Senza nonce nella risposta (CSP rilassata, report-only) come prima.
     *  Prova: tests/js-unit/router-e-nonce.test.js. */
    function rewireInjectedScripts(root, nonce) {
        root.querySelectorAll('script').forEach(orig => {
            if (nonce) {
                // Il browser nasconde il valore dell'attributo (nonce=""), non
                // la proprietà: si legge da `.nonce`.
                const suo = orig.nonce || orig.getAttribute('nonce') || '';
                const tipo = (orig.getAttribute('type') || '').trim();
                if (suo !== nonce && !TIPI_DI_DATI.test(tipo)) return;
            }
