 ...
 *   exportTex()       → Promise<{blob, nome}> (POST → pacchetto TeX nel corpo)
 */
...
export class TeacherContentAdapter {
    ...
    /** POST export → il pacchetto TeX nel corpo della risposta. */
    async exportTex() {
        const csrf = await this._csrf();
        // Dal 21/9/2026 la risposta buona è l'archivio, non un JSON con
        // l'indirizzo di un file scritto su disco.
        const risposta = await wafFetch(`/api/teacher/content/${this.docId}/export`, {
            method: "POST",
            credentials: "same-origin",
            headers: { "Content-Type": "application/x-www-form-urlencoded" },
            body: new URLSearchParams({ _csrf: csrf }).toString(),
        });
        return pacchettoDallaRisposta(risposta, `documento-${this.docId}.zip`);
    }
}
