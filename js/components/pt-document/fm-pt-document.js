...
import { scaricaPacchetto } from "../../modules/core/pacchetto.js";
...
export class FmPtDocument extends LitElement {
    ...
    async _ensureBodyPt() {
        ...
        if (this._isRisdoc && this._adapter?._state && typeof this._adapter._state === "object") {
            ...
            // 21/9/2026 — la spunta dell'intestazione sta nello stesso stato, e
            // la compilazione salvata arriva dopo il giro di `loadIncludeHeader`
            // in connectedCallback: senza questa riga la casella tornava
            // spuntata a ogni riapertura, qualunque cosa avesse scelto il
            // docente.
            if (Object.prototype.hasOwnProperty.call(this._risdocState, "includeHeader")) {
                this._includeHeader = this._risdocState.includeHeader !== false;
            }
        ...
        if (this._isRisdoc) {
            try {
                ...
                await opener([{
                    ...
                    formState: await this._statoDaCompilare(),
                    // ADR-046 — la RIGA aperta, non il modello. Serve al modal
                    // per segnare lo scaricamento quando il docente preme
                    // «⤓ PDF». Viaggia sull'oggetto `doc` e non in `opts`
                    // perché openPreview legge di `opts` cinque chiavi
                    // nominate e butta il resto senza dire niente, mentre gli
                    // oggetti `doc` arrivano intatti agli adapter — è la
                    // stessa strada che fa già `formState`.
                    //
                    // Null quando non c'è una riga da segnare: modifica del
                    // master, bozza tenuta nel browser per scelta
                    // dell'Istituto, compilazione mai salvata. Il modal lo
                    // usa come guardia, e per le verifiche non esiste proprio.
                    compilationId: this._adapter?.compilationId ?? null,
    ...
    /**
     * Quello che il docente ha compilato, nella forma che il server si aspetta.
     *
     * Stava dentro `_exportTex`, e il pacchetto ZIP non ne sapeva niente: i
     * bottoni «ZIP» e «VSCode» chiamavano l'adapter senza passargli niente, e
     * il docente si scaricava il modello vuoto (misurato il 21/9/2026 nei due
     * versi: senza questo stato il pacchetto non contiene quello che ha
     * scritto, con questo sì). Adesso si costruisce in un posto solo e lo
     * usano tutt'e tre.
     *
     * @returns {Promise<{fields: object, state: object, body_pt: Array}>}
     */
    async _statoDaCompilare() {
        let fields = {}, bodyPt = [];
        try {
            ...
            // ADR-026 Step 5 Fix #3 — il body_pt de-idratato così
        ...
        return {
            fields,
            // 21/9/2026 — la spunta «Includi intestazione istituto» viaggia con
            // lo stato: il compilatore la legge da qui.
            state: { ...(this._risdocState || {}), includeHeader: this._includeHeader },
            body_pt: bodyPt,
        };
    }
    ...
    async _exportZip() {
        ...
        try {
            const pacchetto = await this._adapter.exportTex(
                this._isRisdoc ? await this._statoDaCompilare() : null);
            this._status = "✓ ZIP pronto";
            // 21/9/2026 — il pacchetto arriva nel corpo della risposta: non c'è
            // più un indirizzo da aprire, e su disco del server non resta niente.
            scaricaPacchetto(pacchetto);
            await this._segnaScaricata();
        } catch (e) {
            this._status = `✗ ${e.message || e}`;
    ...
    /**
     * ADR-046 — il docente ha portato via il documento: da qui parte la grazia
     * prima che la bozza venga cancellata dal server.
     *
     * Vale solo per i modelli risdoc con una compilazione salvata. Su un
     * documento personalizzato (`teacher_content`) `this._adapter` non ha
     * `compilationId` e non succede niente: quei documenti sono il lavoro del
     * docente e non scadono.
     *
     * Non solleva e non avvisa: se la segnalazione non arriva, la bozza resta
     * dov'è e la raccoglie la rete di fine anno. Un guasto non deve mai
     * togliere niente a nessuno, e non deve nemmeno disturbare chi ha appena
     * scaricato il suo file.
     */
    async _segnaScaricata() {
        const id = this._isRisdoc ? (this._adapter?.compilationId ?? null) : null;
        if (!id) return;
        try {
            const { segnaScaricata } = await import("../../modules/risdoc/segna-scaricata.js");
            await segnaScaricata(id);
        } catch { /* la grazia è un di più: lo scaricamento è già riuscito */ }
    }
