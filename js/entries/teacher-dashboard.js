/**
 * Entry Vite della dashboard docente (views/teacher/dashboard.php).
 *
 * Fino al 2026-09-05 era js/teacher-dashboard.js, uno script CLASSICO che
 * fm-router re-iniettava a ogni navigazione SPA per re-inizializzare i
 * pannelli. Come modulo si esegue una volta sola: l'inizializzazione e'
 * quindi una funzione, chiamata al caricamento e a ogni `fm:navigated`.
 * Ogni blocco cerca la propria radice e ritorna se la pagina non e' la
 * dashboard; i tre listener globali stanno fuori e chiamano l'ultima
 * funzione registrata, cosi' non si accumulano a ogni re-init.
 */
// Ogni richiesta passa da wafFetch: su un 403 `waf_challenge` risolve la
// sfida e ripete (23/9/2026, A-18; prima erano 29 `fetch` dirette).
import { esc, escHtml, fetchCsrf, wafFetch } from "../modules/core/dom-utils.js";
import { descriviDrive, RITORNI, testoPer } from "../modules/features/drive-stato.js";

const csrf = () => fetchCsrf();

// Listener globali: registrati una volta, dispatchano all'istanza corrente.
const _hooks = { fsAccessRefresh: null, syncLogRender: null };
window.addEventListener("fm:fs-access-ready", () => _hooks.fsAccessRefresh?.());
window.addEventListener("fm:sync-log-updated", () => _hooks.syncLogRender?.());
window.addEventListener("storage", (e) => { if (e.key === "fm:syncLog") _hooks.syncLogRender?.(); });

function initTeacherDashboard() {
_hooks.fsAccessRefresh = null;
_hooks.syncLogRender = null;
...
(function() {
    ...
    if (exForm && exResults) {
        ...
        // La scuola della ricerca (14/9/2026). La ricerca guardava solo la
        // scuola attiva, senza mostrarla, e indirizzi, classi e materie
        // arrivavano da tutte le scuole del docente: una classe dell'altra
        // scuola dava zero risultati. A chi ha più di una scuola compare la
        // scelta, che parte dalla scuola attiva e carica le voci di quella
        // scuola; «Tutte le mie scuole» cerca dappertutto (le voci di tutte).
        // Il server accetta solo le scuole del docente (403 le altre).
        const selScuola = exForm.querySelector('select[name="institute_id"]');
        const svuotaVoci = () => {
            for (const sel of exForm.querySelectorAll('select[data-fm-curriculum]')) {
                sel.querySelectorAll('option:not(:first-child)').forEach(o => o.remove());
            }
        };
        // 21/9/2026 — una risposta vecchia non riempie più il menu.
        //
        // Cambiando scuola in fretta (o cambiandola mentre la pagina sta
        // ancora caricando quella attiva) partono due richieste, e non sempre
        // rispondono nell'ordine in cui sono partite: la più lenta arrivava
        // dopo e lasciava nel menu le classi dell'ALTRA scuola. Chi guarda non
        // se ne accorge, perché un elenco di classi plausibile c'è comunque.
        // Stessa guardia della barra laterale: un numero d'ordine, e chi torna
        // in ritardo si fa da parte.
        let giro = 0;
        const caricaVoci = async (scuola) => {
            const mio = ++giro;
            svuotaVoci();
            const url = (scuola && scuola !== 'tutte')
                ? `/api/teacher/curriculum?institute_id=${encodeURIComponent(scuola)}`
                : '/api/teacher/curriculum?scope=all';
            try {
                const r = await wafFetch(url, { credentials: 'same-origin', headers: { 'Accept': 'application/json' } });
                ...
                for (const sel of exForm.querySelectorAll('select[data-fm-curriculum]')) {
                    ...
                    const visti = new Set();
                    const fragments = (cur[kind] || [])
                        .filter(e => e.active !== false)
                        // Con «tutte» la stessa sigla arriva da più scuole: una volta sola.
                        .filter(e => !visti.has(e.code) && visti.add(e.code))
        ...
        const preparaScuole = async (preferita) => {
            if (!selScuola) return '';
            let scuole = [];
            try {
                const r = await wafFetch('/api/teacher/institutes', { credentials: 'same-origin', headers: { 'Accept': 'application/json' } });
                const j = await r.json();
                if (j.ok) scuole = j.institutes || [];
            } catch (_) { /* senza elenco la ricerca resta sulla scuola attiva */ }
            if (scuole.length < 2) {
                selScuola.hidden = true;
                selScuola.disabled = true;
                return '';
            }
            selScuola.innerHTML = scuole
                .map(s => `<option value="${esc(String(s.id))}">${esc(s.name || s.code || String(s.id))}</option>`)
                .join('') + '<option value="tutte">Tutte le mie scuole</option>';
            const valide = new Set([...scuole.map(s => String(s.id)), 'tutte']);
            const attiva = String(selScuola.dataset.scuolaAttiva || '');
            selScuola.value = valide.has(String(preferita)) ? String(preferita)
                : (valide.has(attiva) ? attiva : String(scuole[0].id));
            selScuola.hidden = false;
            selScuola.disabled = false;
            return selScuola.value;
        };
        if (selScuola) {
            selScuola.addEventListener('change', async () => {
                await caricaVoci(selScuola.value);
                // Indirizzo, classe e materia della scuola di prima non valgono qui.
                for (const sel of exForm.querySelectorAll('select[data-fm-curriculum]')) sel.value = '';
                saveState();
            });
        }
        ...
        (async () => {
            ...
            const scuola = await preparaScuole(state?.institute_id);
            await caricaVoci(scuola);
            ...
            if (state) {
                // La scuola l'ha già messa preparaScuole, con le sue voci.
                const resto = { ...state };
                delete resto.institute_id;
                applyState(resto);
                const hasAny = Object.values(resto).some(v => v && v !== '');
        ...
        const restoreRow = async (id, btn) => {
            ...
            try {
                ...
                fd.set('_csrf', tok);
                const r = await wafFetch(`/api/teacher/content/${id}/update`, {
        ...
        exForm.addEventListener('submit', async (ev) => {
            ...
            try {
                const res = await wafFetch('/api/teacher/content?' + params.toString(), { credentials: 'same-origin' });
...
// ADR-038 — frase e comandi vengono da descriviDrive (drive-stato.js), che
// conosce lo stato dell'installazione e quello del collegamento.
(async function() {
    ...
    if (driveFlag) {
        const msg = testoPer(RITORNI, driveFlag);
        if (msg) {
    ...
    function renderActions(azioni) {
        actions.innerHTML = '';
        for (const azione of azioni) {
            if (azione === 'disconnetti') {
                ...
                btn.addEventListener('click', async () => {
                    ...
                    await wafFetch('/teacher/drive/disconnect', {
                        method: 'POST',
                ...
                a.textContent = azione === 'ricollega' ? 'Ricollega Drive' : 'Collega Drive';
    ...
    async function load() {
        try {
            const res = await wafFetch('/teacher/drive/status.json', {
                credentials: 'same-origin',
            ...
            const descrizione = descriviDrive(data);
            setState(descrizione.stato, descrizione.testo);
            renderActions(descrizione.azioni);
        } catch (e) {
...
(function() {
    ...
    _hooks.fsAccessRefresh = refresh;
    refresh();
})();
...
(function() {
    ...
    async function refresh() {
        try {
            const r = await wafFetch('/api/teacher/github/status', { credentials: 'same-origin' });
            const j = await r.json();
            if (!j.ok) {
            ...
            const errPart = c.last_error
                ? ` · ⚠ ${escHtml(c.last_error)}`
                : '';
            pill.dataset.state = c.last_error ? 'error' : 'connected';
            label.innerHTML = `Connesso → <strong>${escHtml(c.repo_owner)}/${escHtml(c.repo_name)}</strong> (${escHtml(c.branch)}) ${escHtml(last)}${errPart}`;
            disBtn.hidden = false;
    ...
    cfgBtn?.addEventListener('click', async () => {
        ...
        try {
            ...
            const r = await wafFetch('/api/teacher/github/configure', {
                method: 'POST', credentials: 'same-origin',
            ...
            // Smoke test
            const t2 = await wafFetch('/api/teacher/github/sync-test', {
                method: 'POST', credentials: 'same-origin',
                headers: { 'Content-Type': 'application/json', 'X-CSRF-Token': token },
    ...
    disBtn?.addEventListener('click', async () => {
        ...
        try {
            const token = await csrf();
            await wafFetch('/api/teacher/github/disconnect', {
                method: 'POST', credentials: 'same-origin',
...
(function() {
    ...
    async function refresh() {
        try {
            const r = await wafFetch('/api/teacher/recovery-key/status', { credentials: 'same-origin' });
            const j = await r.json();
            if (!j.ok) {
                pill.dataset.state = 'error';
                label.textContent = 'Errore: ' + (j.error || 'unknown');
    ...
    let cachedUsername = '';
    wafFetch('/auth/user-info', { credentials: 'same-origin' })
    ...
    genBtn?.addEventListener('click', async () => {
        ...
        try {
            const tok = await csrf();
            const r = await wafFetch('/api/teacher/recovery-key/generate', {
                method: 'POST', credentials: 'same-origin',
    ...
    revBtn?.addEventListener('click', async () => {
        ...
        try {
            const tok = await csrf();
            const r = await wafFetch('/api/teacher/recovery-key/revoke', {
                method: 'POST', credentials: 'same-origin',
                headers: { 'Content-Type': 'application/json', 'X-CSRF-Token': tok },
                body: JSON.stringify({ _csrf: tok }),
            });
    ...
    rotBtn?.addEventListener('click', async () => {
        ...
        try {
            const tok = await csrf();
            // Step 1: revoca
            const r1 = await wafFetch('/api/teacher/recovery-key/revoke', {
                method: 'POST', credentials: 'same-origin',
                headers: { 'Content-Type': 'application/json', 'X-CSRF-Token': tok },
                body: JSON.stringify({ _csrf: tok }),
            });
            ...
            const r2 = await wafFetch('/api/teacher/recovery-key/generate', {
                method: 'POST', credentials: 'same-origin',
                headers: { 'Content-Type': 'application/json', 'X-CSRF-Token': tok },
                body: JSON.stringify({ _csrf: tok }),
            });
...
(function() {
    ...
    async function loadList() {
        ...
        try {
            const r = await wafFetch('/api/teacher/pool/materials?' + params.toString(),
                                  { credentials: 'same-origin' });
            const j = await r.json();
        ...
        const rows = items.map(it => {
            ...
            const recoverBtn = it.already_recovered
                ...
                : `<button class="fm-btn fm-btn--xs fm-btn--primary"
                            ...
                            data-recover-source="${escHtml(it.source || 'teacher_content')}"
            ...
            <tr data-source-id="${it.id}" class="${it.already_recovered ? 'fm-opacity-75' : ''}">
                ...
                <td><strong>${escHtml(it.title)}</strong>
                    ${it.varianti > 1 ? `<span class="fm-muted fm-text-11"> · ${Number(it.varianti)} varianti</span>` : ''}
    ...
    async function myMaterieList() {
        ...
        myMateriePromise = (async () => {
            ...
            const r = await wafFetch('/api/teacher/curriculum?scope=all',
                                  { credentials: 'same-origin' });
            const j = await r.json();
    ...
    async function instituteCatalog(instituteId) {
        ...
        const p = (async () => {
            const r = await wafFetch('/api/teacher/curriculum?institute_id=' + encodeURIComponent(instituteId),
                                  { credentials: 'same-origin' });
            const j = await r.json();
            return {
    ...
    async function onRecoverClick(btn) {
        ...
        // 14/9/2026 — una verifica si recupera con la sua API, e ha bisogno di
        // indirizzo e classe: sono il posto della sua principale.
        const isVerifica = btn.dataset.recoverSource === 'verifica_documents';
        ...
        fb.innerHTML = `
            <div class="fm-info-note">
                ...
                <p class="fm-muted fm-text-11 fm-m-0 fm-lh-base" >
                    ${isVerifica
                        ? 'Per una verifica scegli indirizzo e classe: la copia, con tutte le varianti, nasce lì, in bozza.'
        ...
        document.getElementById('fm-pool-confirm').addEventListener('click', async () => {
            ...
            if (isVerifica && (!targetInd || !targetCls)) {
                fb.innerHTML = '<em>Per una verifica servono indirizzo e classe.</em>';
                return;
            }
            fb.innerHTML = 'Recupero in corso…';
            try {
                const tok = await csrf();
                const r = await wafFetch((isVerifica ? '/api/teacher/pool/recover-verifica/' : '/api/teacher/pool/recover/') + sourceId, {
                    method: 'POST', credentials: 'same-origin',
                    headers: { 'Content-Type': 'application/json', 'X-CSRF-Token': tok },
                    body: JSON.stringify({
                ...
                let msg = isVerifica
                    ? `✅ Verifica copiata nel tuo account: ${Number(j.varianti || 1)} varianti, in bozza (#${j.new_id}).`
                    : `✅ Copiato come #${j.new_id} (tipo ${escHtml(j.content_type)}).`;
...
(function() {
    ...
    async function load() {
        ...
        countEl.textContent = '';
        try {
            const r = await wafFetch('/api/teacher/pool/my-shares', { credentials: 'same-origin' });
            const j = await r.json();
    ...
    unshareBtn.addEventListener('click', async () => {
        ...
        try {
            const tok = await csrf();
            const r = await wafFetch('/api/teacher/pool/unshare', {
                method: 'POST',
                credentials: 'same-origin',
                headers: { 'Content-Type': 'application/json', 'X-CSRF-Token': tok },
...
(async function() {
    ...
    async function loadColleagues() {
        ...
        try {
            const r = await wafFetch('/api/teacher/share/colleagues', { credentials: 'same-origin' });
            const j = await r.json();
    ...
    async function loadGroups() {
        listEl.innerHTML = '<em>Caricamento…</em>';
        try {
            const r = await wafFetch('/api/teacher/share/groups', { credentials: 'same-origin' });
            const j = await r.json();
            if (!j.ok) { listEl.innerHTML = `<em>Errore: ${escHtml(j.error || '?')}</em>`; return; }
            if (!(j.groups || []).length) {
    ...
    function wireRows() {
        listEl.querySelectorAll('[data-sg-del]').forEach(btn => {
            btn.addEventListener('click', async () => {
                ...
                const tok = await csrf();
                const r = await wafFetch(`/api/teacher/share/groups/${id}/delete`, {
                    method: 'POST', credentials: 'same-origin',
    ...
    async function openMembers(groupId, groupName) {
        ...
        try {
            const r = await wafFetch(`/api/teacher/share/groups/${groupId}/members`, { credentials: 'same-origin' });
            const j = await r.json();
        ...
        backdrop.querySelector('#fm-sg-mem-save').addEventListener('click', async () => {
            ...
            const tok = await csrf();
            const r = await wafFetch(`/api/teacher/share/groups/${groupId}/members`, {
                method: 'POST', credentials: 'same-origin',
                headers: { 'X-CSRF-Token': tok, 'Content-Type': 'application/json' },
    ...
    createBtn.addEventListener('click', async () => {
        ...
        const tok = await csrf();
        const r = await wafFetch('/api/teacher/share/groups', {
            method: 'POST', credentials: 'same-origin',
            headers: { 'X-CSRF-Token': tok, 'Content-Type': 'application/json' },
...
(function() {
    ...
    async function callCleanup(confirm) {
        const tok = await csrf();
        const r = await wafFetch('/api/teacher/sync/cleanup-orphans', {
            method: 'POST', credentials: 'same-origin',
            headers: { 'Content-Type': 'application/json', 'X-CSRF-Token': tok },
    ...
    function showResult(j) {
        ...
        let txt = '';
        // 2026-09-10 — se TUTTO risulta senza file non è una pulizia da fare:
        // è la cartella dei dati che il server non trova (è successo davvero,
        // e il pulsante qui sotto avrebbe cancellato tutte le verifiche e le
        // mappe cifrate). Il server rifiuta comunque di cancellare; qui lo si
        // dice, e il pulsante non si abilita.
        if (j.sospetto) {
            txt += `⚠ ${j.messaggio || 'Tutti i documenti risultano senza file: non viene cancellato niente.'}\n\n`;
        }
        if (j.dry_run) {
        ...
        // Con un sospetto il pulsante di cancellazione resta spento.
        return j.sospetto ? 0 : ov.length + om.length;
    }
    ...
    confirmBtn.addEventListener('click', async () => {
        ...
        // 2026-09-10 — la frase «ma i blob sono già mancanti» era un'assicurazione
        // che il server non poteva dare: il giorno in cui cercava nella cartella
        // sbagliata, i blob c'erano tutti.
        if (!(await window.FM.Dialog.confirm('Confermi la cancellazione delle righe senza file? L\'operazione è irreversibile.').catch(() => false))) return;
        confirmBtn.disabled = true;
        confirmBtn.textContent = '🗑 Cancellazione…';
        try {
            const j = await callCleanup(true);
            if (!j.ok) { alert(j.messaggio || ('Errore: ' + (j.error || 'unknown'))); return; }
            showResult(j);
...
if (document.readyState === "loading") {
    document.addEventListener("DOMContentLoaded", initTeacherDashboard, { once: true });
} else {
    initTeacherDashboard();
}
window.addEventListener("fm:navigated", initTeacherDashboard);
