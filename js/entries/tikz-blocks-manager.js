    ...
    async function refreshContent() {
        ...
        if (kind === "template" && tplId) {
            ...
            placeholder.querySelector(".fm-tbm-edit-tpl").addEventListener("click", async () => {
                // Se non arriva, l'avviso l'ha già dato caricaEntry.
                try { await caricaEntry("js/entries/tikz-template-filler.js", { pronto: () => !!window.FM?.openTemplateFiller }); } catch { return; }
            ...
            placeholder.querySelector(".fm-tbm-edit-cm6").addEventListener("click", async () => {
                try { await caricaEntry("js/entries/tikz-editor-modal.js", { pronto: () => !!window.FM?.openTikzModal }); } catch { return; }
