...
import { confirmDialog } from "../modules/editor/tex-dropdown-helpers.js";
...
export function openTemplateFiller(templateId, initialData, onSave, opts = null) {
    ...
    backdrop.addEventListener("click", async (e) => {
        ...
        if (act === "save") {
            ...
            confirmDialog({ title: "Annulla", message: "Annullare le modifiche?", confirmLabel: "Sì, annulla", danger: true })
                .then((ok) => { if (ok) close(); });
        } else if (act === "codeview") {
            const code = getCurrentTikz();
            const cv = el("div", { class: "fm-tplf-codeview" }, [code]);
