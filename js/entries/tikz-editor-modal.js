...
import { testoErrori } from "../modules/editor/tex-errori.js";
import { escHtml as _escapeHtml } from "../modules/core/dom-utils.js";
import { fetchCsrf } from "../modules/core/dom-utils.js";

// Stesso PDF.js della verifica-preview-editor → unificazione pipeline.
// 2026-09-04 — da npm nel bundle Vite invece che da jsdelivr a runtime
// (revisione architetturale 2026-09, rilievo A4); il worker è un asset emesso.
...
async function refreshPreview(previewEl, source) {
    ...
    try {
        ...
        // 422 = il disegno ha errori TeX: è il contenuto del docente, non un
        // guasto dell'applicazione, e si segna come avviso (24/9/2026).
        _dbg(previewEl, resp.ok ? "ok" : resp.status === 422 ? "warn" : "err",
        ...
        if (!resp.ok) {
            ...
            // 24/9/2026 — `errors` sono gli errori di pdflatex, e con
            // `tex_errors` il PDF è uscito lo stesso (`pdf_b64`), a metà.
            // Prima il modal mostrava quel PDF e diceva «ok»: il log compariva
            // solo quando il PDF non usciva affatto. Ora si vedono sempre gli
            // errori, e sotto la figura parziale per capire dove si è rotta.
            const errori = Array.isArray(errBody?.errors) ? errBody.errors : [];
            const err = document.createElement("div");
            err.className = "err";
            err.dataset.texErrors = String(errori.length);
            // Con gli errori: solo loro, con la riga del sorgente; il log
            // intero resta nel registro qui sotto e non spinge la figura
            // parziale fuori dalla vista.
            err.textContent = errori.length > 0
                ? `${testoErrori(errori, source)}\n\n(il log completo di pdflatex è nel registro qui sotto)`
                : `Errore compile:\n\n${  log.substring(0, 8000)}`;
            previewEl.appendChild(err);
            const n = errori.length;
            if (newStatus) {
                newStatus.textContent = n > 0
                    ? `${n} ${n === 1 ? "errore" : "errori"} TeX${errBody?.pdf_b64 ? " — figura parziale" : ""}`
                    : `errore ${resp.status}`;
            }
            _dump(previewEl, `pdflatex log (errore HTTP ${resp.status})`, log, "err");
            if (errBody?.pdf_b64) {
                const parziale = Uint8Array.from(atob(errBody.pdf_b64), (c) => c.charCodeAt(0));
                _dbg(previewEl, "warn", `PDF parziale: ${parziale.length} bytes, con ${n} errori`);
                await _disegnaPdf(previewEl, parziale, "fm-tikz-modal-pdf-canvas fm-tikz-modal-pdf-parziale");
            }
            return;
        }
        ...
        await _disegnaPdf(previewEl, pdfBytes, "fm-tikz-modal-pdf-canvas");
...
/** Disegna la prima pagina di un PDF in un canvas in fondo all'anteprima,
 *  con PDF.js (lo stesso della verifica preview). */
async function _disegnaPdf(previewEl, pdfBytes, classe) {
    ...
    if (previewEl.querySelector(".err")) canvas.style.marginTop = "12px";
