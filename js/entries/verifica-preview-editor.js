 ...
 *
 * Si carica a richiesta da js/modules/features/verifica-preview-modal.js,
 * con `caricaEntry` (core/carica-entry.js); dopo,
...
import { fetchCsrf, wafFetch, escHtml as escapeHtml } from "../modules/core/dom-utils.js";
...
// 2026-09-04 — PDF.js e pako vengono da npm e finiscono nel bundle Vite
// (chunk separati, caricati al primo uso). Fino a qui arrivavano a runtime
// da jsdelivr: nessun fissaggio di versione oltre la stringa, nessuna
// integrità, e un host esterno da autorizzare nel CSP (revisione
// architetturale 2026-09, rilievo A4). Il worker è un asset emesso da Vite.
...
/**
 * Come si chiama il file che il docente scarica.
 *
 * Fino al 22/9/2026 era sempre `verifica_<id>.pdf`, anche quando il modal
 * stava esportando un piano annuale o una relazione finale — e quell'`id`, in
 * modo risdoc, è quello del MODELLO. Il modal è uno solo per cinque cose
 * diverse, e il nome del file era rimasto quello della prima.
 *
 * Il titolo è lì, a portata di mano nello stesso oggetto: lo usa già la barra
 * della finestra due funzioni più in là. Non mancava il dato, mancava la
 * lettura.
 */
function nomeDelFile(doc) {
    const grezzo = String(doc?.title || doc?.variant || "").trim();
    const pulito = grezzo
        .normalize("NFD").replace(/[̀-ͯ]/g, "")   // via gli accenti
        .replace(/[^A-Za-z0-9._ -]+/g, "")                   // via tutto ciò che un filesystem non ama
        .replace(/\s+/g, "_")
        .replace(/_+/g, "_")
        .replace(/^[._-]+|[._-]+$/g, "")
        .slice(0, 80);
    return `${pulito || `documento_${doc?.id ?? ""}`}.pdf`;
}
    ...
    m.innerHTML = `
      <div class="fm-vp-modal" role="dialog" aria-modal="true" aria-labelledby="fm-vp-title">
        ...
        <main class="fm-vp-body">
          ...
          <div class="fm-vp-resizer" data-resizer="sidebar" role="separator" aria-orientation="vertical"
          ...
          <div class="fm-vp-resizer fm-vp-resizer--pdf" data-resizer="pdf" role="separator" aria-orientation="vertical"
               aria-label="Ridimensiona l'editor rispetto all'anteprima PDF" tabindex="0"
               title="Trascina per ridimensionare (frecce ← → da tastiera)"></div>
...
const templateFileAdapter = {
    ...
    async compile(doc, { texOverride = null, activePath = null } = {}) {
        ...
        const csrf = await fetchCsrf();
        // G22.16 — multi-file: invia l'intero albero del doc come bundle così
        // i main con \input{/intestazione} ecc. compilano lato tex-compile.
        // Buffer binari (placeholder, non file reali) esclusi dal bundle.
        const bundleFiles = [];
        const buffers = doc ? State.files.get(doc.id) : null;
        if (buffers) {
            for (const [p, buf] of buffers.entries()) {
                if (buf.binary) continue;
                bundleFiles.push({
                    path: p,
                    content: (p === targetPath && texOverride != null) ? texOverride : buf.text,
                });
            }
        }
...
// Larghezze ridimensionabili (drag handle + frecce tastiera), persistite in
// localStorage. --fm-vp-sidebar-w pilota la prima traccia della grid
// .fm-vp-body; --fm-vp-editor-w (G22.16) pilota la seconda (vedi css).
const SIDEBAR_W_KEY = "fm.vp.sidebarWidth";
const SIDEBAR_W_MIN = 140;
const SIDEBAR_W_MAX = 520;
const EDITOR_W_KEY = "fm.vp.editorWidth";
const EDITOR_W_MIN = 240;
const PDF_W_MIN = 280;
function clampSidebarW(px) {
    return Math.max(SIDEBAR_W_MIN, Math.min(SIDEBAR_W_MAX, Math.round(px)));
}
function bodyWidth() {
    return State.modal?.querySelector(".fm-vp-body")?.clientWidth ?? 1200;
}
function sidebarW() {
    const v = parseInt(State.modal?.style.getPropertyValue("--fm-vp-sidebar-w") || "200", 10);
    return Number.isFinite(v) ? v : 200;
}
function clampEditorW(px) {
    const max = Math.max(EDITOR_W_MIN, bodyWidth() - sidebarW() - PDF_W_MIN);
    return Math.max(EDITOR_W_MIN, Math.min(max, Math.round(px)));
}
function applySidebarWidth(px) {
    const w = clampSidebarW(px);
    State.modal?.style.setProperty("--fm-vp-sidebar-w", `${w  }px`);
    const handle = State.modal?.querySelector('[data-resizer="sidebar"]');
    if (handle) handle.setAttribute("aria-valuenow", String(w));
    try { localStorage.setItem(SIDEBAR_W_KEY, String(w)); } catch (_) {}
    // L'editor clamp dipende dalla sidebar: ri-applica entro i nuovi limiti.
    const curEditor = parseInt(State.modal?.style.getPropertyValue("--fm-vp-editor-w") || "0", 10);
    if (curEditor) applyEditorWidth(curEditor, false);
    return w;
}
function applyEditorWidth(px, persist = true) {
    const w = clampEditorW(px);
    State.modal?.style.setProperty("--fm-vp-editor-w", `${w  }px`);
    const handle = State.modal?.querySelector('[data-resizer="pdf"]');
    if (handle) {
        handle.setAttribute("aria-valuenow", String(w));
        handle.setAttribute("aria-valuemax", String(Math.max(EDITOR_W_MIN, bodyWidth() - sidebarW() - PDF_W_MIN)));
    }
    if (persist) { try { localStorage.setItem(EDITOR_W_KEY, String(w)); } catch (_) {} }
    return w;
}
/** Drag + tastiera + doppio-click per UNA maniglia. leftPane = selettore del
 *  pannello a SINISTRA della maniglia (larghezza = distanza da lì). */
function attachResizerDrag(handle, leftPaneSel, { apply, arrowStep = 16, reset }) {
    const pane = () => State.modal?.querySelector(leftPaneSel);
    let dragging = false;
    const onMove = (e) => {
        if (!dragging) return;
        const p = pane();
        if (p) apply((e.clientX ?? 0) - p.getBoundingClientRect().left);
        e.preventDefault();
    };
    const onUp = () => {
        if (!dragging) return;
    ...
    // Tastiera (WCAG 2.2): ← → regolano la larghezza.
    handle.addEventListener("keydown", (e) => {
        const p = pane();
        if (!p) return;
        if (e.key === "ArrowLeft")  { apply(p.getBoundingClientRect().width - arrowStep); e.preventDefault(); }
        else if (e.key === "ArrowRight") { apply(p.getBoundingClientRect().width + arrowStep); e.preventDefault(); }
    });
    // Doppio click → reset al default.
    handle.addEventListener("dblclick", () => reset());
}
function setupResizers() {
    // ── Maniglia 1: sidebar (filetree ↔ editor) ──
    const sideHandle = State.modal?.querySelector('[data-resizer="sidebar"]');
    const aside = State.modal?.querySelector("[data-filetree]");
    if (sideHandle && aside) {
        sideHandle.setAttribute("aria-valuemin", String(SIDEBAR_W_MIN));
        sideHandle.setAttribute("aria-valuemax", String(SIDEBAR_W_MAX));
        let saved = 200;
        try { const v = parseInt(localStorage.getItem(SIDEBAR_W_KEY) || "", 10); if (Number.isFinite(v)) saved = v; } catch (_) {}
        applySidebarWidth(saved);
        attachResizerDrag(sideHandle, "[data-filetree]", {
            apply: (w) => applySidebarWidth(w),
            reset: () => applySidebarWidth(200),
        });
    }
    // ── Maniglia 2: editor ↔ PDF (G22.16) ──
    const pdfHandle = State.modal?.querySelector('[data-resizer="pdf"]');
    const editorPane = State.modal?.querySelector('[data-pane="editor"]');
    if (pdfHandle && editorPane) {
        pdfHandle.setAttribute("aria-valuemin", String(EDITOR_W_MIN));
        let saved = Math.max(EDITOR_W_MIN, Math.round((bodyWidth() - sidebarW()) / 2));
        try { const v = parseInt(localStorage.getItem(EDITOR_W_KEY) || "", 10); if (Number.isFinite(v)) saved = v; } catch (_) {}
        applyEditorWidth(saved, false);
        attachResizerDrag(pdfHandle, '[data-pane="editor"]', {
            apply: (w) => applyEditorWidth(w),
            reset: () => applyEditorWidth(Math.max(EDITOR_W_MIN, Math.round((bodyWidth() - sidebarW()) / 2))),
        });
    }
    // Window resize: ri-clampa l'editor (il PDF non deve sparire).
    window.addEventListener("resize", () => {
        const cur = parseInt(State.modal?.style.getPropertyValue("--fm-vp-editor-w") || "0", 10);
        if (cur) applyEditorWidth(cur, false);
    });
}
...
async function renderPdfInPane(hostSelector, pdfBytes, pageNum = 1) {
    // La finestra può chiudersi mentre un render è in volo: questa funzione
    // riprende dopo un `await` e `State.modal` a quel punto e' null (lo azzera
    // `closeModal`). Non è un errore, è la corsa fra il render e la
    // chiusura: si esce come si esce da un render superato.
    ...
    const superseded = () => !State.modal || (main && State.pdfRenderGen !== myGen);
...
export async function openPreview(docs, opts = {}) {
    ...
    State.modal.addEventListener("click", async (e) => {
        ...
        if (act === "export-pdf") {
            const doc = State.docs[State.activeIdx];
            const cached = State.cache.get(doc.id);
            // Niente byte = niente scaricamento. Tutto ciò che segue deve
            // stare DOPO questa guardia: qui non è successo niente.
            if (!cached?.pdfBytes) return;
            const blob = bytesToBlob(cached.pdfBytes, "application/pdf");
            const url = URL.createObjectURL(blob);
            const a2 = document.createElement("a");
            a2.href = url;
            a2.download = nomeDelFile(doc);
            a2.click();
            URL.revokeObjectURL(url);
            // ADR-046 — da qui parte la grazia prima della cancellazione.
            //
            // La guardia è `doc.compilationId`, non `State.mode`: il modo
            // «risdoc-template» è vero anche quando il super-admin modifica il
            // modello originale, dove non c'è nessuna compilazione di nessun
            // docente. L'identificativo, invece, esiste solo se una riga
            // salvata c'è davvero — quindi si guarda quello. Per le verifiche,
            // per i documenti personalizzati e per i file dei modelli quella
            // proprietà non viene nemmeno costruita.
            if (doc.compilationId) {
                import("../modules/risdoc/segna-scaricata.js")
                    .then(({ segnaScaricata }) => segnaScaricata(doc.compilationId))
                    .catch(() => { /* la grazia è un di più: il file è già sceso */ });
            }
            return;
        }
