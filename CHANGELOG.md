...
Prima versione dopo la migrazione da progetto-precedente: riassume il lavoro da
...
### Changed
- HKDF/HMAC prefix delle chiavi crypto rinominati `progetto-precedente-*` ->
...
- MySQL view definer rebound `precedente_app@localhost` ->
...
- Grafana datasource + dashboards + provisioning yaml rinominati
  progetto-precedente-* -> pantedu-*. Folder ProgettoPrecedente orfana cancellata via API.
- Promtail config: `/var/log/progetto-precedente-{deploy,waf-blocked}.log` ->
  `/var/log/pantedu-*.log`.
- fail2ban jail+filter `progetto-precedente-waf` rinominato `pantedu-waf` con nuovo
  logpath.
...
- nginx vhost legacy `beta.progetto-precedente.example` + cert Let's Encrypt rimosso
  post-kill progetto-precedente. Cleanup .bak files in sites-enabled.

### Removed
- VPS-only kill di progetto-precedente: stopped+disabled 11 systemd units progetto-precedente,
  rimosso `/var/www/progetto-precedente` (con chattr -i per .env*) e
  `/var/lib/progetto-precedente-data`, droppato DATABASE progetto-precedente + USER
  precedente_app, certbot delete beta.progetto-precedente.example + tex.progetto-precedente.example,
  cleanup `/etc/sudoers.d/progetto-precedente-deploy`, `/etc/progetto-precedente/`,
  `/usr/local/{bin,sbin}/progetto-precedente-*`, userdel -r progetto-precedente.
  Domini `progetto-precedente.example` (hosting legacy) e repo GitHub `vittop89/progetto-precedente`
  NON toccati (archivio storico).

### Security
- Snapshot Hetzner pre-migration `pre-pantedu-migration-20260523-0150`
  conservato per rollback.
- DB dump finale progetto-precedente in `/var/backups/pantedu/final-progetto-precedente/`
  (668K compresso).
...
### Added
- Initial import del codebase ribrandizzato da progetto-precedente.
...
- 66 tabelle DB clonate da progetto-precedente (mantiene compat KMS via
  KMS_MASTER_KEY identica, STORAGE_SIGNING_SECRET identica,
  WAF_HMAC_SECRET identica).

### Architecture (ereditata da progetto-precedente Phase 1-26)
