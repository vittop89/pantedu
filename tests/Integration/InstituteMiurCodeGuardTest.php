...
final class InstituteMiurCodeGuardTest extends TestCase
{
    ...
    {
        $id = $this->repo->upsertCanonical('XXPS99999Z', 'SCUOLA DI PROVA ' . uniqid(), 'Comune Esempio');
    ...
    public function una_riga_sintetica_gia_esistente_resta_usabile(): void
    {
        ...
        $this->pdo->prepare('INSERT INTO institutes (code, name, city, active) VALUES (?, ?, ?, 1)')
            ->execute(['MIUR-STORICA-COMUNE_ESEMPIO', $nome, 'Comune Esempio']);
        $id = (int)$this->pdo->lastInsertId();

        $this->assertSame($id, $this->repo->upsertCanonical('MIUR-STORICA-COMUNE_ESEMPIO', $nome, 'Comune Esempio'));
    }
    ...
    public function un_codice_reale_promuove_la_riga_sintetica(): void
    {
        ...
        $this->pdo->prepare('INSERT INTO institutes (code, name, city, active) VALUES (?, ?, ?, 1)')
            ->execute(['MIUR-PROMUOVERE-COMUNE_ESEMPIO', $nome, 'Comune Esempio']);
        $id = (int)$this->pdo->lastInsertId();

        $this->assertSame($id, $this->repo->upsertCanonical('XXPS88888Y', $nome, 'Comune Esempio'));

        $st = $this->pdo->prepare('SELECT code FROM institutes WHERE id = ?');
        $st->execute([$id]);
        $this->assertSame('XXPS88888Y', (string)$st->fetchColumn(), 'la riga ha adottato il codice vero');
    }
    ...
    public function il_riconoscitore_di_codici_e_quello_che_dice_di_essere(): void
    {
        ...
        foreach (['MIUR-ESEMPIO-COMUNE_ESEMPIO-ART', 'xxps00000a', 'VBPS-0010', ''] as $finto) {
