...
final class TeacherPrintControllerTest extends TestCase
{
    ...
    private bool $dbWasEnabled = false;

    protected function setUp(): void
    {
        $_SESSION = [];
        // OwnershipService e' DB-first: con `database.enabled` a true ignora il
        // JSON iniettato, cerca lo username in `users` e, se non lo trova, non
        // registra nulla ("utente non in DB → nessun assign"). In suite un test
        // precedente ricarica .env (DB_ENABLED=true → pantedu_test), dove
        // "annab"/"mariarossi" non esistono: il test passava solo in isolamento,
        // dove il bootstrap lascia il DB spento. Qui si verifica il cablaggio
        // controller → OwnershipService sul file sandbox, quindi la modalita'
        // file e' forzata esplicitamente e ripristinata in tearDown.
        $this->dbWasEnabled = (bool)AppConfig::get('database.enabled');
        AppConfig::set('database.enabled', false);
        $this->sandbox = sys_get_temp_dir() . '/pantedu_print_' . uniqid();
        mkdir($this->sandbox . '/storage_temp', 0755, true);

        $this->files = new FileService([
            'roots' => ['temp' => $this->sandbox . '/storage_temp'],
    ...
    {
        AppConfig::set('database.enabled', $this->dbWasEnabled);
    ...
    /**
     * 23/9/2026 (A-52): una selezione oltre i 2 MB è 413, come in ogni altra
     * rotta che legge il corpo da Request. Prima questa rispondeva 400
     * (`payload_too_large` preso dal catch generico), PrintInfo 413.
     */
    #[Test]
    public function selection_over_the_limit_returns_413(): void
    {
        $this->asTeacher();
        $grande = $this->samplePayload(['verTitle' => str_repeat('x', 2 * 1024 * 1024)]);
        $res = $this->controller()->generate($this->request($grande));
        $this->assertSame(413, $res->status);
        $this->assertSame('payload_too_large', json_decode((string)$res->body, true)['error'] ?? null);
    }
