...
final class TeacherSubjectServiceTest extends TestCase
{
    ...
    public function togliere_una_materia_la_disattiva_e_non_la_cancella(): void
    {
        ...
        // ADR-035: la spunta e' una riga di curriculum_teacher sulla voce della
        // scuola; togliere la materia la spegne, e la voce della scuola resta
        // una sola, senza copie per docente.
        $st = $this->pdo->prepare(
            'SELECT ct.active FROM curriculum_teacher ct
               JOIN curriculum_entries c ON c.id = ct.curriculum_id
              WHERE c.kind = "materie" AND c.institute_id = ?
                AND c.code = "MAT" AND ct.user_id = ?'
        );
        $st->execute([$this->instId, $this->prof]);
        $this->assertSame('0', (string)$st->fetchColumn(), 'la spunta deve esistere, spenta');

        $voci = $this->pdo->prepare(
            'SELECT COUNT(*) FROM curriculum_entries WHERE institute_id = ? AND kind = "materie" AND code = "MAT"'
        );
        $voci->execute([$this->instId]);
        $this->assertSame(1, (int)$voci->fetchColumn(), 'una voce sola per codice, della scuola');
    }
    ...
    public function rimetterla_la_riaccende_invece_di_duplicarla(): void
    {
        ...
        $this->assertSame(['MAT'], $this->codici());
        $st = $this->pdo->prepare(
            'SELECT COUNT(*) FROM curriculum_teacher ct
               JOIN curriculum_entries c ON c.id = ct.curriculum_id
              WHERE c.kind = "materie" AND c.institute_id = ? AND c.code = "MAT" AND ct.user_id = ?'
        );
        $st->execute([$this->instId, $this->prof]);
        $this->assertSame(1, (int)$st->fetchColumn(), 'una spunta sola, non due');
    }
