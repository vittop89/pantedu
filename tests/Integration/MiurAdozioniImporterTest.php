...
final class MiurAdozioniImporterTest extends TestCase
{
    ...
    protected function setUp(): void
    {
        ...
        $ins->execute([self::CODICE, 'ISTITUTO DI PROVA ADOZIONI', 'Comune Esempio', 'LOMBARDIA']);
    ...
    private function conta(): array
    {
        $st = $this->pdo->prepare(
            'SELECT kind, COUNT(*) AS n FROM curriculum_entries
              WHERE institute_id = ? GROUP BY kind'
        );
    ...
    public function apply_scrive_esattamente_il_piano(): void
    {
        ...
        $st = $this->pdo->prepare(
            'SELECT code, indirizzo FROM curriculum_entries
              WHERE kind = "classi" AND institute_id = ? ORDER BY code'
        );
        $st->execute([$this->instId]);
        $classi = [];
        foreach ($st->fetchAll(PDO::FETCH_ASSOC) as $r) {
    ...
    public function un_indirizzo_gia_a_registro_non_viene_duplicato(): void
    {
        ...
        $this->pdo->prepare(
            'INSERT INTO curriculum_entries (kind, institute_id, code, label, active, shared_with_pool)
             VALUES ("indirizzi", ?, "SCI", "Scientifico", 1, 0)'
        )->execute([$this->instId]);
    ...
    public function una_sezione_senza_indirizzo_viene_agganciata(): void
    {
        ...
        $this->pdo->prepare(
            'INSERT INTO curriculum_entries (kind, institute_id, code, label, indirizzo, active, shared_with_pool)
             VALUES ("classi", ?, "1A", "Classe 1A", NULL, 1, 0)'
        )->execute([$this->instId]);
    ...
    public function le_scuole_non_a_tabella_vengono_ignorate(): void
    {
        $csv = $this->scriviCsv(
            $this->riga('A', 'LICEO SCIENTIFICO', '1')
            . $this->riga('Z', 'LICEO CLASSICO', '1', 'NO', 'XXPS99999Z')
        );
        $plan = $this->importer()->scan($csv, self::CODICE);
