...
final class ActivityLoggerTest extends TestCase
{
    ...
    /**
     * 2026-09-10 — `database.enabled` prima di toccarlo, per rimetterlo com'era.
     *
     * `senza_database_non_esplode()` lo mette a `false` e fino a oggi non lo
     * rimetteva. La configurazione vive nel processo, e la suite gira con
     * `executionOrder="random"`: quindi, a seconda del seme, **ogni prova di
     * integrazione che capitava dopo questa si saltava** con «database non
     * disponibile». Non falliva: si saltava, e il totale restava verde.
     *
     * Misurato con due semi fissi: col seme 1 si saltavano le tre prove di
     * `PublicContentPolicyTest`, coi semi 2 e 3 nessuna. È la stessa forma di
     * guasto che questa sessione insegue da giorni — un verde che non ha
     * guardato — solo che qui a nasconderlo era l'ordine dei test.
     */
    ...
    public function i_beacon_di_telemetria_restano_fuori(): void
    {
        ...
        self::assertFalse(ActivityLogger::shouldLogRequest('POST', 429, '/analytics/nav?v=1'));

        // Un percorso che comincia allo stesso modo ma è un'altra cosa resta dentro.
        self::assertTrue(ActivityLogger::shouldLogRequest('POST', 200, '/analytics/navigazioni'));
    ...
    public function l_indirizzo_non_si_conserva_in_chiaro(): void
    {
        ...
        // Dal 24/9/2026 un'impronta con chiave, non lo SHA-256 nudo.
        self::assertNotNull(\App\Support\ImprontaIp::di('203.0.113.7'));
        self::assertSame(\App\Support\ImprontaIp::di('203.0.113.7'), $stored);
        self::assertNotSame(hash('sha256', '203.0.113.7', true), $stored);
    }
