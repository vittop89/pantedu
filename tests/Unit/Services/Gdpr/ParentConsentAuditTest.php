...
final class ParentConsentAuditTest extends TestCase
{
    ...
    #[Test]
    public function l_indirizzo_del_genitore_si_conserva_come_impronta_con_chiave(): void
    {
        $svc = new ParentConsentService();
        $token = $svc->request(42, 'genitore@example.invalid');
        $svc->confirm($token, '198.51.100.4', 'Mozilla/5.0');

        // Dal 24/9/2026 l'impronta con chiave di ImprontaIp; fino ad allora
        // lo SHA-256 nudo, che per un IPv4 si rovescia provando gli indirizzi.
        $impronta = ImprontaIp::di('198.51.100.4');
        self::assertNotNull($impronta, 'senza segreto la prova non guarderebbe niente');
        $granted = array_values(array_filter($this->audit(), fn($r) => $r['event'] === 'granted'))[0];
        self::assertSame($impronta, $granted['ip_hash']);
        self::assertNotSame(hash('sha256', '198.51.100.4', true), $granted['ip_hash']);
        $conferma = $this->pdo->query('SELECT confirm_ip_hash FROM parent_consents WHERE student_user_id = 42')->fetchColumn();
        self::assertSame($impronta, $conferma, 'la stessa impronta nella riga del consenso');
    }
