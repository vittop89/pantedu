...
final class TosAcceptanceServiceTest extends TestCase
{
    ...
    /**
     * Due versioni dello stesso documento con la stessa `effective_from`, in
     * tutti e due gli ordini di inserimento. Con SQLite le righe tornano
     * nell'ordine in cui sono state scritte, quindi «al contrario» è il caso
     * in cui l'ultima letta è la più bassa.
     *
     * Le coppie 1.9 e 1.10 dicono che il confronto è fra versioni e non fra
     * stringhe: come stringa '1.9' viene dopo '1.10'.
     *
     * @return array<string, array{list<string>, string}>
     */
    public static function versioniConLaStessaData(): array
    {
        return [
            '1.4 poi 1.5'   => [['1.4', '1.5'], '1.5'],
            '1.5 poi 1.4'   => [['1.5', '1.4'], '1.5'],
            '1.9 poi 1.10'  => [['1.9', '1.10'], '1.10'],
            '1.10 poi 1.9'  => [['1.10', '1.9'], '1.10'],
        ];
    }

    /**
     * 23/9/2026. In `docs/legal/versions.json` i Termini 1.4 e 1.5 sono tutti
     * e due in vigore dal 22/9, e la 1.1 e la 1.2 dal 1/9. Le query non hanno
     * ORDER BY e prima vinceva l'ultima riga letta: quale versione fosse
     * vincolante dipendeva dall'ordine in cui il database restituiva le righe.
     * Misurato su MariaDB in una transazione annullata: 1.4 poi 1.5 dava 1.5,
     * 1.5 poi 1.4 dava 1.4.
     *
     * @param list<string> $inserite
     */
    #[Test]
    #[DataProvider('versioniConLaStessaData')]
    public function a_parita_di_data_vince_la_versione_piu_alta(array $inserite, string $attesa): void
    {
        $pdo = $this->pdo();
        $this->addVersion($pdo, 'aup', '1.0', $this->daysFromNow(-90), $this->daysFromNow(-90));
        $this->addVersion($pdo, 'tos', '1.0', $this->daysFromNow(-90), $this->daysFromNow(-90));
        $dal = $this->daysFromNow(-1);
        foreach ($inserite as $versione) {
            $this->addVersion($pdo, 'tos', $versione, $dal, $dal);
        }

        $svc = new TosAcceptanceService($pdo);
        $this->assertSame($attesa, $svc->getCurrentTosVersion(), 'la versione vincolante adesso');
        // La stessa scelta per un'accettazione registrata a posteriori (il
        // flusso della registrazione): la riga deve dire la stessa versione.
        $this->assertSame(
            $attesa,
            $svc->effectiveVersionsAt($this->daysFromNow(0))['tos'],
            'la versione vincolante a una data'
        );
    }

    /**
     * La stessa scelta fra due versioni in preavviso con la stessa data: il
     * modulo di accettazione anticipata propone la più alta.
     *
     * @param list<string> $inserite
     */
    #[Test]
    #[DataProvider('versioniConLaStessaData')]
    public function a_parita_di_data_si_propone_la_pendente_piu_alta(array $inserite, string $attesa): void
    {
        $pdo = $this->pdo();
        $this->addVersion($pdo, 'tos', '1.0', $this->daysFromNow(-90), $this->daysFromNow(-90));
        $this->addVersion($pdo, 'aup', '1.0', $this->daysFromNow(-90), $this->daysFromNow(-90));
        $dal = $this->daysFromNow(30);
        foreach ($inserite as $versione) {
            $this->addVersion($pdo, 'tos', $versione, $this->daysFromNow(0), $dal);
        }

        $this->assertSame($attesa, (new TosAcceptanceService($pdo))->targetVersions()['tos']);
    }

    /**
     * Controprova: la data conta prima del numero. Una versione più bassa ma
     * entrata in vigore dopo resta la vincolante: il confronto delle versioni
     * decide solo a parità di data.
     */
    #[Test]
    public function la_data_piu_recente_vince_sul_numero_di_versione(): void
    {
        $pdo = $this->pdo();
        $this->addVersion($pdo, 'aup', '1.0', $this->daysFromNow(-90), $this->daysFromNow(-90));
        $this->addVersion($pdo, 'tos', '2.0', $this->daysFromNow(-20), $this->daysFromNow(-20));
        $this->addVersion($pdo, 'tos', '1.9', $this->daysFromNow(-5), $this->daysFromNow(-5));
        $this->addVersion($pdo, 'tos', '3.0', $this->daysFromNow(0), $this->daysFromNow(40));
        $this->addVersion($pdo, 'tos', '2.5', $this->daysFromNow(0), $this->daysFromNow(50));

        $svc = new TosAcceptanceService($pdo);
        $this->assertSame('1.9', $svc->getCurrentTosVersion());
        $this->assertSame('1.9', $svc->effectiveVersionsAt($this->daysFromNow(0))['tos']);
        $this->assertSame('2.5', $svc->targetVersions()['tos'], 'fra le pendenti, la più lontana');
    }
}
