...
final class TeacherSectionAssignmentTest extends TestCase
{
    ...
    /** Una voce del vocabolario dell'istituto: quella che il docente spunta. */
    ...
    /**
     * Le voci della scuola che il docente ha spuntato (ADR-035: una riga di
     * `curriculum_teacher` sulla voce, non una copia della voce).
     *
    ...
    {
        $st = $this->db->prepare(
            'SELECT ce.kind, ce.code, ce.indirizzo
               FROM curriculum_teacher ct
               JOIN curriculum_entries ce ON ce.id = ct.curriculum_id
              WHERE ce.institute_id = ? AND ct.user_id = ?
              ORDER BY ce.kind, ce.code'
        );
