...
final class ContractRendererRmTest extends TestCase
{
    ...
    public function rm_option_with_nested_list_renders_nested_OL(): void
    {
        ...
        // G27.dsa (2026-08-31) — asserzioni INVERTITE rispetto a G23.fix6.
        //
        // Il test attendeva l'ASSENZA di F/GF nelle opzioni. G27.dsa ha
        // cambiato intenzione: i marcatori sono emessi per tutte le sezioni
        // che fanno parte della domanda — question, sub-list annidate e
        // options (la cella di una risposta multipla) — e restano esclusi
        // solo da solution e justification. Le opzioni di un RM sono parte
        // dell'esercizio, quindi il docente deve poterle marcare F/GF per
        // le copie BES/DSA come marca gli altri sotto-punti.
        // Cfr. ContractRenderer::renderList, $isQuestion.
        $this->assertStringContainsString('fm-dsa-li-F', $html);
        $this->assertStringContainsString('fm-dsa-li-GF', $html);
        // G27.dsa.fix — struttura UNIFORME a ogni livello (outer, sub,
        // options): [F/GF] [marker esplicito] [content] in flex layout.
        // Il marker non e' piu' quello nativo del browser via
        // `display: list-item`, che finiva dopo i bottoni F/GF.
        $this->assertStringContainsString('fm-dsa-li-num', $html);
        $this->assertStringContainsString('fm-dsa-li-content', $html);
