...
final class ContractRepositoryTest extends TestCase
{
    ...
    // ── Lo schema al salvataggio (23/9/2026, A-76) ───────────────────────
    //
    // save() rifiuta gli errori di schema che il contratto di prima non
    // aveva (ADR-005). Le prove d'integrazione sono in
    // tests/Integration/ContrattoFuoriSchemaTest.php; qui, senza database,
    // il conto degli errori: quelli che c'erano già non contano, anche se il
    // gruppo cambia posto, e uno in più sì.

    /** @return array<string, mixed> un contratto nello schema, con due gruppi */
    private static function contrattoNelloSchema(string $tipoDelSecondo = 'type_VF'): array
    {
        return [
            '$schema' => ContractRepository::SCHEMA_ID,
            'version' => 1,
            'groups'  => [
                ['id' => 'g1', 'type' => 'type_Collect', 'items' => [['id' => 'i1']]],
                ['id' => 'g2', 'type' => $tipoDelSecondo, 'items' => [['id' => 'i2']]],
            ],
        ];
    }

    #[Test]
    public function save_rifiuta_un_errore_di_schema_nuovo_e_lascia_il_deposito_com_era(): void
    {
        $repo = $this->makeRepo(7, 77, self::contrattoNelloSchema());
        $prima = $this->storage->raw('c/test.json');
        $agg = $repo->load(7);
        $agg->appendGroup(['type' => 'Inventato', 'items' => []]);
        $agg->bumpVersion();

        try {
            $repo->save($agg);
            $this->fail('il gruppo fuori schema doveva essere rifiutato');
        } catch (ContractSchemaException $e) {
            $this->assertCount(1, $e->errori);
            $this->assertStringStartsWith('groups[2].type:', $e->errori[0]);
        }
        $this->assertSame($prima, $this->storage->raw('c/test.json'));
    }

    #[Test]
    public function save_accetta_un_contratto_gia_fuori_schema_anche_se_il_gruppo_cambia_posto(): void
    {
        $repo = $this->makeRepo(7, 77, self::contrattoNelloSchema('Inventato'));
        $agg = $repo->load(7);
        $agg->moveGroup('g2', 0);
        $agg->bumpVersion();
        ...
        $raw = json_decode($this->storage->raw('c/test.json'), true);
        $this->assertSame(['g2', 'g1'], array_column($raw['groups'], 'id'));
    }

    #[Test]
    public function save_rifiuta_un_secondo_errore_della_stessa_forma(): void
    {
        $repo = $this->makeRepo(7, 77, self::contrattoNelloSchema('Inventato'));
        $agg = $repo->load(7);
        $agg->appendGroup(['type' => 'AncheQuesto', 'items' => []]);
