...
final class TosAcceptanceMiddlewareTest extends TestCase
{
    ...
    private function login(int $userId = 7, bool $superAdmin = false): void
    {
        ...
        // Claims appena letti: senza, AclPolicy li rilegge dal database
    ...
    /**
     * I tre modi in cui il controllo di accettazione lascia passare.
     *
     * @return array<string, array{0: int, 1: ?bool}> id dell'utente, esito di
     *         hasAccepted (null: il servizio lancia, come con il database giù)
     */
    public static function viaLibera(): array
    {
        return [
            'utente senza id'         => [0, false],
            'accettazione registrata' => [7, true],
            'servizio guasto'         => [7, null],
        ];
    }

    /**
     * Un'eccezione a valle attraversa il gate una volta sola (23/9/2026,
     * revisione architetturale 2026-09, A-1). `$next` stava dentro il try del
     * controllo di accettazione: il catch pensato per il database prendeva
     * anche l'eccezione del controller e lo rieseguiva, e al Kernel arrivava
     * l'errore della seconda esecuzione. Il ramo «servizio guasto» chiamava
     * `$next` dal catch già prima: lì la prova tiene fermo il via libera.
     */
    #[Test]
    #[DataProvider('viaLibera')]
    public function un_eccezione_a_valle_attraversa_il_gate_una_volta(int $userId, ?bool $accettato): void
    {
        $this->setEnforce(true);
        $this->login($userId);
        $servizio = $this->createStub(TosAcceptanceService::class);
        if ($accettato === null) {
            $servizio->method('hasAccepted')->willThrowException(new \RuntimeException('database giù'));
        } else {
            $servizio->method('hasAccepted')->willReturn($accettato);
        }
        $chiamate = 0;
        $next = static function () use (&$chiamate): Response {
            $chiamate++;
            throw new \RuntimeException('controller guasto n.' . $chiamate);
        };

        // Il ramo del servizio guasto scrive in error_log: non nell'output della suite.
        $registro = (string)tempnam(sys_get_temp_dir(), 'pantedu-tos-');
        $prima = ini_set('error_log', $registro);
        try {
            (new TosAcceptanceMiddleware($servizio))->handle($this->mkReq('/area-docente'), $next);
            $this->fail("l'eccezione del controller deve arrivare al Kernel");
        } catch (\RuntimeException $e) {
            $this->assertSame('controller guasto n.1', $e->getMessage(), 'arriva la prima eccezione');
        } finally {
            ini_set('error_log', $prima === false ? '' : $prima);
            @unlink($registro);
        }
        $this->assertSame(1, $chiamate, 'il controller gira una volta');
    }
}
