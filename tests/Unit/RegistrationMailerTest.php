...
final class RegistrationMailerTest extends TestCase
{
    ...
    public function every_registration_email_can_be_replied_to(): void
    {
        ...
        // "rifiutato" e chiede spiegazioni non deve finire in noreply@. La
        // casella la passa il controller (`mail.contact_email`).
        $inviati = [];
        foreach (['pending', 'approved', 'rejected'] as $quale) {
            $rm = $this->rm('operatore@example.net');
            match ($quale) {
                'pending'  => $rm->pending('anna@example.it', 'Anna'),
                'approved' => $rm->approved('anna@example.it', 'Anna', 'anna.rossi'),
                'rejected' => $rm->rejected('anna@example.it', 'Anna'),
            };
            $inviati = array_merge($inviati, $this->sent);
        }

        $this->assertCount(3, $inviati);
    ...
    {
        // 23/9/2026 (A-13): il predefinito era contact@ di produzione, anche
        // per un'altra istanza. Senza casella, il Reply-To è il mittente.
        $this->rm()->rejected('anna@example.it', 'Anna');
        $this->assertStringContainsString("Reply-To: operatore@example.net\r\n", $this->sent[0]['hdrs']);
        $this->assertStringNotContainsString('contact@', $this->sent[0]['hdrs']);
    }
