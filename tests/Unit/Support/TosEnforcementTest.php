...
final class TosEnforcementTest extends TestCase
{
    ...
    private mixed $originalLogs = null;
    ...
    protected function tearDown(): void
    {
        ...
        $this->setConfig('logs', $this->originalLogs);
        $this->setEnv($this->originalFlag);
        TosEnforcement::resetCache();

        foreach (glob($this->tmpStorage . '/{,.}*', GLOB_BRACE) ?: [] as $f) {
            if (is_file($f)) {
                unlink($f);
            }
        }
