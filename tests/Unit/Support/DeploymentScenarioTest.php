...
final class DeploymentScenarioTest extends TestCase
{
    ...
    protected function setUp(): void
    {
        ...
        // Un file corrotto scrive un'anomalia (SostituzioneSuFile, A-38): nel
        // registro della prova, non in quello della copia di lavoro.
        $this->logsPrima = Config::get('app.paths.logs');
        $this->setConfig('app.paths.logs', $this->tmp);
    ...
    {
        $this->resetAll();
        $this->setConfig('app.paths.logs', $this->logsPrima);
        foreach (array_merge(glob($this->tmp . '/config/*') ?: [], glob($this->tmp . '/{,.}*', GLOB_BRACE) ?: []) as $f) {
            if (is_file($f)) {
                @unlink($f);
            }
        }
    ...
    #[Test]
    public function solo_nello_scenario_1_c_e_un_docente_solo(): void
    {
        // 19/9/2026 — decide se a chi studia con la credenziale si propone
        // quella «di un altro docente»: nello scenario 1 non esiste.
        self::assertTrue(DeploymentScenario::isPersonal());
        self::assertFalse(DeploymentScenario::consentePiuDocenti(), 'scenario 1: solo l\'autore');
        self::assertFalse(DeploymentScenario::snapshot()['more_teachers']);

        DeploymentScenario::persist(DeploymentScenario::COLLEAGUES, 'tester', 'apro ai colleghi dopo il parere del DPO');
        $this->resetAll();
        self::assertTrue(DeploymentScenario::consentePiuDocenti(), 'scenario 2: i colleghi');
        self::assertTrue(DeploymentScenario::snapshot()['more_teachers']);
        ...
        $this->resetAll();
        self::assertTrue(DeploymentScenario::consentePiuDocenti(), 'scenario 3: i docenti dell\'Istituto');
    }
    ...
    public function fuori_dallo_scenario_3_il_titolare_e_il_gestore(): void
    {
        ...
        // Un'informativa per scenario (2026-09-06): uso personale e colleghi
        // hanno testi diversi, perche' diversi sono gli interessati.
        self::assertStringEndsWith('/informativa-personale.md', DeploymentScenario::informativaFile());
        self::assertStringEndsWith('/informativa.md', DeploymentScenario::informativaFile(DeploymentScenario::COLLEAGUES));
        self::assertStringEndsWith('/informativa-istituto.md', DeploymentScenario::informativaFile(DeploymentScenario::INSTITUTE));
        self::assertStringContainsString('uso personale', DeploymentScenario::legalDocuments()[0]['label']);
        self::assertStringContainsString('docenti iscritti', DeploymentScenario::legalDocuments(DeploymentScenario::COLLEAGUES)[0]['label']);
    }
