...
final class UserTest extends TestCase
{
    ...
    public function role_helpers_are_mutually_exclusive(): void
    {
        ...
        $doc   = User::fromArray('d', ['role' => 'teacher',       'active' => true]);
        $stud  = User::fromArray('s', ['role' => 'student',       'active' => true]);

        $this->assertTrue($admin->isAdmin());
        $this->assertFalse($admin->isStudent());
        $this->assertTrue($doc->isTeacher());
        $this->assertFalse($doc->isAdmin());
        $this->assertTrue($stud->isStudent());
    }

    #[Test]
    public function admin_and_teacher_can_access_any_section(): void
    {
        $admin = User::fromArray('a', ['role' => 'administrator', 'active' => true]);
        $doc   = User::fromArray('d', ['role' => 'teacher',       'active' => true]);
        // 15/9/2026 — il vecchio valore `collaborator` non apre più niente.
        $vecchio = User::fromArray('c', ['role' => 'collaborator', 'active' => true]);

        $this->assertTrue($admin->canAccessSection('ar5s'));
        $this->assertTrue($doc->canAccessSection('sc2s'));
        $this->assertFalse($vecchio->canAccessSection('sc2s'));
    }
