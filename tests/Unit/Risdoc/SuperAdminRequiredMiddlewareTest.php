...
final class SuperAdminRequiredMiddlewareTest extends TestCase
{
    ...
    {
        // Username unico per test (in origine per la cache statica di
        // AclPolicy, tolta il 23/9/2026): non corrisponde a nessun utente vero.
        $username = 'tester_' . \bin2hex(\random_bytes(4));
        $_SESSION['autenticato'] = true;
        $_SESSION['username']    = $username;
        $_SESSION['user_id']     = 999;
        $_SESSION['user_role']   = $role;
        // AclPolicy::isSuperAdmin() legge i claims della sessione finché sono
        // freschi (Auth::CLAIMS_TTL_SECONDS, 23/9/2026: prima una cache a parte,
        // 'fm_super_admin_cache') → si seminano letti adesso, senza database.
        $_SESSION['is_super_admin'] = $superAdmin;
        $_SESSION['claims_at']      = \time();
    }
