...
TEX;
        ...
        // Sezione `pt_unified` (ADR-026): e' la forma degli schemi reali, e la
        // sola che il fallback di TexBuilder rende dagli items. Senza il flag
        // (fixture pre-ADR) il corpo usciva vuoto — riallineato il 2026-09-04.
        $schema = [
            '$id' => 'test',
            'title' => 'Test',
            'category' => 'MODELLI',
            'sections' => [[
                'title' => 'Profilo della classe',
                'pt_unified' => true,
                'items' => [
                    [
                        'type' => 'nota-textarea',
                    ...
                    // Campo piatto dichiarato nello schema: il fallback rende
                    // solo gli items, non i campi orfani della compilazione.
                    ['type' => 'info-field', 'name' => 'classe_name', 'label' => 'Classe'],
                ],
            ]],
        ];
        ...
        // Sezione e sectionbox dallo skeleton dello schema
        self::assertStringContainsString('\\section*{Profilo della classe}', $texOut);
        self::assertStringContainsString('\\begin{sectionbox}{OSSERVAZIONI}', $texOut);
        // Default preservato; i segnaposto [field-*] sono risolti dallo state
        // della compilazione (classe=3A, sezione=B)
        self::assertStringContainsString('Gli alunni della classe 3A B sono', $texOut);
        self::assertStringNotContainsString('[field-classe]', $texOut);
        // checkboxGroup → itemize con \item[\xcheckbox] / \item[\checkbox]
        self::assertStringContainsString('\\item[\\xcheckbox] attenti', $texOut);
        self::assertStringContainsString('\\item[\\checkbox] distratti', $texOut);
        // User edit preservato
        self::assertStringContainsString('\\item[\\xcheckbox] aggiunta utente', $texOut);
        // Plain string field
    ...
    public function testTexOutputSemanticsAfterUserEdit(): void
    {
        ...
        // Layout "all" del checkboxGroup: itemize con \item[\xcheckbox] label
        // (riallineato il 2026-09-04: prima \xcheckbox{label} inline).
        self::assertStringContainsString('\\item[\\xcheckbox] opzione1', $tex);
        self::assertStringContainsString('\\item[\\xcheckbox] opzione2', $tex);
