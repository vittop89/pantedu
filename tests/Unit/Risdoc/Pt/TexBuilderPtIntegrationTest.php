...
final class TexBuilderPtIntegrationTest extends TestCase
{
    ...
    {
        // Schema senza tex.wrapper → buildFallbackTex. La sezione pt_unified
        // (ADR-026) dichiara i due campi: il fallback rende solo gli items
        // dello schema, non i campi "orfani" della compilazione
        // (fixture riallineata il 2026-09-04: prima non aveva sezioni).
        $schemaPath = $this->writeSchema('s.json', [
            'title' => 'Test',
            '$id'   => 'test',
            'sections' => [[
                ...
                'pt_unified' => true,
                'items' => [
                    ['type' => 'nota-textarea', 'name' => 'rich_field', 'label' => 'Rich'],
                    ['type' => 'info-field', 'name' => 'plain_field', 'label' => 'Plain'],
                ],
            ]],
        ]);
        $builder = new TexBuilder($schemaPath, $this->tmpDir);
