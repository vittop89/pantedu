...
test("ArrowRight a fine span.dots → caret uscito dopo lo span", async ({ bancoEditor }) => {
...
test("ArrowLeft a inizio span.AddTextDSA → caret esce prima dello span", async ({ bancoEditor }) => {
    const page = bancoEditor.page;
    const r = await page.evaluate(() => {
        const wrap = window.FM.__buildSectionForTest("Quesito", "");
        document.body.appendChild(wrap);
...
test("ArrowRight in mezzo a span (NON a boundary) → no preventDefault (navigation normale)", async ({ bancoEditor }) => {
    const page = bancoEditor.page;
    const r = await page.evaluate(() => {
        const wrap = window.FM.__buildSectionForTest("Quesito", "");
        document.body.appendChild(wrap);
