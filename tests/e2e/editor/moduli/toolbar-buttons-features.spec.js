...
test("Bold/italic/underline preservati in _buildBlocksFromTextarea → _toHtml", async ({ bancoEditor }) => {
...
test("Find/replace dialog si apre con flag regex/case/word/insel", async ({ bancoEditor }) => {
    const page = bancoEditor.page;
    const flags = await page.evaluate(async () => {
        ...
        await window.FM.__openFindReplaceDialogForTest({ _focusedTextarea: field }); // 2026-09-05: dialogHost.open e' asincrono
...
test("Find dialog pre-fill con selezione corrente del field", async ({ bancoEditor }) => {
    const page = bancoEditor.page;
