...
test("Caso 1: caret in field vuoto → lista con UN <li> vuoto", async ({ bancoEditor }) => {
...
test("Caso 2: caret in riga con testo → la riga corrente diventa l'unico <li>", async ({ bancoEditor }) => {
    const page = bancoEditor.page;
    const r = await page.evaluate(() => {
        const wrap = window.FM.__buildSectionForTest("Quesito", "");
        document.body.appendChild(wrap);
...
test("Caso 3: selezione multi-line (3 righe) → 3 <li>", async ({ bancoEditor }) => {
    const page = bancoEditor.page;
    const r = await page.evaluate(() => {
        const wrap = window.FM.__buildSectionForTest("Quesito", "");
        document.body.appendChild(wrap);
...
test("Caso 4: selezione multi-line con <div> blocks → 2 <li>", async ({ bancoEditor }) => {
    const page = bancoEditor.page;
    const r = await page.evaluate(() => {
        const wrap = window.FM.__buildSectionForTest("Quesito", "");
        document.body.appendChild(wrap);
