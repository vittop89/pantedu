...
test("Caso A: caret in testo grezzo (no wrapper) → tutta la riga diventa <li>", async ({ bancoEditor }) => {
...
test("Caso B: selezione attraversa 2 righe (testo grezzo) → 2 <li>", async ({ bancoEditor }) => {
    const page = bancoEditor.page;
    const r = await page.evaluate(() => {
        const wrap = window.FM.__buildSectionForTest("Quesito", "");
        document.body.appendChild(wrap);
...
test("Caso C: undo Ctrl+Z dopo insertList ripristina stato pre-azione", async ({ bancoEditor }) => {
    const page = bancoEditor.page;
...
test("Caso E: lista deve apparire IN POSIZIONE delle righe selezionate (non in fondo)", async ({ bancoEditor }) => {
    const page = bancoEditor.page;
    const r = await page.evaluate(() => {
        const wrap = window.FM.__buildSectionForTest("Quesito", "");
        document.body.appendChild(wrap);
...
test("Caso D: keyboard shortcut Ctrl+Z su contenteditable triggers undo", async ({ bancoEditor }) => {
    const page = bancoEditor.page;
    const r = await page.evaluate(async () => {
        const wrap = window.FM.__buildSectionForTest("Quesito", "");
        document.body.appendChild(wrap);
