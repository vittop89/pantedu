...
test("Bold <b>: innerHTML→block.content→_toHtml preserva tag", async ({ bancoEditor }) => {
...
test("Italic <i>: roundtrip preserva tag", async ({ bancoEditor }) => {
    const page = bancoEditor.page;
    const r = await page.evaluate(() => {
        const wrap = window.FM.__buildSectionForTest("Quesito", "");
        document.body.appendChild(wrap);
...
test("Underline <u>: roundtrip preserva tag", async ({ bancoEditor }) => {
    const page = bancoEditor.page;
    const r = await page.evaluate(() => {
        const wrap = window.FM.__buildSectionForTest("Quesito", "");
        document.body.appendChild(wrap);
...
test("Mix B+I+U: tutti tag preservati post-roundtrip", async ({ bancoEditor }) => {
    const page = bancoEditor.page;
    const r = await page.evaluate(() => {
        const wrap = window.FM.__buildSectionForTest("Quesito", "");
        document.body.appendChild(wrap);
...
test("wrapSnippet('<b>','</b>') con selezione → wrap in <b> visibile", async ({ bancoEditor }) => {
    const page = bancoEditor.page;
    const r = await page.evaluate(() => {
        const wrap = window.FM.__buildSectionForTest("Quesito", "");
        document.body.appendChild(wrap);
...
test("wrapSnippet collapsed → typed char STAYS dentro <b> (ZWS placeholder)", async ({ bancoEditor }) => {
    const page = bancoEditor.page;
    const r = await page.evaluate(() => {
        const wrap = window.FM.__buildSectionForTest("Quesito", "");
        document.body.appendChild(wrap);
...
test("Reload pipeline: ContractRenderer .fm-collection → _extractRawWithTikz preserva <b>/<i>/<u>", async ({ bancoEditor }) => {
    const page = bancoEditor.page;
    const r = await page.evaluate(() => {
...
test("Helper _updateInlineFormatBtnState: rileva ancestor <b>/<i>/<u>", async ({ bancoEditor }) => {
    const page = bancoEditor.page;
    const r = await page.evaluate(() => {
        const wrap = window.FM.__buildSectionForTest("Quesito", "");
        document.body.appendChild(wrap);
...
test("Enter → <div> wrappers: tag inline DENTRO div preservati", async ({ bancoEditor }) => {
    const page = bancoEditor.page;
...
test("Enter → <p> wrappers (Firefox-style): tag inline preservati", async ({ bancoEditor }) => {
    const page = bancoEditor.page;
    const r = await page.evaluate(() => {
        const wrap = window.FM.__buildSectionForTest("Quesito", "");
        document.body.appendChild(wrap);
...
test("Toggle B: selezione FULL su <b> → unwrap (remove formato)", async ({ bancoEditor }) => {
    const page = bancoEditor.page;
    const r = await page.evaluate(() => {
        const wrap = window.FM.__buildSectionForTest("Quesito", "");
        document.body.appendChild(wrap);
...
test("Toggle B: selezione PARZIALE dentro <b> → split", async ({ bancoEditor }) => {
    const page = bancoEditor.page;
    const r = await page.evaluate(() => {
        const wrap = window.FM.__buildSectionForTest("Quesito", "");
        document.body.appendChild(wrap);
...
test("Toggle B: caret IN MEZZO a <b>aaa|bbb</b> → split, aaa resta bold", async ({ bancoEditor }) => {
    const page = bancoEditor.page;
    const r = await page.evaluate(() => {
        const wrap = window.FM.__buildSectionForTest("Quesito", "");
        document.body.appendChild(wrap);
...
test("Toggle B: caret a FINE di <b>aaa|</b> → exit, typing fuori non-bold", async ({ bancoEditor }) => {
    const page = bancoEditor.page;
    const r = await page.evaluate(() => {
        const wrap = window.FM.__buildSectionForTest("Quesito", "");
        document.body.appendChild(wrap);
...
test("Toggle B: caret a INIZIO di <b>|aaa</b> → exit prima, typing fuori non-bold", async ({ bancoEditor }) => {
    const page = bancoEditor.page;
    const r = await page.evaluate(() => {
        const wrap = window.FM.__buildSectionForTest("Quesito", "");
        document.body.appendChild(wrap);
...
test("Wrap selezione: selezione PRESERVATA su content del nuovo <b>", async ({ bancoEditor }) => {
    const page = bancoEditor.page;
    const r = await page.evaluate(() => {
        const wrap = window.FM.__buildSectionForTest("Quesito", "");
        document.body.appendChild(wrap);
...
test("Toggle B: selezione SENZA <b> → applica wrap", async ({ bancoEditor }) => {
    const page = bancoEditor.page;
    const r = await page.evaluate(() => {
        const wrap = window.FM.__buildSectionForTest("Quesito", "");
        document.body.appendChild(wrap);
...
test("Toggle I/U: alias strong/em riconosciuti come b/i", async ({ bancoEditor }) => {
    const page = bancoEditor.page;
    const r = await page.evaluate(() => {
        const wrap = window.FM.__buildSectionForTest("Quesito", "");
        document.body.appendChild(wrap);
...
test("Copy: selezione di testo DENTRO <u>ccc</u> include <u> wrapper", async ({ bancoEditor }) => {
    const page = bancoEditor.page;
    const r = await page.evaluate(() => {
        const wrap = window.FM.__buildSectionForTest("Quesito", "");
        document.body.appendChild(wrap);
...
test("Paste: strip commenti HTML (StartFragment/EndFragment) + tag vuoti", async ({ bancoEditor }) => {
    const page = bancoEditor.page;
    const r = await page.evaluate(() => {
        const wrap = window.FM.__buildSectionForTest("Quesito", "");
        document.body.appendChild(wrap);
...
test("_buildBlocksFromTextarea: strip tag inline vuoti residuali", async ({ bancoEditor }) => {
    const page = bancoEditor.page;
    const r = await page.evaluate(() => {
        const wrap = window.FM.__buildSectionForTest("Quesito", "");
        document.body.appendChild(wrap);
...
test("Paste: singleton <div>g</div> → unwrap (no newline indesiderata)", async ({ bancoEditor }) => {
    const page = bancoEditor.page;
    const r = await page.evaluate(() => {
        const wrap = window.FM.__buildSectionForTest("Quesito", "");
        document.body.appendChild(wrap);
...
test("Paste: multi-line <div>a</div><div>b</div> → inline + <br> separator", async ({ bancoEditor }) => {
    const page = bancoEditor.page;
    const r = await page.evaluate(() => {
        const wrap = window.FM.__buildSectionForTest("Quesito", "");
        document.body.appendChild(wrap);
...
test("Paste: <div><i>ccc</i><u>ddd</u></div> con multi inline children → unwrap", async ({ bancoEditor }) => {
    const page = bancoEditor.page;
    const r = await page.evaluate(() => {
        const wrap = window.FM.__buildSectionForTest("Quesito", "");
        document.body.appendChild(wrap);
...
test("Paste: inter-element whitespace \\n/\\t da clipboard parser → strip (no newline visive)", async ({ bancoEditor }) => {
    const page = bancoEditor.page;
    const r = await page.evaluate(() => {
        const wrap = window.FM.__buildSectionForTest("Quesito", "");
        document.body.appendChild(wrap);
...
test("Toggle B su nested <b><i><u>aaa</u></i></b>: middle preserva <i>+<u>", async ({ bancoEditor }) => {
    const page = bancoEditor.page;
...
test("Toggle U su <b><i><u>aaaa</u></i></b>: middle preserva <b><i>", async ({ bancoEditor }) => {
    const page = bancoEditor.page;
    const r = await page.evaluate(() => {
        const wrap = window.FM.__buildSectionForTest("Quesito", "");
        document.body.appendChild(wrap);
...
test("Toggle I su <b><i><u>aaaa</u></i></b>: middle preserva <u> interno", async ({ bancoEditor }) => {
    const page = bancoEditor.page;
    const r = await page.evaluate(() => {
        const wrap = window.FM.__buildSectionForTest("Quesito", "");
        document.body.appendChild(wrap);
...
test("DIAG: 40 chars 'aaa...' wrappato <b><i><u>: 3 toggle B/I/U sequenza", async ({ bancoEditor }) => {
    const page = bancoEditor.page;
    const r = await page.evaluate(() => {
        const wrap = window.FM.__buildSectionForTest("Quesito", "");
        document.body.appendChild(wrap);
...
test("Keydown Ctrl+U/I/B: handler attached + intercetta (preventDefault + toggle)", async ({ bancoEditor }) => {
    const page = bancoEditor.page;
    const r = await page.evaluate(() => {
        const wrap = window.FM.__buildSectionForTest("Quesito", "");
        document.body.appendChild(wrap);
...
test("Normalize: <i><div><b><u>aaa</u></b></div></i> → <div><i><b><u>aaa</u></b></i></div>", async ({ bancoEditor }) => {
    const page = bancoEditor.page;
    const r = await page.evaluate(() => {
        const wrap = window.FM.__buildSectionForTest("Quesito", "");
        document.body.appendChild(wrap);
...
test("Chain B+U+I sulla STESSA selezione: ogni toggle trova ancestor correttamente", async ({ bancoEditor }) => {
    const page = bancoEditor.page;
...
test("Bold inside list <li>: tag preserved in nested block.content", async ({ bancoEditor }) => {
    const page = bancoEditor.page;
    const r = await page.evaluate(() => {
        const wrap = window.FM.__buildSectionForTest("Quesito", "");
        document.body.appendChild(wrap);
