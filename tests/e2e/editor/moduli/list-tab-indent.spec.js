...
test("Tab dentro <li>: indent → sub-list nested attaccata al li precedente", async ({ bancoEditor }) => {
...
test("Tab senza previous-sibling: no-op (browser convention)", async ({ bancoEditor }) => {
    const page = bancoEditor.page;
    const r = await page.evaluate(() => {
        const wrap = window.FM.__buildSectionForTest("Quesito", "");
        document.body.appendChild(wrap);
...
test("Shift+Tab in li nested: outdent → li sibling del nonno-li", async ({ bancoEditor }) => {
    const page = bancoEditor.page;
    const r = await page.evaluate(() => {
        const wrap = window.FM.__buildSectionForTest("Quesito", "");
        document.body.appendChild(wrap);
...
test("Enter su <li> VUOTO: outdent + rimuovi il li (exit list)", async ({ bancoEditor }) => {
    const page = bancoEditor.page;
    const r = await page.evaluate(() => {
        const wrap = window.FM.__buildSectionForTest("Quesito", "");
        document.body.appendChild(wrap);
...
test("Tab triggered via keydown: indent funziona end-to-end", async ({ bancoEditor }) => {
    const page = bancoEditor.page;
...
test("Conversione TEX preserva nesting dopo Tab indent", async ({ bancoEditor }) => {
    const page = bancoEditor.page;
    const r = await page.evaluate(() => {
