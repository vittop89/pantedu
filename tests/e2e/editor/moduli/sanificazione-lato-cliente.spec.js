/**
 * La ripulitura del contenuto scritto nell'editor, dentro al browser.
 * Riscrittura di g24_xss_sanitization.spec.js.
 *
 * Prima di salvare, l'editor ripulisce quel che il docente ha scritto o
 * incollato: via gli script, via gli indirizzi `javascript:`, via i gestori
 * `onclick`, via le cornici. È la stessa regola che applica il server, ripetuta
 * qui perché chi scrive se ne accorga subito e non a salvataggio fatto — e
 * perché due controlli sono meglio di uno.
 *
 * Il grassetto, i collegamenti veri e il testo restano dove sono: una pulizia
 * che porta via anche quelli sarebbe un difetto suo.
 *
...
test("il contenuto di un blocco perde script, indirizzi eseguibili e gestori", async ({ bancoEditor }) => {
...
test("dove serve solo testo, ogni marcatura viene tolta", async ({ bancoEditor }) => {
    const page = bancoEditor.page;
...
test("la ripulitura avviene prima del salvataggio, non dopo", async ({ bancoEditor }) => {
    const page = bancoEditor.page;
    await page.waitForFunction(() => typeof window.FM?.__buildBlocksFromTextareaForTest === "function", null, { timeout: 10000 });
