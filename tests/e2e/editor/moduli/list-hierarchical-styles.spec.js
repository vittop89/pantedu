...
test("Preset alpha-decimal: data-fm-list-style settato, no type attr", async ({ bancoEditor }) => {
...
test("Preset roman-alpha: data-fm-list-style=roman-alpha", async ({ bancoEditor }) => {
    const page = bancoEditor.page;
    const r = await page.evaluate(() => {
        const wrap = window.FM.__buildSectionForTest("Quesito", "");
        document.body.appendChild(wrap);
...
test("Preset decimal-zero: data-fm-list-style=decimal-zero", async ({ bancoEditor }) => {
    const page = bancoEditor.page;
    const r = await page.evaluate(() => {
        const wrap = window.FM.__buildSectionForTest("Quesito", "");
        document.body.appendChild(wrap);
...
test("Preset paren / arrow-bullet / star-circle settano data-fm-list-style", async ({ bancoEditor }) => {
    const page = bancoEditor.page;
    const r = await page.evaluate(() => {
...
test("Round-trip preset: insert → buildBlocks → _toHtml preserva data-fm-list-style", async ({ bancoEditor }) => {
    const page = bancoEditor.page;
    const r = await page.evaluate(() => {
...
test("Default ol/ul: nessun preset (default CSS nesting via descendant selectors)", async ({ bancoEditor }) => {
    const page = bancoEditor.page;
    const r = await page.evaluate(() => {
        const wrap = window.FM.__buildSectionForTest("Quesito", "");
        document.body.appendChild(wrap);
