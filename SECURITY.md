...
branch `main`. Non sono previste back-port di security fix su versioni precedenti.
...
Scrivi a **<{{OPERATORE_EMAIL}}>** — è lo stesso `Contact` pubblicato in
[security.txt](https://pantedu.eu/.well-known/security.txt) (RFC 9116).

### Canale Alternativo

**GitHub Security Advisories** sul repository pubblico
(→ https://github.com/vittop89/pantedu/security/advisories/new) è il canale
ufficiale di GitHub, ma al 2026-09-23 la segnalazione privata delle
vulnerabilità non è ancora attivata su quel repository: prima di usarlo,
verifica che il pulsante «Report a vulnerability» sia presente nella scheda
Security. Se non c'è, o se stai lavorando sul repository di sviluppo
(privato, non accetta segnalazioni esterne), usa l'email.
...
Contatto: {{OPERATORE_EMAIL}} (vedi «Segnalare una Vulnerabilità» sopra)
