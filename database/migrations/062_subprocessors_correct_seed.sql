...
--
-- 060 aveva inserito "Aruba S.p.A. — Web hosting" che era ereditato dalla
...
--   - Aruba: SOLO registrar dominio (DNS) → non sub-processor di trattamento dati
--   - Google: opt-in OAuth + Drive (resta)
--
-- Strategy: marca Aruba come inactive (preserva history audit), inserisce
-- Hetzner + Backblaze come active. Niente DROP/UPDATE distruttivo —
-- mantiene tracciabilità delle revisioni.
--
-- Rollback: UPDATE subprocessors SET active=1 WHERE name='Aruba S.p.A.';
--           DELETE FROM subprocessors WHERE name IN ('Hetzner Online GmbH','Backblaze Inc.');
-- ═════════════════════════════════════════════════════════════════════════

-- Deattiva il vecchio Aruba (hosting legacy in dismissione, no più trattamento)
UPDATE subprocessors
   SET active = 0,
       service_description = 'Hosting legacy in dismissione — dominio resta su Aruba (registrar DNS)'
 WHERE name = 'Aruba S.p.A.';
